...
if (!isset($_GET['Prid']) and !isset($_SESSION['ProcessingPick'])) {
	...
	echo '<div class="row">
<div class="col-xs-4">
			<a href="' . $RootPath . '/SelectPickingLists.php" class="btn btn-info">' . _('Select a Pick List') . '</a>
		</div></div>
		
		<br />';
	echo prnMsg(_('This page can only be opened if a pick list has been selected Please select a pick list first'), 'error');
...
<div class="col-xs-4">
			...
			echo  prnMsg(_('There are no ordered items with a quantity left to deliver. There is nothing left to invoice')),'</div>';
			include('includes/footer.php');
			exit;
...
<div class="col-xs-4">
		...
		echo  prnMsg(_('This pick list item could not be retrieved. Please select another pick list'), 'warn');
		include('includes/footer.php');
		exit;
...
echo '<br /><p align="right">
		<a href="' . $RootPath . '/SelectPickingLists.php" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> To Pick Lists') . '</a></p><br />';
	
	
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Pick List: ') . str_pad($_SESSION['ProcessingPick'], 10, '0', STR_PAD_LEFT) . _(' for Order No: ') . $_SESSION['Items' . $identifier]->OrderNo . '</h1></a></div>';



echo '<h4 class="page-header">
			' . ' ' . _('Customer Code') . ' :<strong> ' . $_SESSION['Items' . $identifier]->DebtorNo . '</strong><br />
				' . _('Customer Name') . ' :<strong> ' . $_SESSION['Items' . $identifier]->CustomerName . '</strong></h4>
			';
...
foreach ($_SESSION['Items' . $identifier]->LineItems as $LnItm) {
	...
	if ($LnItm->Controlled == 1) {
		if (isset($_POST['ProcessPickList'])) {
			...
			echo '<td class="number"><input type="hidden" name="' . $LnItm->LineNumber . '_QtyDispatched"  value="' . $LnItm->QtyDispatched . '" /><a href="' . $RootPath . '/PickingListsControlled.php?identifier=' . $identifier . '&amp;LineNo=' . $LnItm->LineNumber . '" class="btn btn-info">' . locale_number_format($LnItm->QtyDispatched, $LnItm->DecimalPlaces) . '</a></td>';
		}
	} else {
		if (isset($_POST['ProcessPickList'])) {
			echo '<td class="number">' . locale_number_format($LnItm->QtyDispatched, $LnItm->DecimalPlaces) . '</td>';
		} else {
			echo '<td class="has-error"><input tabindex="' . $j . '" type="text" ' . ($j == 1 ? 'autofocus="autofocus" ' : '') . ' required="required" class="form-control" title="' . _('Enter the quantity to charge the customer for, that has been dispatched') . '" name="' . $LnItm->LineNumber . '_QtyDispatched" maxlength="12" size="12" value="' . locale_number_format($LnItm->QtyDispatched, $LnItm->DecimalPlaces) . '" /></td>';
		}
	}
	...
	if ($LnItm->Controlled == 1) {
		if (!isset($_POST['ProcessPickList'])) {
			echo '<td><a href="' . $RootPath . '/PickingListsControlled.php?identifier=' . $identifier . '&amp;LineNo=' . $LnItm->LineNumber . '" class="btn btn-info">';
...
if (isset($_POST['ProcessPickList']) and $_POST['ProcessPickList'] != '') {
		...
		foreach ($_SESSION['Items' . $identifier]->LineItems as $OrderLine) {
			...
			if (($CheckNegRow['mbflag'] == 'B' or $CheckNegRow['mbflag'] == 'M') and mb_substr($OrderLine->StockID, 0, 4) != 'ASSET') {
				if ($CheckNegRow['quantity'] < $OrderLine->QtyDispatched) {
					echo  prnMsg(_('Invoicing the selected order would result in negative stock. The system parameters are set to prohibit negative stocks from occurring. This invoice cannot be created until the stock on hand is corrected.'), 'error', $OrderLine->StockID . ' ' . $CheckNegRow['description'] . ' - ' . _('Negative Stock Prohibited')),'</div>';
					$NegativesFound = true;
				}
	...
	if (DB_num_rows($Result) != count($_SESSION['Items' . $identifier]->LineItems)) {
		...
		echo '<br />';
		echo prnMsg(_('This order has been changed or invoiced since this delivery was started to be confirmed') . '. ' . _('Processing halted') . '. ' . _('To enter and confirm this dispatch') . _(' the order must be re-selected and re-read again to update the changes made by the other user'), 'error');
	...
	while ($MyRow = DB_fetch_array($Result)) {
		...
		if ($_SESSION['Items' . $identifier]->LineItems[$MyRow['orderlineno']]->Quantity != $MyRow['quantity'] or $_SESSION['Items' . $identifier]->LineItems[$MyRow['orderlineno']]->QtyInv != $MyRow['qtyinvoiced']) {
			...
			echo '<br /><p class="text-info">' . _('Orig order for') . ' ' . $MyRow['orderlineno'] . ' ' . _('has a quantity of') . ' ' . $MyRow['quantity'] . ' ' . _('and an invoiced qty of') . ' ' . $MyRow['qtyinvoiced'] . ' ' . _('the session shows quantity of') . ' ' . $_SESSION['Items' . $identifier]->LineItems[$MyRow['orderlineno']]->Quantity . ' ' . _('and quantity invoice of') . ' ' . $_SESSION['Items' . $identifier]->LineItems[$MyRow['orderlineno']]->QtyInv .'</p><br />';

			echo prnMsg(_('This order has been changed or invoiced since this delivery was started to be confirmed') . ' ' . _('Processing halted.') . ' ' . _('To enter and confirm this dispatch, it must be re-selected and re-read again to update the changes made by the other user'), 'error');

			echo '<div class="row">
			<div class="col-xs-4">
<a href="' . $RootPath . '/SelectPickiingLists.php" class="btn btn-info">' . _('Select a pick list to maintain') . '</a></div></div><br />';
...
<a target="_blank" href="' . $PrintDispatchNote . '" class="btn btn-warning">' . _('Print Packing Slip') . '</a></div>
<div class="col-xs-4"><a target="_blank" href="' . $PrintLabels . '" class="btn btn-warning">' . _('Print Customer Labels') . '</a></div>';

	echo '<div class="col-xs-4"><a href="' . $RootPath . '/SelectPickingLists.php" class="btn btn-info">' . _('Select another pick list for processing') . '</a></div></div><br />';
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Pick List Status') . '</label>
	...
	echo '</select></div></div>';
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Consignment Note Ref') . '</label>
			<input tabindex="' . $j . '" type="text" data-type="no-illegal-chars" class="form-control" title="' . _('Enter the consignment note reference to enable tracking of the delivery in the event of customer proof of delivery issues') . '" maxlength="15" size="15" name="Consignment" value="' . $_POST['Consignment'] . '" /></div>
		</div>';
	++$j;
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('No Of Packages in Delivery') . '</label>
	...
	echo '<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Pick List Comments') . '</label>
			<textarea tabindex="' . $j . '" name="Comments" pattern=".{0,20}" class="form-control">' . reverse_escape($_SESSION['Items' . $identifier]->Comments) . '</textarea></div>
		</div>';

	++$j;
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Order Internal Comments') . '</label>
	...
	if (($_SESSION['Items' . $identifier]->Status != 'Shipped') or (in_array($ARSecurity, $_SESSION['AllowedPageSecurityTokens']))) {
		...
		++$j;
		echo '<div class="col-xs-4">
				<input type="submit" class="btn btn-info" tabindex="' . $j . '" name="ProcessPickList" value="' . _('Process Pick List') . '" />
			</div></div><br />
