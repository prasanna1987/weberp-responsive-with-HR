...
if (isset($_POST['submit'])) {
	...
	if (mb_strlen($_POST['TermsIndicator']) < 1) {
		$InputError = 1;
		echo prnMsg(_('The payment terms name must exist'),'error');
		$Errors[$i] = 'TermsIndicator';
		$i++;
	}
	if (mb_strlen($_POST['TermsIndicator']) > 2) {
		$InputError = 1;
		echo prnMsg(_('The payment terms name must be two characters or less'),'error');
		$Errors[$i] = 'TermsIndicator';
		$i++;
	}
	if (empty($_POST['DayNumber']) OR !is_numeric(filter_number_format($_POST['DayNumber'])) OR filter_number_format($_POST['DayNumber']) <= 0){
		$InputError = 1;
		echo prnMsg( _('The number of days or the day in the following month must be numeric') ,'error');
		$Errors[$i] = 'DayNumber';
		$i++;
	}
	if (empty($_POST['Terms']) OR mb_strlen($_POST['Terms']) > 40) {
		$InputError = 1;
		echo prnMsg( _('The terms description must be forty characters or less') ,'error');
		$Errors[$i] = 'Terms';
		$i++;
	}
	/*
	...
	if ($_POST['DayNumber']>360 AND !empty($_POST['DaysOrFoll'])) {
		$InputError = 1;
		echo prnMsg( _('When the check box is checked to indicate that the term expects a number of days after which accounts are due') . ', ' . _('the number entered should be less than 365 days') ,'error');
		$Errors[$i] = 'DayNumber';
		$i++;
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN DebtorsMaster
	...
	if ($myrow[0] > 0) {
		echo prnMsg( _('Cannot delete this payment term because customer accounts have been created referring to this term'),'warn');
		...
		$myrow = DB_fetch_row($result);
		if ($myrow[0] > 0) {
			echo prnMsg( _('Cannot delete this payment term because supplier accounts have been created referring to this term'),'warn');
			...
			$result = DB_query($sql);
			echo prnMsg( _('The payment term definition record has been deleted') . '!','success');
		}
	}
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Payment Terms.') . '</h3></div>
...
<table id="general-table" class="table table-bordered">';
	...
	printf('<tr><td>%s</td>
			...
			<td>%s</td>
			<td>%s</td>
			<td><a href="%s?SelectedTerms=%s" class="btn btn-info">' . _('Edit') . '</a></td>
			<td><a href="%s?SelectedTerms=%s&amp;delete=1" class="btn btn-danger" onclick="return confirm(\'' . _('Are you sure you wish to delete this payment term?') . '\');">' . _('Delete') . '</a></td>
			</tr>',
...
<div class="col-xs-12"><div class="block">';
	...
	if (isset($SelectedTerms)) {
		...
		echo '<div class="block-title">
				<h3>' . _('Update Payment Terms.') . '</h3></div>
			';
		echo '<div class="row">
				<div class="col-xs-4">
<div class="form-group"><label class="col-md-8 control-label">' . _('Term Code') . '</label>
				' . $_POST['TermsIndicator'] . '</div></div></div>';
		...
		echo '<div class="block-title"><h3>' . _('New Payment Terms.') . '</h3></div>';
		echo '<div class="row">
				<div class="col-xs-3">
<div class="form-group has-error"><label class="col-md-8 control-label">' . _('Term Code') . '</label>
				<input type="text" name="TermsIndicator"' . (in_array('TermsIndicator',$Errors) ? 'class="form-control"' : '' ) .' autofocus="autofocus" required="required" pattern="[0-9a-ZA-Z_]*" title="' . _('A 2 character code to identify this payment term. Any alpha-numeric characters can be used') . '" value="' . $_POST['TermsIndicator'] . '" class="form-control" size="3" maxlength="2" /></div>
			</div></div>';
	}

	echo '<div class="row">
				<div class="col-xs-4">
<div class="form-group has-error"><label class="col-md-8 control-label">' .  _('Terms Description'). '</label>
			<input type="text"' . (in_array('Terms',$Errors) ? 'class="form-control"' : '' ) .' name="Terms" ' . (isset($SelectedTerms)? 'autofocus="autofocus"': '') . ' required="required" value="'.$_POST['Terms']. '" class="form-control" title="' . _('A description of the payment terms is required') . '" size="35" maxlength="40" /></div>
		</div>
		
				<div class="col-xs-4">
<div class="form-group"><label class="col-md-8 control-label">' . _('Due After A Given No. Of Days').'</label>
    ...
    echo '  /></div>
				...
				<div class="col-xs-4">
<div class="form-group has-error"><label class="col-md-8 control-label">' . _('Days (Or Day In Following Month)').'</label>
			<input type="text" ' . (in_array('DayNumber',$Errors) ? 'class="form-control"' : '' ) .' name="DayNumber" required="required" class="form-control"  size="4" maxlength="3" value="';
	if ($DaysBeforeDue !=0) {
