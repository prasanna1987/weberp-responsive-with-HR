...
$Title = _('Maintenance Of Petty Cash Expenses For a Tab Type ');
...
if (!isset($_GET['delete']) and (ContainsIllegalCharacters($SelectedCode) or mb_strpos($SelectedCode, ' ') > 0)) {
	$InputError = 1;
	echo prnMsg(_('The petty cash tab type should not contain any of the following characters " \' - &amp; or a space'), 'error');
}
...
if (isset($_POST['submit'])) {
	...
	if ($InputError != 1) {
		...
		if ($CheckRow[0] > 0) {
			$InputError = 1;
			echo prnMsg(_('The Expense') . ' ' . $_POST['codeexpense'] . ' ' . _('already exists in this Tab Type'), 'error');
		} else {
	...
	echo prnMsg(_('Expense code') . ' ' . $SelectedCode . ' ' . _('for Tab Type') . ' ' . $SelectedTab . ' ' . _('has been deleted'), 'success');
	unset($_GET['delete']);
}
...
<div class="form-group"> <label class="col-md-8 control-label">', _('Tab Type'), '</label>
			<select required="required" class="form-control" name="SelectedTab">';
...
if (isset($_POST['process']) or isset($SelectedTab)) {
	echo '<div class="row"><div class="col-xs-4">
			<a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '" class="btn btn-default">', _('Back to Tab Type '), ' ',  '</a>
		</div></div><br />
';
	echo '<form method="post" action="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '">';
	echo '<input type="hidden" name="FormID" value="', $_SESSION['FormID'], '" />';
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>', _('Expense Codes for Tab Type: '), ' ', $SelectedTab, '</h3></div>
...
<table id="general-table" class="table table-bordered">
	...
	while ($MyRow = DB_fetch_array($Result)) {
		echo '<tr class="striped_row">
			...
			<td><a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '?SelectedCode=', $MyRow['codeexpense'], '&amp;delete=yes&amp;SelectedTab=', $SelectedTab, '" onclick="return confirm(\'' . _('Are you sure you wish to delete this expense code?') . '\', \'Confirm Delete\', this);" class="btn btn-danger">' . _('Delete') . '</a></td>
		</tr>';
	}
	//END WHILE LIST LOOP
...
';
	if (!isset($_GET['delete'])) {
		...
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Expense Code To Add'), '</label>
