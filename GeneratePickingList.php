...
if ($_SESSION['RequirePickingNote'] == 0) {
	...
	echo prnMsg(_('The system is not configured for picking lists. A configuration parameter is required where picking slips are required. Please consult your system administrator.'), 'info');
...
if ((!isset($_GET['TransNo']) or $_GET['TransNo'] == '') and !isset($_POST['TransDate'])) {
	...
	echo '<br /><form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '" method="post" name="form">
		...
		<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Date of Delivery') . '  ' . '</label>
			<input type="text" required="required" autofocus="autofocus" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="TransDate" maxlength="10" size="11" value="' . date($_SESSION['DefaultDateFormat'], mktime(date('m'), date('Y'), date('d') + 1)) . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('From Location') . '  ' . '</label>
			<select required="required" name="loccode" class="form-control">';
...
<div class="form-group"><br />
			<input type="submit" class="btn btn-success" name="Process" value="' . _('Print Picking Lists') . '" />
		</div></div></div><br />
...
if (isset($_POST['TransDate']) or (isset($_GET['TransNo']) and $_GET['TransNo'] != 'Preview')) {
	...
	if (DB_num_rows($Result) == 0) {
		...
		include('includes/header.php');
		echo '<br />';
		echo prnMsg(_('Unable to Locate any orders for this criteria '), 'info');
		echo '<br />
			<div class="row">
<div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '" class="btn btn-info">' . _('Enter Another Date') . '</a></div>
			</div>
...
if ($ListCount == 0) {
	$Title = _('Print Picking List Error');
	include('includes/header.php');
	echo prnMsg( _('There are no picking lists to print'), 'error');
	include('includes/footer.php');
	exit;
} else {
