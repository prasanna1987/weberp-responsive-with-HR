...
prnMsg( _('The activity amount is not numeric and you elected to print customer relative to a certain amount of activity') . ' - ' . _('this level of activity must be specified in the local currency') .'.', 'error');
	...
	if(DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
	  echo   
 prnMsg( _('The customer List could not be retrieved by the SQL because') . ' - ' . DB_error_msg() ),'</div>';
	...
	if(DB_num_rows($CustomersResult) == 0) {
	  ...
	  include('includes/header.php');
	  echo  
prnMsg( _('This report has no output because there were no customers retrieved'), 'error' ),'</div>';
	  echo '<p align="right"><a href="' . $RootPath . '/index.php" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
	...
	$pdf->OutputD('nERP' . '_CustomerList_' . date('Y-m-d').'.pdf');//UldisN
	$pdf->__destruct();
	exit;
...
<select name="Areas[]" multiple="multiple" class="form-control">';
	...
	echo '<div class="col-xs-3">
        <div class="form-group"> <label class="col-md-8 control-label">
' . _('For Salesperson:') . '</label>
			<select name="SalesPeople[]" multiple="multiple" class="form-control">
...
<input type="text" class="form-control" name="ActivityAmount" size="8" maxlength="8" value="0" /></div>
		</div></div>';
