...
if ((isset($_GET['Type']) AND $_GET['Type']=='Receipts')
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1> ' .// Icon title.
		_('Bank Account Matching - Receipts') . '</h1></a></div>';// Page title.
	...
	$TypeName =_('Payments');
	echo '<div class="block-header"><a href="" class="header-title-link"><h1> ' .// Icon title.
		_('Bank Account Matching - Payments') . '</h1></a></div>';// Page title.

} else {

	echo prnMsg(_('This page must be called with a bank transaction type') . '. ' . _('It should not be called directly'),'error');
...
echo '<div class="text-info">' . _('Match nERP Receipts and Payments to your Bank Statement.  Check your bank statement and click the check-box when you find the matching transaction.') . '</div><br />';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Bank Account') . '</label>
			<select tabindex="1" autofocus="autofocus" name="BankAccount" class="form-control">';
...
<div class="form-group has-error"> <label class="col-md-8 control-label"> ' . $TypeName . ' ' . _('from') . '</label>
		<input tabindex="3" type="text" name="AfterDate" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" size="12" maxlength="10" required="required" onchange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')" value="' . $_POST['AfterDate'] . '" /></div>
	</div>';

echo '<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label"> ' . $TypeName . ' ' . _('to') . '</label>
		<input tabindex="2" type="text" name="BeforeDate" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" size="12" maxlength="10" required="required" onchange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')" value="' . $_POST['BeforeDate'] . '" /></div>
	</div></div>';
echo '<div class="row">
		<div class="col-xs-6">
<div class="form-group"> <label class="col-md-12 control-label"> ' . $TypeName . ' ' . _('Type') . ' </label>
		<select tabindex="4" name="Ostg_or_All" class="form-control">';

if ($_POST['Ostg_or_All']=='All'){
	echo '<option selected="selected" value="All">' . _('All') . ' ' . $TypeName . ' ' . _('in the date range') . '</option>';
	echo '<option value="Ostdg">' . _('Unmatched') . ' ' . $TypeName . ' ' . _('only') . '</option>';
} else {
	echo '<option value="All">' . _('All') . ' ' . $TypeName . ' ' . _('in the date range') . '</option>';
	echo '<option selected="selected" value="Ostdg">' . _('Unmatched') . ' ' . $TypeName . ' ' . _('only') . '</option>';
}
echo '</select></div>
	</div>';

echo '<div class="col-xs-6">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Display') . ' ' . $TypeName . ' </label>
	<select tabindex="5" name="First20_or_All" class="form-control">';
if ($_POST['First20_or_All']=='All'){
	echo '<option selected="selected" value="All">' . _('All') . ' ' . $TypeName . ' ' . _('in the date range') . '</option>';
	echo '<option value="First20">' . _('Only the first 20') . ' ' . $TypeName . '</option>';
} else {
	echo '<option value="All">' . _('All') . ' ' . $TypeName . ' ' . _('in the date range') . '</option>';
	echo '<option selected="selected" value="First20">' . _('Only the first 20') . ' ' . $TypeName . '</option>';
}
...
echo '</div>
	...
	<div class="row">
	<div class="col-xs-4">
		<input tabindex="6" type="submit" class="btn btn-info" name="ShowTransactions" value="' . _('Show') . ' " /></div>

';

if (isset($_POST['BankAccount'])) {
   echo '
	<div class="col-xs-4"><a href="' . $RootPath . '/BankReconciliation.php?Account=' . $_POST['BankAccount'] . '" class="btn btn-info">' . _('Show reconciliation') . '</a></div>';
}
echo '</div><br />';
...
if (!Is_Date($_POST['BeforeDate'])){
	$InputError =1;
	echo prnMsg(_('The date entered for the field to show') . ' ' . $TypeName . ' ' . _('before') . ', ' .
...
if (!Is_Date($_POST['AfterDate'])){
	$InputError =1;
	echo prnMsg( _('The date entered for the field to show') . ' ' . $Type . ' ' . _('after') . ', ' .
		_('is not entered in a recognised date format') . '. ' . _('Entry is expected in the format') . ' ' .
		$_SESSION['DefaultDateFormat'],'error');
}
...
<table id="general-table" class="table table-bordered">
	...
	while ($myrow=DB_fetch_array($PaymentsResult)) {
		...
		if (ABS($Outstanding)<0.009){ /*the payment is cleared dont show the check box*/
			...
			printf('<tr class="striped_row">
					...
					<td colspan="2"><input type="text" maxlength="15" size="15" class="form-control" name="AmtClear_%s" /></td>
				</tr>',
					$myrow['ref'],
					$myrow['banktranstype'],
					$DisplayTranDate,
	...
	echo '</tbody>
			...
			<div class="col-xs-4">
				...
				<input type="submit" class="btn btn-info" name="Update" value="' . _('Update Matching') . '" />
			</div></div><br />
';
}
