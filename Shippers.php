...
if ( isset($_POST['submit']) ) {
	...
	if (mb_strlen($_POST['ShipperName']) >40) {
		$InputError = 1;
		echo prnMsg( _('The shipper\'s name must be forty characters or less'), 'error');
		$Errors[$i] = 'ShipperName';
		$i++;
	} elseif( trim($_POST['ShipperName']) == '' ) {
		$InputError = 1;
		echo prnMsg( _('The shipper\'s name may not be empty'), 'error');
		$Errors[$i] = 'ShipperName';
		$i++;
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'SalesOrders'
	...
	if ($myrow[0]>0) {
		$CancelDelete = 1;
		echo '<br />';
		echo prnMsg( _('Cannot delete this shipper because sales orders have been created using this shipper') . '. ' . _('There are'). ' '.
		...
		if ($myrow[0]>0) {
			$CancelDelete = 1;
			echo '<br />';
			echo prnMsg( _('Cannot delete this shipper because invoices have been created using this shipping company') . '. ' . _('There are').  ' ' .
			...
			if ($_SESSION['Default_Shipper']==$SelectedShipper) {
				...
				$CancelDelete = 1;
				echo '<br />';
				echo prnMsg( _('Cannot delete this shipper because it is defined as the default shipping company in the configuration file'), 'error');
				...
				$result = DB_query($sql);
				echo '<br />';
				echo prnMsg( _('The shipper record has been deleted'), 'success');;
			}
		}
	}
	unset($SelectedShipper);
...
<table id="general-table" class="table table-bordered">
	...
	while ($myrow = DB_fetch_array($result)) {
		printf('<tr class="striped_row">
			<td>%s</td>
			<td>%s</td>
			<td><a href="%sSelectedShipper=%s" class="btn btn-info">' .  _('Edit') . '</a></td>
			<td><a href="%sSelectedShipper=%s&amp;delete=1" class="btn btn-danger" onclick="return confirm(\'' . _('Are you sure you wish to delete this shipper?') . '\');">' .  _('Delete'). '</a></td>
			</tr>',
			$myrow[0],
			$myrow[1],
...
<div class="col-xs-12">';
	...
	if (isset($SelectedShipper)) {
		...
		echo '
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' .  _('Shipper Code').'</label>' . $_POST['Shipper_ID'] . '</div></div>';
	} else {
		echo '
			';
	}
	...
	echo '
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' .  _('Shipper Name') .'</label>
			
				<input type="text" class="form-control" name="ShipperName"'. (in_array('ShipperName',$Errors) ? 'class="inputerror"' : '' ) . ' value="'. $_POST['ShipperName'] .'" size="35" maxlength="40" />
			</div>
		</div>
