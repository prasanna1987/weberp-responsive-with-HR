...
echo '<div class="block-header"><a href="" class="header-title-link"><h1> ',// Icon title.
	_('Location Maintenance'), '</h1></a></div>';// Page title.
...
if(isset($_POST['submit'])) {
	...
	if(trim($_POST['LocCode']) == '') {
		$InputError = 1;
		echo prnMsg(_('The location code may not be empty'), 'error');
	}
	if($_POST['CashSaleCustomer']!='') {

		if($_POST['CashSaleBranch']=='') {
			echo prnMsg(_('A cash sale customer and branch are necessary to fully setup the counter sales functionality'),'error');
			$InputError =1;
			...
			if(DB_num_rows($result)==0) {
				$InputError = 1;
				echo prnMsg(_('The cash sale customer for this location must be defined with both a valid customer code and a valid branch code for this customer'),'error');
			}
		}
	...
	if(isset($SelectedLocation) AND $InputError !=1) {
		...
		echo prnMsg(_('The location record has been updated'),'success');
		unset($_POST['LocCode']);
		...
		echo prnMsg(_('The new location record has been added and new stock locations inserted for all existing stock items for the new location. Existing users have been authorized for this location'),'success');
...
// PREVENT DELETES IF DEPENDENT RECORDS
	...
	if($myrow[0]>0) {
		$CancelDelete = 1;
		echo prnMsg(_('Cannot delete this location because sales orders have been created delivering from this location <br />There are ' . $myrow[0] . ' sales orders with this Location code'),'warn');
		...
		if($myrow[0]>0) {
			$CancelDelete = 1;
			echo prnMsg(_('Cannot delete this location because stock movements have been created using this location'),'warn');
			...
			if($myrow[0]>0) {
				$CancelDelete = 1;
				echo prnMsg(_('Cannot delete this location because location stock records exist that use this location and have a quantity on hand greater than 0'),'warn');
				...
				if($myrow[0]>0) {
					$CancelDelete = 1;
					echo prnMsg(_('Cannot delete this location because it is the default location for a user') . '. ' . _('The user record must be modified first'),'warn');
					...
					if($myrow[0]>0) {
						$CancelDelete = 1;
						echo prnMsg(_('Cannot delete this location because it is the default location for a bill of material') . '. ' . _('The bill of materials must be modified first'),'warn');
						...
						if($myrow[0]>0) {
							$CancelDelete = 1;
							echo prnMsg(_('Cannot delete this location because it is used by some work centre records'),'warn');
							...
							if($myrow[0]>0) {
								$CancelDelete = 1;
								echo prnMsg(_('Cannot delete this location because it is used by some work order records'),'warn');
								...
								if($myrow[0]>0) {
									$CancelDelete = 1;
									echo prnMsg(_('Cannot delete this location because it is used by some branch records as the default location to deliver from'),'warn');
									...
									if($myrow[0]>0) {
										$CancelDelete = 1;
										echo prnMsg(_('Cannot delete this location because it is used by some purchase order records as the location to receive stock into'),'warn');
	...
	if(! $CancelDelete) {
		...
		echo prnMsg(_('Location') . ' ' . $SelectedLocation . ' ' . _('has been deleted') . '!', 'success');
		unset ($SelectedLocation);
...
or deletion of the records*/
	...
	if(DB_num_rows($result)==0) {
		echo prnMsg(_('There are no locations that match up with a tax province record to display. Check that tax provinces are set up for all dispatch locations'),'error');
	}
...
*/
	printf('<tr class="striped_row">
			...
			<td class="noprint"><a href="%sSelectedLocation=%s" class="btn btn-info">' . _('Edit') . '</a></td>
			<td class="noprint"><a href="%sSelectedLocation=%s&amp;delete=1" class="btn btn-danger" onclick="return confirm(\'' . _('Are you sure you wish to delete this inventory location?') . '\');">' . _('Delete') . '</a></td>
			</tr>',
			$myrow['loccode'],
			$myrow['locationname'],
			$myrow['description'],
...
if(isset($SelectedLocation)) {
	echo '<div class="row">
<div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-default">' . _('Back To Locations List') . '</a></div></div>';
}
echo '<br />';
...
echo '<div class="block">';
	if(isset($SelectedLocation)) {
		...
		echo '<div class="block-title">
				<h3' . _('Amend Location details') . '</h3></div>
			';
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Location Code') . '</label>
				' . $_POST['LocCode'] . '</div>
			</div>';
		...
		echo '<div class="block-title">
				<h3' . _('New Location details') . '</h3></div>
				';
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Location Code') . '</label>
				<input type="text" class="form-control" autofocus="autofocus" required="required" title="' . _('Enter up to five characters for the inventory location code') . '" data-type="no-illegal-chars" name="LocCode" value="' . $_POST['LocCode'] . '" size="5" maxlength="5" /></div>
			</div>';
	}
...
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Location Name') . '' . '</label>
			<input type="text" name="LocationName" class="form-control" required="required" value="'. $_POST['LocationName'] . '" title="' . _('Enter the inventory location name this could be either a warehouse or a factory') . '" namesize="51" maxlength="50" /></div></div>
			
		<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Contact for deliveries') . '' . '</label>
			<input type="text" class="form-control" name="Contact" required="required" value="' . $_POST['Contact'] . '" title="' . _('Enter the name of the responsible person to contact for this inventory location') . '" size="31" maxlength="30" /></div></div></div>
			
		<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Delivery Address 1 (Building)') . '' . '</label>
			<input type="text" name="DelAdd1" class="form-control" value="' . $_POST['DelAdd1'] . '" size="41" maxlength="40" /></div></div>
			
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Delivery Address 2 (Street)') . '' . '</label>
			<input type="text" class="form-control" name="DelAdd2" value="' . $_POST['DelAdd2'] . '" size="41" maxlength="40" /></div></div>
			
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Delivery Address 3 (Suburb)') . '' . '</label>
			<input type="text" name="DelAdd3" class="form-control" value="' . $_POST['DelAdd3'] . '" size="41" maxlength="40" /></div></div>
			</div>
			
		<div class="row">	
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Delivery Address 4 (City)') . '' . '</label>
			<input type="text" class="form-control" name="DelAdd4" value="' . $_POST['DelAdd4'] . '" size="41" maxlength="40" /></div></div>
			
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Delivery Address 5 (Zip Code)') . '' . '</label>
			<input type="text" class="form-control" name="DelAdd5" value="' . $_POST['DelAdd5'] . '" size="21" maxlength="20" /></div></div>
			
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Country') . '</label>
			<select name="DelAdd6" class="form-control">';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Telephone No') . '' . '</label>
			<input name="Tel" type="tel" class="form-control" pattern="[0-9+\-\s()]*" value="' . $_POST['Tel'] . '" size="31" maxlength="30" title="' . _('The phone number should consist of numbers, spaces, parentheses, or the + character') . '" /></div></div>
			
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Fax No') . '' . '</label>
			<input name="Fax" class="form-control" type="tel" pattern="[0-9+\-\s()]*" value="' . $_POST['Fax'] . '" size="31" maxlength="30" title="' . _('The fax number should consist of numbers, parentheses, spaces or the + character') . '"/></div></div>';
			
		// Email address:
		echo '
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label"><label for="Email">', _('Email'), ':</label></td>
			<input id="Email" class="form-control" maxlength="55" name="Email" size="31" type="email" value="', $_POST['Email'], '" /></div></div></div>';
		// Tax Province:
		echo '<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Tax Province') . '' . '</label>
			<select name="TaxProvince" class="form-control">';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Allow internal requests?') . '</label>
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Use for Work Order Productions?') . '</label>
			<select name="UsedForWO" class="form-control">';
	...
	echo '
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">', _('GL Account Code - leave it in blank if not needed'), '</label>
			<input data-type="no-illegal-chars" id="GLAccountCode" class="form-control" maxlength="20" name="GLAccountCode" size="20" type="text" value="', $_POST['GLAccountCode'], '" /></div></div></div>';
	// Allow or deny the invoicing of items in this location:
	echo '<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">', _('Allow Invoicing from this location?'), '</label>
			<select name="AllowInvoicing" class="form-control">
	...
	<tr><?php echo _('Enable Warehouse Management') . ':'; ?></td>
	<input type='checkbox' name='Managed'<?php if($_POST['Managed'] == 1) echo ' checked';?>></td></tr>
	*/
	echo '
		<div class="col-xs-4">
