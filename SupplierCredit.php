...
/*Now retrieve supplier information - name, currency, default ex rate, terms, tax rate etc */
	...
	echo prnMsg(_('To enter a supplier credit note the supplier must first be selected from the supplier selection screen'),'warn');
	echo '<br /><p align="center"><a href="' . $RootPath . '/SelectSupplier.php" class="btn btn-default">' . _('Select A Supplier to Enter an Credit Note For') . '</a></p>';
	include('includes/footer.php');
	exit;
...
if (isset($_POST['GRNS'])
	...
	echo '<br /><p class="text-info">' .
		_('You should automatically be forwarded to the entry of credit notes against goods received page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/SuppCreditGRNs.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.
		</p><br />';
	include('includes/footer.php');
	exit;
}
if (isset($_POST['Shipts'])){
	...
	echo '<br /><p class="text-info">
		' . _('You should automatically be forwarded to the entry of credit notes against shipments page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/SuppShiptChgs.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.
		</p><br />';
	include('includes/footer.php');
	exit;
}
if (isset($_POST['GL'])
	...
	echo '<br /><p class="text-info">
		' . _('You should automatically be forwarded to the entry of credit notes against the general ledger page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/SuppTransGLAnalysis.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.
		</p><br />';
	include('includes/footer.php');
	exit;
}
if (isset($_POST['Contracts'])
	AND $_POST['Contracts'] == _('Contracts')){
		...
		echo '<p class="text-info">
				' . _('You should automatically be forwarded to the entry of supplier credit notes against contracts page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh'). ') ' . '<a href="' . $RootPath . '/SuppContractChgs.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.
			</p>
			<br />';
		exit;
}
if (isset($_POST['FixedAssets'])
	AND $_POST['FixedAssets'] == _('Fixed Assets')){
		...
		echo '<p class="text-info">
				' . _('You should automatically be forwarded to the entry of invoices against fixed assets page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh'). ') ' . '<a href="' . $RootPath . '/SuppFixedAssetChgs.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.
			</p>
			<br />';
		exit;
}
...
echo '<tr>
		<th>' . $_SESSION['SuppTrans']->SupplierID . ' - ' . $_SESSION['SuppTrans']->SupplierName . '</th>
		<th>' .  $_SESSION['SuppTrans']->CurrCode . '</th>
...
echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Supplier Credit Note Reference') . '</label>
		<td><input type="text" required="required" class="form-control" size="20" maxlength="20" name="SuppReference" value="' . $_SESSION['SuppTrans']->SuppReference . '" /></div></div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Credit Note Date') . ' (' . _('in format') . ' ' . $_SESSION['DefaultDateFormat'] . ') </label>
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Exchange Rate') . '</label>
		<input type="text" class="form-control" size="11" maxlength="10" name="ExRate" value="' . locale_number_format($_SESSION['SuppTrans']->ExRate,'Variable') . '" /></div>
	</div>
	</div><br />
';
...
if (count($_SESSION['SuppTrans']->GRNs)>0){   /*if there are some GRNs selected for crediting then */
	...
	echo '<div class="sub-header"></div><div class="row gutter30">
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Purchase Order Details') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
	...
	<tr>
					...
					<th>' . _('Line Total in') . ' ' . $_SESSION['SuppTrans']->CurrCode . '</th>
				</tr></thead>';
	echo $TableHeader;
	$TotalGRNValue=0;
	...
	echo '<tr><td colspan="5" class="number">' . _('<strong>Total Value of Goods Credited</strong>') . ':</td>
		<td class="number">' . locale_number_format($TotalGRNValue,$_SESSION['SuppTrans']->CurrDecimalPlaces) . '</td></tr>';
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Shipment Credits') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
				<tr>
					<th colspan="2"></th>
...
if (count( $_SESSION['SuppTrans']->Assets) > 0){   /*if there are any fixed assets on the invoice*/
	...
	echo '<br />
		<div class="sub-header"></div><div class="row gutter30">
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Fixed Asset Credits') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
		';
	$TableHeader = '<thead>
...
<table id="general-table" class="table table-bordered">
	...
	<tr><th>' . _('Contract') . '</th>
						<th>' . _('Narrative') . '</th>
						<th>' . _('Amount in') . ' ' . $_SESSION['SuppTrans']->CurrCode . '</th>
					</tr></thead>';
	echo $TableHeader;
...
<table id="general-table" class="table table-bordered">
			<tr>
				<td><strong>' . _('Credit Amount in Supplier Currency') . ':</strong></td>
	...
	echo '<table class="selection">
			<tr>
				<td><strong>' . _('Credit Amount in Supplier Currency') .
		  ':</strong></td>
		  	<td colspan="2" class="number"><input type="text" size="12" class="form-control" maxlength="10" name="OvAmount" value="' . locale_number_format($_SESSION['SuppTrans']->OvAmount,$_SESSION['SuppTrans']->CurrDecimalPlaces) . '" /></td></tr>';
}

echo '<tr>
		<td><strong>Tax Calculaions</strong></td>
...
foreach ($_SESSION['SuppTrans']->Taxes as $Tax) {
	...
	echo '<tr>
			<td><strong>'  . $Tax->TaxAuthDescription . '</strong></td>
			<td>';
	...
	if (!isset($_POST['OverRideTax'])
		...
		echo  ' <input type="text" class="form-control" name="TaxRate' . $Tax->TaxCalculationOrder . '" maxlength="4" size="4" value="' . locale_number_format($_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * 100,2) . '" />%';
		...
		echo '' . locale_number_format($_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount,$_SESSION['SuppTrans']->CurrDecimalPlaces);
		...
		echo '<input type="text" class="form-control" size="12" maxlength="12" name="TaxAmount'  . $Tax->TaxCalculationOrder . '"  value="' . locale_number_format(round($_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount,$_SESSION['SuppTrans']->CurrDecimalPlaces),$_SESSION['SuppTrans']->CurrDecimalPlaces) . '" />';
...
echo '<tr>
		<td><strong>' . _('Credit Note Total') . '</strong></td>
...
<div class="col-xs-4">
<div class="form-group"><br />
			<input type="submit" class="btn btn-success" name="PostCreditNote" value="' . _('Enter Credit Note') . '" />
		</div></div></div>';
...
then do the updates and inserts to process the credit note entered */
	...
	if ( $TaxTotal + $_SESSION['SuppTrans']->OvAmount <= 0){
		$InputError = True;
		echo prnMsg(_('The credit note as entered cannot be processed because the total amount of the credit note is less than or equal to 0') . '. ' . 	_('Credit notes are expected to be entered as positive amounts to credit'),'warn');
	} elseif (mb_strlen($_SESSION['SuppTrans']->SuppReference) < 1){
		$InputError = True;
		echo prnMsg(_('The credit note as entered cannot be processed because the there is no suppliers credit note number or reference entered') . '. ' . _('The supplier credit note number must be entered'),'error');
	} elseif (!Is_Date($_SESSION['SuppTrans']->TranDate)){
		$InputError = True;
		echo prnMsg(_('The credit note as entered cannot be processed because the date entered is not in the format') . ' ' . $_SESSION['DefaultDateFormat'], 'error');
	} elseif (DateDiff(Date($_SESSION['DefaultDateFormat']), $_SESSION['SuppTrans']->TranDate, 'd') < 0){
		$InputError = True;
		echo prnMsg(_('The credit note as entered cannot be processed because the date is after today') . '. ' . _('Purchase credit notes are expected to have a date prior to or today'),'error');
	}elseif ($_SESSION['SuppTrans']->ExRate <= 0){
		$InputError = True;
		echo prnMsg(_('The credit note as entered cannot be processed because the exchange rate for the credit note has been entered as a negative or zero number') . '. ' . _('The exchange rate is expected to show how many of the suppliers currency there are in 1 of the local currency'),'warn');
		...
		echo prnMsg(_('The credit note total as entered is less than the sum of the shipment charges') . ', ' . _('the general ledger entries (if any) and the charges for goods received') . '. ' . _('There must be a mistake somewhere') . ', ' . _('the credit note as entered will not be processed'),'error');
		$InputError = True;
	} else {
		...
		/*Loop through the GL Entries and create a debit posting for each of the accounts entered */
			...
			*/
			...
			if ($debug == 1 AND abs(($_SESSION['SuppTrans']->OvAmount/ $_SESSION['SuppTrans']->ExRate) - $LocalTotal)>0.004){
				echo prnMsg(_('The total posted to the credit accounts is') . ' ' . $LocalTotal . ' ' . _('but the sum of OvAmount converted at ExRate') . ' = ' . ($_SESSION['SuppTrans']->OvAmount / $_SESSION['SuppTrans']->ExRate),'error');
			}
	...
	/*Now insert the credit note into the SuppTrans table*/
		...
		echo prnMsg(_('Supplier credit note number') . ' ' . $CreditNoteNo . ' ' . _('has been processed'),'success');
		echo '<br /><p align="center"><a href="' . $RootPath . '/SupplierCredit.php?&SupplierID=' .$_SESSION['SuppTrans']->SupplierID . '" class="btn btn-info">' . _('Enter another Credit Note for this Supplier') . '</a></p>';
		unset($_SESSION['SuppTrans']->GRNs);
		unset($_SESSION['SuppTrans']->Shipts);
		unset($_SESSION['SuppTrans']->GLCodes);
		unset($_SESSION['SuppTrans']);
