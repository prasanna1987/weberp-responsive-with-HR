...
Class Cart {
	...
	function remove_from_cart($LineNumber, $UpdateDB='No', $identifier=0){
		...
		if (!isset($LineNumber) OR $LineNumber=='' OR $LineNumber < 0){ /* over check it */
			echo prnMsg(_('No Line Number passed to remove_from_cart, so nothing has been removed.'), 'error');
			return;
		}
		if ($UpdateDB=='Yes'){
			if ($this->Some_Already_Delivered($LineNumber)==0){
				...
				echo prnMsg( _('Deleted Line Number'). ' ' . $LineNumber . ' ' . _('from existing Order Number').' ' . $_SESSION['ExistingOrder' . $identifier], 'success');
			} else {
				...
				echo prnMsg(_('Removed Remaining Quantity and set Line Number '). ' ' . $LineNumber . ' ' . _('as Completed for existing Order Number').' ' . $_SESSION['ExistingOrder'], 'success');
			}
		}
	...
	function GetTaxes($LineNumber){
		...
		if (DB_num_rows($GetTaxRatesResult)==0){
			echo prnMsg(_('It appears that taxes are not defined correctly for this customer tax group') ,'error');
		} else {
			$i=1;
			while ($myrow = DB_fetch_array($GetTaxRatesResult)){
	...
	function GetFreightTaxes () {
		...
		if ($TaxCatRow = DB_fetch_array($TaxCatQuery)) {
		  $TaxCatID = $TaxCatRow['taxcatid'];
  		  ...
  		  echo prnMsg( _('Cannot find tax category Freight which must always be defined'),'error');
		  exit();
		}
