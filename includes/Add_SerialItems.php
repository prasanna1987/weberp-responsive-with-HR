		...
		if(isset($_POST['SerialNo' . $i]) AND mb_strlen($_POST['SerialNo' . $i])>0){
			...
			if(is_numeric(filter_number_format($_POST['Qty'.$i]))){
				if(strlen(substr(strrchr(filter_number_format($_POST['Qty'.$i]), "."), 1))>$DecimalPlaces){
					echo '<br />';
					echo prnMsg(_('Please correct input quantity ').' '.$_POST['Qty'.$i].' '._('to').' '.$DecimalPlaces.' '._('decimalplaces'),'error');
					$AddThisBundle = false;
					$SerialError = true;
				...
				echo '<br />';
				echo prnMsg(_('The input quantity must be numeric'),'error');
				$AddThisBundle = false;
				$SerialError = true;
				unset($LineItem->SerialItems[$_POST['SerialNo'.$i]]);
			}
			...
			if ($ItemMustExist){
				...
				if ($ExistingBundleQty >0 OR ($ExistingBundleQty==1 and $IsCredit=true)){
					...
					if (((filter_number_format($_POST['Qty' . $i]))*$InOutModifier) > $ExistingBundleQty){
						if ($LineItem->Serialised ==1){
							echo '<br />';
							echo  prnMsg ( $_POST['SerialNo' . $i] . ' ' .
								 _('has already been sold'),'warn' ),'</div>';
							$AddThisBundle = false;
						} elseif ($ExistingBundleQty==0) { /* and its a batch */
							echo '<br />';
							echo  prnMsg ( _('There is none of') . ' '. $_POST['SerialNo' . $i] .
								' '. _('remaining').'.', 'warn');
							$AddThisBundle = false;
						} else {
							echo '<br />';
						 	echo  prnMsg (  _('There is only'). ' ' . $ExistingBundleQty .
        	        ...
        	        echo '<br />';
	                echo prnMsg( '<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($_POST['SerialNo'. $i]) . '" target=_blank>' . $_POST['SerialNo'. $i]. '</a> ' ._('not available') . '...' , '', 'info' );
				...
				if ($LineItem->Serialised){
					...
					if ($NewQty == 1 AND $ExistingQty != 0){
						echo prnMsg( '<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target=_blank>' . $NewSerialNo. '</a> : '. _('The Serial Number being added exists with a Quantity that is not Zero (0)!'), 'error' ),'</div>';
						$SerialError = true;
					} elseif ($NewQty == -1 AND $ExistingQty != 1){
						echo prnMsg( '<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target=_blank>' . $NewSerialNo. '</a> : '. _('The Serial Number being removed exists with a Quantity that is not One (1)!'), 'error');
						$SerialError = true;
					}
				}
...
if ( isset($_POST['AddSequence']) AND $_POST['AddSequence']!='') {
	...
	$EndNo   = $_POST['EndNo'];
	if ($BeginNo > $EndNo){
		echo prnMsg( _('To Add Items Sequentially, the Begin Number must be less than the End Number'), 'error');
	} else {
...
' . '<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a> '. _('has already been sold'). '.';
									$AddThisBundle = false;
								} elseif ($ExistingBundleQty==0) { /* and its a batch */
									echo '' . _('There is none of'). ' <a href="/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a> '. _('remaining') .'</p></div>';
									$AddThisBundle = false;
							} else {
									echo '<div class="row"><p class="text-info">' .  _('There is only') . ' ' . $ExistingBundleQty . ' '. _('of') . ' ' .
												'<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a> ' . _('remaining') . '. '.
												_('The entered quantity will be reduced to the remaining amount left of this batch/bundle/roll'),'</p></div>';
				...
				echo '<br />';
				echo  prnMsg( '<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a>  ' . _('not available') ,'', 'Notice' ),'</div>';
			}
			...
			if ($LineItem->Serialised){
				$ExistingQty = ValidBundleRef($StockID, $LocationOut, $NewSerialNo);
				if ($NewQty == 1 AND $ExistingQty != 0){
					echo prnMsg( '<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a>: '. _('The Serial Number being added exists with a Quantity that is not Zero (0)!'), 'error' ),'</div>';
					$SerialError = true;
				} elseif ($NewQty == -1 AND $ExistingQty != 1){
					echo prnMsg( '<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a> : '. _('The Serial Number being removed exists with a Quantity that is not One (1)!'), 'error');
					$SerialError = true;
				}
			}
			if (!$SerialError){
...
if (isset($_GET['REVALIDATE']) || isset($_POST['REVALIDATE'])) {
		...
		if ($ItemMustExist){
			...
			if ($ExistingBundleQty >0){
				$AddThisBundle = true;
					...
					if ($NewQty > $ExistingBundleQty){
						if ($LineItem->Serialised ==1){
								echo '<div class="row"><p class="text-info">' . '<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a> '. _('has already been sold'). '.';
								$AddThisBundle = false;
							} elseif ($ExistingBundleQty==0) { /* and its a batch */
								echo '' . _('There is none of'). ' <a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a> '. _('remaining') .'</p></div>';
								$AddThisBundle = false;
						} else {
								echo '<div class="row"><p class="text-info">' .  _('There is only') . ' ' . $ExistingBundleQty . ' '. _('of') . ' ' .
											'<a href="'.$RootPath.'/StockSerialItemResearch.php?serialno='. urlencode($NewSerialNo) . '" target="_blank" class="btn btn-info">' . $NewSerialNo. '</a> ' . _('remaining') . '. '.
	...
	if ($invalid_imports > 0){
		echo  prnMsg( _('Finished Validating Items') . ' : ' . $invalid_imports . ' ' . _('problems found. Please research and correct them') . '.', 'warn');
	} else {
		echo prnMsg( _('Finished Validating Items').' with NO errors', 'success' );
	}
