...
Class Offer {
		...
		if ($Update=='') {
			foreach ($this->LineItems as $LineItems) {
				if ($LineItems->Deleted==False) {
					...
					if (DB_error_no()==0) {
						echo prnMsg( _('The offer for').' '.$LineItems->StockID.' '._('has been inserted into the database'), 'success');
						...
						echo prnMsg( _('The offer for').' '.$LineItems->StockID.' '._('could not be inserted into the database'), 'error');
			...
			foreach ($this->LineItems as $LineItem) {
				if ($LineItem->Deleted==false){ //Update only the LineItems which is not flagged as deleted
					...
					$result = DB_query($sql,$ErrMsg,$DbgMsg,true);
					if (DB_error_no()==0) {
						echo prnMsg( _('The offer for').' '.$LineItem->StockID.' '._('has been updated in the database'), 'success');
						$this->OfferMailText .= $LineItem->Quantity.' '.$LineItem->Units.' '._('of').' '.$LineItem->StockID.' '._('at a price of').
							' '.$this->CurrCode.$LineItem->Price."\n";
					} else {
						echo prnMsg( _('The offer for').' '.$LineItem->StockID.' '._('could not be updated in the database'), 'error');
						include('includes/footer.php');
						exit;
					}
					...
					$result = DB_query($sql,$ErrMsg,$DbgMsg,true);
					if (DB_error_no() == 0) {
						echo prnMsg(_('The offer for').' '.$LineItem->StockID.' ' ._('has been deleted in the database'), 'info');
	...
	function EmailOffer() {
		...
		$Message=(_('This email is automatically generated by nERP')."\n" .
