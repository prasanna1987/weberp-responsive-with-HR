			...
			if ($ExistingBundleQty >0){
				...
				if (filter_number_format($_POST['Qty' . $i]) > $ExistingBundleQty){
					if ($LineItem->Serialised ==1){
						echo '<p class="text-info">' . $_POST['SerialNo' . $i] . " " . _('has already been sold'),'</p>';
						$AddThisBundle = false;
					} elseif ($ExistingBundleQty==0) { /* and its a batch */
						echo '<p class="text-info">' . _('There is none of') . ' ' . $_POST['SerialNo' . $i] . ' ' . _('left'),'</p>';
						$AddThisBundle = false;
					} else {
					 	echo '<p class="text-info">' . _('There is only') . ' ' . $ExistingBundleQty . ' ' . _('of') . ' ' . $_POST['SerialNo' . $i] . ' ' . _('remaining') . '. ' . _('The entered quantity will be reduced to the remaining amount left of this batch/bundle/roll'),'</p>';
