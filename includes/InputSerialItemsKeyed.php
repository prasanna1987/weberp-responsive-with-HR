...
foreach ($LineItem->SerialItems as $Bundle){
	...
	echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Delete=' . $Bundle->BundleRef . '&amp;StockID=' . $LineItem->StockID . '&amp;LineNo=' . $LineNo .'&amp;identifier=' . $identifier . $CreditInvoice . '" class="btn btn-danger">' .  _('Delete'). '</a></td>
		</tr>';
...
if ($LineItem->Serialised==1){
	echo '<tr><td class="number"><strong>' .  _('Total Quantity'). ': ' . locale_number_format($TotalQuantity,$LineItem->DecimalPlaces) . '</strong></td></tr>';
} else {
	echo '<tr>
...
if ($EditControlled){
	foreach ($LineItem->SerialItems as $Bundle){
		...
		echo '<tr>
				<td valign="top"><input type="text" class="form-control" name="SerialNo'. $StartAddingAt .'" value="' .$Bundle->BundleRef.'" size="21"  maxlength="20" /></td>';
		...
		if ($LineItem->Serialised==1){
			...
			echo '<td><input type="text" class="form-control" name="Qty' . $StartAddingAt .'" size="11" value="'. locale_number_format($Bundle->BundleQty, $LineItem->DecimalPlaces). '" maxlength="10" /></td><td><input type="text" name="ExpiryDate' . $StartAddingAt . '" size="11" value="' . $Bundle->ExpiryDate . '" class="date" maxlength="10" /></td></tr>';
		} else {
			echo '<td><input type="text" class="form-control" name="Qty' . $StartAddingAt .'" size="11" value="'. locale_number_format($Bundle->BundleQty, $LineItem->DecimalPlaces). '" maxlength="10" /></tr>';
		}
...
for ($i=0;$i < 10;$i++){
	...
	echo '<tr>
			<td valign="top"><input type="text" class="form-control" name="SerialNo'. ($StartAddingAt+$i) .'" size="21"  maxlength="20" /></td>';
	...
	if ($LineItem->Serialised==1){
		...
		echo '<td><input type="text" class="form-control" name="Qty' . ($StartAddingAt+$i) .'" size="11"  maxlength="10" /></td>
				<td><input type="text" class="date" name="ExpiryDate' . ($StartAddingAt+$i) .'" size="11" value="" maxlength="10" /></td>
			</tr>';
	} else {
		echo '<td><input type="text" class="form-control" name="Qty' . ($StartAddingAt+$i) .'" size="11"  maxlength="10" /></td></tr>';
	}
}
...
echo '</table></div>
		...
		<div class="row" align="center">
			<input type="submit" class="btn btn-success" name="AddBatches" value="'. _('Submit'). '" />
		</div>
