...
// http://www.netelity.com/forum/showthread.php?tid=8133
session_name('PHPSESSIDnERPteam');
session_start();
...
if(basename($_SERVER['SCRIPT_NAME'])=='Logout.php'){
	...
	case  UL_CONFIGERR:
		...
		echo '<br /><br /><br />';
		prnMsg(_('Your user role does not have any access defined for nERP. There is an error in the security setup for this user account'),'error');
	...
	case  UL_MAINTENANCE:
		$demo_text = '<font size="3" color="red"><b>' .  _('system maintenance') . '</b></font><br /><b>' . _('nERP is not available right now') . '<br />' . _('during maintenance of the system') . '</b>';
		die(include($PathPrefix . 'includes/Login.php'));
...
if ($_SESSION['HTTPS_Only']==1){
	if ($_SERVER['HTTPS']!='on'){
		prnMsg(_('nERP is configured to allow only secure socket connections. Pages must be called with https://') . ' .....','error');
		exit;
	}
}
...
if (!isset($AllowAnyone)){
	if ((!in_array($PageSecurity, $_SESSION['AllowedPageSecurityTokens']) OR !isset($PageSecurity))) {
		...
		include($PathPrefix . 'includes/header.php');
		echo '<h4 class="text-danger"><strong>' . _('The security settings on your account do not permit you to access this function') . '</strong></h4>
							';
...
if (sizeof($_POST) > 0 AND !isset($AllowAnyone)) {
	/*Security check to ensure that the form submitted is originally sourced from nERP with the FormID = $_SESSION['FormID'] - which is set before the first login*/
