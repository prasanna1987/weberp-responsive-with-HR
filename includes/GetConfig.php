...
if(isset($ForceConfigReload) AND $ForceConfigReload==true OR !isset($_SESSION['CompanyDefaultsLoaded'])) {
	...
	if (DB_error_no()!=0){
		/* the table may not exist with the pagesecurity field in it if it is an older nERP database
...
/* Also reads all the company data set up in the company record and returns an array */
	...
	if (DB_num_rows($ReadCoyResult)==0) {
      		echo '<br /><b>';
		echo prnMsg( _('The company record has not yet been set up') . '</b><br />' . _('From the system setup tab select company maintenance to enter the company information and system preferences'),'error',_('CRITICAL PROBLEM'));
		exit;
	} else {
