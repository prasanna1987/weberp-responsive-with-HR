...
if (isset($_POST['update'])) {
	$fp = fopen($_FILES['ImportFile']['tmp_name'], "r");
 	...
 	while (!feof ($fp)) {
    	...
    	if ($FieldValues[0]!='') {
			...
			$msg = new xmlrpcmsg("nERP.xmlrpc_InsertStockItem", array($stockitem, $user, $password));
	...
	if (DB_num_rows($result)==0) {
		...
		prnMsg( _('Select a csv file containing the details of the parts that you wish to import into nERP. '). '<br />' .
