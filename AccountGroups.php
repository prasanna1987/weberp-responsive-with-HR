...
if(isset($_POST['MoveGroup'])) {
	...
	$result = DB_query($sql,$ErrMsg,$DbgMsg);
	echo '<div class="row"><div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '" class="btn btn-info">' . _('Review Account Groups') . '</a></div></div><br />';
	echo prnMsg( _('All accounts in the account group:') . ' ' . $_POST['OriginalAccountGroup'] . ' ' . _('have been changed to the account group:') . ' ' . $_POST['DestinyAccountGroup'],'success');
}
...
if(isset($_POST['submit'])) {
	...
	if($myrow[0] != 0 AND $_POST['SelectedAccountGroup'] == '') {
		$InputError = 1;
		echo prnMsg( _('The account group name already exists in the System'),'error');
		$Errors[$i] = 'GroupName';
		$i++;
	}
	if(ContainsIllegalCharacters($_POST['GroupName'])) {
		$InputError = 1;
		echo prnMsg( _('The account group name cannot contain the character') . " '&' " . _('or the character') ."' '",'error');
		$Errors[$i] = 'GroupName';
		$i++;
	}
	if(mb_strlen($_POST['GroupName'])==0) {
		$InputError = 1;
		echo prnMsg( _('The account group name must be at least one character long'),'error');
		$Errors[$i] = 'GroupName';
		$i++;
	}
	if($_POST['ParentGroupName'] !='') {
		if(CheckForRecursiveGroup($_POST['GroupName'],$_POST['ParentGroupName'])) {
			$InputError =1;
			echo prnMsg(_('The parent account group selected appears to result in a recursive account structure - select an alternative parent account group or make this group a top level account group'),'error');
			...
			echo prnMsg(_('Since this account group is a child group, the sequence in the Trial Balance, the section in the accounts and whether or not the account group appears in the balance sheet or profit and loss account are all properties inherited from the parent account group. Any changes made to these fields will have no effect.'),'warn');
		}
	}
	if(!ctype_digit($_POST['SectionInAccounts'])) {
		$InputError = 1;
		echo prnMsg( _('The section in accounts must be an integer'),'error');
		$Errors[$i] = 'SectionInAccounts';
		$i++;
	}
	if(!ctype_digit($_POST['SequenceInTB'])) {
		$InputError = 1;
		echo prnMsg( _('The sequence in the Trial Balance must be an integer'),'error');
		$Errors[$i] = 'SequenceInTB';
		$i++;
	}
	if(!ctype_digit($_POST['SequenceInTB']) OR $_POST['SequenceInTB'] > 10000) {
		$InputError = 1;
		echo prnMsg( _('The sequence in the Trial Balance must be numeric and less than') . ' 10,000','error');
		$Errors[$i] = 'SequenceInTB';
		$i++;
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'ChartMaster'
	...
	if($myrow['groups']>0) {
		echo prnMsg( _('Cannot delete this account group because general ledger accounts have been created using this group'),'warn');
	echo '<br />' . _('There are') . ' ' . $myrow['groups'] . ' ' . _('general ledger accounts that refer to this account group');
		...
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Parent Group') . '' . '</label>
				<select tabindex="2" ' . (in_array('ParentGroupName',$Errors) ?  'class="selecterror"' : '' ) . '  name="DestinyAccountGroup" class="form-control">';
...
<div class="form-group"><br />
				<input tabindex="6" type="submit" class="btn btn-info" name="MoveGroup" value="' . _('Move Group') . '" /></div></div>
		  </div>
		 <br />';
		...
		if($myrow['groupnames']>0) {
			echo prnMsg( _('Cannot delete this account group because it is a parent account group of other account group(s)'),'warn');
			...
			$result = DB_query($sql,$ErrMsg,$DbgMsg);
			echo prnMsg( $_GET['SelectedAccountGroup'] . ' ' . _('group has been deleted') . '!','success');
		}
...
<table id="general-table" class="table table-bordered">
	...
	while($myrow = DB_fetch_array($result)) {
		...
		echo '<td class="noprint"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'] . '?SelectedAccountGroup=' . urlencode($myrow['groupname']), ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Edit') . '</a></td>';
		echo '<td class="noprint"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'] . '?SelectedAccountGroup=' . urlencode($myrow['groupname']), ENT_QUOTES,'UTF-8') . '&amp;delete=1" onclick="return confirm(\'' . _('Are you sure you wish to delete this account group?') . '\');" class="btn btn-danger">' . _('Delete')  . '</a></td></tr>';
...
if(!isset($_GET['delete'])) {
	...
	if(isset($_GET['SelectedAccountGroup'])) {
		...
		if(DB_num_rows($result) == 0) {
			echo prnMsg( _('The account group name does not exist in the System'),'error');
...
<div class="block-title"><h3>', _('Edit Account Group Details'), '</h3></div>
		...
		echo '<br />
			<div class="block">
<div class="block-title"><h3>', _('New Account Group Details'), '</h3></div>
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Account Group Name'), '</label>
			<input autofocus="autofocus" data-type="no-illegal-chars" placeholder="' . _('Enter the account group name') . '" ' . (in_array('GroupName',$Errors) ?  '"class=inputerror" ' : '' ) . 'maxlength="30" minlength="3" name="GroupName" required="required" size="30" tabindex="1" type="text" class="form-control" value="' . $_POST['GroupName'] . '" title="' . _('A unique name for the account group must be entered - at least 3 characters long and less than 30 characters long. Only alpha numeric characters can be used.') . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Parent Group'), '</label>
			<select ',
				( in_array('ParentGroupName',$Errors) ? 'class="selecterror" ' : '' ),
				'name="ParentGroupName" tabindex="2" class="form-control">';
	echo '<option ',
	...
	echo '</select></div>
		...
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Section In Accounts'), '</label>
			<select ',
				( in_array('SectionInAccounts',$Errors) ? 'class="selecterror" ' : '' ),
				'name="SectionInAccounts" tabindex="3" class="form-control">';
	...
	echo '</select></div>
		...
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Profit and Loss'), '</label>
			<select name="PandL" tabindex="4" class="form-control" title="', _('Select YES if this account group will contain accounts that will consist of only profit and loss accounts or NO if the group will contain balance sheet account'), '">';
	if($_POST['PandL']!=0 ) {
	...
	echo '</select></div>
		...
		<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Sequence In Trial Balance'), '</label>
			<td><input class="form-control" maxlength="4" name="SequenceInTB" required="required" tabindex="5" type="text" value="', $_POST['SequenceInTB'], '" title="', _('Enter the sequence number that this account group and its child general ledger accounts should display in the trial balance'), '" /></div>
		</div>
		</div>
...
echo '<div class="row">
			<div class="col-xs-4">
					<button name="submit" class="btn btn-info" tabindex="6" type="submit" value="Update">', _('Update'), '</button> </div><div class="col-xs-4">
					<button onclick="window.location=\'AccountGroups.php\'" class="btn btn-default" type="button">', _('Return'), '</button></div></div><br />
';
                    }
...
<div class="row">
			<div class="col-xs-4">
					<button name="submit" tabindex="6" type="submit" class="btn btn-success" value="Insert">', _('Insert'), '</button></div>
                    <div class="col-xs-4">
					<button onclick="window.location=\'menu_data.php?Application=GL\'" type="button" class="btn btn-default">', _('Return'), '</button></div></div><br />
';
                    }					
