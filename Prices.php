...
if (DB_num_rows($result)==0) {
	echo prnMsg( _('The part code entered does not exist in the system') . '. ' . _('Only valid items can have prices entered against them'),'error');
	$InputError=1;
}

if (!isset($Item)) {
	echo '<p>';
	echo prnMsg (_('An item must first be selected before this page is called') . '. ' . _('The product selection page should call this page with a valid product code'),'error');
...
if ($myrow[1]=='K') {
	echo prnMsg(_('The part selected is a kit set item') .', ' . _('these items explode into their components when selected on an order') . ', ' . _('prices must be set up for the components and no price can be set for the whole kit'),'error');
	exit;
}
...
if (isset($_POST['submit'])) {
	...
	if (!is_numeric(filter_number_format($_POST['Price'])) OR $_POST['Price']=='') {
		$InputError = 1;
		echo prnMsg( _('The price entered must be numeric'),'error');
	}
	...
	if ($_POST['EndDate']!='') {
		if (FormatDateForSQL($_POST['EndDate'])!='9999-12-31'){
			if (! Is_Date($_POST['EndDate']) AND $_POST['EndDate']!=''){
				$InputError =1;
				echo prnMsg (_('The date this price is be in effect to must be entered in the format') . ' ' . $_SESSION['DefaultDateFormat'],'error');
			}
			...
			if (Date1GreaterThanDate2(Date($_SESSION['DefaultDateFormat']),$_POST['EndDate']) AND $_POST['EndDate']!='' AND FormatDateForSQL($_POST['EndDate'])!='9999-12-31'){
				$InputError =1;
				echo prnMsg(_('The end date is expected to be after today. There is no point entering a new price where the effective date is before today!'),'error');
			}
		}
	}
	...
	if ($myrow[0]!=0 AND !isset($_POST['OldTypeAbbrev']) AND !isset($_POST['OldCurrAbrev'])) {
		echo prnMsg( _('This price has already been entered. To change it you should edit it') , 'warn');
		$InputError =1;
	}
	...
	if (isset($_POST['OldTypeAbbrev']) AND isset($_POST['OldCurrAbrev']) AND mb_strlen($Item)>1 AND $InputError !=1) {
		...
		echo prnMsg(_('The price has been updated'),'success');
	...
	/*Selected price is null cos no item selected on first time round so must be adding a	record must be submitting new entries in the new price form */
		...
		ReSequenceEffectiveDates ($Item, $_POST['TypeAbbrev'], $_POST['CurrAbrev']) ;
		echo prnMsg(_('The new price has been inserted'),'success');
	}
...
//the link to delete a selected record was clicked instead of the submit button
	...
	$result = DB_query($sql,$ErrMsg);
	echo prnMsg( _('The selected price has been deleted'),'success');
...
<table id="general-table" class="table table-bordered">
			...
			<tr>
				...
				<td><input type="submit" class="btn btn-info" name="NewPart" value="' . _('Review Prices') . '" /></td><th colspan="4"></th>
			</tr>
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		if (in_array(5, $_SESSION['AllowedPageSecurityTokens'])) {
			echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Item=' . $myrow['stockid'] . '&amp;TypeAbbrev=' . $myrow['typeabbrev'] . '&amp;CurrAbrev=' . $myrow['currabrev'] . '&amp;Price=' . locale_number_format($myrow['price'],$myrow['currdecimalplaces']) . '&amp;StartDate=' . $myrow['startdate'] . '&amp;EndDate=' . $myrow['enddate'] . '&amp;Edit=1" class="btn btn-info">' . _('Edit') . '</a></td>
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Item=' . $myrow['stockid'] . '&amp;TypeAbbrev=' . $myrow['typeabbrev'] . '&amp;CurrAbrev=' . $myrow['currabrev'] . '&amp;StartDate=' . $myrow['startdate'] . '&amp;EndDate=' . $myrow['enddate'] . '&amp;delete=yes" onclick="return confirm(\'' . _('Are you sure you wish to delete this price?') . '\');" class="btn btn-danger">' . _('Delete') . '</a></td>';
		}
		echo '</tr>';
	...
	echo prnMsg(_('There are no prices set up for this part'),'warn');
}
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Currency') . '</label>
			<select name="CurrAbrev" class="form-control">';
while ($myrow = DB_fetch_array($result)) {
	echo '<option ';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Sales Type Price List') . '</label>
			<select name="TypeAbbrev" class="form-control">';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Price Effective From')  . '</label>
			<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="StartDate" required="required" size="10" maxlength="10" title="' . _('Enter the date from which this price should take effect.') . '" value="' . $_POST['StartDate'] . '" /></div></div></div>';
echo '<div class="row"><div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Price Effective To')  . '</label>
			<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="EndDate" size="10" maxlength="10" title="' . _('Enter the date to which this price should be in effect to, or leave empty if the price should continue indefinitely') . '" value="' . $_POST['EndDate'] . '" />';
echo '<input type="hidden" name="Item" value="' . $Item.'" /></div></div>';
echo '<div class="col-xs-4 has-error">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Price') . '</label>
         
          <input type="text" class="form-control" required="required" name="Price" size="12" maxlength="11" value="';
...
<input type="submit" name="submit" value="' . _('Submit') . '" class="btn btn-success" />
</div>';
