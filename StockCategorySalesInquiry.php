...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Sales By Category By Item Inquiry') . '';
echo '</h1></a></div>';
...
<div>
	...
	<input tabindex="4" type="submit" class="btn btn-success" name="ShowSales" value="' . _('Show') . '" />
</div>
</div>
</div>
</form>
	...
	while ($SalesRow=DB_fetch_array($SalesResult)) {
		if ($CategoryID != $SalesRow['categoryid']) {
			if ($CategoryID !='') {
				...
				echo '<tr>
					<td colspan="2" class="number">' . _('<strong>Category Total</strong>') . '</td>
