...
if (!isset($_POST['supplierid'])) {
	...
	if (DB_num_rows($result)==0) {
		echo prnMsg(_('There are no offers outstanding that you are authorised to deal with'), 'information');
	} else {
		echo '<div class="block-header"><a href="" class="header-title-link"><h1>
             ' . ' ' . _('Select Supplier') . '</h1></a></div>';
...
<div class="col-xs-12">';		 
		...
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Select Supplier') . '</label>
...
<div class="form-group"> <br />
					<input type="submit" class="btn btn-info" name="select" value="' . _('Enter Information') . '" />
				</div>
				</div>
			</div>
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Supplier Offers') . '
		</h1></a></div>';
...
<table id="general-table" class="table table-bordered">
	...
	if (sizeOf($Accepts)>0){
		$MailText=_('This email has been automatically generated by the nERP installation at'). ' ' . $_SESSION['CompanyRecord']['coyname'] . "\n";
		...
		if (sizeOf($Recipients) == 0) {
			echo prnMsg( _('There are no members of the Offers Received Result Recipients email group'), 'warn');
		...
		if($result){
			echo prnMsg(_('The accepted offers from').' ' . $SupplierName.' ' . _('have been converted to purchase orders and an email sent to')
				.' ' . $Email."\n"._('Please review the order contents').' ' . '<a href="' . $RootPath .
				'/PO_Header.php?ModifyOrderNumber=' . $OrderNo.'">' . _('here') . '</a>', 'success');
		}else{
			echo prnMsg(_('The accepted offers from').' ' . $SupplierName.' ' . _('have been converted to purcharse orders but failed to mail, you can view the order contents').' ' . '<a href="' . $RootPath.'/PO_Header.php?ModifyOrderNumber=' . $OrderNo.'">' . _('here') . '</a>','warn');
		}
	}
	if (sizeOf($RejectsArray)>0){
		$MailText=_('This email has been automatically generated by the nERP installation at').' '.
		...
		if($result){
			echo prnMsg(_('The rejected offers from').' ' . $SupplierName.' ' . _('have been removed from the system and an email sent to')
				.' ' . $Email, 'success');
		}else{
			echo prnMsg(_('The rejected offers from').' ' . $SupplierName.' ' . _('have been removed from the system and but no email was not sent to')
				.' ' . $Email, 'warn');

		}
	}
	echo prnMsg(_('All offers have been processed, and emails sent where appropriate'), 'success');
}
include('includes/footer.php');
