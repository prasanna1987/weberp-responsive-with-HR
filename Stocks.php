...
if (isset($_FILES['ItemPicture']) AND $_FILES['ItemPicture']['name'] !='') {
	...
	if (!in_array ($ImgExt, $SupportedImgExt)) {
		...
		echo prnMsg( _('No tmp directory set. You must have a tmp directory set in your PHP for upload of files. '),'warn');
		 	$UploadTheFile ='No';
	}
...
if (isset($_POST['submit'])) {
	...
	if (!isset($_POST['Description']) OR mb_strlen($_POST['Description']) > 50 OR mb_strlen($_POST['Description'])==0) {
		$InputError = 1;
		echo prnMsg (_('The stock item description must be entered and be fifty characters or less') . '. ' . _('It cannot be a zero length string either') . ' - ' . _('a description is required'),'error');
		$Errors[$i] = 'Description';
		$i++;
	}
	if (mb_strlen($_POST['LongDescription'])==0) {
		$InputError = 1;
		echo prnMsg (_('The stock item description cannot be a zero length string') . ' - ' . _('a long description is required'),'error');
		$Errors[$i] = 'LongDescription';
		$i++;
	}
	if (mb_strlen($StockID) ==0) {
		$InputError = 1;
		echo prnMsg (_('The Stock Item code cannot be empty'),'error');
		$Errors[$i] = 'StockID';
		$i++;
	}
	if (ContainsIllegalCharacters($StockID) OR mb_strpos($StockID,' ')) {
		$InputError = 1;
		echo prnMsg(_('The stock item code cannot contain any of the following characters') . " - ' &amp; + \" \\ ." . _('or a space'),'error');
		$Errors[$i] = 'StockID';
		$i++;
		$StockID='';
	}
	if (mb_strlen($_POST['Units']) >20) {
		$InputError = 1;
		echo prnMsg(_('The unit of measure must be 20 characters or less'),'error');
		$Errors[$i] = 'Units';
		$i++;
	}
	if (mb_strlen($_POST['BarCode']) >20) {
		$InputError = 1;
		echo prnMsg(_('The barcode must be 20 characters or less'),'error');
		$Errors[$i] = 'BarCode';
		$i++;
	}
	if (!is_numeric(filter_number_format($_POST['Volume']))) {
		$InputError = 1;
		echo prnMsg (_('The volume of the packaged item in cubic metres must be numeric') ,'error');
		$Errors[$i] = 'Volume';
		$i++;
	}
	if (filter_number_format($_POST['Volume']) <0) {
		$InputError = 1;
		echo prnMsg(_('The volume of the packaged item must be a positive number'),'error');
		$Errors[$i] = 'Volume';
		$i++;
	}
	if (!is_numeric(filter_number_format($_POST['GrossWeight']))) {
		$InputError = 1;
		echo prnMsg(_('The weight of the packaged item in Gross Weight must be numeric'),'error');
		$Errors[$i] = 'GrossWeight';
		$i++;
	}
	if (filter_number_format($_POST['GrossWeight'])<0) {
		$InputError = 1;
		echo prnMsg(_('The weight of the packaged item must be a positive number'),'error');
		$Errors[$i] = 'GrossWeight';
		$i++;
	}
	if (!is_numeric(filter_number_format($_POST['NetWeight']))) {
		$InputError = 1;
		echo prnMsg(_('The net weight of the item in Net Weight must be numeric'),'error');
		$Errors[$i] = 'NetWeight';
		$i++;
	}
	if (filter_number_format($_POST['NetWeight'])<0) {
		$InputError = 1;
		echo prnMsg(_('The net weight of the item must be a positive number'),'error');
		$Errors[$i] = 'NetWeight';
		$i++;
	}
	if (!is_numeric(filter_number_format($_POST['EOQ']))) {
		$InputError = 1;
		echo prnMsg(_('The economic order quantity must be numeric'),'error');
		$Errors[$i] = 'EOQ';
		$i++;
	}
	if (filter_number_format($_POST['EOQ']) <0) {
		$InputError = 1;
		echo prnMsg (_('The economic order quantity must be a positive number'),'error');
		$Errors[$i] = 'EOQ';
		$i++;
	}
	if ($_POST['Controlled']==0 AND $_POST['Serialised']==1){
		$InputError = 1;
		echo prnMsg(_('The item can only be serialised if there is lot control enabled already') . '. ' . _('Batch control') . ' - ' . _('with any number of items in a lot/bundle/roll is enabled when controlled is enabled') . '. ' . _('Serialised control requires that only one item is in the batch') . '. ' . _('For serialised control') . ', ' . _('both controlled and serialised must be enabled'),'error');
		$Errors[$i] = 'Serialised';
		$i++;
	}
	if ($_POST['NextSerialNo']!=0 AND $_POST['Serialised']==0){
		$InputError = 1;
		echo prnMsg(_('The item can only have automatically generated serial numbers if it is a serialised item'),'error');
		$Errors[$i] = 'NextSerialNo';
		$i++;
	}
	if ($_POST['NextSerialNo']!=0 AND $_POST['MBFlag']!='M'){
		$InputError = 1;
		echo prnMsg(_('The item can only have automatically generated serial numbers if it is a manufactured item'),'error');
		$Errors[$i] = 'NextSerialNo';
		$i++;
	}
	if (($_POST['MBFlag']=='A'
		...
		$InputError = 1;
		echo prnMsg(_('Assembly/Kitset/Phantom/Service/Labour items cannot also be controlled items') . '. ' . _('Assemblies/Dummies/Phantom and Kitsets are not physical items and batch/serial control is therefore not appropriate'),'error');
		$Errors[$i] = 'Controlled';
		$i++;
	}
	if (trim($_POST['CategoryID'])==''){
		$InputError = 1;
		echo prnMsg(_('There are no inventory categories defined. All inventory items must belong to a valid inventory category,'),'error');
		$Errors[$i] = 'CategoryID';
		$i++;
	}
	if (!is_numeric(filter_number_format($_POST['Pansize']))) {
		$InputError = 1;
		echo prnMsg(_('Pansize quantity must be numeric'),'error');
		$Errors[$i] = 'Pansize';
		$i++;
	}
	if (!is_numeric(filter_number_format($_POST['ShrinkFactor']))) {
		$InputError = 1;
		echo prnMsg(_('Shrinkage factor quantity must be numeric'),'error');
		...
		if ($New==0) { /*so its an existing one */
			...
			*/
			...
			if ($OldMBFlag != $_POST['MBFlag']){
				if (($OldMBFlag == 'M' OR $OldMBFlag=='B') AND ($_POST['MBFlag']=='A' OR $_POST['MBFlag']=='K' OR $_POST['MBFlag']=='D' OR $_POST['MBFlag']=='G')){ /*then need to check that there is no stock holding first */
					...
					if ($StockQtyRow[0]!=0 AND $OldMBFlag!='G'){
						$InputError=1;
						echo prnMsg( _('The make or buy flag cannot be changed from') . ' ' . $OldMBFlag . ' ' . _('to') . ' ' . $_POST['MBFlag'] . ' ' . _('where there is a quantity of stock on hand at any location') . '. ' . _('Currently there are') . ' ' . $StockQtyRow[0] .  ' ' . _('on hand') , 'errror');
					}
					/* don't allow controlled/serialized  */
					if ($_POST['Controlled']==1){
						$InputError=1;
						echo prnMsg( _('The make or buy flag cannot be changed from') . ' ' . $OldMBFlag . ' ' . _('to') . ' ' . $_POST['MBFlag'] . ' ' . _('where the item is to be lot controlled') . '. ' . _('Kitset, phantom, dummy and assembly items cannot be lot controlled'), 'error');
					}
				}
				...
				if ($_POST['MBFlag']=='K') {
					...
					if ($ChkSalesOrds[0]!=0){
						$InputError = 1;
						echo prnMsg( _('The make or buy flag cannot be changed to a kitset where there is a quantity outstanding to be delivered on sales orders') . '. ' . _('Currently there are') .' ' . $ChkSalesOrds[0] . ' '. _('outstanding'), 'error');
					}
				}
				...
				if ($_POST['MBFlag']=='K' OR $_POST['MBFlag']=='A' OR $_POST['MBFlag']=='D') {
					...
					if ($ChkPurchOrds[0]!=0){
						$InputError = 1;
						echo prnMsg( _('The make or buy flag cannot be changed to'). ' ' . $_POST['MBFlag'] . ' '. _('where there is a quantity outstanding to be received on purchase orders') . '. ' . _('Currently there are'). ' ' . $ChkPurchOrds[0] . ' '. _('yet to be received'). 'error');
					}
				}
				...
				if (($OldMBFlag=='M' OR $OldMBFlag =='K' OR $OldMBFlag=='A' OR $OldMBFlag=='G') AND ($_POST['MBFlag']=='B' OR $_POST['MBFlag']=='D')) {
					...
					if ($ChkBOM[0]!=0){
						$InputError = 1;
						echo prnMsg( _('The make or buy flag cannot be changed from manufactured, kitset or assembly to'). ' ' . $_POST['MBFlag'] . ' '. _('where there is a bill of material set up for the item') . '. ' . _('Bills of material are not appropriate for purchased or dummy items'), 'error');
					}
				}
				...
				if (($OldMBFlag=='M' OR $OldMBFlag =='B' OR $OldMBFlag=='D' OR $OldMBFlag=='G') AND ($_POST['MBFlag']=='A' OR $_POST['MBFlag']=='K')) {
					...
					if ($ChkBOM[0]!=0){
						$InputError = 1;
						echo prnMsg( _('The make or buy flag cannot be changed from manufactured, purchased or dummy to a kitset or assembly where the item is a component in a bill of material') . '. ' . _('Assembly and kitset items are not appropriate as components in a bill of materials'), 'error');
					}
				}
			}
			...
			if ($OldControlled != $_POST['Controlled'] AND $StockQtyRow[0]!=0){
				$InputError=1;
				echo prnMsg( _('You can not change a Non-Controlled Item to Controlled (or back from Controlled to non-controlled when there is currently stock on hand for the item') , 'error');

			}
			if ($OldSerialised != $_POST['Serialised'] AND $StockQtyRow[0]!=0){
				$InputError=1;
				echo prnMsg( _('You can not change a Serialised Item to Non-Serialised (or vice-versa) when there is a quantity on hand for the item') , 'error');
			}
			...
			for ($i=0;$i<$_POST['PropertyCounter'];$i++){
				if ($_POST['PropNumeric' .$i]==1){
					if ( filter_number_format($_POST['PropValue' . $i]) < $_POST['PropMin' . $i] OR filter_number_format($_POST['PropValue' . $i]) > $_POST['PropMax' . $i]){
							$InputError = 1;
							echo prnMsg(_('The property value should between') . ' '.$_POST['PropMin' . $i] . ' ' . _('and') . $_POST['PropMax' . $i],'error');
					}
				}
			}
			...
			if ($InputError == 0){
				...
				DB_Txn_Commit();
				echo prnMsg( _('Stock Item') . ' ' . $StockID . ' ' . _('has been updated'), 'success');
				echo '<br />';
			}
			...
			if (DB_num_rows($result)==1){
				echo prnMsg(_('The stock code entered is actually already in the database - duplicate stock codes are prohibited by the system. Try choosing an alternative stock code'),'error');
				$InputError = 1;
				$Errors[$i] = 'StockID';
				$i++;
			} else {
				...
				if (DB_error_no() ==0) {
					...
					DB_Txn_Commit();
					if (DB_error_no() ==0) {
						echo prnMsg( _('New Item') .' ' . '<a href="SelectProduct.php?StockID=' . $StockID . '"  class="btn btn-info">' . $StockID . '</a> '. _('has been added to the system') .
							'<br />' . _('The item cost and pricing must also be setup') .
							'<br />' . '<a target="_blank" href="StockCostUpdate.php?StockID=' . $StockID . '"  class="btn btn-info">' . _('Enter Item Cost') . '</a>
							<br />' . '<a target="_blank" href="Prices.php?Item=' . $StockID . '"  class="btn btn-info">' . _('Enter Item Prices') . '</a> ','success');
						echo '<br />';
...
//						unset($_POST['CategoryID']);
		...
		echo '<br />' ;
		echo prnMsg( _('Validation failed, no updates or deletes took place'), 'error');
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'StockMoves'
	...
	if ($myrow[0]>0) {
		$CancelDelete = 1;
		echo prnMsg( _('Cannot delete this stock item because there are stock movements that refer to this item'),'warn');
		...
		if ($myrow[0]>0) {
			$CancelDelete = 1;
			echo prnMsg( _('Cannot delete this item record because there are bills of material that require this part as a component'),'warn');
			...
			if ($myrow[0]>0) {
				$CancelDelete = 1;
				echo prnMsg( _('Cannot delete this item record because there are existing sales orders for this part'),'warn');
				...
				if ($myrow[0]>0) {
					$CancelDelete = 1;
					echo prnMsg(_('Cannot delete this item because sales analysis records exist for it'),'warn');
					...
					if ($myrow[0]>0) {
						$CancelDelete = 1;
						echo prnMsg(_('Cannot delete this item because there are existing purchase order items for it'),'warn');
						...
						if ($myrow[0]!=0) {
							$CancelDelete = 1;
							echo prnMsg( _('Cannot delete this item because there is currently some stock on hand'),'warn');
							...
							if ($myrow[0]!=0) {
								$CancelDelete = 1;
								echo prnMsg( _('Cannot delete this item because there are offers for this item'),'warn');
								...
								if ($myrow[0]!=0) {
									$CancelDelete = 1;
									echo prnMsg( _('Cannot delete this item because there are tenders for this item'),'warn');
	...
	if ($CancelDelete==0) {
		...
		echo prnMsg(_('Deleted the stock master record for') . ' ' . $StockID . '....' .
...
if (isset($StockID) && $StockID != '' && $InputError == 0){
	echo '<div class="row">
<div class="col-xs-4">
<input type="submit" name="PreviousItem" value="Prev" class="btn btn-info" value="" ' . ($hasPrev ? '' : 'disabled') . ' /></div>
				<div align="right"><input type="submit" value="Next" class="btn btn-info" name="NextItem" value="" ' . ($hasNext ? '' : 'disabled') . ' /></div>
			</div>
		<br />';
}
...
<div class="form-group has-error"> <label class="col-md-12 control-label">' .  _('Item Code'). '</label>
				<input type="text" ' . (in_array('StockID',$Errors) ?  'class="inputerror"' : '' ) . ' data-type="no-illegal-chars" autofocus="autofocus" required="required"  value="'.$StockID.'" name="StockID" size="20" maxlength="20" class="form-control"  title ="'._('Input the stock code, the following characters are prohibited:') . ' \' &quot; + . &amp; \\ &gt; &lt;" placeholder="'._('alpha-numeric only').'" /></div>
			</div>';
	} else {
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Item Code'). '</label>
				' . $StockID . '<input type="hidden" name ="StockID" value="' . $StockID . '" /></div>
			</div>';
	}
...
echo '<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Part Description') . ' (' . _('short') . ')</label>
		<input ' . (in_array('Description',$Errors) ?  'class="inputerror"' : '' ) .' type="text" ' . ($New==0?'autofocus="autofocus"':'') . ' name="Description" required="required" class="form-control" size="52" maxlength="50" value="' . stripslashes($Description) . '" />
	</div></div>';
...
//		echo '
//				<div class="col-xs-4">
//<div class="form-group"> <label class="col-md-12 control-label">' . $LanguagesArray[$LanguageId]['LanguageName'] . ' ' . _('Description') . '</label>
//				<input type="text" class="form-control" name="'. $PostVariableName . '" size="52" maxlength="50" value="' . $_POST[$PostVariableName] . '" title="' . _('This language translation of the item will be used in invoices and credits to customers who are defined to use this language. The language translations to maintain here can be configured in the system parameters page') .  '" /></div>
//			</div>';
//	}
//}
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Part Description') . ' (' . _('long') . ')</label>
		<textarea ' . (in_array('LongDescription',$Errors) ?  'class="texterror"' : '' ) .'  name="LongDescription" class="form-control">' . stripslashes($LongDescription) . '</textarea></div>
	</div></div>';
...
//		}
//		echo '<div class="col-xs-4">
//<div class="form-group"> <label class="col-md-12 control-label">' . $LanguagesArray[$LanguageId]['LanguageName'] . ' ' . _('Long Description') . '</label>
//				<textarea name="'. $PostVariableName . '" class="form-control">' . stripslashes(AddCarriageReturns($_POST[$PostVariableName])) . '</textarea></div>
//			</div>';
//	}
//}
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Category') . '</label>
		<select name="CategoryID" class="form-control" onchange="ReloadForm(ItemForm.UpdateCategories)">';
...
echo '</select><a target="_blank" href="'. $RootPath . '/StockCategories.php" class="btn btn-info">' . _('Add or Modify Stock Categories') . '</a></div>
	</div>';
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Economic Order Quantity') . '</label>
		<input ' . (in_array('EOQ',$Errors) ?  'class="inputerror"' : '' ) .'   type="text" class="form-control" name="EOQ" size="12" maxlength="10" value="' . locale_number_format($_POST['EOQ'],'Variable') . '" /></div></div></div>';

echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Packaged Volume (metres cubed)') . '</label>
		<input ' . (in_array('Volume',$Errors) ?  'class="inputerror"' : '' ) .'   type="text" class="form-control" name="Volume" size="12" maxlength="10" value="' . locale_number_format($_POST['Volume'],'Variable') . '" /></div>
	</div>';

echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Packaged Gross Weight (KGs)') . '</label><input ' . (in_array('GrossWeight',$Errors) ?  'class="inputerror"' : '' ) .'   type="text" class="form-control" name="GrossWeight" size="12" maxlength="10" value="' . locale_number_format($_POST['GrossWeight'],'Variable') . '" /></div>
	</div>';

echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Net Weight (KGs)') . '</label><input ' . (in_array('NetWeight',$Errors) ?  'class="inputerror"' : '' ) .'   type="text" class="form-control" name="NetWeight" size="12" maxlength="10" value="' . locale_number_format($_POST['NetWeight'],'Variable') . '" /></div>
	</div></div>';

	echo '<div class="row">
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Units of Measure') . '</label>
		<select ' . (in_array('Description',$Errors) ?  'class="selecterror"' : '' ) .'  name="Units" class="form-control">';
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Stock Type') . '</label>
		<select name="MBFlag" class="form-control">';
if ($_POST['MBFlag']=='A'){
...
if ($_POST['Discontinued']==1){
	echo '<option selected="selected" value="1">' . _('In-active') . '</option>';
} else {
	echo '<option value="1">' . _('In-active') . '</option>';
}
echo '</select></div>
	</div></div>';

echo '<div class="row"><div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Batch, Serial or Lot Control') . '</label>
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Serialised') . '</label><select ' . (in_array('Serialised',$Errors) ?  'class="selecterror"' : '' ) .'  name="Serialised" class="form-control">';
...
echo '</select><i>'  . _('This has no effect if the item is not Controlled') . '</i></div>
	</div>';

if ($_POST['Serialised']==1 AND $_POST['MBFlag']=='M'){
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Next Serial No (>0 for auto numbering)') . '</label>
			<input ' . (in_array('NextSerialNo',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="NextSerialNo" size="15" maxlength="15" class="form-control" value="' . $_POST['NextSerialNo'] . '" /></div>
	</div></div></div>';
} else {
	echo '<div class="col-xs-4">
<div class="form-group"><input type="hidden" name="NextSerialNo" value="0" /></div></div></div>';
}

echo '<div class="row"><div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Perishable') . '</label>
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Decimal Places for display Quantity') . '</label>
		<input type="text" class="form-control" name="DecimalPlaces" size="1" maxlength="1" value="' . $_POST['DecimalPlaces'] . '" /></div>
	</div>';
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Bar Code') . '</label>
		<input ' . (in_array('BarCode',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" class="form-control" name="BarCode" size="22" maxlength="20" value="' . $BarCode . '" /></div>
	</div></div>';
...
echo '<div class="row"><div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Discount Category') . '</label>
		<input type="text" class="form-control" name="DiscountCategory" size="2" maxlength="2" value="' . $DiscountCategory . '" /></div>
	</div>';

echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Tax Category') . '</label>
		<select name="TaxCat" class="form-control">';
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Pan Size') . ':</label></td>
		<input class="form-control" id="PanSize" maxlength="6" name="Pansize" size="6" title="' . _('Order multiple. It is the minimum packing quantity.') . '" type="text" value="' . locale_number_format($_POST['Pansize'],0) . '" /></div>
	</div></div>
	<div class="row">
	 <div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Shrinkage Factor') . ':</label></td>
		<input class="form-control" id="ShrinkageFactor" maxlength="6" name="ShrinkFactor" size="6" title="' . _('Amount by which an output falls short of the estimated or planned output.') . '" type="text" value="' . locale_number_format($_POST['ShrinkFactor'],0) . '" /></div>
	</div>
 <div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('HSN/SAC Code') . ':</label></td>
		<input class="form-control" id="HsnCode" maxlength="15" name="HsnCode" size="6" type="text" value="' . $_POST['HsnCode'] . '" /></div>
	</div>	
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Item Category Properties') . '</h3></div>
	...
	switch ($PropertyRow['controltype']) {
	 	case 0; //textbox
	 		if ($PropertyRow['numericvalue']==1) {
				...
				echo '<input type="text" class="form-control" name="PropValue' . $PropertyCounter . '" size="20" maxlength="100" value="' . locale_number_format($PropertyValue,'Variable') . '" />';
				...
				echo '<input type="text" class="form-control" name="PropValue' . $PropertyCounter . '" size="20" maxlength="100" value="' . $PropertyValue . '" />';
			}
	 		break;
	 	case 1; //select box
	 		$OptionValues = explode(',',$PropertyRow['defaultvalue']);
			echo '<select name="PropValue' . $PropertyCounter . '" class="form-control">';
...
<div><input type="submit" name="submit" class="btn btn-success" value="' . _('Add New Item') . '" /></div>';
	echo '<div class="col-xs-4"><input type="submit" name="UpdateCategories" class="btn btn-info" style="visibility:hidden;width:1px" value="' . _('Categories') . '" /></div></div><br />
';
	...
	echo '<div class="row" align="center">
<div><input type="submit" name="submit" value="' . _('Update') . '" class="btn btn-info" /></div>';
	echo '<div class="col-xs-4"><input type="submit" class="btn btn-info" name="UpdateCategories" style="visibility:hidden;width:1px" value="' . _('Categories') . '" /></div></div>';
	echo '<br />';
	echo prnMsg( _('Only click the Delete button if you are sure you wish to delete the item!') . '<br />' . _('Checks will be made to ensure that there are no stock movements, sales analysis records, sales order items or purchase order items for the item') . '. ' . _('No deletions will be allowed if they exist') . '.', 'warn', _('warn'));
	echo '<br /><div class="row" align="center"><div>
		<input type="submit" name="delete" class="btn btn-danger" value="' . _('Delete This Item') . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');" /></div></div><br />
';
}
