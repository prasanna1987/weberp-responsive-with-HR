...
if ((isset($_POST['UpdateStatus']) AND $_POST['UpdateStatus'] != '')) {
	...
	if ($_SESSION['ExistingOrder'] == 0) {
		echo prnMsg(_('This is a new order. It must be created before you can change the status'), 'warn');
		$OKToUpdateStatus = 0;
		...
		if ($_POST['Status'] == 'Authorised') {
			if ($AuthorityLevel > $OrderTotal) {
				...
				$OKToUpdateStatus = 0;
				echo prnMsg(_('You do not have permission to authorise this purchase order') . '.<br />' . _('This order is for') . ' ' . $_SESSION['PO' . $identifier]->CurrCode . ' ' . $OrderTotal . '. ' . _('You can only authorise up to') . ' ' . $_SESSION['PO' . $identifier]->CurrCode . ' ' . $AuthorityLevel . '.<br />' . _('If you think this is a mistake please contact your systems administrator'), 'warn');
			}
		}
		...
		if ($_POST['Status'] == 'Rejected' OR $_POST['Status'] == 'Cancelled') {
			if (!isset($_SESSION['ExistingOrder']) OR $_SESSION['ExistingOrder'] != 0) {
				...
				if ($_SESSION['PO' . $identifier]->Any_Already_Received() == 1) {
					...
					echo prnMsg(_('This order cannot be cancelled or rejected because some of it has already been received') . '. ' . _('The line item quantities may be modified to quantities more than already received') . '. ' . _('Prices cannot be altered for lines that have already been received') . ' ' . _('and quantities cannot be reduced below the quantity already received'), 'warn');
				}
				$ShipmentExists = $_SESSION['PO' . $identifier]->Any_Lines_On_A_Shipment();
				if ($ShipmentExists != false) {
					$OKToUpdateStatus = 0; //not ok to update the status
					echo prnMsg(_('This order cannot be cancelled or rejected because there is at least one line that is allocated to a shipment') . '. ' . _('See shipment number') . ' ' . $ShipmentExists, 'warn');
				}
			...
			if ($OKToUpdateStatus == 1) { // none of the order has been received
				if ($AuthorityLevel > $OrderTotal) {
					...
					$OKToUpdateStatus = 0;
					echo prnMsg(_('You do not have permission to reject this purchase order') . '.<br />' . _('This order is for') . ' ' . $_SESSION['PO' . $identifier]->CurrCode . ' ' . $OrderTotal . '. ' . _('Your authorisation limit is set at') . ' ' . $_SESSION['PO' . $identifier]->CurrCode . ' ' . $AuthorityLevel . '.<br />' . _('If you think this is a mistake please contact your systems administrator'), 'warn');
				}
			} //$OKToUpdateStatus == 1
		...
		if ($_POST['Status'] == 'Pending') {
			...
			if ($_SESSION['PO' . $identifier]->Any_Already_Received() == 1) {
				...
				echo prnMsg(_('This order could not have the status changed back to pending because some of it has already been received. Quantities received will need to be returned to change the order back to pending.'), 'warn');
			}
			...
			if (($AuthorityLevel > $OrderTotal OR $_SESSION['UserID'] == $_SESSION['PO' . $identifier]->Initiator) AND $OKToUpdateStatus == 1) {
				...
				$OKToUpdateStatus = 0;
				echo prnMsg(_('You do not have permission to change the status of this purchase order') . '.<br />' . _('This order is for') . ' ' . $_SESSION['PO' . $identifier]->CurrCode . ' ' . $OrderTotal . '. ' . _('Your authorisation limit is set at') . ' ' . $_SESSION['PO' . $identifier]->CurrCode . ' ' . $AuthorityLevel . '.<br />' . _('If you think this is a mistake please contact your systems administrator'), 'warn');
...
if (isset($_POST['EnterLines']) OR isset($_POST['AllowRePrint'])) {
	...
	if (!isset($_POST['AllowRePrint'])) { // user only hit update not "Enter Lines"
		...
		echo prnMsg(_('You should automatically be forwarded to the entry of the purchase order line items page') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/PO_Items.php?identifier=' . $identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
...
if (isset($_POST['ChangeSupplier'])) {
	if ($_SESSION['PO' . $identifier]->Status == 'Pending' AND $_SESSION['UserID'] == $_SESSION['PO' . $identifier]->Initiator) {
		...
		if ($_SESSION['PO' . $identifier]->Any_Already_Received() == 0) {
			...
			echo '<br />';
			echo prnMsg(_('Cannot modify the supplier of the order once some of the order has been received'), 'warn');
		}
	}
...
if (isset($_POST['SearchSuppliers'])) {
	if (mb_strlen($_POST['Keywords']) > 0 AND mb_strlen($_SESSION['PO' . $identifier]->SupplierID) > 0) {
		echo prnMsg(_('Supplier name keywords have been used in preference to the supplier code extract entered'), 'warn');
	}
	...
	if (DB_num_rows($result_SuppSelect) == 1) {
		...
		echo prnMsg(_('No supplier records contain the selected text') . ' - ' . _('please alter your search criteria and try again'), 'info');
	}
...
if (isset($_POST['Select'])) {
	...
	if (($AuthRow = DB_fetch_array($AuthResult) and $AuthRow['cancreate'] == 0)) {
		...
		echo prnMsg(_('You do not have the authority to raise Purchase Orders for') . ' ' . $myrow['suppname'] . '. ' . _('Please Consult your system administrator for more information.') . '<br />' . _('You can setup authorisations') . ' ' . '<a href="PO_AuthorisationLevels.php">' . _('<strong>here</strong>') . '</a>', 'warn');
		include('includes/footer.php');
		exit;
	}
...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO' . $identifier]->SupplierID) OR $_SESSION['PO' . $identifier]->SupplierID == '') {
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Purchase Order: Select Supplier') . '</h1></a></div>';
...
echo '<p align="left"><a href="' . $RootPath . '/PO_SelectOSPurchOrder.php?identifier=' . $identifier . '" class="btn btn-info">' . _('Back to Orders') . '</a></p>';
	echo '
	<div class="row">
<div class="col-xs-3">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier name-part or full') . '</label>
		<input type="text" name="Keywords" class="form-control" autofocus="autofocus" size="20" maxlength="25" /></div>
		</div>
		
<div class="col-xs-3">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier code-part or full') . '</label>
		<input type="text" class="form-control" name="SuppCode" size="15" maxlength="18" /></div>
	</div>
	
	<div class="col-xs-3">
<div class="form-group"><br />

	<input type="submit" class="btn btn-success" name="SearchSuppliers" value="' . _('Search') . '" />
	</div>
	</div></div>
...
<table id="general-table" class="table table-bordered">
		...
		while ($myrow = DB_fetch_array($result_SuppSelect)) {
			...
			echo '<tr class="striped_row">
				<td><input type="submit" style="width:100%" name="Select" class="btn btn-info" value="' . $myrow['supplierid'] . '" /></td>
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>
			' . $_SESSION['PO' . $identifier]->SupplierName . ' <br /><small> ' . _('All amounts stated in:') . '
			<strong>' . $_SESSION['PO' . $identifier]->CurrCode . '</strong></small></h1></a></div>';
	...
	if ($_SESSION['ExistingOrder']) {
		echo'<h4 class="btn btn-default"><strong>'. _('Edit Purchase Order Number:') . ' ' . $_SESSION['PO' . $identifier]->OrderNo.'</strong></h4><br />';
	}

	if (isset($Purch_Item)) {
		/*This is set if the user hits the link from the supplier purchasing info shown on SelectProduct.php */
		echo prnMsg(_('Purchase Item(s) with this code') . ': ' . $Purch_Item, 'info');
		...
		/* the link */
		echo '<p align="left"><a href="' . $RootPath . '/PO_Items.php?NewItem=' . $Purch_Item . '&identifier=' . $identifier . '">' . _('Enter Line Item to this purchase order') . '</a></p>';
...
<div class="col-xs-6">
<div class="block">
<div class="block-title"><h2>' . _('Order Initiation Details') . '</h2></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
		';
	//sub table starts
	...
	echo '<tr>
		...
		<tr>
			<td class="has-error">' . _('Delivery Date') . ':</td>
			<td class="has-error"><input type="text" required="required" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="DeliveryDate" size="11" value="' . $_POST['DeliveryDate'] . '" /></td>
		</tr>';
	...
	echo '<tr>
		...
		<tr>
			...
			<td><input type="text" name="Requisition" class="form-control " size="16" maxlength="15" title="' . _('Enter our purchase requisition reference if needed') . '" value="' . $_POST['Requisition'] . '" /></td>
		</tr>
		<tr>
	...
	if ($_SESSION['PO' . $identifier]->AllowPrintPO == 0 AND empty($_POST['RePrint'])) {
		echo '<tr>
				...
				<td><select name="RePrint" class="form-control" onchange="ReloadForm(form1.AllowRePrint)">
					<option selected="selected" value="0">' . _('No') . '</option>
					<option value="1">' . _('Yes') . '</option>
				</select></td>';
		echo '<td><input type="submit" class="btn btn-info" name="AllowRePrint" value="Update" /></td></tr>';
	} elseif ($Printed) {
		echo '<tr>
				<td colspan="2" align="center"><a target="_blank" class="btn btn-warning"  href="' . $RootPath . '/PO_PDFPurchOrder.php?OrderNo=' . $_SESSION['ExistingOrder'] . '&amp;identifier=' . $identifier . '">' . _('Reprint Now') . '</a></td></tr>';
	} //$Printed
	...
	echo '
            <div class="col-xs-6">
			<div class="block">
<div class="block-title"><h2>' . _('Order Status') . '</h2></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">';

	if ($_SESSION['ExistingOrder'] != 0 AND $_SESSION['PO' . $identifier]->Status == 'Printed') {
		echo '<tr>
				<td colspan="2" align="center"><a href="' . $RootPath . '/GoodsReceived.php?PONumber=' . $_SESSION['PO' . $identifier]->OrderNo . '&amp;identifier=' . $identifier . '" class="btn btn-success">' . _('Receive') . '</a></td>
			</tr>';
	}
	...
	if ($_SESSION['PO' . $identifier]->Status == '') { //then its a new order
		...
		echo '<tr>
				<td>' . _('Status') . ' :  </td>
				<td><select name="Status" onchange="ReloadForm(form1.UpdateStatus)" class="form-control">';
		...
		echo '</select></td>
			...
			<tr>
				<td>' . _('Comment') . ':</td>
				<td><input type="text" name="StatusComments" class="form-control" size="50" /></td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2" align="center"><input type="submit" name="UpdateStatus" class="btn btn-info" value="' . _('Update Status') . '" /></td>
			</tr>';
	...
	echo '</table></div></div></div>
		...
		<div class="col-xs-6">
		<div class="block">
		<div class="block-title"><h2>' . _('Location Details') . '</h2></div>
		';
	/*nested table level1 */

	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Location') . '</label>
				<select required="required" name="StkLocation" onchange="ReloadForm(form1.LookupDeliveryAddress)" class="form-control">';
	...
	echo '</select>
		<input type="submit" name="LookupDeliveryAddress" class="btn btn-info" value="' . _('Select Location') . '" /></div>
		</div>';
	...
	if (!isset($_POST['StkLocation']) OR $_POST['StkLocation'] == '') {
		...
		if (DB_num_rows($LocnAddrResult) == 1) {
			...
			echo prnMsg(_('The default stock location set up for this user is not a currently defined stock location') . '. ' . _('Your system administrator needs to amend your user record'), 'error');
		}
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Delivery Contact') . '</label>
			<input type="text" class="form-control" name="Contact" size="41"  title="' . _('Enter the name of the contact at the delivery address - normally our warehouse person at that warehouse') .  '" value="' . $_SESSION['PO' . $identifier]->Contact . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 1 </label>
			<input type="text" class="form-control" name="DelAdd1" size="41" maxlength="40" value="' . $_POST['DelAdd1'] . '" /></div>
		</div>
		</div>
		<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 2 </label>
			<input type="text" class="form-control" name="DelAdd2" size="41" maxlength="40" value="' . $_POST['DelAdd2'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 3 </label>
			<input type="text" class="form-control" name="DelAdd3" size="41" maxlength="40" value="' . $_POST['DelAdd3'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 4 </label>
			<input type="text" class="form-control" name="DelAdd4" size="41" maxlength="40" value="' . $_POST['DelAdd4'] . '" /></div>
		</div>
		</div>
		<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 5 </label>
			<input type="text" class="form-control" name="DelAdd5" size="21" maxlength="20" value="' . $_POST['DelAdd5'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 6 </label>
			<input type="text" class="form-control" name="DelAdd6" size="16" maxlength="15" value="' . $_POST['DelAdd6'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Phone') . '</label>
			<input type="tel" name="Tel" pattern="[0-9+\-\s()]*" size="31" maxlength="30" class="form-control" value="' . $_SESSION['PO' . $identifier]->Tel . '" /></div>
		</div>
		</div>
		<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Delivery By') . '</label>
			<select name="DeliveryBy" class="form-control">';
	...
	<div class="block">
	<div class="block-title"><h2>' . _('Supplier Details') . '</h2></div>
			<div class="row">';


	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier Contact') . '</label>
				<select name="SupplierContact" class="form-control">';
	...
	echo '</select> </div>
		...
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 1 </label>
			<input type="text" class="form-control" name="SuppDelAdd1" size="41" maxlength="40" value="' . $_POST['SuppDelAdd1'] . '" /></div>
		</div>
		
		
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 2 </label>
			<input type="text" class="form-control" name="SuppDelAdd2" size="41" maxlength="40" value="' . $_POST['SuppDelAdd2'] . '" /></div>
		</div>
		</div>
		...
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 3 </label>
			<input type="text" class="form-control" name="SuppDelAdd3" size="41" maxlength="40" value="' . $_POST['SuppDelAdd3'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 4 </label>
			<input type="text" class="form-control" name="SuppDelAdd4" size="41" maxlength="40" value="' . $_POST['SuppDelAdd4'] . '" /></div>
		</div>
		
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 5 </label>
			<input type="text" class="form-control" name="SuppDelAdd5" size="41" maxlength="20" value="' . $_POST['SuppDelAdd5'] . '" /></div>
		</div>
		</div>
		...
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address') . ' 6 </label>
			<input type="text" class="form-control" name="SuppDelAdd6" size="16" maxlength="15" value="' . $_POST['SuppDelAdd6'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Phone') . '</label>
			<input type="tel" class="form-control" name="SuppTel" pattern="[0-9+\-\s()]*" size="31" maxlength="30" value="' . $_SESSION['PO' . $identifier]->SuppTel . '" /></div>
		</div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Payment Terms') . '</label>
	...
	//echo '
//	<div class="col-xs-4">
//<div class="form-group"> <label class="col-md-8 control-label">' . _('Delivery To') . '</label>
//			<input type="text" class="form-control" name="Port" size="31" value="' . $_POST['Port'] . '" /></div>
//		</div>
//		
//		';
...
<div class="form-group"><label class="col-md-8 control-label">' . _('Exchange Rate') . '' . '</label>
				<input type="text" name="ExRate" class="form-control" value="' . locale_number_format($_POST['ExRate'], 5) . '" class="number" size="11" /></div></div></div>';
	} else {
	...
	<div class="col-xs-7">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Comments');
	...
	echo '</label>
		...
		<textarea name="Comments" class="form-control">' . stripcslashes($_POST['Comments']) . '</textarea></div>
			</div>
	...
	echo '<div class="col-xs-6">
<div class="form-group"><br />
			<input type="submit" class="btn btn-success" name="EnterLines" value="' . _('Enter Line Items') . '" />
		</div></div>
		</div>
		';
