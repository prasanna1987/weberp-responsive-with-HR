...
if (!isset($_POST['FromDate']) OR !isset($_POST['ToDate']) OR $InputError==1){
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' '
		. _('Orders Invoiced Report') . '</h1></a></div>';
...
<div class="col-xs-12">';
	...
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />
		...
		<input type="text" required="required" autofocus="autofocus" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="FromDate" maxlength="10" size="11" value="' . Date($_SESSION['DefaultDateFormat'], Mktime(0,0,0,Date('m'),Date('d')-1,Date('y'))) . '" /></div></div>
		
		<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Date to which orders are to be listed') . '</label>
		<input type="text" required="required" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="ToDate" maxlength="10" size="11" value="' . Date($_SESSION['DefaultDateFormat']) . '" /></div></div>
	...
	echo '</select></div>
        ...
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Inventory Location') . '</label>
			<select required="required" name="Location" class="form-control">
...
if (DB_error_no()!=0){
	include('includes/header.php');
	echo prnMsg(_('An error occurred getting the orders details'),'',_('Database Error')),'</div>';
	if ($debug==1){
		echo prnMsg( _('The SQL used to get the orders that failed was') . '<br />' . $sql, '',_('Database Error')),'</div>';
	}
	include ('includes/footer.php');
	exit;
} elseif (DB_num_rows($Result)==0){
  	include('includes/header.php');
	echo  prnMsg(_('There were no orders found in the database within the period from') . ' ' . $_POST['FromDate'] . ' ' . _('to') . ' '. $_POST['ToDate'] . '. ' . _('Please try again selecting a different date range'), 'warn');
	if ($debug==1) {
		echo  prnMsg(_('The SQL that returned no rows was') . '<br />' . $sql,'',_('Database Error')),'</div>';
	}
	include('includes/footer.php');
	exit;
}
...
$pdf->OutputD('nERP' . '_OrdersInvoiced_' . date('Y-m-d') . '.pdf');
$pdf->__destruct();
?>
