...
if (!isset($_GET['OrderNo'])) {
	...
	echo '<br />';
	echo prnMsg( _('This page must be called with a purchase order number to review'), 'error');

	echo '<div class="row"><div class="col-xs-4">
		<a href="'. $RootPath . '/PO_SelectPurchOrder.php" class="btn btn-info">' . _('Outstanding Purchase Orders') . '</a></div>
		</div><br />
';
...
if (DB_num_rows($GetOrdHdrResult)!=1) {
	...
	if (DB_num_rows($GetOrdHdrResult) == 0){
		echo prnMsg ( _('Unable to locate this PO Number') . ' '. $_GET['OrderNo'] . '. ' . _('Please search for another one') . '. ' . _('The order requested could not be retrieved') . ' - ' . _('the querry returned either 0 or several purchase orders'), 'error');
	} else {
		echo prnMsg ( _('The order requested could not be retrieved') . ' - ' . _('the querry returned either several purchase orders'), 'error');
	}
        echo '<div class="row"><div class="col-xs-4"><a href="'. $RootPath . '/PO_SelectPurchOrder.php" class="btn btn-info">' . _('Outstanding Purchase Orders') . '</a></div>
					</div>
				';
...
echo '<p align="left"><a href="' . $RootPath . '/PO_SelectPurchOrder.php" class="btn btn-info">' . _('Back to POs') . '</a></p>';
...
<table id="general-table" class="table table-bordered">
		...
		<tr>
			...
			<td><a href="SelectSupplier.php?SupplierID='.$myrow['supplierid'].'" class="btn btn-info">' . $myrow['supplierid'] . '</a></td>
			<td>' . _('Supplier Name'). '</td>
			<td><a href="SelectSupplier.php?SupplierID='.$myrow['supplierid'].'" class="btn btn-info">' . $myrow['suppname'] . '</a></td>
		</tr>
		<tr>
			<td>' . _('Ordered On'). '</td>
...
if ($myrow['dateprinted'] == ''){
	...
	echo '<a href="PO_PDFPurchOrder.php?OrderNo='. $_GET['OrderNo'] .'" class="btn btn-warning">' .  _('Print')  . '</a>';
} else {
	echo _('Printed on').' '. ConvertSQLDate($myrow['dateprinted']). '&nbsp; &nbsp;';
	echo '<a href="PO_PDFPurchOrder.php?OrderNo='. $_GET['OrderNo'] .'" class="btn btn-warning">' .  _('Print a Copy')  . '</a>';
}
