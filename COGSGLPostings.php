...
//the link to delete a selected record was clicked instead of the submit button
	...
	$result = DB_query($sql);
	echo prnMsg( _('The cost of sales posting code record has been deleted'),'info');
	unset ($SelectedCOGSPostingID);
}
...
if (!isset($SelectedCOGSPostingID)) {
	...
	if (DB_num_rows($result)>0){
		...
		echo prnMsg (_('The following cost of sales posting records that do not have valid general ledger code specified - these records must be amended.'),'error');
...
<table id="general-table" class="table table-bordered">
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			printf('<tr class="striped_row">
					...
					<td>%s</td>
					<td>%s</td>
					<td><a href="%sSelectedCOGSPostingID=%s" class="btn btn-info">' . _('Edit') . '</a></td>
					<td><a href="%sSelectedCOGSPostingID=%s&amp;delete=yes" onclick="return confirm(\'' . _('Are you sure you wish to delete this COGS GL posting record?') . '\');" class="btn btn-danger">' .  _('Delete') . '</a></td></tr>',
					$myrow['area'],
					$myrow['stkcat'],
					$myrow['salestype'],
					$myrow['accountname'],
...
<table id="general-table" class="table table-bordered">
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			printf('<tr class="striped_row">
				...
				<td>%s</td>
				<td><a href="%sSelectedCOGSPostingID=%s" class="btn btn-info">' . _('Edit') . '</a></td>
				<td><a href="%sSelectedCOGSPostingID=%s&amp;delete=yes" onclick="return confirm(\'' . _('Are you sure you wish to delete this COGS GL posting record?') . '\');" class="btn btn-danger">' . _('Delete') . '</a></td>
				</tr>',
				$myrow['area'],
				$myrow['stkcat'],
				$myrow['salestype'],
...
if (isset($SelectedCOGSPostingID)) {
	echo '<div class="row"><div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') .'" class="btn btn-info">' . _('Show all cost of sales posting records') . '</a></div></div><br />';
}
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Area') . '</label>
			<select tabindex="1" name="Area" class="form-control">
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Stock Category') . '</label>
		<select tabindex="2" name="StkCat" class="form-control">
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Sales Type') . ' / ' . _('Price List') . '</label>
		<select tabindex="3" name="SalesType" class="form-control">
			<option value="AN">' . _('Any Other') . '</option>';
...
echo '</select></div>
	...
	<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Post to GL account') . '</label>
		<select tabindex="4" name="GLCode" class="form-control">';
