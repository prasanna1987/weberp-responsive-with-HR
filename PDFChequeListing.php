...
<div class="col-xs-12">';
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Enter the date from which cheques are to be listed') . '</label>
				<input type="text" name="FromDate" maxlength="10" size="11" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" value="' . Date($_SESSION['DefaultDateFormat']) . '" /></div></div>
			';
	 echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Enter the date to which cheques are to be listed') . '</label>
	 		<input type="text" name="ToDate" maxlength="10" size="11"  class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" value="' . Date($_SESSION['DefaultDateFormat']) . '" /></div>
	</div>';
	 echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Bank Account') . '</label>';
	 ...
	 echo '<select name="BankAccount" class="form-control">';
	 ...
	 echo '<div class="row">
	 <div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Email the report off') . '</label>
...
if (DB_error_no()!=0){
	...
	$Title = _('Payment Listing');
	include('includes/header.php');
  	echo prnMsg (_('There were no bank transactions found in the database within the period from') . ' ' . $_POST['FromDate'] . ' ' . _('to') . ' ' . $_POST['ToDate'] . '. ' ._('Please try again selecting a different date range or account'), 'error');
	include('includes/footer.php');
  	exit;
}
...
while ($myrow=DB_fetch_array($Result)){
	...
	if (DB_error_no()!=0){
		$Title = _('Payment Listing');
		include('includes/header.php');
   		echo prnMsg(_('An error occurred getting the GL transactions'),'error');
		if ($debug==1){
				prnMsg( _('The SQL used to get the receipt header information that failed was') . ':<br />' . $sql, 'error');
		}
		include('includes/footer.php');
...
$ReportFileName = 'nERP' . '_ChequeListing_' . date('Y-m-d').'.pdf';
...
if ($_POST['Email']=='Yes'){
	...
	if(sizeOf($ChkListingRecipients) == 0){
		echo  prnMsg(_('There are no member in Check Listing Recipients email group,  no mail will be sent'),'error');
		include('includes/footer.php');
		exit;
	}
