...
if (!isset($_SESSION['Items' . $identifier])){
	...
	if (DB_num_rows($result)==0) {
		echo prnMsg(_('Your user account does not have a valid default inventory location set up. Please contact your system administrator to modify your user account.'),'error');
		...
		if ($myrow['cashsalecustomer']=='' OR $myrow['cashsalebranch']==''){
			echo prnMsg(_('It is first necessary to define a cash sales customer for the location that is your default location. Please contact your system administrator for setup.'),'error');
			include('includes/footer.php');
			exit;
		}
		...
		if (DB_num_rows($result)==0){
			...
			if ($debug==1){
				echo '<br />' . _('The querry that failed to get the branch details was') . ':<br />' . $sql;
			}
			include('includes/footer.php');
			exit;
		}
...
if (isset($_POST['CancelReturn'])) {
	...
	echo prnMsg(_('This return has been cancelled as requested'),'success');
	echo '<p align="center"><a href="' .htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Back to Counter Returns') . '</a></p>';
	include('includes/footer.php');
	exit;
	...
	echo '' . $_SESSION['Items' . $identifier]->CustomerName . '<br /><small> ' . _('Counter Return') . ' ' . _('to') . ' ' . $_SESSION['Items' . $identifier]->LocationName . ' ' . _('inventory') . ' (' . _('all amounts in') . ' ' . $_SESSION['Items' . $identifier]->DefaultCurrency . ')';
	echo '</small></h1></a></div>';
}
...
if (isset($_POST['Search']) OR isset($_POST['Next']) OR isset($_POST['Prev'])){
	...
	if ($_POST['Keywords']!='' AND $_POST['StockCode']=='') {
		$msg='<p class="text-info">' . _('Item description has been used in search') . '.</p>';
	} else if ($_POST['StockCode']!='' AND $_POST['Keywords']=='') {
		$msg='<p class="text-info">' . _('Item Code has been used in search') . '.</p>';
 ...
 if (isset($_POST['SelectingReturnItems'])
	...
	while ($i<=$_SESSION['QuickEntries']
		...
		if (DB_num_rows($KitResult)==0){
			echo prnMsg( _('The item code') . ' ' . $NewItem . ' ' . _('could not be retrieved from the system and has not been added to the return'),'warn');
			...
			if ($myrow['mbflag']=='K'){	/*It is a kit set item */
				...
				echo prnMsg(_('Phantom assemblies cannot be returned, these items exist only as bills of materials used in other manufactured items. The following item has not been added to the return:') . ' ' . $NewItem, 'warn');
			} else if ($myrow['controlled']==1){
				echo prnMsg(_('The system does not currently allow for counter returns of lot controlled or serialised items'),'warn');
			} else if ($NewItemQty<=0) {
				echo prnMsg(_('Only items entered with a positive quantity can be added to the return'),'warn');
		...
		if (isset($_POST['Quantity_' . $ReturnItemLine->LineNumber])){
			...
			$Quantity = round(filter_number_format($_POST['Quantity_' . $ReturnItemLine->LineNumber]),$ReturnItemLine->DecimalPlaces);
				...
				if (ABS($ReturnItemLine->Price - filter_number_format($_POST['Price_' . $ReturnItemLine->LineNumber]))>0.01){
					...
					echo prnMsg(_('Recalculated the price from the GP % entered - the GP % was') . ' ' . $ReturnItemLine->GPPercent . '  the new GP % is ' . filter_number_format($_POST['GPPercent_' . $ReturnItemLine->LineNumber]),'info');
			...
			if ($Quantity<0 or $Price <0 or $DiscountPercentage >100 or $DiscountPercentage <0){
				echo prnMsg(_('The item could not be updated because you are attempting to set the quantity returned to less than 0 or the price less than 0 or the discount more than 100% or less than 0%'),'warn');
...
<table id="general-table" class="table table-bordered">
	...
	foreach ($_SESSION['Items' . $identifier]->LineItems as $ReturnItemLine) {
		...
		echo '<td><input class="form-control" tabindex="2" type="text" name="Quantity_' . $ReturnItemLine->LineNumber . '" required="required" size="6" maxlength="6" value="' . locale_number_format($ReturnItemLine->Quantity,$ReturnItemLine->DecimalPlaces) . '" />';

		echo '</td>
				<td>' . $ReturnItemLine->Units . '</td>
				<td><input class="form-control" type="text" name="Price_' . $ReturnItemLine->LineNumber . '" required="required" size="16" maxlength="16" value="' . locale_number_format($ReturnItemLine->Price,$_SESSION['Items' . $identifier]->CurrDecimalPlaces) . '" /></td>
				<td><input class="form-control" type="text" name="Discount_' . $ReturnItemLine->LineNumber . '" required="required" size="5" maxlength="4" value="' . locale_number_format(($ReturnItemLine->DiscountPercent * 100),2) . '" /></td>
				<td><input class="form-control" type="text" name="GPPercent_' . $ReturnItemLine->LineNumber . '" required="required" size="3" maxlength="40" value="' . locale_number_format($ReturnItemLine->GPPercent,2) . '" /></td>
		...
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier . '&Delete=' . $ReturnItemLine->LineNumber . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');" class="btn btn-danger">' . _('Delete') . '</a></td></tr>';

		if ($_SESSION['AllowOrderLineItemNarrative'] == 1){
			echo '<tr class="striped_row">
					<td valign="top" colspan="11">' . _('Narrative') . ':<textarea name="Narrative_' . $ReturnItemLine->LineNumber . '" class="form-control">' . stripslashes(AddCarriageReturns($ReturnItemLine->Narrative)) . '</textarea><br /></td></tr>';
		} else {
...
<div class="col-xs-12">';
	...
	echo '<div class="row">
		<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' .  _('Returned By') .'</label>
		<input type="text" size="25" required="required" maxlength="25" name="DeliverTo" class="form-control" value="' . stripslashes($_SESSION['Items' . $identifier]->DeliverTo) . '" /></div>
	</div>';
	echo '<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' .  _('Contact Phone Number') .'</label>
		<input type="tel" size="25" maxlength="25" name="PhoneNo" class="form-control"  required="required" title="' . _('A phone number is required for counter returns') . '" value="' . stripslashes($_SESSION['Items' . $identifier]->PhoneNo) . '" /></div>
	</div>';

	echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Contact Email') . '</label>
			<input type="email" size="25" class="form-control" maxlength="30" name="Email" value="' . stripslashes($_SESSION['Items' . $identifier]->Email) . '" /></div></div></div>';

	echo '<div class="row">
			<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' .  _('Customer Reference') .'</label>
			<input type="text" size="25" maxlength="25" class="form-control" name="CustRef" value="' . stripcslashes($_SESSION['Items' . $identifier]->CustRef) . '" /></div>
		</div>';
	echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Sales person'). '</label>
	...
	echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' .  _('Reason for Return') .'</label>
		<textarea name="Comments" class="form-control">' . stripcslashes($_SESSION['Items' . $identifier]->Comments)  . '</textarea></div>
	</div>';
	echo '</div>'; //end the sub table in the first column of master table
	echo '<div class="row">'; //for the master table
	...
	echo '<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Paid to Customer') . '</label>
			<input type="text" class="form-control" name="AmountPaid"  required="required" maxlength="12" size="12" value="' . $_POST['AmountPaid'] . '" /></div>
		</div>';

	echo '</div></div>'; //end the sub table in the second column of master table
	echo '</div>';	//end of column/row/master table
}
	if (!isset($_POST['ProcessReturn'])){
		echo '<br />
				<div class="row">
				<div class="col-xs-4">
					<input type="submit" class="btn btn-info" name="Recalculate" value="' . _('Re-Calculate') . '" />
					</div>
				<div class="col-xs-4">	
					<input type="submit" class="btn btn-success" name="ProcessReturn" value="' . _('Submit') . '" />
				</div>
				
		
			<div class="col-xs-4">	
				<input type="submit" class="btn btn-danger" name="CancelReturn" value="' . _('Cancel Return') . '" onclick="return confirm(\'' . _('Are you sure you wish to cancel this return?') . '\');" />
			</div>
...
if (isset($_POST['ProcessReturn']) AND $_POST['ProcessReturn'] != ''){
	...
	if ($_SESSION['Items' . $identifier]->LineCounter == 0){
		echo prnMsg(_('There are no line items on this return. Please enter line items to return'),'error');
		$InputError = true;
	}
	if (abs(filter_number_format($_POST['AmountPaid']) -round($_SESSION['Items' . $identifier]->total+filter_number_format($_POST['TaxTotal']),$_SESSION['Items' . $identifier]->CurrDecimalPlaces))>=0.01) {
		echo prnMsg(_('The amount entered as payment to the customer does not equal the amount of the return. Please correct amount and re-enter'),'error');
		$InputError = true;
	}
	...
	/*company record read in on login with info on GL Links and debtors GL account*/
		...
		if ($_SESSION['CompanyRecord']==0){
			...
			echo prnMsg( _('The company information and preferences could not be retrieved. Please contact your system administrator'), 'error');
			include('includes/footer.php');
			exit;
		}
	...
	// *************************************************************************
		...
		if ($_SESSION['InvoicePortraitFormat']==0){
			echo '<div class="col-xs-4">' . ' ' . '<a target="_blank" class="btn btn-warning" href="'.$RootPath.'/PrintCustTrans.php?FromTransNo='.$CreditNoteNo.'&InvOrCredit=Credit&PrintPDF=True">' .  _('Print this credit note'). ' </a></div>';
		} else {
			echo '<div class="col-xs-4">' . ' ' . '<a target="_blank" class="btn btn-warning" href="'.$RootPath.'/PrintCustTransPortrait.php?FromTransNo='.$CreditNoteNo.'&InvOrCredit=Credit&PrintPDF=True">' .  _('Print this credit note'). ' </a></div>';
		}
		echo '<div class="col-xs-4"><a href="' .htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Back to Counter Returns') . '</a></div></div><br />';
...
' . _('Select a Stock Category') . ' </label><select tabindex="1" name="StockCat" class="form-control">';
		...
		<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">
<?php echo _('Description-part or full'); ?></label>
		<input tabindex="2" type="text" class="form-control" name="Keywords" <?php isset($SearchResult)? '': 'autofocus="autofocus"' ?> size="20" maxlength="25" value="<?php if (isset($_POST['Keywords'])) echo $_POST['Keywords']; ?>" /></div></div>

		<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label"><?php echo _('Stock ID-part or full'); ?></label>
		<input tabindex="3" class="form-control" type="text" name="StockCode" size="15" maxlength="18" value="<?php if (isset($_POST['StockCode'])) echo $_POST['StockCode']; ?>" /></div>
		...
		<input tabindex="4" type="submit" class="btn btn-success" name="Search" value="<?php echo _('Search'); ?>" /></div>
        </div>
        <div class="col-xs-4">
        <div class="form-group"> 
		<input tabindex="5" type="submit" class="btn btn-info" name="QuickEntry" value="<?php echo _('Quick Entry'); ?>" /></div>
</div>
</div>
...
<table class="table">
			<tr>
					<td align="left"><input name="PreviousList" type="hidden" value="'.strval($Offset-1).'" /><input type="submit" name="Prev" class="btn btn-default" value="'._('Previous').'" /></td>
					<td align="center"><input name="SelectingReturnItems" type="hidden" value="1" /><input tabindex="'.strval($j+8).'" type="submit" value="'._('Add to Returns').'" class="btn btn-success" /></td>
					<td align="right"><input name="NextList" type="hidden" value="'.strval($Offset+1).'" /><input tabindex="'.strval($j+9).'" name="Next" class="btn btn-default" type="submit" value="'._('Next').'" /></td>
					</tr></table></div></div>';
...
';
			...
			while ($myrow=DB_fetch_array($SearchResult)) {
				...
				printf('<tr class="striped_row">
						...
						<td class="number">%s</td>
						<td class="number">%s</td>
						<td class="has-error"><input class="form-control"  tabindex="'.strval($j+7).'" type="text"  required="required" ' . ($i==0?'autofocus="autofocus"':'') . ' size="6" name="ReturnQty%s" value="0" /><input type="hidden" name="StockID%s" value="%s" /></td>
						</tr>',
						$myrow['stockid'],
...
<table class="table">
					...
					<tr>
					<td align="left"><input name="previous" type="hidden" value="'. strval($Offset-1).'" /><input tabindex="'.strval($j+7).'" type="submit" name="Previous" value="'._('Previous').'" class="btn btn-default" /></td>
					<td align="center"><input name="SelectingReturnItems" type="hidden" value="1" /><input tabindex="'.strval($j+8).'" type="submit" value="'._('Add to Returns').'" class="btn btn-success" /></td>
					<td align="right"><input name="NextList" type="hidden" value="'.strval($Offset+1).'" /><input tabindex="'.strval($j+9).'" name="Next" type="submit" value="'._('Next').'" class="btn btn-default" /></td>
					</tr></table></div></div><br /><br /><br /><br />
...
<table id="general-table" class="table table-bordered">
		...
		for ($i=1;$i<=$_SESSION['QuickEntries'];$i++){
	 		...
	 		/* Do not display colum unless customer requires po line number by sales order line*/
	 		echo '<td><input type="text" class="form-control" name="part_' . $i . '" ' . ($i==1 ? 'autofocus="autofocus" ': '') . 'size="21" data-type="no-illegal-chars" title="' . _('Enter a part code to be returned. Part codes can contain any alpha-numeric characters underscore or hyphen.') . '" maxlength="20" /></td>
		...
		<div class="row">
		<div class="col-xs-4">	
					<input type="submit" name="PartSearch" class="btn btn-success" value="' . _('Search') . '" />
				</div>
		</div>
				
				
				<div align="center">
					<input type="submit" name="QuickEntry" class="btn btn-success" value="' . _('Quick Entry') . '" />
					</div>
