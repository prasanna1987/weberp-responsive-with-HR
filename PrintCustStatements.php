...
/*Figure out who all the customers in this range are */
	...
	if (DB_Num_Rows($StatementResults) == 0){
		...
		echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Print Customer Account Statements') . '</h1></a></div>';
		echo prnMsg( _('There were no Customers matching your selection of ') . $_POST['FromCust'] . ' - ' . $_POST['ToCust'] . '.' , 'error');
	...
	if ($_POST['EmailOrPrint']=='print') {
		...
		echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Emailing Customer Account Statements') . '</h1></a></div>';
...
$pdf->SetFont('arsenal');
	...
	if (isset($pdf) AND $_POST['EmailOrPrint']=='print'){
		$pdf->OutputD('nERP' . '_CustStatements_' . date('Y-m-d') . '.pdf');
		$pdf->__destruct();
	} elseif (!isset($pdf)) {
		...
		if ($_POST['EmailOrPrint']=='print') {
			include('includes/header.php');
			echo '<br />
				<div class="alert alert-warning alert-dismissable" id="MessageContainerHead">
              <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' ,prnMsg( _('There were no statements to print'),'warn');
		} else {
			echo '<br />
				<div class="alert alert-warning alert-dismissable" id="MessageContainerHead">
              <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' ,prnMsg( _('There were no statements to email'),'warn');
		}
		echo'<br />';	
		include('includes/footer.php');
	}
...
<div class="col-xs-12">';
        ...
        echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' , _('Starting Customer code)') , '</label>
				<input type="text" class="form-control" maxlength="10" size="8" name="FromCust" value="0" /></div></div>
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' ,  _('Ending Customer code)') , '</label>
				<input type="text" maxlength="10" size="8" name="ToCust" class="form-control" value="zzzzzz" /></div>
			</div>
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' , _('Option') , '</label>
				<select name="EmailOrPrint" class="form-control">
					<option selected="selected" value="print">', _('Print') , '</option>
					<option value="email">', _('Email to customer contacts') , '</option>
					</select></div>
			</div>
			</div>
				<div class="row">
...
<div class="form-group">
				<input type="submit" name="PrintPDF" value="' . _('Submit').'" class="btn btn-success" />
			</div></div>';
        echo '</div>
              </form>';
	}
