...
echo '<div class="block-header"><a href="" class="header-title-link"><h1> ', // Icon title.
	$PageTitleText, '</h1></a></div>';// Page title.

include('includes/SQL_CommonFunctions.inc');

if(isset($_POST['PaymentCancelled'])) {
	echo prnMsg(_('Payment Cancelled since cheque was not printed'), 'warn');
...
if((isset($_POST['UpdateHeader']) AND $_POST['BankAccount']=='')
	...
	echo prnMsg(_('A bank account must be selected to make this payment from'), 'warn');
...
if(isset($_GET['SupplierID'])) {
	...
	if(DB_num_rows($Result)==0) {
		...
		echo prnMsg( _('The supplier code is not a currently defined supplier code') . '. ' ,'warn');
		include('includes/footer.php');
		exit;
...
if(isset($_POST['Paymenttype']) AND $_POST['Paymenttype']!='') {
	...
	if($myrow[0] == 1) {
		if(empty($_POST['ChequeNum'])) {
			echo prnMsg(_('The cheque number should not be empty'),'warn');
			$Errors[] = 'ChequeNum';
		} else {
			...
			if($cheqrow[0]>0) {
				echo prnMsg(_('The cheque has already been used'),'warn');
				$Errors[] = 'ChequeNum';
			}
		}
	}
...
if(isset($_POST['CommitBatch']) AND empty($Errors)) {
	...
	*/
	...
	if($TotalAmount==0 AND
		...
		echo prnMsg( _('This payment has no amounts entered and will not be processed'),'warn');
		include('includes/footer.php');
		exit;
	}

	if($_POST['BankAccount']=='') {
		echo prnMsg( _('No bank account has been selected so this payment cannot be processed'),'warn');
		include('includes/footer.php');
		exit;
	}
	...
	// it is a supplier payment by cheque and haven't printed yet so print cheque
		...
		if(empty($_POST['ChequeNum'])) {
			echo prnMsg(_('There are no Check Number input'),'error');
			include('includes/footer.inc');
			exit;
		} elseif(!is_numeric($_POST['ChequeNum'])) {//check if this cheque no has been used
			echo prnMsg(_('The cheque no should be numeric'),'error');
			include('includes/footer.inc');
			exit;
		} else {
			$cheqsql = "SELECT count(chequeno) FROM  supptrans WHERE chequeno='" . $_POST['ChequeNum'] . "'";
			...
			$cheqrow = DB_fetch_row($cheqresult);
			if($cheqrow[0]>0) {
				echo prnMsg(_('The cheque has already been used'),'error');
				include('includes/footer.inc');
				exit;
			}
		}
		...
		echo '<br /><p align="right">
			<a href="' . $RootPath . '/PrintCheque.php?ChequeNum=' . $_POST['ChequeNum'] . '&amp;identifier=' . $identifier . '" class="btn btn-default">' . _('Print Cheque using pre-printed stationery') . '</a></p>
			...
			<div class="row">
			<div class="col-xs-4"><input type="submit" class="btn btn-info" name="ChequePrinted" value="' , _('Yes / Continue') , '" /></div>
			<div class="col-xs-4"><input type="submit" class="btn btn-info" name="PaymentCancelled" value="' , _('No / Cancel Payment') , '" /></div>
			<div class="col-xs-4">' , _('Payment amount') , ' = ' , $_SESSION['PaymentDetail'.$identifier]->Amount ,
			'</div></div>
			</form>';
		...
		DB_Txn_Commit();
		echo prnMsg(_('Payment') . ' ' . $TransNo . ' ' . _('has been successfully entered'),'success');
...
echo '<br /><div class="row">';
		...
		if(isset($LastSupplier) and $LastSupplier!='') {
			...
			echo '
<div class="col-xs-4"><a href="' . $RootPath . '/SupplierAllocations.php?AllocTrans=' . $TransRow['id'] . '" class="btn btn-info">' . _('Allocate this payment') . '</a></div>';
			echo '<div class="col-xs-4"><a href="' . $RootPath . '/Payments.php?SupplierID=' . $LastSupplier . '" class="btn btn-info">' . _('Enter another Payment for') . ' ' . $SupplierRow['suppname'] . '</a></div>';
		} else {
			echo '<div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Enter another General Ledger Payment') . '</a></div>';
		}
		echo '</div><br />';
	}
 ...
 /* User hit delete the receipt entry from the batch */
	...
	if(is_numeric($_POST['GLManualCode'])) {
		...
		if(DB_num_rows($Result)==0) {
			...
			echo prnMsg( _('The Cheque/Voucher number has already been used') . ' - ' . _('This GL analysis item could not be added'),'error');
		} else {
			$myrow = DB_fetch_array($Result);
			...
			if($_SESSION['ProhibitJournalsToControlAccounts'] == 1) {
				if($_SESSION['CompanyRecord']['gllink_debtors'] == '1' AND $_POST['GLManualCode'] == $_SESSION['CompanyRecord']['debtorsact']) {
					echo prnMsg(_('Payments involving the debtors control account cannot be entered. The general ledger debtors ledger (AR) integration is enabled so control accounts are automatically maintained. This setting can be disabled in System Configuration'), 'warn');
	 			...
	 			if($_SESSION['CompanyRecord']['gllink_creditors'] == '1' AND
					($_POST['GLManualCode'] == $_SESSION['CompanyRecord']['creditorsact'] OR $_POST['GLManualCode'] == $_SESSION['CompanyRecord']['grnact'])) {
	 				echo prnMsg(_('Payments involving the creditors control account or the GRN suspense account cannot be entered. The general ledger creditors ledger (AP) integration is enabled so control accounts are automatically maintained. This setting can be disabled in System Configuration'), 'warn');
	 				$AllowThisPosting = false;
	 			}
	 			if($_POST['GLManualCode'] == $_SESSION['CompanyRecord']['retainedearnings']) {
	 				echo prnMsg(_('Payments involving the retained earnings control account cannot be entered. This account is automtically maintained.'), 'warn');
	 				$AllowThisPosting = false;
	 			}
	 		}
	 		if($AllowThisPosting) {
		...
		echo prnMsg( _('The cheque number has already been used') . ' - ' . _('This GL analysis item could not be added'),'error');
	} elseif($_POST['GLCode'] == '') {
			echo prnMsg( _('No General Ledger code has been chosen') . ' - ' . _('so this GL analysis item could not be added'),'warn');
	} else {
...
if(isset($_POST['BankAccount']) AND $_POST['BankAccount']!='') {
	...
	if(DB_num_rows($result)==1) {
		...
		echo prnMsg( _('The bank account number') . ' ' . $_POST['BankAccount'] . ' ' . _('is not set up as a bank account with a valid general ledger account'),'error');
	}
}
...
echo '<form action="' , htmlspecialchars($_SERVER['PHP_SELF'] . '?identifier=' . $identifier) , '" method="post">
	...
	<h3 class="page-header">' , _('Payment');
...
echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Bank Account'), '</label>
		<select autofocus="autofocus" class="form-control" name="BankAccount" onchange="ReloadForm(UpdateHeader)" required="required" title="', _('Select the bank account that the payment has been made from'), '">';
...
if(DB_num_rows($AccountsResults)==0) {
	echo '</select></div>
		...
		<p />';
	echo prnMsg( _('Bank Accounts have not yet been defined. You must first') . ' <a href="' . $RootPath . '/BankAccounts.php">' . _('<strong>define the bank accounts</strong>') . '</a> ' . _('and general ledger accounts to be affected'),'warn');
	include('includes/footer.php');
	exit;
} else {
...
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Date'), '</label>
		<input class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" maxlength="10" name="DatePaid" onchange="isDate(this, this.value, ', "'", $_SESSION['DefaultDateFormat'], "'", ')" required="required" size="11" type="text" value="', $_SESSION['PaymentDetail'.$identifier]->DatePaid, '" /></div>
	</div>';
...
if(DB_num_rows($result) == 0) {
	echo prnMsg( _('No currencies are defined yet. Payments cannot be entered until a currency is defined'),'error');
	
} else {
	include('includes/CurrenciesArray.php');// To get the currency name from the currency code.
	if($_SESSION['PaymentDetail'.$identifier]->SupplierID == '') {
		echo '<select name="Currency" class="form-control" onchange="ReloadForm(UpdateHeader)" required="required">';
		...
		echo '<select disabled="disabled" class="form-control">';
		while($myrow=DB_fetch_array($result)) {
			echo '<option ';
			if($_SESSION['PaymentDetail'.$identifier]->Currency == $myrow['currabrev']) {
				echo 'selected="selected" ';
		...
		echo '</select>';
		echo ' ', _('It is a supplier payment so it should be in the supplier\'s currency'), '</div></div></div>
		';// Sometimes some people pay in a currency different from supplier's document currency (e.g. invoice in euros paid in dollars).
...
if($_SESSION['PaymentDetail'.$identifier]->AccountCurrency != $_SESSION['PaymentDetail'.$identifier]->Currency AND isset($_SESSION['PaymentDetail'.$identifier]->AccountCurrency)) {
	...
	if(isset($SuggestedExRate)) {
		$SuggestedExRateText = '' . _('Suggested rate:') . ' 1 ' . $_SESSION['PaymentDetail'.$identifier]->AccountCurrency . ' = '	. locale_number_format($SuggestedExRate, 'Variable') . ' ' . $_SESSION['PaymentDetail'.$identifier]->Currency . '';
	} else {
		$SuggestedExRateText = '1 ' . $_SESSION['PaymentDetail'.$identifier]->AccountCurrency . ' = ? ' . $_SESSION['PaymentDetail'.$identifier]->Currency;
	}
	echo '<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Payment Exchange Rate'), '</label>
			<input class="form-control" maxlength="12" name="ExRate" size="14" title="', _('The exchange rate between the currency of the bank account and the currency of the payment'), '" type="text" value="', $_POST['ExRate'], '" /></div>
		</div>
		<div class="col-xs-8">
<div class="form-group"><br />
 <label class="col-md-12 control-label text-info">  ', $SuggestedExRateText, ' ', _('The exchange rate between the currency of the bank account and the currency of the payment'), '</label></div></div>
...
if($_SESSION['PaymentDetail'.$identifier]->AccountCurrency != $_SESSION['CompanyRecord']['currencydefault'] AND isset($_SESSION['PaymentDetail'.$identifier]->AccountCurrency)) {
	...
	if(isset($SuggestedFunctionalExRate)) {
		$SuggestedFunctionalExRateText = '<strong>' . _('Suggested rate:') . ' 1 ' . $_SESSION['CompanyRecord']['currencydefault'] . ' = ' . locale_number_format($SuggestedFunctionalExRate, 'Variable') . ' ' . $_SESSION['PaymentDetail'.$identifier]->AccountCurrency . '</strong>';
	} else {
		$SuggestedFunctionalExRateText = '1 ' . $_SESSION['CompanyRecord']['currencydefault'] . ' = ? ' . $_SESSION['PaymentDetail'.$identifier]->AccountCurrency;
	}
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Default Exchange Rate'), '</label>
			<input class="form-control" maxlength="16" name="FunctionalExRate" pattern="[0-9\.,]*" required="required" size="18" type="text" value="', $_POST['FunctionalExRate'], '" /> ', $SuggestedFunctionalExRateText, ' ', _('The exchange rate between the default currency and the currency of the bank account'), '</div>
		</div>';
}
echo '<div class="col-xs-4">
...
echo '
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Cheque Number') . '</label>
		<input maxlength="8" name="ChequeNum" size="10" type="text" class="form-control" value="' . $_POST['ChequeNum'] . '" ' . $ErrClass . ' /> ' . _('(if using pre-printed stationery)') . '</div>
	</div></div>';
...
<div class="form-group"> <label class="col-md-8 control-label">', _('Reference'), '</label>
		<input maxlength="50" name="BankTransRef" class="form-control" size="52" type="text" value="', stripslashes($_POST['BankTransRef']), '" /> <i>', _('Reference on Bank Transactions Inquiry'), '.</i></div>
	</div>';
...
<div class="form-group"> <label class="col-md-8 control-label">', _('Narrative'), '</label>
		<input maxlength="200" class="form-control" name="Narrative" size="52" type="text" value="', stripslashes($_POST['Narrative']), '" /> <i>', _('Narrative on General Ledger Account Inquiry'), '.</i></div>
	</div>';

echo '
	<div class="col-xs-4"><div class="form-group"> <br /><input name="PreviousCurrency" type="hidden" value="', $_POST['Currency'], '" />
	<input type="hidden" name="PreviousBankAccount" value="' . $_SESSION['PaymentDetail'.$identifier]->Account . '" />
	<input name="UpdateHeader" type="submit" value="', _('Update'), '" class="btn btn-info" />
		</div></div></div>
...
/* Set upthe form for the transaction entry for a GL Payment Analysis item */
	...
	echo '<h3 class="sub-header">' . _('General Ledger Payment Analysis Entry') . '</h3><br />
		<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Select Tag') . '</label>
	...
	if(isset($_POST['GLManualCode'])) {
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Enter GL Account Manually') . '</label>
				<input type="text" class="form-control" name="GLManualCode" maxlength="12" size="12" onchange="return inArray(this, GLCode.options,\'' . _('The account code') . ' \' + this.value + \' ' . _('doesnt exist') . '\')" value="'. $_POST['GLManualCode'] .'" /></div>
			</div>';
	} else {
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Enter GL Account Manually') . '</label>
				<input type="text" class="form-control" name="GLManualCode" maxlength="12" size="12" onchange="return inArray(this, GLCode.options,\'' . _('The account code') . ' \' + this.value + \' ' . _('doesnt exist') . '\')" /></div>
			</div></div>';
	}

	//echo '<div class="col-xs-4">
//<div class="form-group"> <label class="col-md-8 control-label">' . _('Select GL Group') . '</label>
//			<select name="GLGroup" class="form-control" onchange="return ReloadForm(UpdateCodes)">';
//
...
//		echo '</select></div>
//			</div></div>';
//		echo prnMsg(_('No General ledger account groups have been set up yet') . ' - ' . _('payments cannot be analysed against GL accounts until the GL accounts are set up'),'error')
//                 ;
//	} else {
//		echo '<option value=""></option>';
//		while($myrow=DB_fetch_array($result)) {
...
//		<div class="row">
//		<div class="col-xs-4">
//				<input type="submit" name="UpdateCodes" value="Select" class="btn btn-info" /></div>
//				</div><br />';
//	}
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Select GL Account') . '</label>
	...
	if(DB_num_rows($result)==0) {
		echo '</select></div></div>';
		echo prnMsg(_('No General ledger accounts have been set up yet') . ' - ' . _('payments cannot be analysed against GL accounts until the GL accounts are set up'),'error');
	} else {
		echo '<option value=""></option>';
		while($myrow=DB_fetch_array($result)) {
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Cheque/Voucher Number') . '</label>
			<input type="text" class="form-control" name="Cheque" maxlength="12" size="12" /></div>
		</div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('GL Narrative') . '</label>
				<input maxlength="200" class="form-control" name="GLNarrative" size="52" type="text" value="' . stripslashes($_POST['GLNarrative']) . '" /></div>
			</div></div>';
	} else {
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('GL Narrative') . '</label>
				<input maxlength="200" class="form-control" name="GLNarrative" size="52" type="text" /></div>
			</div></div>';
	}
echo '<div class="row">';
	if(isset($_POST['GLAmount'])) {
		echo '
				<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Amount') . ' (' . $_SESSION['PaymentDetail'.$identifier]->Currency . ')</label>
				<input type="text" required="required" name="GLAmount" maxlength="12" size="12" class="form-control" value="' . $_POST['GLAmount'] . '" /></div>
			</div>';
	} else {
		echo '
				<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Amount') . ' (' . $_SESSION['PaymentDetail'.$identifier]->Currency . ')</label>
				<input type="text" required="required" name="GLAmount" maxlength="12" size="12" class="form-control" value="0" /></div>
			</div>';
	}
...
<table id="general-table" class="table table-bordered">
		...
		foreach($_SESSION['PaymentDetail'.$identifier]->GLItems as $PaymentItem) {
			...
			echo '<tr>
				...
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'] . '?identifier=' . $identifier) . '&amp;Delete=' . $PaymentItem->ID . '" onclick="return confirm(\'' . _('Are you sure you wish to delete this payment analysis item?') . '\');" class="btn btn-danger">' . _('Delete') . '</a></td>
				</tr>';
...
<input type="submit" name="CommitBatch" class="btn btn-success" value="' . _('Process Payment') . '" /></div></div><br />';
	}
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>', _('Supplier Transactions Payment Entry'), '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
				<tr>
					<th>' . _('Date') . '</th>
		...
		while ($myrow = DB_fetch_array($Result)) {
			...
			echo '<tr>
					...
					<td><input type="text" class="form-control" id="'.$myrow['id'].'" name="paid'.$myrow['id'].'" value="'.$_POST['paid'.$myrow['id']].'" />
		...
		echo '</table></div></div></div></div>
				<div class="row"><div class="col-xs-3"><div class="form-group"> <label class="col-md-8 control-label">
' . _('Total Input Amount').'</label><input type="text" class="form-control" id="ttl" value="0" readonly></div></div>' .'<div class="col-xs-4"><div class="form-group"> <br /> <input type="button" data-ids="' . $ids . '" value="' . _('Update') . '" onclick="update1(\''.$ids.'\')" id="update" class="btn btn-info" /></div></div></div>
				';
...
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Supplier Narrative'), '</label>
			<input class="form-control" maxlength="200" name="gltrans_narrative" size="52" type="text" value="', stripslashes($_POST['gltrans_narrative']), '" /> ', _('Supplier narrative in general ledger transactions. If blank, it uses the bank narrative.'), '</div>
		</div>';
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Supplier Reference'), '</label>
			<input class="form-control" maxlength="20" name="supptrans_suppreference" size="22" type="text" value="', stripslashes($_POST['supptrans_suppreference']), '" /> ', _('Supplier reference in supplier transactions. If blank, it uses the payment type.'), '</div>
		</div>';
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Transaction Text'), '</label>
			<input class="form-control" maxlength="200" name="supptrans_transtext" size="52" type="text" value="', stripslashes($_POST['supptrans_transtext']), '" /> ', _('Transaction text in supplier transactions. If blank, it uses the bank narrative.'), '</div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">',
				_('Amount of Payment'), ' ', $_SESSION['PaymentDetail'.$identifier]->Currency, '</label>
			<input class="form-control" maxlength="12" id="Amount" name="Amount" size="13" type="text" value="', $_SESSION['PaymentDetail'.$identifier]->Amount, '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label"><input name="SuppName" type="hidden" value="', $_SESSION['PaymentDetail'.$identifier]->SuppName, '" />',
				_('Amount of Discount'), ' ', $_SESSION['PaymentDetail'.$identifier]->Currency, '</label>
			<input class="form-control" maxlength="12" name="Discount" size="13" type="text" value="', $_SESSION['PaymentDetail'.$identifier]->Discount, '" /></div>
		</div>
	<div class="col-xs-4">
<div class="form-group"><br /><input type="submit" class="btn btn-info" name="CommitBatch" onClick=payVerify("Amount","ttl") value="' . _('Accept and Process Payment') . '" /></div></div></div>';
}
echo '
	</form>';
