...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' .// Icon title.
	_('Access Permissions Maintenance') . '</h1></a></div>';// Page title.
...
if (isset($_POST['submit']) OR isset($_GET['remove']) OR isset($_GET['add']) ) {
	...
	if ($AllowDemoMode){
		$InputError =1;
		echo prnMsg('The demo functionality is crippled to prevent access problems. No changes will be made','warn');
	}
	...
	if (isset($_POST['SecRoleName']) AND mb_strlen($_POST['SecRoleName'])<4){
		$InputError = 1;
		echo prnMsg(_('The role description entered must be at least 4 characters long'),'error');
	}
	...
	if ($myrow[0]>0) {
		echo prnMsg( _('Cannot delete this role because user accounts are setup using it'),'warn');
		echo '<br />' . _('There are') . ' ' . $myrow[0] . ' ' . _('user accounts that have this security role setting');
	} else {
		...
		$result = DB_query($sql);
		echo prnMsg( $_GET['SecRoleName'] . ' ' . _('security role has been deleted') . '!','success');
...
<table id="general-table" class="table table-bordered">
	...
	while($myrow = DB_fetch_array($result)) {
		...
		printf('<tr class="striped_row">
			<td>%s</td>
			<td><a href="%s&amp;SelectedRole=%s" class="btn btn-info">' . _('Edit') . '</a></td>
			<td><a href="%s&amp;SelectedRole=%s&amp;delete=1&amp;SecRoleName=%s" class="btn btn-danger" onclick="return confirm(\'' . _('Are you sure you wish to delete this role?') . '\');">' . _('Delete') . '</a></td>
			</tr>',
			$myrow['secrolename'],
...
if (isset($SelectedRole)) {
	echo '<br /><div class="row"><div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Back to Roles') . '</a></div></div>';
}
...
if (isset($SelectedRole)) {
	...
	if ( DB_num_rows($result) == 0 ) {
		echo prnMsg( _('The selected role is no longer available.'),'warn');
	} else {
...
echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Role') . '</label>
		<input type="text" class="form-control" name="SecRoleName" pattern=".{4,}" size="40" maxlength="40" value="' . $_POST['SecRoleName'] . '" required="true" title="'._("The role description entered must be at least 4 characters long").'" /></div>
	</div>';
...
<tr>';
	...
	if (DB_num_rows($Result)>0 ) {
		echo '<th colspan="3">' . _('Assigned Security Tokens') . '</th>';
		echo '<th colspan="3">' . _('Available Security Tokens') . '</th>';
	}
	echo '</tr>';
	...
	while($AvailRow = DB_fetch_array($Result)) {
		...
		if (in_array($AvailRow['tokenid'],$TokensUsed)){
			printf('<tr class="striped_row">
					<td>%s</td>
					<td>%s</td>
					<td><a href="%sSelectedRole=%s&amp;remove=1&amp;PageToken=%s" onclick="return confirm(\'' . _('Are you sure you wish to delete this security token from this role?') . '\');" " class="btn btn-danger">' . _('Remove') . '</a></td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					</tr>',
			...
			printf('<tr class="striped_row">
					...
					<td>%s</td>
					<td>%s</td>
					<td><a href="%sSelectedRole=%s&amp;add=1&amp;PageToken=%s" class="btn btn-success">' . _('Add') . '</a></td>
					</tr>',
					$AvailRow['tokenid'],
					$AvailRow['tokenname'],
					htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8')  . '?',
