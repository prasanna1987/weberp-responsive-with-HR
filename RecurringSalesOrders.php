...
if ((!isset($_SESSION['Items'.$identifier]) OR $_SESSION['Items'.$identifier]->ItemsOrdered == 0) AND $NewRecurringOrder=='Yes'){
	echo prnMsg(_('A new recurring order can only be created if an order template has already been created from the normal order entry process') . '. ' . _('To enter an order template select sales order entry from the orders tab of the main menu'),'error');
...
if (isset($_POST['DeleteRecurringOrder'])){
	...
	echo prnMsg(_('Successfully deleted recurring sales order template number') . ' ' . $_POST['ExistingRecurrOrderNo'],'success');

	echo '<br /><div class="row" align="center">
<a href="'.$RootPath.'/SelectRecurringSalesOrder.php" class="btn btn-default">' .  _('Back To Recurring Orders')  . '</a></div><br />';
...
If (isset($_POST['Process'])) {
	...
	If (!Is_Date($_POST['StartDate'])){
		$InputErrors =1;
		echo prnMsg(_('The last recurrence or start date of this recurring order must be a valid date in the format') . ' ' . $_SESSION['DefaultDateFormat'],'error');
	}
	If (!Is_Date($_POST['StopDate'])){
		$InputErrors =1;
		echo prnMsg(_('The end date of this recurring order must be a valid date in the format') . ' ' . $_SESSION['DefaultDateFormat'],'error');
	}
	If (Date1GreaterThanDate2 ($_POST['StartDate'],$_POST['StopDate'])){
		$InputErrors =1;
		echo prnMsg(_('The end date of this recurring order must be after the start date'),'error');
	}
	...
	if ($InputErrors == 0 ){  /*Error checks above all passed ok so lets go*/
		...
		if ($NewRecurringOrder=='Yes'){
			...
			echo prnmsg(_('The new recurring order template has been added'),'success');
			...
			echo prnmsg(_('The recurring order template has been updated'),'success');
		}

	echo '<div class="row">
<div class="col-xs-4"><a href="'.$RootPath.'/SelectOrderItems.php?NewOrder=Yes" class="btn btn-default">' .  _('Back to Sales  Orders')  . '</a></div>';

	echo '<div class="col-xs-4"><a href="'.$RootPath.'/SelectRecurringSalesOrder.php" class="btn btn-default">' .  _('Back to Recurrinng Orders')  . '</a></div></div><br />';
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' '. _('Recurring Order for Customer') .' : ' . $_SESSION['Items'.$identifier]->CustomerName  . '</h1></a></div>';
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Order Line Details') . '</h3></div>
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Order Header Details') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">';
echo '<tr>
	<td>' .  _('Deliver To') .':</td>
...
if ($NewRecurringOrder=='Yes'){
	echo '<tr>
	<td>' .  _('Start Date') .':</td>
	<td><input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="StartDate" size="11" maxlength="10" value="' . $_POST['StartDate'] .'" /></td></tr>';
} else {
	echo '<tr>
...
echo '<tr>
	<td>' .  _('Finish Date') .':</td>
	<td><input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="StopDate" size="11" maxlength="10" value="' . $_POST['StopDate'] .'" /></td></tr>';
...
if ($_SESSION['Items'.$identifier]->AllDummyLineItems()==true){
	...
	echo '<tr><td>' . _('Invoice Automatically') . ':</td>
		<td><select name="AutoInvoice" class="form-control">';
	...
	echo '</select></td></tr></table></div></div></div></div>';
} else {
...
<input type="submit" name="DeleteRecurringOrder" class="btn btn-danger" value="' . _('Delete Recurring Order') . ' ' . $_POST['ExistingRecurrOrderNo'] . '" onclick="return confirm(\'' . _('Are you sure you wish to delete this recurring order template?') . '\');" /></div>';
if ($NewRecurringOrder=='Yes'){
	echo '<input type="hidden" name="NewRecurringOrder" value="Yes" />';
	echo '<div class="col-xs-6">
<input type="submit" name="Process" class="btn btn-success" value="' . _('Submit') . '" /></div>';
} else {
	...
	echo '<div class="col-xs-6">
<input type="submit" name="Process" class="btn btn-success" value="' . _('Update') . '" /></div>';
