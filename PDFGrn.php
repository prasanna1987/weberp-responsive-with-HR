...
if ($NoOfGRNs >0){
	...
	for ($i=1;$i<=$NoOfGRNs;$i++) {
		...
		$LeftOvers = $pdf->addTextWrap($FormDesign->Data->Column4->x,$Page_Height-$YPos,$FormDesign->Data->Column4->Length,$FormDesign->Data->Column4->FontSize, $SuppliersQuantity, 'left');
		...
		$LeftOvers = $pdf->addTextWrap($FormDesign->Data->Column6->x,$Page_Height-$YPos,$FormDesign->Data->Column6->Length,$FormDesign->Data->Column6->FontSize, $OurUnitsQuantity, 'left');
    ...
    $pdf->OutputD('nERP' . '_GRN_' . $GRNNo . '_' . date('Y-m-d').'.pdf');
    $pdf->__destruct();
} else { //there were not GRNs to print
	$Title = _('GRN Error');
	include('includes/header.php');
	echo   prnMsg(_('There were no GRNs to print'),'warn');
	echo '<br /><p align="right"><a href="'.$RootPath.'/menu_data.php?Application=stock" class="btn btn-default">' .  _('<i class="fa fa-hand-o-left fa-fw"></i> to Menu') . '</a></p><br />';
