...
if (isset($_POST['UpdateDatabase']) OR isset($_POST['RefreshAllocTotal'])) {
	...
	if (!isset($_SESSION['Alloc'])) {
		echo prnMsg(
			_('Allocations can not be processed again') . '. ' .
				...
				_('to avoid this error in future'),
			'info');
	...
	// loop through amounts allocated using AllocnItm->ID for each record
		...
		if (isset($_POST['Amt' . $AllocCounter])) {
			...
			if (filter_number_format($_POST['Amt' . $AllocCounter]) < 0) {
				echo prnMsg(_('Amount entered was negative') . '. ' . _('Only positive amounts are allowed') . '.','warn');
				$_POST['Amt' . $AllocCounter] = 0;
			}
	...
	if ($TotalAllocated + $_SESSION['Alloc']->TransAmt > 0.008) {
		echo prnMsg(_('Allocation could not be processed because the amount allocated is more than the').' ' . $_SESSION['Alloc']->TransTypeName  . ' '._('being allocated') . '<br />' . _('Total allocated').' = ' . $TotalAllocated . ' '._('and the total amount of the') .' ' . $_SESSION['Alloc']->TransTypeName  . ' ' . _('was').' ' . -$_SESSION['Alloc']->TransAmt,'error');
		$InputError=1;
	}
}
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Allocate Receipts') . '</h1></a></div>';
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h2>' . $_SESSION['Alloc']->DebtorNo . ' - ' . $_SESSION['Alloc']->CustomerName . '</h2></div>
...
<table id="general-table" class="table table-bordered">
	...
	if ($_SESSION['Alloc']->TransExRate != 1) {
			echo '<br />' . _('Amount in customer currency') . ' <strong>' . locale_number_format(-$_SESSION['Alloc']->TransAmt,$_SESSION['Alloc']->CurrDecimalPlaces) . '</strong><i> (' .  _('converted into local currency at an exchange rate of'). ' ' . $_SESSION['Alloc']->TransExRate . ')</i>';
	}
	echo '</th>
		</tr>
		<tr>
			<th>' . _('Transaction') . '<br />' . _('Type') . '</th>
			<th>' . _('Transaction') . '<br />' . _('Number') . '</th>
			<th>' . _('Transaction') . '<br />' . _('Date') . '</th>
			<th>' . _('Total') . '<br />' . _('Receipts') . '</th>
	...
	echo '<tr>
			<td colspan="5" class="number"><strong>' . _('Total Allocated').':</strong></td>
			<td class="number"><strong>' . locale_number_format($TotalAllocated,$_SESSION['Alloc']->CurrDecimalPlaces) . '</strong></td>';
	$j++;
	echo '<td colspan="2">
			<input tabindex="'.$j.'" type="submit" class="btn btn-info" name="RefreshAllocTotal" value="' . _('Recalculate Total To Allocate') . '" /></td>
        </tr>
		<tr>
			<td colspan="5" class="number"><strong>' . _('Left to allocate') . '</strong></td>
		...
		<div class="row">
			<div class="col-xs-4">
<input tabindex="' . $j . '" type="submit" class="btn btn-info" name="UpdateDatabase" value="' . _('Process Allocations') . '" />
</div>
<div class="col-xs-4">
			<input tabindex="' . $j . '" type="submit" class="btn btn-info" name="Cancel" value="' . _('Cancel') . '" />
		</div>
        </div><br />
        </form>
';
	...
	if (DB_num_rows($result)==0) {
		echo prnMsg(_('No outstanding receipts or credits to be allocated for this customer'),'info');
		include('includes/footer.php');
		exit;
	}
...
<table id="general-table" class="table table-bordered">';
	...
	while ($myrow = DB_fetch_array($result)) {
		echo '<tr class="striped_row">
				...
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8'). '?AllocTrans=' . $myrow['id'] . '" class="btn btn-info">' . _('Allocate') . '</a></td>
			</tr>';
	}
	...
	if ($NoOfUnallocatedTrans == 0) {
		echo prnMsg(_('There are no allocations to be made'),'info');
	}
	else {
...
';
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		$AllocateLink = '<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8'). '?AllocTrans=' . $myrow['id'] . '" class="btn btn-info">' . _('Allocate') . '</a>';

		if ( $CurrentDebtor != $myrow['debtorno'] ) {
			if ( $CurrentTransaction > 1 ) {
				echo '<tr class="striped_row">
			<td colspan="6"><strong>' . _('Balance') . '</strong></td>
						<td class="number"><strong>' . locale_number_format($Balance,$CurrDecimalPlaces)  . '</strong></td>
	...
	if (!isset($Balance)) {
		...
		echo '<tr class="striped_row">
				<td colspan="6" class="number"><strong>' . _('Balance') . '</strong></td>
				<td><strong>' . locale_number_format($Balance,$CurrDecimalPlaces)  . '</strong></td>
				<td><strong>' . $CurrCode . '</strong></td>
			</tr>
		</table></div></div></div>';
	}
