...
if (!isset($_POST['FromDate'])){
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' '. _('Stock Transaction Listing') . '</h1></a>
		</div>';
...
<div class="col-xs-12">';
	echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">
		...
		<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Enter the date from which the transactions are to be listed') . '</label>
			<input type="text" required="required" autofocus="autofocus" name="FromDate" maxlength="10" size="11" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" value="' . Date($_SESSION['DefaultDateFormat']) . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Enter the date to which the transactions are to be listed') . '</label>
			<input type="text" required="required" name="ToDate" maxlength="10" size="11" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" value="' . Date($_SESSION['DefaultDateFormat']) . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Transaction type') . '</label>
	...
	echo '<div class="row">
			<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('For Stock Location') . '</label>
...
if (DB_error_no()!=0){
	...
	include('includes/header.php');
	echo prnMsg(_('An error occurred getting the transactions'),'error');
	include('includes/footer.php');
	exit;
	...
	include('includes/header.php');
	echo '<br />';
	echo   prnMsg (_('There were no transactions found in the database between the dates') . ' ' . $_POST['FromDate'] . ' ' . _('and') . ' '. $_POST['ToDate']  . '<br />' ._('Please try again selecting a different date'), 'info');
	include('includes/footer.php');
  	exit;
}
...
$ReportFileName = 'nERP' . '_StockTransListing_' . date('Y-m-d').'.pdf';
