...
if (!isset($_GET['SelectedShipment'])){
	...
	echo prnMsg( _('This page is expected to be called with the shipment number to show the costing for'), 'error');
...
if (DB_num_rows($GetShiptHdrResult)==0) {
	echo '<br />';
	echo prnMsg( _('Shipment') . ' ' . $_GET['SelectedShipment'] . ' ' . _('could not be located in the database') , 'error');
	include ('includes/footer.php');
	exit;
}
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Shipment Details') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
	
	<tr>
		<td><strong>' .  _('Shipment') .': </strong></td>
		<td><strong>' . $_GET['SelectedShipment'] . '</strong></td>
		<td><strong>' .  _('From').' ' . $HeaderData['suppname'] . '</strong></td>
	</tr>
	<tr>
		<td>' . _('Vessel'). ': </td>
...
if (DB_num_rows($GetShiptCostsResult)==0) {
	echo '<br />';
	echo prnMsg (_('No General Cost Records exist for Shipment') . ' ' . $_GET['SelectedShipment'] . ' ' . _('in the database'), 'error');
	include ('includes/footer.php');
	exit;
}
...
if (DB_error_no() !=0 OR DB_num_rows($GetShiptCostsResult)==0) {
	echo '<br />';
	echo prnMsg ( _('No Item Cost Records exist for Shipment') . ' ' . $_GET['SelectedShipment'] . ' ' . _('in the database'), 'error');
	include ('includes/footer.php');
	exit;
}
	...
	if (isset($_POST['Close'])){
		while ($myrow=DB_fetch_array($LineItemsResult)){
		  if ($myrow['totqtyinvoiced'] < $myrow['totqtyrecd']){
			 echo  prnMsg(_('Cannot close a shipment where the quantity received is more than the quantity invoiced. Check the item') . ' ' . $myrow['itemcode'] . ' - ' . $myrow['itemdescription'],'warn');
			 unset($_POST['Close']);
		  }
		}
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Items on shipment'). '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">';
...
/*  Item / Qty Inv/  FX price/ Local Val/ Portion of chgs/ Shipt Cost/ Std Cost/ Variance/ Var % */
	...
	echo '<tr class="striped_row">
			...
			<td>' . locale_number_format($myrow['totqtyinvoiced'],'Variable') . '</td>
			<td>' . locale_number_format($myrow['totqtyrecd'],'Variable') . '</td>
			<td>' . locale_number_format($ItemCharges,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
			<td>' . locale_number_format($PortionOfCharges,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
			<td>' . locale_number_format($ItemShipmentCost,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
			<td>' . locale_number_format($StdCostUnit,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
			<td>' . locale_number_format($Variance,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
			<td>' . $VariancePercentage . '%</td>
		</tr>';
	}
}
echo '<tr>
		<td colspan="3" class="number"><strong>>' .  _('Total Shipment Charges'). '</strong>></td>
		<td>' . locale_number_format($TotalInvoiceValueOfShipment,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
		<td>' . locale_number_format($TotalCostsToApportion,$_SESSION['CompanyRecord']['decimalplaces'])  . '</td>
	</tr>';

echo '<tr>
		<td colspan="6" class="number">' . _('Total Value of all variances on this shipment') . '</td>
		<td>' . locale_number_format($TotalShiptVariance,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
	</tr>';
...
<div class="block-title"><h3>' . _('Shipment Charges Against Products'). '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">';
...
while ($myrow=DB_fetch_array($ChargesResult)) {
	...
	echo '<tr class="striped_row">
		...
		<td>' . $myrow['stockid'] . '</td>
		<td>' . locale_number_format($myrow['value'],$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
		</tr>';
...
echo '<tr>
		<td colspan="5" class="number"><strong>' .  _('Total Charges Against Shipment Items'). ':</strong></td>
		<td>' . locale_number_format($TotalItemShipmentChgs,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
	</tr>';
...
echo '<div class="block">
<div class="block-title"><h3>' . _('General Shipment Charges') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">';
...
while ($myrow=DB_fetch_array($ChargesResult)) {
	...
	echo '<tr class="striped_row">
		...
		<td>' . $myrow['suppreference'] . '</td>
		<td>' . ConvertSQLDate($myrow['trandate']) . '</td>
		<td>' . locale_number_format($myrow['value'],$_SESSION['CompanyRecord']['decimalplaces']) . '</td></tr>';
...
echo '<tr>
	<td class="number" colspan="4"><strong>' .  _('Total General Shipment Charges'). ':</strong></td>
	<td>' . locale_number_format($TotalGeneralShipmentChgs,$_SESSION['CompanyRecord']['decimalplaces']) . '</td></tr>';

echo '</table></div></div>';

echo '</td>
	</tr>
	</table></div></div></div><br />'; //major table close
...
<div class="form-group"> <label class="col-md-12 control-label">'._('Update Standard Costs') .'</label>
<select name="UpdateCost" class="form-control">
...
<div class="form-group"> <br />
		<input type="submit" class="btn btn-danger" name="Close" value="'. _('Confirm OK to Close'). '" />
		</div>
		</div><br />
		</div>
		</form>
...
/*also need to make sure the purchase order lines that were on this shipment are completed so no more can be received in against the order line */
	...
	echo prnMsg( _('Shipment'). ' ' . $_GET['SelectedShipment'] . ' ' . _('has been closed'),'success' ),'</div>';
	if ($_SESSION['CompanyRecord']['gllink_stock']==1) {
		echo '<br />';
		echo   prnMsg ( _('All variances were posted to the general ledger'),'info' ),'</div>';
	}
	If (isset($_POST['UpdateCost']) AND $_POST['UpdateCost']=='Yes'){
		echo '<br />';
		echo prnMsg ( _('All shipment items have had their standard costs updated'),'success' ),'</div>';
	}
}
