...
if (isset($_POST['UpdateData'])){
	...
	if (DB_num_rows($result)==0) {
		echo prnMsg (_('The entered item code does not exist'),'error',_('Non-existent Item'));
...
<table id="general-table" class="table table-bordered">
		<tr>
			<th>' . _('Item Code') . '</th><td><input type="text" class="form-control" name="StockID" value="' . $StockID . '"  maxlength="20" /></td><td><input type="submit" class="btn btn-info"" name="Show" value="' . _('Show') . '" /></td>
		</tr>
		<tr>
...
if (($myrow['mbflag']=='D' AND $myrow['stocktype'] != 'L')
   ...
   echo prnMsg(_('Cost information cannot be modified for kits assemblies or service items') . '. ' . _('Please select a different part'),'warn');
...
';
	...
	if ($myrow['mbflag']=='M'){
		echo '<tr>
				...
				<td class="number"><input type="text" class="form-control" name="MaterialCost" value="' . locale_number_format($myrow['materialcost'],$_SESSION['StandardCostDecimalPlaces']) . '" /></td>
			</tr>
			<tr>
				<td>' . _('Standard Labour Cost Per Unit') . '</td>
				<td class="number"><input type="text" class="form-control" name="LabourCost" value="' . locale_number_format($myrow['labourcost'],$_SESSION['StandardCostDecimalPlaces']) . '" /></td>
			</tr>
			<tr>
				<td>' . _('Standard Overhead Cost Per Unit') . '</td>
				<td class="number"><input type="text" class="form-control" name="OverheadCost" value="' . locale_number_format($myrow['overheadcost'],$_SESSION['StandardCostDecimalPlaces']) . '" /></td>
			</tr>';
    ...
    echo '</table></div></div></div>
         <br />
			 ...
			 <div>
                  <input type="submit" class="btn btn-success" name="UpdateData" value="' . _('Update') . '" />
             </div> </div>
...
if ($myrow['mbflag']!='D'){
	echo '<div class="row">
	 <div class="col-xs-2"><a href="' . $RootPath . '/StockStatus.php?StockID=' . $StockID . '" class="btn btn-info">' . _('Stock Status') . '</a></div>';
	echo '<div class="col-xs-2"><a href="' . $RootPath . '/StockMovements.php?StockID=' . $StockID . '" class="btn btn-info">' . _('Stock Movements') . '</a></div>';
	echo '<div class="col-xs-2"><a href="' . $RootPath . '/StockUsage.php?StockID=' . $StockID . '" class="btn btn-info">' . _('Stock Usage')   . '</a></div>
';
	echo '
	<div class="col-xs-2"><a href="' . $RootPath . '/SelectSalesOrder.php?SelectedStockItem=' . $StockID . '" class="btn btn-info">' . _('Open Sales Orders') . '</a></div>';
	echo '<div class="col-xs-4"><a href="' . $RootPath . '/SelectCompletedOrder.php?SelectedStockItem=' . $StockID . '" class="btn btn-info">' . _('Completed Sales Orders') . '</a></div></div><br />
';
}
echo '
      </form>';
