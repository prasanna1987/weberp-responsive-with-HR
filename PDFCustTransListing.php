...
if (!isset($_POST['Date'])){
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Customer Transaction Listing').
			'</h1></a></div>';
...
' . _('Enter the date for which the transactions are to be listed') . '</label>
				<input type="text" name="Date" maxlength="10" size="11" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker"
 value="' . Date($_SESSION['DefaultDateFormat']) . '" /></div>
			</div>';
...
$ReportFileName = 'nERP' . '_CustTransListing_' . date('Y-m-d').'.pdf';
