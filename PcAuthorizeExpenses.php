...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>', _('Authorisation of Petty Cash Expenses'), '
		</h1></a></div>';
...
<table id="general-table" class="table table-bordered">
	...
	while ($MyRow = DB_fetch_array($Result)) {
		...
		if (isset($_POST['Submit']) and $_POST['Submit'] == _('Update') and isset($_POST[$MyRow['counterindex']])) {
			...
			DB_Txn_Commit();
			 echo prnMsg(_('Expenses have been correctly authorised'), 'success');
			unset($_POST['Submit']);
			unset($SelectedTabs);
		...
		//Generate download link for expense receipt, or show text if no receipt file is found.
			...
			if (DB_num_rows($ReceiptResult) > 0) { //If receipt exists in database
				...
				$ReceiptText = '<a href="' . $ReceiptPath . '" download="ExpenseReceipt-' . mb_strtolower($SelectedTabs) . '-[' . $MyRow['date'] . ']-[' . $MyRow['counterindex'] . ']" class="btn btn-warning"">' . _('Download attachment') . '</a>';
...
<div class="form-group"> <label class="col-md-8 control-label">', _('Authorise expenses on'), '</label>
