...
if ((!(Is_Date($_POST['FromDate'])) OR (!Is_Date($_POST['ToDate']))) AND (isset($_POST['View']))) {
	echo prnMsg( _('Incorrect date format used, please re-enter'), error);
	unset($_POST['View']);
}
...
<div class="form-group has-error"> <label class="col-md-12 control-label">' .  _('From Date') . ' ' . $_SESSION['DefaultDateFormat']  . '</label>
		<input tabindex="1" type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="FromDate" size="11" maxlength="10" required="required" value="' .$_POST['FromDate']. '" onchange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')"/></div>
	</div>
	<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' .  _('To Date') . ' ' . $_SESSION['DefaultDateFormat']  . '</label>
		<td><input tabindex="2" type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="ToDate" size="11" maxlength="10" required="required" value="' . $_POST['ToDate'] . '" onchange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')"/></div>
	</div>';

// Show user selections
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('User ID'). '</label>
		<select tabindex="3" name="SelectedUser" class="form-control">
...
echo '<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Table '). '</label>
		<select tabindex="4" name="SelectedTable" class="form-control">
			<option value="ALL">' . _('All') . '</option>';
...
// Show the text
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Containing text') . '</label>
		<input type="text" class="form-control" name="ContainingText" size="20" maxlength="20" value="'. $_POST['ContainingText'] . '" /></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"><br />
		<input tabindex="5" class="btn btn-info" type="submit" name="View" value="' . _('View') . '" />
	</div>
	</div>
	</div>
	</form>';
