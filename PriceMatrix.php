...
echo '<p align="left"><a href="' . $RootPath . '/SelectProduct.php" class="btn btn-info">' . _('Back to Items') . '</a></p>';
if (isset($_POST['submit'])) {
	...
	if (!is_numeric(filter_number_format($_POST['QuantityBreak']))){
		echo prnMsg( _('The quantity break must be entered as a positive number'),'error');
		$InputError =1;
	}

	if (filter_number_format($_POST['QuantityBreak'])<=0){
		echo prnMsg( _('The quantity of all items on an order in the discount category') . ' ' . $StockID . ' ' . _('at which the price will apply is 0 or less than 0') . '. ' . _('Positive numbers are expected for this entry'),'warn');
		$InputError =1;
	}
	if (!is_numeric(filter_number_format($_POST['Price']))){
		echo prnMsg( _('The price must be entered as a positive number'),'warn');
		$InputError =1;
	}
	...
	ie the page has called itself with some user input */
		...
		$result = DB_query($sql,$ErrMsg);
		echo prnMsg( _('The price matrix record has been added'),'success');
		echo '<br />';
		unset($_POST['StockID']);
...
/*the link to delete a selected record was clicked instead of the submit button */
	...
	$result = DB_query($sql,$ErrMsg);
	echo prnMsg( _('The price matrix record has been deleted'),'success');
	echo '<br />';
}
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Customer Price List') . ' (' . _('Sales Type') . ')</label>';
...
if(isset($_GET['StockID'])){
	...
	echo prnMsg(_('You must select a stock item first before set a price maxtrix'),'error');
...
<div class="form-group"> <label class="col-md-12 control-label">'. _('Price Effective From Date') . '</label>
	<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="StartDate" required="required" size="11" maxlength="10" title="' . _('Enter the date from which this price should take effect.') . '" value="' . $_POST['StartDate'] . '" /></div></div>';
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Price Effective To Date') . '</label>
			<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="EndDate" size="11" maxlength="10" title="' . _('Enter the date to which this price should be in effect to, or leave empty if the price should continue indefinitely') . '" value="' . $_POST['EndDate'] . '" />';
echo '</div></div>';

echo '<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Quantity Break') . '</label>
		<input class="form-control' . (in_array('QuantityBreak',$Errors) ? ' inputerror' : '') . '" tabindex="3" required="required" type="number" name="QuantityBreak" size="10" value="'. $_POST['QuantityBreak'].'" maxlength="10"  /></div>
	</div></div>
	
	<div class="row">
		<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Price') . ' </label>
		<input class="form-control' . (in_array('Price',$Errors) ? ' inputerror' : '') . '" tabindex="4" type="text" required="required" name="Price" value="'.$_POST['Price'].'" title="' . _('The price to apply to orders where the quantity exceeds the specified quantity') . '" size="5" maxlength="5" /></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"><br />
		<input tabindex="5" type="submit" class="btn btn-success" name="submit" value="' . _('Enter Information') . '" />
	</div>
	</div>
	</div>
	<br />';
...
while ($myrow = DB_fetch_array($result)) {
    ...
    if (in_array(5, $_SESSION['AllowedPageSecurityTokens'])){
	    printf('<tr class="striped_row">
			...
			<td class="number">%s</td>
						<td><a href="%s" class="btn btn-info">'._('Edit').'</a></td>
