...
if (isset($_POST['submit']) AND isset($_POST['NewDatabase'])) {
	...
	if(mb_strlen($_POST['NewDatabase'])>32
		...
		prnMsg (_('The new company database has been created for' . ' ' . htmlspecialchars($_POST['NewCompany'], ENT_QUOTES, 'UTF-8') . '. ' . _('The company details and parameters should now be set up for the new company. NB: Only a single user "admin" is defined with the password "nERP" in the new company database. A new system administrator user should be defined for the new company and this account deleted immediately.')), 'info');
