...
if (!isset($_GET['TransferNo'])){
	if (isset($_POST['TransferNo'])){
		if (is_numeric($_POST['TransferNo'])){
			...
			echo prnMsg(_('The entered transfer reference is expected to be numeric'),'error');
...
<div class="col-xs-12">';
		...
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Print Stock Transfer Note').'  ' . '</label>
				<input type="text" class="form-control"  name="TransferNo" maxlength="10" size="11" /></div>
			</div>
			';
		echo '
			<div class="col-xs-4">
<div class="form-group"> <br />
				<input type="submit" class="btn btn-info" name="Process" value="' . _('Print Transfer Note') . '" />
			</div>
            </div>
			</div>
			</form></div></div>';
...
<div class="col-xs-12">';
		...
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Transfer docket to reprint Shipping Labels') . '</label>
		...
		echo '
			<div class="col-xs-4">
<div class="form-group"><br />
				<input type="submit" class="btn btn-warning" name="Print" value="' . _('Print Shipping Labels') .'" />
			</div>';
		echo '</div>
		</div>
			</form></div></div>';
...
if (DB_num_rows($result) == 0){
	...
	include ('includes/header.php');
	echo prnMsg(_('There was no transfer found with number') . ': ' . $_GET['TransferNo'], 'error');
	echo '<p align="left"><a href="PDFStockTransfer.php" class="btn btn-default">' . _('Try Again')  . '</a></p>';
	include ('includes/footer.php');
	exit;
}
...
$pdf->OutputD('nERP' . '_StockTransfer_' . date('Y-m-d') . '.pdf');
$pdf->__destruct();
?>