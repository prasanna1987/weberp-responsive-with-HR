...
if (isset($_POST['submit'])) {
	...
	*/
	if (mb_strlen($_POST['X_PastDueDays1']) > 3 OR !is_numeric($_POST['X_PastDueDays1']) ) {
		$InputError = 1;
		echo prnMsg(_('First overdue deadline days must be a number'),'error');
	} elseif (mb_strlen($_POST['X_PastDueDays2'])  > 3 OR !is_numeric($_POST['X_PastDueDays2']) ) {
		$InputError = 1;
		echo prnMsg(_('Second overdue deadline days must be a number'),'error');
	} elseif (mb_strlen($_POST['X_DefaultCreditLimit']) > 12 OR !is_numeric($_POST['X_DefaultCreditLimit']) ) {
		$InputError = 1;
		echo prnMsg(_('Default Credit Limit must be a number'),'error');
	} elseif (mb_strstr($_POST['X_RomalpaClause'], "'") OR mb_strlen($_POST['X_RomalpaClause']) > 5000) {
		$InputError = 1;
		echo prnMsg(_('The Romalpa Clause may not contain single quotes and may not be longer than 5000 chars'),'error');
		...
		$InputError = 1;
		echo prnMsg(_('No less than 1 and more than 99 Quick entries allowed'),'error');
	} elseif (!is_numeric($_POST['X_MaxSerialItemsIssued']) or $_POST['X_MaxSerialItemsIssued'] < 1) {
		$InputError = 1;
		echo prnMsg(_('The maximum number of serial numbers issued must be numeric and greater than zero'), 'error');
	} 
elseif (mb_strlen($_POST['X_FreightChargeAppliesIfLessThan']) > 12 OR !is_numeric($_POST['X_FreightChargeAppliesIfLessThan']) ) {
		$InputError = 1;
		echo prnMsg(_('Freight Charge Applies If Less Than must be a number'),'error');
		...
		$InputError = 1;
		echo prnMsg(_('Standard Cost Decimal Places must be a number between 0 and 4'),'error');
		...
		$InputError = 1;
		echo prnMsg(_('Financial period per year must be a number between 1 and 12'),'error');
	} elseif (mb_strlen($_POST['X_TaxAuthorityReferenceName']) >25) {
		$InputError = 1;
		echo prnMsg(_('The Tax Authority Reference Name must be 25 characters or less'),'error');
		...
		$InputError = 1;
		echo prnMsg(_('Over Charge Proportion must be a percentage'),'error');
		...
		$InputError = 1;
		echo prnMsg(_('Default maximum number of records to display must be between 1 and 500'),'error');
	}elseif (mb_strlen($_POST['X_MaxImageSize']) > 3 OR !is_numeric($_POST['X_MaxImageSize']) OR
		$_POST['X_MaxImageSize'] < 1 ) {
		$InputError = 1;
		echo prnMsg(_('The maximum size of item image files must be between 50 and 500 KB'),'error');
	}
	//elseif (!IsEmailAddress($_POST['X_FactoryManagerEmail'])){
//		$InputError = 1;
//		echo prnMsg(_('The Factory Manager Email address does not appear to be valid'),'error');
//	}elseif (!IsEmailAddress($_POST['X_PurchasingManagerEmail'])){
//		$InputError = 1;
//		echo prnMsg(_('The Purchasing Manager Email address does not appear to be valid'),'error');
//	}elseif (!IsEmailAddress($_POST['X_InventoryManagerEmail']) AND $_POST['X_InventoryManagerEmail']!=''){
//		$InputError = 1;
//		echo prnMsg(_('The Inventory Manager Email address does not appear to be valid'),'error');
//	}
elseif (mb_strlen($_POST['X_FrequentlyOrderedItems']) > 2 OR !is_numeric($_POST['X_FrequentlyOrderedItems'])) {
		$InputError = 1;
		echo prnMsg(_('The number of frequently ordered items to display must be numeric'),'error');
	}elseif (strlen($_POST['X_SmtpSetting']) != 1 OR !is_numeric($_POST['X_SmtpSetting'])){
		$InputError = 1;
		echo prnMsg(_('The SMTP setting should be selected as Yes or No'),'error');
	}elseif (strlen($_POST['X_QualityLogSamples']) != 1 OR !is_numeric($_POST['X_QualityLogSamples'])){
		$InputError = 1;
		echo prnMsg(_('The Quality Log Samples setting should be selected as Yes or No'),'error');
	} elseif (mb_strstr($_POST['X_QualityProdSpecText'], "'") OR mb_strlen($_POST['X_QualityProdSpecText']) > 5000) {
		$InputError = 1;
		echo prnMsg(_('The Quality ProdSpec Text may not contain single quotes and may not be longer than 5000 chars'),'error');
	} elseif (mb_strstr($_POST['X_QualityCOAText'], "'") OR mb_strlen($_POST['X_QualityCOAText']) > 5000) {
		$InputError = 1;
		echo prnMsg(_('The Quality COA Text may not contain single quotes and may not be longer than 5000 chars'),'error');
	}
		...
		if($DefaultTheme != $_POST['X_DefaultTheme']) {// If not equal, update the default theme.
			...
			if($fhandle) {
				...
				if(!fwrite($fhandle,$content)) {
					echo prnMsg(_('Cannot write to the configuration file.'), 'error');
				} else {
					echo prnMsg(_('The configuration file was updated.'), 'info');
				}
				fclose($fhandle);
			} else {
				echo prnMsg(_('Cannot open the configuration file.'), 'error');
			}
...
//new number must be shown
		...
		echo prnMsg( _('System configuration updated'),'success');
...
//	<td><select name="X_DefaultDateFormat" class="form-control">
...
//			<td><select name="X_DefaultTheme" class="form-control">';
...
//		</tr>';
//} else {
	echo '<input type="hidden" name="X_DefaultDateFormat" value="d/m/Y" />';
	echo '<input type="hidden" name="X_DefaultTheme" value="xenos" />';
//}
// ---------- New section:
...
echo '<tr><td>' . _('First Overdue Deadline in (days)') . ':</td>
	<td class="has-error"><input type="text" class="form-control" required="required"  pattern="(?!^0\d+$)[\d]+" title="'._('The input must be integer').'" name="X_PastDueDays1" value="' . $_SESSION['PastDueDays1'] . '" size="3" maxlength="3" /></td>
...
echo '<tr><td>' . _('Second Overdue Deadline in (days)') . ':</td>
	<td class="has-error"><input type="text" class="form-control" required="required"  pattern="(?!^0\d+$)[\d]+" title="'._('The input must be integer').'" name="X_PastDueDays2" value="' . $_SESSION['PastDueDays2'] . '" size="3" maxlength="3" /></td>
	<td>' . _('As above but the next level of overdue') . '</td></tr>';


// DefaultCreditLimit
echo '<tr><td>' . _('Default Credit Limit') . ':</td>
	<td class="has-error"><input type="text" class="form-control" required="required" title="'._('The input must be numeric').'" name="X_DefaultCreditLimit" value="' . $_SESSION['DefaultCreditLimit'] . '" size="12" maxlength="12" /></td>
	<td>' . _('The default used in new customer set up') . '</td></tr>';

// Check Credit Limits
echo '<tr><td>' . _('Check Credit Limits') . ':</td>
	<td><select name="X_CheckCreditLimits" class="form-control">
...
echo '<tr><td>' . _('Show Settled Last Month') . ':</td>
	<td><select name="X_Show_Settled_LastMonth" class="form-control">
...
echo '<tr><td>' . _('Romalpa Clause') . ':</td>
	<td><textarea name="X_RomalpaClause" class="form-control">' . $_SESSION['RomalpaClause'] . '</textarea></td>
...
echo '<tr><td>' . _('Quick Entries') . ':</td>
	<td class="has-error"><input type="text" class="form-control" required="required" pattern="[1-9][\d]{0,1}" name="X_QuickEntries" value="' . $_SESSION['QuickEntries'] . '" size="3" maxlength="2" /></td>
...
echo '<tr><td>' . _('Maximum number of serial numbered items that can be issued') . ':</td>
	<td class="has-error"><input type="number" class="form-control" name="X_MaxSerialItemsIssued" value="' . $_SESSION['MaxSerialItemsIssued'] . '" size="3" required="required" maxlength="10" /></td>
...
echo '<tr><td>' . _('Frequently Ordered Items') . ':</td>
	<td><input type="text" class="form-control" pattern="(?!^0[1-9]+$)[\d]{1,2}" name="X_FrequentlyOrderedItems" value="' . $_SESSION['FrequentlyOrderedItems'] . '" size="3" maxlength="2" /></td>
...
echo '<tr><td>' . _('Sales Order Allows Same Item Multiple Times') . ':</td>
	<td><select name="X_SO_AllowSameItemMultipleTimes" class="form-control">
...
echo '<tr>
		...
		<td><select name="X_AllowOrderLineItemNarrative" class="form-control">
	...
	</tr> <input type="hidden" name="X_ItemDescriptionLanguages[]" value="0" />';
//ItemDescriptionLanguages
if (!isset($_POST['X_ItemDescriptionLanguages'])){
...
//		<td><select name="X_ItemDescriptionLanguages[]" size="5" multiple="multiple" class="form-control">';
//
...
//echo '<tr><td>' . _('Google Translator API Key') . ':</td>
//	<td><input type="text" class="form-control" name="X_GoogleTranslatorAPIKey" size="25" maxlength="50" value="' . $_SESSION['GoogleTranslatorAPIKey'] . '" /></td>
//	<td>' . _('Google Translator API Key to allow automatic translations. More info at https://cloud.google.com/translate/')  . '</td></tr>';
echo '<input type="hidden" name="X_GoogleTranslatorAPIKey" value="" />';
echo '<input type="hidden" name="X_RequirePickingNote" value="0" />';
//'RequirePickingNote'
//echo '<tr><td>' . _('A picking note must be produced before an order can be delivered') . ':</td>
//	<td><select name="X_RequirePickingNote" class="form-control">
...
echo '<tr><td>' . _('Auto Update Exchange Rates Daily') . ':</td>
	<td><select name="X_UpdateCurrencyRatesDaily" class="form-control">
...
echo '<tr><td>' . _('Source Exchange Rates From') . ':</td>
	<td><select name="X_ExchangeRateFeed" class="form-control">
...
echo '<tr><td>' . _('Format of Packing Slips') . ':</td>
	<td><select name="X_PackNoteFormat" class="form-control">
...
//	<td><select name="X_InvoicePortraitFormat" class="form-control">
...
//	</tr>';
echo '<input type="hidden" name="X_InvoicePortraitFormat" value="1" />';
//Default Invoice Quantity
echo '<tr><td>' . _('Invoice Quantity Default') . ':</td>
	<td><select name="X_InvoiceQuantityDefault" class="form-control">
...
echo '<tr><td>' . _('Show company details on packing slips') . ':</td>
	<td><select name="X_DefaultBlindPackNote" class="form-control">
...
echo '<tr>
		...
		<td><select name="X_WorkingDaysWeek" class="form-control">
...
echo '<tr>
		...
		<td><select name="X_DispatchCutOffTime" class="form-control">';
for ($i=0; $i < 24; $i++ )
...
echo '<tr><td>' . _('Allow Sales Of Zero Cost Items') . ':</td>
	<td><select name="X_AllowSalesOfZeroCostItems" class="form-control">
...
echo '<tr><td>' . _('Controlled Items Must Exist For Crediting') . ':</td>
	<td><select name="X_CreditingControlledItems_MustExist" class="form-control">
...
echo '<td><select name="X_DefaultPriceList" class="form-control">';
...
echo '<td><select name="X_Default_Shipper" class="form-control">';
if( DB_num_rows($result) == 0 ) {
...
//	<td><select name="X_DoFreightCalc" class="form-control">
...
echo '<input type="hidden" name="X_DoFreightCalc" value="0" />';
//FreightChargeAppliesIfLessThan
//echo '<tr><td>' . _('Apply freight charges if an order is less than') . ':</td>
//	<td class="has-error"><input type="text" class="form-control" title="'._('The input must be numeric').'" name="X_FreightChargeAppliesIfLessThan" size="12" maxlength="12" value="' . $_SESSION['FreightChargeAppliesIfLessThan'] . '" /></td>
...
echo '<input type="hidden" name="X_FreightChargeAppliesIfLessThan" value="0" />';
// AutoDebtorNo
echo '<tr><td>' . _('Create Customer Codes Automatically') . ':</td>
	<td><select name="X_AutoDebtorNo" class="form-control">';
...
echo '<tr><td>' . _('Create Supplier Codes Automatically') . ':</td>
	<td><select name="X_AutoSupplierNo" class="form-control">';
...
echo '<td><select name="X_DefaultTaxCategory" class="form-control">';
if( DB_num_rows($result) == 0 ) {
	echo '<option selected="selected" value="">' . _('Unavailable') . '</option>';
} else {
	while( $row = DB_fetch_array($result) ) {
...
echo '<tr><td>' . _('Tax Authority Reference Name') . ':</td>
	<td><input type="text" class="form-control" name="X_TaxAuthorityReferenceName" size="16" maxlength="25" value="' . $_SESSION['TaxAuthorityReferenceName'] . '" /></td>
	<td>' . _('This parameter is what is displayed on tax invoices and credits for the tax authority of the company eg. in India this would be GST No:')  . '</td></tr>';

// CountryOfOperation

//echo '<tr><td>' . _('Country Of Operation') . ':</td>';
//echo '<td><select name="X_CountryOfOperation" class="form-control">';
//echo '<option selected="selected" value="">' . _('Unavailable') . '</option>';
...
echo '<input type="hidden" name="X_CountryOfOperation" value="IN" />';
// StandardCostDecimalPlaces
echo '<tr><td>' . _('Standard Cost Decimal Places') . ':</td>
	<td><select name="X_StandardCostDecimalPlaces" class="form-control">';
for ($i=0; $i <= 4; $i++ )
...
echo '<tr><td>' . _('Number Of Periods Of StockUsage') . ':</td>
	<td><select name="X_NumberOfPeriodsOfStockUsage" class="form-control">';
for ($i=1; $i <= 12; $i++ )
...
echo '<tr><td>' . _('Show order values on GRN') . ':</td>
	<td><select name="X_ShowValueOnGRN" class="form-control">
...
echo '<tr><td>' . _('Check Quantity Charged vs Deliver Qty') . ':</td>
	<td><select name="X_Check_Qty_Charged_vs_Del_Qty" class="form-control">
...
echo '<tr><td>' . _('Check Price Charged vs Order Price') . ':</td>
	<td><select name="X_Check_Price_Charged_vs_Order_Price" class="form-control">
...
echo '<tr><td>' . _('Allowed Over Charge %') . ':</td>
	<td class="has-error"><input type="text" class="form-control" pattern="(?!^0\d+$)[\d]{1,2}|(100)" required="required" title="'._('The input must between 0 and 100').'" name="X_OverChargeProportion" size="4" maxlength="3" value="' . $_SESSION['OverChargeProportion'] . '" placeholder="'._('integer between 0 and 100').'" /></td>
...
echo '<tr><td>' . _('Allowed Over Receive %') . ':</td>
	<td class="has-error"><input type="text" class="form-control" pattern="(?!^0\d+$)[\d]{1,2}|(100)" required="required" title="'._('The input must between 0 and 100').'" name="X_OverReceiveProportion" size="4" maxlength="3" value="' . $_SESSION['OverReceiveProportion'] . '" /></td>
...
echo '<tr><td>' . _('Purchase Order Allows Same Item Multiple Times') . ':</td>
	<td><select name="X_PO_AllowSameItemMultipleTimes" class="form-control">
...
// AutoAuthorisePO
echo '<tr><td>' . _('Automatically authorise purchase orders if user has authority') . ':</td>
	<td><select name="X_AutoAuthorisePO" class="form-control">
...
echo '<tr><td>' . _('Financial Year Ends On') . ':</td>
	<td><select name="X_YearEnd" class="form-control">';
...
echo '<tr><td>' . _('Report Page Length') . ':</td>
	<td><input type="text" class="form-control" pattern="(?!^0\d*$)[\d]{1,3}" title="'._('The input should be between 1 and 999').'" placeholder="'._('1 to 999').'" name="X_PageLength" size="4" maxlength="6" value="' . $_SESSION['PageLength'] . '" /></td><td>&nbsp;</td>
</tr>';

//DefaultDisplayRecordsMax
echo '<tr>
		<td>' . _('Default Maximum Number of Records to Show') . ':</td>
		<td class="has-error"><input type="text" class="form-control" pattern="(?!^0\d*$)[\d]{1,3}" required="required" title="'._('The records should be between 1 and 999').'" name="X_DefaultDisplayRecordsMax" size="4" maxlength="3" value="' . $_SESSION['DefaultDisplayRecordsMax'] . '" /></td>
...
// ShowStockidOnImage
echo '<input type="hidden" name="X_ShowStockidOnImages" value="0" />';
//echo '<tr>
//		<td>' . _('Show Stockid on images') . ':</td>
//		<td><select name="X_ShowStockidOnImages" class="form-control">
...
echo '<tr>
		...
		<td class="has-error"><input type="text" class="form-control" pattern="(?!^0\d*$)[\d]{1,3}" required="required" title="'._('The input should be between 1 and 999').'" placeholder="'._('1 to 999').'" name="X_MaxImageSize" size="4" maxlength="3" value="' . $_SESSION['MaxImageSize'] . '" /></td>
...
echo '<tr><td>' . _('Number Of Month Must Be Shown') . ':</td>
		  <td class="has-error"><input type="text" class="form-control" pattern="(?!^0\d*$)[\d]+" required="required" title="'._('input must be positive integer').'" placeholder="'._('positive integer').'" name="X_NumberOfMonthMustBeShown" size="4" maxlength="3" value="' . $_SESSION['NumberOfMonthMustBeShown'] . '" /></td>
		  <td>' . _('Number of month must be shown on report can be changed with this parameters ex: in Customer Inquiry')  . '</td>
      </tr>';
echo '<input type="hidden" name="X_part_pics_dir" value="part_pics" />';
//$part_pics_dir
//echo '<tr><td>' . _('The directory where images are stored') . ':</td>
//	 <td><select name="X_part_pics_dir" class="form-control">';
//
//
...
echo '<input type="hidden" name="X_reports_dir" value="reports" />';
//$reports_dir
//echo '<tr><td>' . _('The directory where reports are stored') . ':</td>
//	<td><select name="X_reports_dir" class="form-control">';
//
//$DirHandle = dir($CompanyDirectory);
//
...
echo '<input type="hidden" name="X_HTTPS_Only" value="0" />';
// HTTPS_Only
//echo '<tr><td>' . _('Only allow secure socket connections') . ':</td>
//	<td><select name="X_HTTPS_Only" class="form-control">
...
//	</tr>';
echo '<input type="hidden" name="X_DB_Maintenance" value="0" />';
/*Perform Database maintenance DB_Maintenance*/
//echo '<tr><td>' . _('Perform Database Maintenance At Logon') . ':</td>
//	<td><select name="X_DB_Maintenance" class="form-control">';
...
echo '<input type="hidden" value="DokuWiki" name="X_WikiApp" />
     <input type="hidden" name="X_WikiPath" value="' . $_SESSION['WikiPath'] . '" />
	 <input type="hidden" value="0" name="X_geocode_integration" />
	 ';

echo '<tr><td>' . _('Extended Customer Information') . ':</td>
        <td><select name="X_Extended_CustomerInfo" class="form-control">';
...
echo '<tr><td>' . _('Extended Supplier Information') . ':</td>
        <td><select name="X_Extended_SupplierInfo" class="form-control">';
...
echo '<tr><td>' . _('Prohibit GL Journals to Control Accounts') . ':</td>
	<td><select name="X_ProhibitJournalsToControlAccounts" class="form-control">';
...
echo '<tr>
		...
		<td><select name="X_ProhibitPostingsBefore" class="form-control">';
...
echo '<tr><td>' . _('Inventory Costing Method') . ':</td>
	<td><select name="X_WeightedAverageCosting" class="form-control">';

if ($_SESSION['WeightedAverageCosting']==1){
	echo  '<option selected="selected" value="1">' . _('Weighted Average') . '</option>';
	echo  '<option value="0">' . _('Standard') . '</option>';
} else {
	echo  '<option selected="selected" value="0">' . _('Standard') . '</option>';
	echo  '<option value="1">' . _('Weighted Average') . '</option>';
}

echo '</select></td><td>' . _('nERP allows inventory to be costed based on the weighted average of items in stock or full standard costing with price variances reported. The selection here determines the method used and the general ledger postings resulting from purchase invoices and shipment closing') . '</td></tr>';

echo '<tr><td>' . _('Auto Issue Components') . ':</td>
		<td>
		<select name="X_AutoIssue" class="form-control">';
...
echo '</select></td><td>' . _('When items are manufactured it is possible for the components of the item to be automatically decreased from stock in accordance with the Bill of Material setting') . '</td></tr>' ;

echo '<tr><td>' . _('Prohibit Negative Stock') . ':</td>
		<td>
		<select name="X_ProhibitNegativeStock" class="form-control">';
...
echo '<input type="hidden" name="X_MonthsAuditTrail" value="1" />';
//echo '<tr><td>' . _('Months of Audit Trail to Retain') . ':</td>
//	<td class="has-error"><input type="text" class="form-control" pattern="(?!^0\d+$)[\d]{1,2}" required="required" name="X_MonthsAuditTrail" size="3" maxlength="2" value="' . $_SESSION['MonthsAuditTrail'] . '" /></td><td>' . _('If this parameter is set to 0 (zero) then no audit trail is retained. An audit trail is a log of which users performed which additions updates and deletes of database records.') . '</td>
//</tr>';


//Which messages to log
echo '<input type="hidden" name="X_LogSeverity" value="3" />';
//echo '<tr><td>' . _('Log Severity Level') . ':</td><td><select name="X_LogSeverity" class="form-control">';
...
echo '<input type="hidden" name="X_LogPath" value="companies/" />';
//echo '<tr><td>' . _('Path to log files') . ':</td>
//	<td><input type="text" name="X_LogPath" class="form-control" size="40" maxlength="79" value="' . $_SESSION['LogPath'] . '" /></td><td>' . _('The path to the directory where the log files will be stored. Note the apache user must have write permissions on this directory.') . '</td>
//</tr>';

//DefineControlledOnWOEntry
echo '<tr><td>' . _('Controlled Items Defined At Work Order Entry') . ':</td>
	<td><select name="X_DefineControlledOnWOEntry" class="form-control">
...
echo '<tr><td>' . _('Auto Create Work Orders') . ':</td>
		<td>
		<select name="X_AutoCreateWOs" class="form-control">';
...
echo '<tr><td>' . _('Default Factory Location') . ':</td>
	<td><select name="X_DefaultFactoryLocation" class="form-control">';
...
echo '<tr><td>' . _('Factory Manager Email Address') . ':</td>
	<td><input type="email" class="form-control" name="X_FactoryManagerEmail" size="50" maxlength="50" value="' . $_SESSION['FactoryManagerEmail'] . '" /></td>
...
echo '<tr><td>' . _('Purchasing Manager Email Address') . ':</td>
	<td><input type="email" class="form-control" name="X_PurchasingManagerEmail" size="50" maxlength="50" value="' . $_SESSION['PurchasingManagerEmail'] . '" /></td>
	<td>' . _('The email address for the purchasing manager, used to receive notifications by the tendering system')  . '</td></tr>';

echo '<tr><td>' . _('Inventory Manager Email Address') . ':</td>
	<td><input type="email" class="form-control" name="X_InventoryManagerEmail" size="50" maxlength="50" value="' . $_SESSION['InventoryManagerEmail'] . '" /></td>
...
echo '<tr>
	...
	<td>
		<select type="text" name="X_SmtpSetting" class="form-control">';
...
echo '<tr><td>' . _('Text for Quality Product Specification') . ':</td>
	<td><textarea name="X_QualityProdSpecText" class="form-control">' . $_SESSION['QualityProdSpecText'] . '</textarea></td>
	<td>' . _('This text appears on product specifications') . '</td></tr>';
echo '<tr><td>' . _('Text for Quality Product Certifications') . ':</td>
	<td><textarea name="X_QualityCOAText" class="form-control">' . $_SESSION['QualityCOAText'] . '</textarea></td>
	<td>' . _('This text appears on product certifications') . '</td></tr>';
echo '<tr>
	<td>' . _('Auto Log Quality Samples'). '</td>
	<td>
		<select type="text" name="X_QualityLogSamples" class="form-control">';
...
echo '</table></div></div></div>
		<div class="row" align="center"><div><input type="submit" class="btn btn-success" name="submit" value="' . _('Submit') . '" /></div>
    </div><br />
