...
if (isset($_POST['PrintPDF'])) {
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
		echo prnMsg(_('The sell through support items to claim could not be retrieved by the SQL because') . ' - ' . DB_error_msg(),'error');
	...
	if (DB_num_rows($ClaimsResult) == 0) {
		...
		include('includes/header.php');
		echo  prnMsg(_('No sell through support items retrieved'), 'warn');
		echo '<p align="right"><a href="' .$RootPath .'/menu_data.php?Application=Sales" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a><p>';
		if ($debug==1){
		//  echo '<br />' .  $SQL;
		}
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' '
		. _('Sell Through Support Claims Report') . '</h1></a></div>';
...
<div class="col-xs-12">';
        ...
        echo '
				...
				<div class="row">
					<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Sales Made From') . ' (' . _('in the format') . ' ' . $_SESSION['DefaultDateFormat'] . ')</label>
						<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="FromDate" size="11" maxlength="10" value="' . $_POST['FromDate'] . '" /></div>
					</div>
					<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Sales Made To') . ' (' . _('in the format') . ' ' . $_SESSION['DefaultDateFormat'] . ')</label>
						<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="ToDate" size="11" maxlength="10" value="' . $_POST['ToDate'] . '" /></div>
					</div>
				
				<div class="col-xs-4">
        <div class="form-group"> <br />
					<input type="submit" name="PrintPDF" class="btn btn-info" value="' . _('Create Claims Report') . '" />
				</div>';
        echo '</div> </div>
              </form>
			  </div>
