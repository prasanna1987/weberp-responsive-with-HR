...
if ((!isset($_GET['TransNo']) or $_GET['TransNo']=='') and !isset($_POST['TransDate'])){
	...
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post" name="form">
		...
		<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Create picking lists for all deliveries to be made on').'  ' . '
</label>
		...
		<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('From Warehouse').'  ' . '</label>
...
<div class="form-group"><br />
			<input type="submit" class="btn btn-info" name="Process" value="' . _('Print Picking Lists') . '" />
		</div>
        </div>
		</div><br />
		</form>';
...
if (isset($_POST['TransDate'])
	...
	if (DB_num_rows($result)==0){
		...
		echo '<br />
				<div class="row">
<div class="col-xs-4"><a href="'. $RootPath . '/PDFPickingList.php" class="btn btn-info">' . _('Enter Another Date') . '</a></div>
				</div>
...
if ($ListCount == 0){
	...
	$pdf->OutputD('nERP' . '_PickingLists_' . date('Y-m-d') . '.pdf');
	$pdf->__destruct();
}
?>