...
if (!isset($_GET['OrderNo']) AND !isset($_POST['OrderNo'])) {
	...
	echo '<br />';
	echo prnMsg(_('Select a Purchase Order Number to Print before calling this page'), 'error');
	echo '<br />
				
				<div class="row">
					<div class="col-xs-4">
					<a href="' . $RootPath . '/PO_SelectOSPurchOrder.php" class="btn btn-info">' . _('Outstanding Purchase Orders') . '</a></div>
					<div class="col-xs-4">
					<a href="' . $RootPath . '/PO_SelectPurchOrder.php" class="btn btn-info">' . _('Purchase Order Inquiry') . '</a></div>
						</div>
	...
	echo '<div class="text-danger">' . _('This page must be called with a purchase order number to print');
	echo '</div><br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=PO" class="btn btn-default">' . _('<Back to Menu') . '</a></p><br />';
	exit;
}
...
if (isset($_POST['PrintOrEmail']) AND isset($_POST['EmailTo'])) {
	if ($_POST['PrintOrEmail'] == 'Email' AND !IsEmailAddress($_POST['EmailTo'])) {
		include('includes/header.php');
		echo  prnMsg(_('The email address entered does not appear to be valid. No emails have been sent.'), 'warn');
		include('includes/footer.php');
		exit;
	}
}
...
if (isset($OrderNo) AND $OrderNo != '' AND $OrderNo > 0 AND $OrderNo != 'Preview') {
	...
	if (DB_num_rows($result) == 0) {
		...
		include('includes/header.php');
		echo '<br />';
		echo prnMsg(_('Unable to Locate Purchase Order Number') . ' : ' . $OrderNo . ' ', 'error');
		echo '<br />
			
			<div class="row">
				<div class="col-xs-4"><a href="' . $RootPath . '/PO_SelectOSPurchOrder.php" class="btn btn-info">' . _('Outstanding Purchase Orders') . '</a></div>
				<div class="col-xs-4"><a href="' . $RootPath . '/PO_SelectPurchOrder.php" class="btn btn-info">' . _('Purchase Order Inquiry') . '</a></div>
				</div>
			<br />';
		include('includes/footer.php');
		exit();
		...
		if ($POHeader['status'] != 'Authorised' AND $POHeader['status'] != 'Printed') {
			include('includes/header.php');
			echo  prnMsg(_('Purchase orders can only be printed once they have been authorised') . '. ' . _('This order is currently at a status of') . ' ' . _($POHeader['status']), 'warn');
			include('includes/footer.php');
			exit;
		}
		...
		if ($ViewingOnly == 0) {
			if ($POHeader['allowprint'] == 0) {
				...
				echo  prnMsg(_('Purchase Order Number') . ' ' . $OrderNo . ' ' . _('has previously been printed') . '. ' . _('It was printed on') . ' ' . ConvertSQLDate($POHeader['dateprinted']) . '<br />' . _('To re-print the order it must be modified to allow a reprint') . '<br />' . _('This check is there to ensure that duplicate purchase orders are not sent to the supplier resulting in several deliveries of the same supplies'), 'warn');

				echo '<br /><div class="row">
 					<div class="col-xs-3"><a href="' . $RootPath . '/PO_PDFPurchOrder.php?OrderNo=' . $OrderNo . '&ViewingOnly=1" class="btn btn-warning">' . _('Print This Order as a Copy') . '</a></div>
 					<div class="col-xs-3"><a href="' . $RootPath . '/PO_Header.php?ModifyOrderNumber=' . $OrderNo . '" class="btn btn-info">' . _('Modify the order to allow a real reprint') . '</a></div>
					<div class="col-xs-3"><a href="' . $RootPath . '/PO_SelectPurchOrder.php" class="btn btn-info">' . _('Select another order') . '</a></div>
					<div class="col-xs-3"><a href="' . $RootPath . '/menu_data.php?Application=PO" class="btn btn-default">' . _('Back to the menu') . '</a></div></div><br />
';
...
if (isset($MakePDFThenDisplayIt) or isset($MakePDFThenEmailIt)) {
	...
	if ($MakePDFThenDisplayIt) {
		$pdf->OutputD('nERP' . '_PurchaseOrder_' . $OrderNo . '_' . date('Y-m-d') . '.pdf');
		$pdf->__destruct();
	} else {
		/* must be MakingPDF to email it */

		$PdfFileName = 'nERP' . '_PurchaseOrder_' . $OrderNo . '_' . date('Y-m-d') . '.pdf';
		...
		if($_SESSION['SmtpSetting'] == 0){//use the mail service provice by the server.
			...
			echo  prnMsg(_('The SMTP settings are wrong, please ask administrator for help'),'error');
			exit;
			include('includes/footer.php');
		}
		...
		if ($Success == 1) {
			...
			echo  prnMsg(_('Purchase Order') . ' ' . $OrderNo . ' ' . _('has been emailed to') . ' ' . $_POST['EmailTo'] . ' ' . _('as directed'), 'success');

		} else { //email failed
			$Title = _('Email a Purchase Order');
			include('includes/header.php');
			
			echo  prnMsg(_('Emailing Purchase order') . ' ' . $OrderNo . ' ' . _('to') . ' ' . $_POST['EmailTo'] . ' ' . _('failed'), 'error');
		}
	}
...
/* There was enough info to either print or email the purchase order */
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Print or Email the Order') . '</label>
            <select name="PrintOrEmail" class="form-control">';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Show Amounts on the Order') . '</label>
		<select name="ShowAmounts" class="form-control">';
	...
	if ($_POST['PrintOrEmail'] == 'Email') {
		...
		if (DB_num_rows($ContactsResult) > 0) {
			echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Email to') . '</label><select name="EmailTo" class="form-control">';
			...
			echo prnMsg(_('There are no contacts defined for the supplier of this order') . '. ' . _('You must first set up supplier contacts before emailing an order'), 'error');
			echo '<br />';
		}
	} else {
	...
	echo '
         <div class="row">
          <div class="col-xs-4">    <input type="submit" name="DoIt" value="' . _('OK') . '" class="btn btn-info" />
         </div>
         </div><br />
         </form>';
