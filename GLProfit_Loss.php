...
if (isset($_POST['FromPeriod']) and ($_POST['FromPeriod'] > $_POST['ToPeriod'])){
	echo prnMsg(_('The selected period from is actually after the period to') . '! ' . _('Please reselect the reporting period'),'error');
...
if ((!isset($_POST['FromPeriod'])
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1> ' .// Icon title.
		_('Print Profit and Loss Report') . '</h1></a></div>';// Page title.
...
<h4 class="text-danger"><strong>', _('OR'), '</strong></h4>
			</div>
		</div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Report Type').'</label>
		...
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Include zero balance accounts?') . '</label>
			 <div class="checkbox"><label><input type="checkbox" title="' . _('Check this box to display all accounts including those accounts with no balance') . '" name="ShowZeroBalances"></label></div></div>
		</div>
		</div>
...
<div class="form-group">
			<input type="submit" name="ShowPL" class="btn btn-info" value="' ._('Show') . '" />
		</div>
		</div>
...
<div class="form-group"> 
	...
	if ($NumberOfMonths > 12){
		include('includes/header.php');
		echo '<p>';
		echo prnMsg(_('A period up to 12 months in duration can be specified') . ' - ' . _('the system automatically shows a comparative for the same period from the previous year') . ' - ' . _('it cannot do this if a period of more than 12 months is specified') . '. ' . _('Please select an alternative period range'),'error');
	...
	if (DB_error_no() != 0) {
		...
		include('includes/header.php');
		echo prnMsg( _('No general ledger accounts were returned by nERP because') . ' - ' . DB_error_msg(),'error');
	...
	if (DB_num_rows($AccountsResult)==0){
		...
		include('includes/header.php');
		echo '<br />';
		echo prnMsg( _('There were no entries to print out for the selections specified'),'warn' );
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=GL" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
		include('includes/footer.php');
		exit;
	}
...
<table id="general-table" class="table table-bordered"> ' .// Icon title.
		_('Profit and Loss Statement') . '<br /><small>' .// Page title, reporting statement.
