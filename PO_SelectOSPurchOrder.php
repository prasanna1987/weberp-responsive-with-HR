...
if (isset($OrderNumber) AND $OrderNumber != '') {
	if (!is_numeric($OrderNumber)) {
		echo '<br /><p class="text-danger"><strong>' . _('The Order Number entered') . ' <u>' . _('MUST') . '</u> ' . _('be numeric') . '.</strong></p><br />';
		unset($OrderNumber);
	} else {
		echo '<p class="text-success"><strong>'. _('Order Number') . ' - ' . $OrderNumber.'</strong></p>';
	}
} else {
	if (isset($SelectedSupplier)) {
		echo '<br />
				<p class="text-info"><strong>' . _('For supplier') . ': ' . $SelectedSupplier . ' ' . _('and') . ' ';
		echo '<input type="hidden" name="SelectedSupplier" value="' . $SelectedSupplier . '" /></strong>
				</p>';
	}
...
if (isset($_POST['SearchParts'])) {
	if (isset($_POST['Keywords']) AND isset($_POST['StockCode'])) {
		echo '<p class="text-info"><strong>' . _('Stock description keywords have been used in preference to the Stock code extract entered') . '.</p></strong>';
	}
...
if (!isset($OrderNumber) or $OrderNumber == '') {
	if (isset($SelectedSupplier)) {
		echo '<br /><p align="right"><a href="' . $RootPath . '/PO_Header.php?NewOrder=Yes&amp;SupplierID=' . $SelectedSupplier . '" class="btn btn-default">' . _('<i class="fa fa-plus"></i> Purchase Order') . '</a></p>';
	} else {
...
<input type="text" name="OrderNumber" class="form-control" autofocus="autofocus" maxlength="8" size="9" /> </div></div>
...
<div class="form-group"> <label class="col-md-8 control-label">
	...
	if (!isset($_POST['Status']) OR $_POST['Status'] == 'All') {
		echo '<option selected="selected" value="Pending_Authorised">' . _('All') . '</option>';
	} else {
		echo '<option value="Pending_Authorised">' . _('All') . '</option>';
	}
...
<div class="form-group"> <label class="col-md-2 control-label">
		' . _('Orders Between') . '</label>
<div class="col-md-4">
		<div class="input-group input-daterange" data-date-format="mm/dd/yyyy">
			<input type="text" id="example-daterange1" name="DateFrom" value="' . ConvertSQLDate($DateFrom) . '"  class="form-control" size="10" placeholder="From" /><span class="input-group-addon"></span>
		
			<input type="text" id="example-daterange2" name="DateTo" placeholder="To" value="' . ConvertSQLDate($DateTo) . '"  class="form-control" size="10" /></div>
			</div>
			</div>
			
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Show PO Details?') . '</label>
<input type="checkbox" name="PODetails" ' . $Checked . ' /></div></div></div>
<div class="row">
<div class="col-xs-4">
...
echo '<br /><h4 class="text"><strong>' . _('To search for purchase orders for a specific part use the part selection facilities below') . '</strong></h4>';
echo '<br />
		<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">';

echo '' . _('Stock category') . '</label>
...
echo '<input type="text" class="form-control" name="Keywords" size="20" maxlength="25" /></div>
		</div>
		';
...
echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <input type="submit" class="btn btn-success" name="SearchParts" value="' . _('Search') . '" />
</div>
</div>
<div class="col-xs-4">
<div class="form-group">
<input type="submit" name="ResetPart" class="btn btn-info" value="' . _('Show All') . '" /></div>
		</div>
	</div>';
...
<table id="general-table" class="table table-bordered">
	...
	if (isset($_POST['PODetails'])) {
		$BalHead = '<th>' . _('Balance') .' (' . _('Stock ID') . '--' . _('Quantity') . ' )</th>';
	} else {
		$BalHead = '';
	}
	echo '<tr>
	...
	while ($myrow = DB_fetch_array($PurchOrdersResult)) {
		...
		if ($myrow['status'] == 'Printed') {
			$ReceiveOrder = '<a href="' . $RootPath . '/GoodsReceived.php?PONumber=' . $myrow['orderno'] . '" class="btn btn-success">' . _('Receive') . '</a>';
		} else {
			$ReceiveOrder = '';
		}
		if ($myrow['status'] == 'Authorised' AND $myrow['allowprint'] == 1) {
			$PrintPurchOrder = '<a target="_blank" class="btn btn-warning" href="' . $RootPath . '/PO_PDFPurchOrder.php?OrderNo=' . $myrow['orderno'] . '">' . _('Print') . '</a>';
		} elseif ($myrow['status'] == 'Authorisied' AND $myrow['allowprint'] == 0) {
			$PrintPurchOrder = _('Printed');
		} elseif ($myrow['status'] == 'Printed') {
			$PrintPurchOrder = '<a target="_blank" class="btn btn-danger" href="' . $RootPath . '/PO_PDFPurchOrder.php?OrderNo=' . $myrow['orderno'] . '&amp;realorderno=' . $myrow['realorderno'] . '&amp;ViewingOnly=2">
				' . _('Reprint') . '</a>';
		} else {
