...
echo '<div class="row">
<div class="col-xs-4">
		<a href="'. $RootPath . '/SelectWorkOrder.php" class="btn btn-default">' . _('Back to Work Orders'). '</a></div>
		';
if(isset($SelectedWO)){

	echo '<div class="col-xs-4"><a href="'. $RootPath . '/WorkOrderCosting.php?WO=' .  $SelectedWO . '" class="btn btn-default">' . _('Back to Costing'). '</a>
		</div>';
}
echo '</div><br />';
...
if (!isset($SelectedWO) OR !isset($StockID)) {
	...
	echo '<div class="row" align="center">
			<a href="' . $RootPath . '/SelectWorkOrder.php" class="btn btn-info">' .  _('Select a work order to receive') . '</a>
		</div>';
	echo   prnMsg(_('This page can only be opened if a work order has been selected. Please select a work order to receive first'),'info');
...
if (isset($_POST['Process'])){ //user hit the process the work order receipts entered.
	...
	if($WORow['controlled']==1){ //controlled
		if ($WORow['serialised']==1){ //serialised
			...
			for ($i=0;$i<$_POST['CountOfInputs'];$i++){
				if(isset($_POST['Qty' . $i]) AND trim($_POST['Qty' . $i]) != '' AND !is_numeric($_POST['Qty' . $i])) {
					$InputError = true;
					echo prnMsg(_('The quantity entered is not numeric - a number is expected'),'error');
				}
		...
		if (!empty($_POST['ExpiryDate'])){
			if(!is_date($_POST['ExpiryDate'])){
				$InputError = true;
				echo prnMsg(_('The Expiry Date should be in date format'),'error');
			}
		}
	...
	if ($QuantityReceived + $WORow['qtyrecd'] > $WORow['qtyreqd'] *(1+$_SESSION['OverReceiveProportion']/100)){
		echo prnMsg(_('The quantity received is greater than the quantity required even after allowing for the configured allowable over-receive proportion. If this is correct then the work order must be modified first.'),'error');
		$InputError=true;
	}
		...
		//need to test if the serialised item exists first already
			if (trim($_POST['SerialNo' .$i]) != '' AND  ($_SESSION['DefineControlledOnWOEntry']==0
					...
					if ($AlreadyExistsRow[0]>0){
						echo prnMsg(_('The serial number entered already exists. Duplicate serial numbers are prohibited. The duplicate item is:') . ' ' . $_POST['SerialNo'.$i] ,'error');
						$InputError = true;
					}
			}
...
/************************ BEGIN SQL TRANSACTIONS ************************/
	...
	//Recalculate the standard for the item if there were no items previously received against the work order
				...
				//need to test if the batch/lot exists first already
					if (trim($_POST['BatchRef' .$i]) != "" AND (is_numeric($_POST['Qty' . $i]) AND ABS($_POST['Qty' . $i]>0))){
						...
						if ($AlreadyExistsRow[0]>0){
							...
							echo prnMsg(_('The input quantity should not be negative since there are no this lot no existed'),'error');
							include('includes/footer.php');
							exit;
						}
						$ErrMsg =  _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('The serial stock item record could not be inserted because');
		...
		echo   prnMsg(_('The receipt of') . ' ' . $QuantityReceived . ' ' . $WORow['units'] . ' ' . _('of')  . ' ' . $_POST['StockID'] . ' - ' . $WORow['description'] . ' ' . _('against work order') . ' '. $_POST['WO'] . ' ' . _('has been processed'),'info');
		echo '<div class="row" align="center"><a href="' . $RootPath . '/SelectWorkOrder.php" class="btn btn-info">' . _('Select a different work order for receiving finished stock against'). '</a></div>';
		unset($_POST['WO']);
		unset($_POST['StockID']);
...
if ($WORow['closed']==1){
	echo   prnMsg(_('The selected work order has been closed and variances calculated and posted. No more receipts of manufactured items can be received against this work order. You should make up a new work order to receive this item against.'),'info');
...
<table id="general-table" class="table table-bordered">
		...
		if($WORow['perishable']==1){
			echo '<td>' . _('Expiry Date') . ':<td>
			      <td class="has-error"><input type="text" maxlength="10" size="11" name="ExpiryDate" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker"
 required /></td>';
		}

		echo '<td>' . _('Received Into') . ':</td>
			<td><select name="IntoLocation" class="form-control">';
...
if($WORow['controlled']==1){ //controlled
	...
	if ($WORow['serialised']==1){ //serialised
		...
		echo '<br />
			<div class="row" align="center">
				<input type="submit" class="btn btn-success" name="Process" value="' . _('Process Manufactured Items Received') . '" />
			</div><br />';
	} else { //controlled but not serialised - just lot/batch control
		echo '<tr>
	...
	echo '<tr>
			...
			<td><input type="text" class="form-control" name="Qty" required="required" title="'._('The input must be numeric').'" placeholder="'._('Received Qty').'" /></td>
		</tr>
		</table></div></div></div>';
	echo '<br />
		<div class="row" align="center">
