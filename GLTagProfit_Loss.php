...
<div class="form-group"> 
	...
	if (DB_num_rows($AccountsResult)==0){
		...
		include('includes/header.php');
		echo '<br />';
		echo prnMsg( _('There were no entries to print out for the selections specified'),'info');
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=GL" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
		include('includes/footer.php');
		exit;
	}
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Statement of Income and Expenditure for Tag'). ' ' . $myrow[0]. ' ' . _('during the'). ' ' . $NumberOfMonths . ' ' . _('months to'). ' ' . $PeriodToDate . '</h3></div>
...
';
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		...
		if ($_POST['Detail']==_('Detailed')){
			...
			$ActEnquiryURL = '<a href="' . $RootPath . '/GLAccountInquiry.php?FromPeriod=' . urlencode($_POST['FromPeriod']) . '&amp;ToPeriod=' . urlencode($_POST['ToPeriod']) . '&amp;Account=' . urlencode($myrow['account']) . '&amp;Show=Yes" class="btn btn-info">' . $myrow['account'] . '</a>';
	...
	echo '<tr>
		...
		<div class="row" align="center">
			...
			<input type="submit" class="btn btn-info" name="SelectADifferentPeriod" value="'._('Select A Different Period').'" /><div>
		</div></div><br />
';
}
