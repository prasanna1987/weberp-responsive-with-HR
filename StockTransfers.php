...
if(isset($_POST['CheckCode'])) {
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1> ',// Icon title.
		_('Select Item to Transfer'), '</h1></a></div>';// Page title.
...
<table id="general-table" class="table table-bordered">
	...
	while($myrow = DB_fetch_array($result)) {
		echo '<tr>
				...
				<td><a href="' . $RootPath . '/StockTransfers.php?StockID='.$myrow['stockid'].'&amp;Description='.$myrow['description'].'&amp;NewTransfer=Yes&amp;Quantity='. filter_number_format($_POST['Quantity']).'&amp;From='.$_POST['StockLocationFrom'].'&amp;To='.$_POST['StockLocationTo'].'" class="btn btn-info">'
				._('Transfer') . '</a></td>
			</tr>';
...
if($NewTransfer) {
	...
	if(DB_num_rows($result) == 0) {
		echo prnMsg( _('Unable to locate Stock Code').' '.mb_strtoupper($_POST['StockID']), 'error');
		...
		if($myrow['mbflag']=='D' OR $myrow['mbflag']=='A' OR $myrow['mbflag']=='K') {
			echo prnMsg(_('The part entered is either or a dummy part or an assembly or a kit-set part') . '. ' . _('These parts are not physical parts and no stock holding is maintained for them') . '. ' . _('Stock Transfers are therefore not possible'),'warn');
			echo '.<hr />';
			echo '<div class="row"><div class="col-xs-4"><a href="' . $RootPath . '/StockTransfers.php?NewTransfer=Yes" class="btn btn-info">' . _('Enter another Transfer') . '</a></div></div>';
			unset($_SESSION['Transfer']);
			include('includes/footer.php');
			exit;
		}
...
if(isset($_POST['EnterTransfer']) ) {
	...
	if($_SESSION['Transfer']->StockLocationFrom==$_SESSION['Transfer']->StockLocationTo) {
		echo '<br />';
		echo prnMsg( _('The locations to transfer from and to must be different'), 'error');
		$InputError = true;
	}
...
/*All inputs must be sensible so make the stock movement records and update the locations stocks */
		...
		if($_SESSION['ProhibitNegativeStock']==1
			...
			echo prnMsg( _('There is insufficient stock to make this transfer and nERP is setup to prevent negative stock'), 'warn');
			include('includes/footer.php');
			exit;
		}
...
/*Insert the StockSerialMovements and update the StockSerialItems  for controlled items*/
		...
		echo prnMsg(_('An inventory transfer of').' ' . $_SESSION['Transfer']->TransferItem[0]->StockID . ' - ' . $_SESSION['Transfer']->TransferItem[0]->ItemDescription . ' '. _('has been created from').' ' . $_SESSION['Transfer']->StockLocationFrom . ' '. _('to') . ' ' . $_SESSION['Transfer']->StockLocationTo . ' '._('for a quantity of').' ' . $_SESSION['Transfer']->TransferItem[0]->Quantity,'success');
		echo '<br /><a href="PDFStockTransfer.php?TransferNo='.$TransferNumber.'" class="btn btn-warning">' . _('Print Transfer Note') . '</a>';
		unset($_SESSION['Transfer']);
		include('includes/footer.php');
		exit;
	}
...
<div class="form-group"> <label class="col-md-8 control-label">' .  _('Description-part or full'). '</label>
		<input type="text" class="form-control" name="StockText" title="'._('You can key in part of stock description or left this and ')._('Partial Stock Code')._('blank and click ')._('Check Part').' '._('to view the whole stock list').'" size="21" value="' . $_GET['Description'] .'" /></div></div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Stock ID-part or full'). '</label>';
if(isset($_POST['StockID'])) {
	echo '<input type="text" class="form-control" title="'._('You can key in partial of the stock code or just left this blank to click').' '._('Check Part').'" name="StockCode" size="21" value="' . $_POST['StockID'] .'" maxlength="20" />';
} else {
	echo '<input type="text" class="form-control" title="'._('You can key in partial of the stock code or just left this blank to click').' '._('Check Part').'" name="StockCode" size="21" value="" maxlength="20" />';
}
echo '</div></div></div>
<div class="row" align="center">
<input type="submit" name="CheckCode" class="btn btn-success" value="'._('Search').'" /></div>
...
if(isset($_SESSION['Transfer']->TransferItem[0]->ItemDescription)
	...
	echo '<br />
<div class="row">
			' . $_SESSION['Transfer']->TransferItem[0]->ItemDescription . ' ('._('In Units of').' ' . $_SESSION['Transfer']->TransferItem[0]->PartUnit . ' )</div>';
}

echo '<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('From Stock Location').'</label>
		<select name="StockLocationFrom" class="form-control">';
...
<div class="form-group"> <label class="col-md-8 control-label">' .  _('To Stock Location').' </label>
		<select name="StockLocationTo" class="form-control"> ';
...
if(isset($_SESSION['Transfer']->TransferItem[0]->Controlled)
	...
	echo '<strong>' . _('Transfer Quantity').'</strong><input type="hidden" name="Quantity" value="' . locale_number_format($_SESSION['Transfer']->TransferItem[0]->Quantity) . '" /><br /><div style="padding-top:9px;"><a href="' . $RootPath .'/StockTransferControlled.php?StockLocationFrom='.$_SESSION['Transfer']->StockLocationFrom.'" class="btn btn-info">' . $_SESSION['Transfer']->TransferItem[0]->Quantity . '</a></div>';
} elseif(isset($_SESSION['Transfer']->TransferItem[0]->Controlled)) {
	echo '<div class="form-group"> <label class="col-md-8 control-label">' . _('Transfer Quantity').'</label><input type="text" class="form-control" name="Quantity" title="'._('The transfer quantity cannot be zero').'" size="12" maxlength="12" value="' . locale_number_format($_SESSION['Transfer']->TransferItem[0]->Quantity) . '" /></div>';
} else {
	echo '<div class="form-group"> <label class="col-md-8 control-label">' . _('Transfer Quantity').'</label><input type="text" class="form-control"  title="'._('The transfer quantity cannot be zer0').'" name="Quantity" size="12" maxlength="12" value="0" /></div>';
}

echo '</div></div>
	<div class="row" align="center">
		<div>
		<input type="submit" name="EnterTransfer" class="btn btn-success" value="' . _('Submit') . '" />
		</div></div>
		';
...
if(isset($_SESSION['Transfer'])) {
	echo '<div class="sub-header"></div><br /><div class="row">
		<div class="col-xs-3">
		<a href="'.$RootPath.'/StockStatus.php?StockID=' . $StockID . '" class="btn btn-info">' . _('Show Stock Status') . '</a>';
	echo '</div>
		<div class="col-xs-3"><a href="'.$RootPath.'/StockMovements.php?StockID=' . $StockID . '" class="btn btn-info">' . _('Show Movements') . '</a>';
	echo '</div>
		<div class="col-xs-3"><a href="'.$RootPath.'/StockUsage.php?StockID=' . $StockID . '&amp;StockLocation=' . $_SESSION['Transfer']->StockLocationFrom . '" class="btn btn-info">' . _('Show Stock Usage') . '</a>';
	echo '</div>
		<div class="col-xs-3"><a href="'.$RootPath.'/SelectSalesOrder.php?SelectedStockItem=' . $StockID . '&amp;StockLocation=' . $_SESSION['Transfer']->StockLocationFrom . '" class="btn btn-info">' . _('Search Outstanding Sales Orders') . '</a>';
	echo '</div></div><br />
	<div class="row">
		<div class="col-xs-3"><a href="'.$RootPath.'/SelectCompletedOrder.php?SelectedStockItem=' . $StockID . '" class="btn btn-info">' . _('Search Completed Sales Orders') . '</a></div></div><br />';
}
echo '
	</form></div>
    </div>';
