...
/* Javier */	$ListCount = 1;
	...
	if ($myrow['printedpackingslip']==1 AND ($_GET['Reprint']!='OK' OR !isset($_GET['Reprint']))){
		...
		echo   prnMsg( _('The packing slip for order number') . ' ' . $_GET['TransNo'] . ' ' .
			...
			'<br />' . _('This check is there to ensure that duplicate packing slips are not produced and dispatched more than once to the customer'), 'warn' ),'</div><br />';
	      echo '<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/PrintCustOrder.php?TransNo=' . $_GET['TransNo'] . '&Reprint=OK" class="btn btn-warning">'
		. _('Do a Re-Print') . ' (' . _('On Pre-Printed Stationery') . ') ' . _('Even Though Previously Printed') . '</a><div>' .
		'<div class="col-xs-4"><a href="' . $RootPath. '/PrintCustOrder_generic.php?TransNo=' . $_GET['TransNo'] . '&Reprint=OK" class="btn btn-warning">' .  _('Do a Re-Print') . ' (' . _('Plain paper') . ' - ' . _('A4') . ' ' . _('landscape') . ') ' . _('Even Though Previously Printed'). '</a></div></div>';

		echo '<br /';
		echo '<h4>'. _('Or select another Order Number to Print').'</h4>';
	        echo '<div class="row">
<div class="col-xs-4"><a href="'. $RootPath . '/SelectSalesOrder.php" class="btn btn-info">' . _('Outstanding Sales Orders') . '</a></div>
<div class="col-xs-4"><a href="'. $RootPath . '/SelectCompletedOrder.php" class="btn btn-info">' . _('Completed Sales Orders') . '</a></div>	
				</div>
...
/* END Brought from class.pdf.php constructor */
	...
	$Title = _('Print Packing Slip Error');
	include('includes/header.php');
	echo '<p class="text-info">' .  _('There were no outstanding items on the order to deliver. A dispatch note cannot be printed').
		'</p><br />
		<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/SelectSalesOrder.php" >' .  _('Print Another Packing Slip/Order').
		'</a></div>' . '
<div class="col-xs-4"><a href="' . $RootPath . '/menu_data.php?Application=Sales">' . _('<i class="fa fa-hand-o-left fa-fw"></i>
To Menu') . '</a></div></div><br />';
	include('includes/footer.php');
	exit;
