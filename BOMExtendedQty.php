...
if (isset($_POST['PrintPDF'])) {
	...
	if (DB_error_no() !=0) {
		...
		echo prnMsg( _('The Quantiy Extended BOM Listing could not be retrieved by nERP because') . ' '  . DB_error_msg(),'error');
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Return') . '</a></p><br />';
		if ($debug==1){
			echo '<br />' . $sql;
		}
	...
	if ($ListCount == 0) {
		...
		include('includes/header.php');
		echo prnMsg(_('There were no items for the selected assembly'),'error');
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p><br />';
		include('includes/footer.php');
		exit;
	} else {
...
<div class="col-xs-12">';
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">
		...
		<div class="row">
<div class="col-xs-3">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Part') . '</label>
			<input type="text" class="form-control" autofocus="autofocus" required="required" name="Part" size="20" title="' . _('Enter the item code that you wish to display the extended bill of material for') . '" /></div>
		</div>
		<div class="col-xs-3">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Quantity') . '</label>
			<input type="text" class="form-control" required="required" name="Quantity" size="4" /></div>
		</div>
		<div class="col-xs-3">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Parts Option') . '</label>
