...
if (isset($_POST['submit'])) {
	...
	echo    prnMsg(_('Creating first level'),'info');
	flush();
	...
	echo    prnMsg(_('Creating other levels'),'info');
	flush();
	$compctr = 1;
	while ($compctr > 0) {
		$lctr++;
	...
	echo    prnMsg(_('Creating bomlevels table'),'info');
	flush();
	...
	echo    prnMsg(_('Creating levels table'),'info');
	flush();
	...
	echo    prnMsg(_('Levels table has been created'),'info');
	flush();
	...
	// mrpdemands
	echo    prnMsg(_('Creating requirements table'),'info');
	flush();
	...
	echo    prnMsg(_('Loading requirements from sales orders'),'info');
	flush();
	...
	echo    prnMsg(_('Loading requirements from work orders'),'info');
	flush();
		...
		$sql = "INSERT INTO mrprequirements	(part,
								   ...
								   SELECT mrpdemands.stockid,
									 ...
									 WHERE mrpdemands.stockid = stockmaster.stockid
										AND stockmaster.discontinued = 0";
		$result = DB_query($sql);
		echo    prnMsg(_('Loading requirements based on mrpdemands'),'info');
		flush();
	}
	if ($_POST['UseRLDemands'] == 'y') {
		...
		$result = DB_query($sql);
		echo    prnMsg(_('Loading requirements based on reorder level'),'info');
		flush();
	}
	...
	echo    prnMsg(_('Creating supplies table'),'info');
	flush();
	...
	echo    prnMsg(_('Loading supplies from purchase orders'),'info');
	flush();
	...
	echo    prnMsg(_('Loading supplies from inventory on hand'),'info');
	flush();
	...
	echo    prnMsg(_('Loading supplies from work orders'),'info');
	flush();
	$sql = "INSERT INTO mrpsupplies	(id,
									 part,
									 duedate,
	...
	// In the following section, create mrpsupplies from open purchase orders,
	// open work orders, and current quantity onhand from locstock
	echo    prnMsg(_('Creating planned orders table'),'info');
	flush();
	...
	for ($Level = $MaxLevel; $Level >= $MinLevel; $Level--) {
		...
		echo prnMsg('------ ' . _('Processing level') .' ' . $Level . ' ------','info');
		flush();
		$result = DB_query($sql);
		while ($myrow=DB_fetch_array($result)) {
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Last Run Details') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
				
				<tr>
					<td>' . _('Last Run On') . ':</td><td>' . $myrow['runtime'] . '</td>
				</tr>
				<tr>
					<td>' . _('Location') . ':</td>
				...
				<tr>
					<td>' . _('Use Economic Order Quantity?') . ':</td>
					<td>' . $useeoq . '</td>
				</tr>
				<tr>
...
<div class="col-xs-12">';
	...
	echo '<div class="block">
			<div class="block-title">
				<h3>' . _('This Run Details') . '</h3></div>
			<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Location') . '</label>
				<select required="required" autofocus="autofocus" name="location[]" multiple="multiple" class="form-control">
...
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Allowed Variance (days)') . '</label>
			<input type="text" required="required" name="Leeway" class="form-control" size="4" value="' . $Leeway . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' ._('Use MRP Demands?') . '</label>
			<input type="checkbox" name="UseMRPDemands" value="y" checked="checked" /></div>
		</div></div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' ._('Use Reorder Level Demands?') . '</label>
			<input type="checkbox" name="UseRLDemands" value="y" checked="checked" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' ._('Use Economic Order Quantity') . '</label>
			<input type="checkbox" name="EOQFlag" value="y" checked="checked" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' ._('Use Pan Size?') . '</label>
			<input type="checkbox" name="PanSizeFlag" value="y" checked="checked" /></div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' ._('Use Shrinkage?') . '</label>
			<input type="checkbox" name="ShrinkageFlag" value="y" checked="checked" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <br />
			<input type="submit" class="btn btn-info" name="submit" value="' . _('Run') . '" />
		</div>
        </div>
		 </div>
		</form> </div></div> </div><br />
