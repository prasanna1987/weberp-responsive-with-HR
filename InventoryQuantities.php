...
If (isset($_POST['PrintPDF'])) {
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
	   echo prnMsg( _('The Inventory Quantity report could not be retrieved by nERP because') . ' '  . DB_error_msg(),'error');
	...
	if (DB_num_rows($result)==0){
			...
			include('includes/header.php');
			echo prnMsg(_('There were no items with inventory quantities'),'error');
		echo '<br /><p align="right"><a href="' . $RootPath . '/index.php" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
			include('includes/footer.php');
			exit;
	}
...
/*Print out the grand totals */
	...
	include('includes/header.php');
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Inventory Quantities Report') . '<br /><small>';
echo '</small></h1></a></div>';
...
<div class="col-xs-12">';
	echo '
		...
		<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Inventory Type') . '</label>
			<select name="Selection" class="form-control">
				<option selected="selected" value="All">' . _('All') . '</option>
				<option value="Multiple">' . _('Only Items With Multiple Locations') . '</option>
				</select></div>
		</div>';
