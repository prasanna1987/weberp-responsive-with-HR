...
/*First off do the stock check file stuff */
	if ($_POST['MakeStkChkData']=='New'){
		...
		if (DB_error_no() !=0) {
			...
			echo prnMsg(_('The inventory quantities could not be added to the freeze file because') . ' ' . DB_error_msg(),'error');
			echo '<br /><p align="center"><a href="' . $RootPath . '/StockCheck.php" class="btn btn-default">' . _('Back to Search') . '</a></p>';
			if ($debug==1){
		  			echo '<br />' . $sql;
			}
	...
	if ($_POST['MakeStkChkData']=='AddUpdate'){
		...
		if (DB_error_no() !=0) {
			...
			include('includes/header.php');
			echo prnMsg(_('The old quantities could not be deleted from the freeze file because') . ' ' . DB_error_msg(),'error');
			echo '<br /><p align="center"><a href="' . $RootPath . '/StockCheck.php" class="btn btn-default">' . _('Back to search') . '</a></p>';
			if ($debug==1){
		  			echo '<br />' . $sql;
			}
		...
		if (DB_error_no() !=0) {
			...
			include('includes/header.php');
			echo '<p align="center"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-default" >' . _('Back to search') . '</a>';
			echo prnMsg( _('Added to the stock check file successfully'),'success');
			include('includes/footer.php');
			exit;
		}
	}
	...
	if (DB_error_no() !=0) {
		...
		include('includes/header.php');
		echo prnMsg( _('The inventory quantities could not be retrieved by the querry because') . ' ' . DB_error_msg(),'error');
		echo '<br /><p align="center"><a href="' . $RootPath . '/StockCheck.php" class="btn btn-default">' . _('Back to search') . '</a></p>';
		if ($debug==1){
		  	echo '<br />' . $SQL;
		}
	...
	if (DB_num_rows($InventoryResult) ==0) {
		$Title = _('Stock Count Sheets - Problem Report');
		include('includes/header.php');
		echo prnMsg(_('Before stock count sheets can be printed, a copy of the stock quantities needs to be taken - the stock check freeze. Make a stock check data file first'),'error');
		echo '<br /><p align="center"><a href="' . $RootPath . '/StockCheck.php" class="btn btn-default">' . _('Back to search') . '</a></p>';
		include('includes/footer.php');
		exit;
	}
	...
	While ($InventoryCheckRow = DB_fetch_array($InventoryResult)){
		...
		if (isset($_POST['ShowInfo']) and $_POST['ShowInfo']==true){
			...
			$DemandResult = DB_query($sql,'','',false,false);
			if (DB_error_no() !=0) {
				echo prnMsg(_('The demand for this product from') . ' ' . $myrow['loccode'] . ' ' . _('cannot be retrieved because') . ' - ' . DB_error_msg(),'error');
				if ($debug==1){
...
<div class="col-xs-12">';
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Inventory Categories(Use Ctrl/Shift key)') . '</label>
				<select class="form-control" autofocus="autofocus" required="required" minlength="1" size="12" name="Categories[]" multiple="multiple">';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Location') . '</label>
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Action for Stock Check Freeze') . '</label>
			<select name="MakeStkChkData" class="form-control">';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Show system quantity on sheets?') . '</label>
			';
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Only items with non zero quantities?') . '</label>
			';
...
<div class="form-group"> 
			<input type="submit" class="btn btn-warning" name="PrintPDF" value="' . _('Print') . '" />
		</div>
		</div>
		</div>
		</form></div></div>';
