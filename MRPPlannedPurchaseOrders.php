...
if ( !DB_table_exists('mrprequirements') ) {
	...
	echo '<br />';
	echo prnMsg( _('The MRP calculation must be run before you can run this report') . '<br />' .
			_('To run the MRP calculation click') . ' ' . '<a href="' . $RootPath . '/MRP.php">' . _('<strong>here</strong>') . '</a>', 'error');
...
if ( isset($_POST['PrintPDF']) OR isset($_POST['Review']) ) {
	...
	if (DB_error_no() !=0) {
		...
		include('includes/header.php');
		echo prnMsg( _('The MRP planned purchase orders could not be retrieved by nERP because') . ' '  . DB_error_msg(),'error');
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('Menu') . '</a></p><br />';
		if ($debug==1){
			echo '<br />' . $sql;
		}
		include('includes/footer.php');
	...
	if (DB_num_rows($result)==0){ //then there is nothing to print
		...
		include('includes/header.php');
		echo prnMsg(_('There were no items with planned purchase orders'),'info');
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _(' Menu') . '</a></p><br />';
		include('includes/footer.php');
		exit;
	}
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h4>' . _('Consolidation') . ': ' . $_POST['Consolidation'] . '&nbsp;&nbsp;&nbsp;&nbsp;'
						  . _('Cutoff Date') . ': ' . $_POST['cutoffdate'] . '</h4></div>
...
<table id="general-table" class="table table-bordered">
		...
		while ($myrow = DB_fetch_array($result)){
			...
			echo '<tr class="striped_row">
				<td><a href="' . $RootPath . '/PO_Header.php?NewOrder=Yes&amp;SelectedSupplier=' . urlencode($preferredsupplier) . '&amp;StockID=' . urlencode($myrow['part']) . '&amp;Quantity=' . urlencode($myrow['supplyquantity']/$conversionfactor) . '" class="btn btn-success">' . _('Convert') . '</a></td>
				<td>' . '<a href="' . $RootPath . '/SelectProduct.php?StockID=' . urlencode($myrow['part']) . '" class="btn  btn-info">' . $myrow['part'] . '</a>' .  '<input type="hidden" name="' . $j . '_part" value="' . $myrow['part']. '" /></td>
				<td>' . $myrow['description'] . '</td>
				<td>' . ConvertSQLDate($myrow['mrpdate']) . '</td>
				<td>' . ConvertSQLDate($myrow['duedate']) . '</td>
				<td>' . locale_number_format($myrow['supplyquantity'],$myrow['decimalplaces']) . '</td>
				<td>' . locale_number_format($myrow['computedcost'],$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
				<td>' . locale_number_format($myrow['supplyquantity'] * $myrow['computedcost'],$_SESSION['CompanyRecord']['decimalplaces']) . '</td>';

			if ($_POST['Consolidation']!='None') {
				echo '<td>' . $myrow['consolidatedcount'] . '</td>';
			}
			echo '</tr>';
		...
		// Print out the grand totals
		echo '<tr>
				<td colspan="3">' . _('<strong>Number of Purchase Orders</strong>') .': ' . ($j-1) . '</td>
				<td colspan="5">' . _('<strong>Total Cost</strong>') . ': ' . locale_number_format($Total_ExtCost,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
			</tr>
			</table>
			</div>
			</div>
		...
		echo '<br /><p align="right"><a class="noprint btn btn-default" href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '">', _('Back'), '</a></p>';
		include('includes/footer.php');
...
<div class="col-xs-12">';
	echo '
		<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">
			...
			<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Consolidation') . '</label>
				
					<select required="required" name="Consolidation" class="form-control">
			...
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Print Option') . '</label>
				
					<select name="Fill" class="form-control">
			...
			<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Cut Off Date') . '</label>
				
					<input type="text" class="form-control" required="required" class="date" name="cutoffdate" autofocus="autofocus" maxlength="10" size="11" value="' . date($_SESSION['DefaultDateFormat']) . '" />
				</div>
			</div>
			</div>
			<div class="row">
...
<div class="form-group"> 
				<input type="submit" class="btn btn-success" name="Review" value="' . _('Review') . '" /></div></div>
				<div class="col-xs-4">
<div class="form-group">  <input type="submit" name="PrintPDF" class="btn btn-warning" value="' . _('Print PDF') . '" />
			</div>
		</div>
		</div>
		</form></div></div>';
