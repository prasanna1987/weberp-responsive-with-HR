...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' .
		_('Dispatch Tax Province Maintenance') . '</h1></a></div>';
...
if(isset($_POST['submit'])) {
	...
	if(ContainsIllegalCharacters($_POST['TaxProvinceName'])) {
		$InputError = 1;
		echo prnMsg( _('The tax province name cannot contain any of the illegal characters'),'error');
	}
	if(trim($_POST['TaxProvinceName']) == '') {
		$InputError = 1;
		echo prnMsg( _('The tax province name may not be empty'), 'error');
	}
	...
	if($_POST['SelectedTaxProvince']!='' AND $InputError !=1) {
		...
		if( $myrow[0] > 0 ) {
			$InputError = 1;
			echo prnMsg( _('The tax province cannot be renamed because another with the same name already exists.'),'error');
		} else {
			...
			if( DB_num_rows($result) != 0 ) {
				...
				if(!$result) {
					echo prnMsg(_('Tax province name changed'),'success');
				}
			} else {
				$InputError = 1;
				echo prnMsg( _('The tax province no longer exists'),'error');
			}
		}
	} elseif($InputError !=1) {
		...
		if( $myrow[0] > 0 ) {
			...
			$InputError = 1;
			echo prnMsg( _('The tax province cannot be created because another with the same name already exists'),'error');
		...
		if(!$result) {
			echo prnMsg(_('Errors were encountered adding this tax province'),'error');
		} else {
			echo prnMsg(_('New tax province added'),'success');
		}
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'stockmaster'
	...
	if( DB_num_rows($result) == 0 ) {
		// This is probably the safest way there is
		echo prnMsg( _('Cannot delete this tax province because it no longer exists'),'warn');
	} else {
		$myrow = DB_fetch_row($result);
		$OldTaxProvinceName = $myrow[0];
		...
		$myrow = DB_fetch_row($result);
		if($myrow[0]>0) {
			echo prnMsg( _('Cannot delete this tax province because at least one stock location is defined to be inside this province'),'warn');
			...
			$result = DB_query($sql);
			echo prnMsg( $OldTaxProvinceName . ' ' . _('tax province and any tax rates set for it have been deleted'),'success');
		}
	} //end if
	unset ($SelectedTaxProvince);
...
<table id="general-table" class="table table-bordered">
	...
	while($myrow = DB_fetch_row($result)) {
		echo '<tr class="striped_row">
				<td>' . $myrow[1] . '</td>
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?SelectedTaxProvince=' . $myrow[0] . '" class="btn btn-info">' . _('Edit') . '</a></td>
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?SelectedTaxProvince=' . $myrow[0] . '&amp;delete=1" class="btn btn-danger">' . _('Delete')  . '</a></td>
			</tr>';
...
if(isset($SelectedTaxProvince)) {
	echo '<div class="row">
		<div class="col-xs-4">	<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Back to Tax Provinces') . '</a>
		</div></div>
<br />';
}
...
if(! isset($_GET['delete'])) {
	...
	if(isset($SelectedTaxProvince)) {
		...
		$result = DB_query($sql);
		if( DB_num_rows($result) == 0 ) {
			echo prnMsg( _('Could not retrieve the requested tax province, please try again.'),'warn');
			unset($SelectedTaxProvince);
		} else {
...
<div class="form-group has-error"> <label class="col-md-12 control-label">' . _('Tax Province Name') . '' . '</label>
			<input type="text" pattern="(?!^ *$)[^\\><+-]+" required="true" title="'._('The tax province cannot be left blank and includes illegal characters').'" placeholder="'._('Within 30 legal characters').'" class="form-control" name="TaxProvinceName" size="30" maxlength="30" value="' . $_POST['TaxProvinceName'] . '" /></div>
		</div>
		';
