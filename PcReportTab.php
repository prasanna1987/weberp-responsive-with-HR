...
<div class="form-group"> 
			<input type="submit" name="ShowTB" value="' . _('Show') . '" class="btn btn-success" />
			</div></div>
			<div class="col-xs-4">
<div class="form-group">
	...
	if (DB_error_no() != 0){
		include('includes/header.php');
		echo prnMsg(_('An error occurred getting the orders details'),'',_('System Error'));
		if ($debug == 1){
			echo prnMsg( _('The querry used to get the orders that failed was') . '<br />' . $SQL, '',_('System Error'));
		}
		include ('includes/footer.php');
		exit;
	} elseif (DB_num_rows($TabDetail) == 0){
	  	include('includes/header.php');
		echo prnMsg(_('There were no expenses found in the system within the period from') . ' ' . $_POST['FromDate'] . ' ' . _('to') . ' ' . $_POST['ToDate'] . '. ' . _('Please try again selecting a different date range'),'warn');
		if ($debug == 1) {
			echo prnMsg(_('The querry that returned no rows was') . '<br />' . $SQL,'',_('System Error'));
		}
		include('includes/footer.php');
		exit;
	}
...
<input type="submit" class="btn btn-info" name="SelectDifferentDate" value="' . _('Back to Search') . '" /></div>';
...
<table id="general-table" class="table table-bordered">
			<thead>
				<tr>
					<th class="ascending">' . _('Expense') . '</th>
	...
	echo '</tbody>
			...
			<tr>
				<td colspan="2" class="number">' . _('<strong>Balance as on</strong>') . ' ' .$_POST['ToDate'] . ':</td>
