...
/*User hit the button to enter line items -
  then meta refresh to Contract_Items.php*/
	...
	if(mb_strlen($_SESSION['Contract'.$identifier]->ContractRef)<5){
		echo prnMsg(_('The contract reference must be entered (and be longer than 5 characters) before the requirements of the contract can be setup'),'warn');
		$InputError = true;
	}
	...
	if (isset($_POST['EnterContractBOM']) AND !$InputError){
		...
		echo '<br />';
		echo prnMsg(_('You should automatically be forwarded to the entry of the Contract line items page') . '. ' .
		_('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/ContractBOM.php?identifier='.$identifier . '" class="btn btn-info">' . _('click here') . '</a> ' . _('to continue'),'info');
	...
	if (isset($_POST['EnterContractRequirements']) AND !$InputError){
		...
		echo '<br />';
		echo prnMsg(_('You should automatically be forwarded to the entry of the Contract requirements page') . '. ' .
		_('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' .
		'<a href="' . $RootPath . '/ContractOtherReqts.php?identifier=' . $identifier . '" class="btn btn-info">' . _('click here') . '</a> ' . _('to continue'),'info');
		include('includes/footer.php');
		exit;
	}
...
if (isset($_FILES['Drawing']) AND $_FILES['Drawing']['name'] !='' AND $_SESSION['Contract'.$identifier]->ContractRef!='') {
	...
	$filename1 = $_SESSION['part_pics_dir'] . '/Contracts/' . $_SESSION['Contract'.$identifier]->ContractRef;
	//mkdir($filename);
...
/*The cancel button on the header screen - to delete the contract */
	...
	if(!isset($_SESSION['ExistingContract']) OR $_SESSION['ExistingContract']!=0) {
		...
		if($_SESSION['Contract'.$identifier]->Status==2){
			$OK_to_delete = false;
			echo prnMsg( _('The contract has already been converted to an order.Order must be deleted first before the contract can be deleted'),'warn');
		}
	}
	...
	if ($OK_to_delete==true){
		...
		echo prnMsg( _('Contract').' '.$_SESSION['Contract'.$identifier]->ContractRef.' '._('has been cancelled'), 'success');
...
if (isset($_POST['CommitContract']) OR isset($_POST['CreateQuotation'])){
	...
	if (mb_strlen($_POST['ContractRef']) < 2){
		echo prnMsg(_('The contract reference should be more than 2 characters long. Please correct the contract reference before proceeding.'),'error');
		$InputError = true;
	}
	if(ContainsIllegalCharacters($_POST['ContractRef'])){
		echo prnMsg(_('The contract reference cannot contain any spaces, slashes, or inverted commas. Please correct the contract reference before proceeding.'),'error');
		$InputError = true;
	}
	...
	if (DB_num_rows($result)==1 AND $_SESSION['Contract'.$identifier]->Status ==0){
		echo prnMsg(_('The contract reference cannot be the same as a previously created stock item. Please modify the contract reference before continuing'),'error');
		$InputError=true;
	}
	if (mb_strlen($_POST['ContractDescription'])<10){
		echo prnMsg(_('The contract description is expected to be more than 10 characters long. Please correct the contract description in full before proceeding.'),'error');
		$InputError = true;
	}
	if (! Is_Date($_POST['RequiredDate'])){
		echo prnMsg (_('The date the contract is required to be completed by must be entered in the format') . ' ' . $_SESSION['DefaultDateFormat'],'error');
		$InputError =true;
	}
	if (Date1GreaterThanDate2(Date($_SESSION['DefaultDateFormat']),$_POST['RequiredDate']) AND $_POST['RequiredDate']!=''){
		echo prnMsg(_('The date that the contract is to be completed by is expected to be a date in the future. Make the required date a date after today before proceeding.'),'error');
		$InputError =true;
	}
	...
	if (DB_num_rows($result)==1){ // then we have an existing contract with this contractref
		...
		if ($ExistingContract['debtorno'] != $_SESSION['Contract'.$identifier]->DebtorNo){
			echo prnMsg(_('The contract reference cannot be the same as a previously created contract for another customer. Please modify the contract reference before continuing'),'error');
			$InputError=true;
		}
		...
		if($ExistingContract['status']<=1 AND ! $InputError){
			...
			echo prnMsg(_('The changes to the contract have been saved to the system'),'success');
		}
		if ($ExistingContract['status']==1 AND ! $InputError){
			...
			echo prnMsg(_('The contract quotation has been updated based on the new contract cost and margin'),'success');
			echo '<br /><a href="' .$RootPath . '/SelectSalesOrder.php?OrderNumber=' .  $_SESSION['Contract'.$identifier]->OrderNo . '&amp;Quotations=Quotes_Only" class="btn btn-default">' . _('Go to Quotation') . ' ' .  $_SESSION['Contract'.$identifier]->OrderNo . '</a>';
		...
		echo prnMsg(_('The new contract has been added to the system'),'success');
...
//Check if the item exists already
	...
	echo prnMsg(_('The contract has been converted into quotation number') . ' ' . $OrderNo,'info');
...
if (isset($_POST['SearchCustomers'])){
	...
	if (($_POST['CustKeywords']!='') AND (($_POST['CustCode']!='') OR ($_POST['CustPhone']!=''))) {
		echo prnMsg( _('Customer Branch Name keywords have been used in preference to the Customer Branch Code entered'), 'warn');
	}
	if (($_POST['CustCode']!='') AND ($_POST['CustPhone']!='')) {
		echo prnMsg(_('Customer Branch Code has been used in preference to the Customer Branch Phone Number entered'), 'warn');
	}
	...
	if (DB_num_rows($result_CustSelect)==0){
		echo prnMsg(_('No Customer Branch records match the search criteria') . ' - ' . _('please try again') . ' - ' . _('Note a Customer Branch Name may be different to the Customer Name'),'info');
	}
 ...
 * so parse the $Select string into debtorno and branch code */
	...
	if (DB_num_rows($result)==0){
		echo prnMsg(_('The customer details were unable to be retrieved'),'error');
		if ($debug==1){
			echo prnMsg(_('The querry used that failed to get the customer details was:') . '<br />' . $sql,'error');
		}
	} else {
		...
		if ($_SESSION['CheckCreditLimits'] > 0){  /*Check credit limits is 1 for warn and 2 for prohibit contracts */
			...
			if ($_SESSION['CheckCreditLimits']==1 AND $CreditAvailable <=0){
				echo prnMsg(_('The') . ' ' . $_SESSION['Contract'.$identifier]->CustomerName . ' ' . _('account is currently at or over their credit limit'),'warn');
...
if (!isset($_SESSION['Contract'.$identifier]->DebtorNo)
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Contract: Select Customer') . '</h1></a></div>';
	...
	echo '<div class="row">
				...
				<label class="col-md-8 control-label">' . _('Customer Branch-part or full') . '</label>
			 <input tabindex="1" type="text" name="CustKeywords" class="form-control" autofocus="autofocus" size="20" maxlength="25" />
			 </div></div>
			 
			<div class="col-xs-4">
                <div class="form-group"> 
				<label class="col-md-8 control-label">' .  _('Branch Code-part or full'). '</label>
			<input tabindex="2" class="form-control" type="text" name="CustCode" data-type="no-illegal-chars" title="' . _('Enter an extract of the customer code to search for. Customer codes can only contain alpha-numeric characters, underscore or hyphens') . '" size="15" maxlength="18" /></div></div>
		...
		<div class="row" align="center">
			...
			<input tabindex="4" type="submit" name="SearchCustomers" class="btn btn-success" value="' . _('Search') . '" />
			</div><br />
	...
	if (isset($result_CustSelect)) {
		...
		echo '<div class="sub-header"></div><br /><div class="col-xs-12">
<div class="table-responsive">
		...
		while ($myrow=DB_fetch_array($result_CustSelect)) {
			...
			echo '<td><input type="submit" name="Submit'.$j.'" value="' . $myrow['brname'] . '" class="btn btn-info" /></td>
...
<div class="col-xs-12">';
	...
	if (DB_num_rows($wcresults)==0){
		...
		echo '<div class="row">
<div class="col-xs-4"><a href="'.$RootPath.'/WorkCentres.php" class="btn btn-info">' . _('Work Centre Maintenance') . '</a></div></div>';
		include('includes/footer.php');
		exit;
	}
	...
	echo '<div class="row">
		...
		<label class="col-md-8 control-label">' . _('Contract Reference') . '</label>
				';
	if ($_SESSION['Contract'.$identifier]->Status==0) {
		...
		echo '<input type="text" name="ContractRef" autofocus="autofocus" class="form-control" required="required" size="21" title="' . _('Enter the contract reference. This reference will be used as the item code so no more than 20 alpha-numeric characters or underscore') . '" data-type="no-illegal-chars" maxlength="20" value="' . $_SESSION['Contract'.$identifier]->ContractRef . '" />';
	} else {
...
<a target="_blank" href="'. $RootPath . '/StockCategories.php" class="btn btn-info">' . _('Add or Modify Contract Categories') . '</a></div></div></div>';
	...
	echo '<div class="row">
        ...
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Location') . '</label>
			<select name="LocCode" class="form-control">';
	while ($myrow=DB_fetch_array($result)){
	...
	echo '
				<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Default Work Centre') . ' </label>';

	echo '<select name="DefaultWorkCentre" class="form-control">';
	...
	echo '</select></div>
		...
		<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Contract Description') . '</label>
		<textarea name="ContractDescription" class="form-control" style="width:100%" required="required" title="' . _('A description of the contract is required') . '" minlength="5" rows="5" cols="40">' . $_SESSION['Contract'.$identifier]->ContractDescription . '</textarea></div>
		</div>
		</div
		><div class="row">
		';
		if($_SESSION['Contract'.$identifier]->ContractRef=='') { 
		echo '<div class="col-xs-4"></div>';
		}
		else { 
			echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' .  _('Drawing') . ' ' . implode(", ", $SupportedImgExt) . ' ' . _('format') .'</label>
	...
	if (extension_loaded('gd') && function_exists('gd_info') && isset($_SESSION['Contract'.$identifier]->ContractRef) && !empty($_SESSION['Contract'.$identifier]->ContractRef)){
	//echo ($_SESSION['Contract'.$identifier]->ContractRef);	
	echo '<a href="'.$imagefile.'" download><img src="'.$imagefile.'" width=64'.
		'&amp;height=64'.
		'" alt="" /></a>';
//	$StockImgLink = '<img src="GetContractImage.php?automake=1&amp;textcolor=FFFFFF&amp;bgcolor=CCCCCC'.
//		'&amp;ContractID='.($_SESSION['Contract'.$identifier]->ContractRef).
//		'&amp;text='.
//		'&amp;width=64'.
//		'&amp;height=64'.
//		'" alt="" />';
...
if (isset($_POST['ClearImage']) ) {
	...
	echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Required Date') . '</label>
			<input type="text" required="required" id="example-datepicker" class="form-control input-datepicker-close" name="RequiredDate" maxlength="10" data-date-format="dd/mm/yyyy" size="11" value="' . $_SESSION['Contract'.$identifier]->RequiredDate . '" /></div>
		</div>';

	echo '<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Customer Reference') . '</label>
			<input type="text" name="CustomerRef" required="required" title="' . _('Enter the reference that the customer uses for this contract') . '" size="21" maxlength="20" value="' . $_SESSION['Contract'.$identifier]->CustomerRef . '" class="form-control" /></div>
		</div>
		</div>
		';
	...
	<div class="row">
	<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Gross Profit') . ' %</label>
		<input class="number form-control" type="text" name="Margin" required="required" size="6" maxlength="6" value="' . locale_number_format($_SESSION['Contract'.$identifier]->Margin, 2) . '" /></div>
		</div>';

	if ($_SESSION['CompanyRecord']['currencydefault'] != $_SESSION['Contract'.$identifier]->CurrCode){
		echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . $_SESSION['Contract'.$identifier]->CurrCode . ' ' . _('Exchange Rate') . '</label>
				<input class="number form-control" type="text" name="ExRate"  required="required" title="' . _('The exchange rate between the customer\'s currency and the functional currency of the business must be entered') . '" size="10" maxlength="10" value="' . locale_number_format($_SESSION['Contract'.$identifier]->ExRate,'Variable') . '" /></div></div>
			';
	} else {
		echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label"></label><input type="hidden" name="ExRate" value="' . locale_number_format($_SESSION['Contract'.$identifier]->ExRate,'Variable') . '" /></div></div>';
	}

	echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Contract Status') . '</label>
			';
	...
	if ($_SESSION['Contract'.$identifier]->Status >=1) {
		echo '<div class="row">
				<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Quotation Reference/Sales Order No') . '</label>
				<a href="' . $RootPath . '/SelectSalesOrder.php?OrderNumber=' . $_SESSION['Contract'.$identifier]->OrderNo . '&amp;Quotations=Quotes_Only" class="btn btn-info">' .  $_SESSION['Contract'.$identifier]->OrderNo . '</a></div>
			</div></div>';
	}
	if ($_SESSION['Contract'.$identifier]->Status!=2 and isset($_SESSION['Contract'.$identifier]->WO)) {
		echo '<div class="row">
		<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Contract Work Order Ref') . '</label>
				' . $_SESSION['Contract'.$identifier]->WO . '</div>
			</div></div>';
	}
	echo '<br />';
...
<div class="table-responsive">
	...
	echo '</table></div></td>'; //end of contract BOM table
	echo '<td valign="top">
			<div class="table-responsive">
			<table id="general-table" class="table table-bordered">
				<tr>
	...
	echo '</table></div></td></tr></table></div></div>';
	echo '<br />';
	echo'<div class="col-xs-12">
<div class="table-responsive">
			<table id="general-table" class="table table-bordered">
...
';
	...
	<div class="col-xs-4">
        <div class="form-group">
			<input type="submit" name="EnterContractBOM" class="btn btn-info" value="' . _('Items Required') . '" />
			</div>
			</div>
			<div class="col-xs-4">
        <div class="form-group">
			<input type="submit" name="EnterContractRequirements" class="btn btn-info" value="' . _('Other Requirements') .'" />';
			echo '</div></div></div>';
	if($_SESSION['Contract'.$identifier]->Status==0){ // not yet quoted
		echo '
	<div class="col-xs-4">
        <div class="form-group"><input type="submit" name="CommitContract" value="' . _('Save Changes') .'" class="btn btn-success" /></div></div>';
	} elseif($_SESSION['Contract'.$identifier]->Status==1){ //quoted but not yet ordered
		echo '
	<div class="col-xs-4">
        <div class="form-group"><input type="submit" name="CommitContract" class="btn btn-success" value="' . _('Update Quotation') .'" /></div></div>';
	}
	if($_SESSION['Contract'.$identifier]->Status==0){ //not yet quoted
		echo '
	<div class="col-xs-4">
        <div class="form-group"><input type="submit" name="CreateQuotation" class="btn btn-success" value="' . _('Create Quotation') .'" />
			</div></div>';
	} else {
		//echo '</div>';
	}
	...
	<div class="col-xs-4">
        <div class="form-group">
				 ...
				 <input type="submit" name="CancelContract" class="btn btn-danger" value="' . _('Cancel') . '" />
			  </div></div></div>';
	}
	echo '</div></div></form>';
