...
if (isset($_POST['submit'])) {
	...
	if ($_POST['CodeExpense'] == '' or $_POST['CodeExpense'] == ' ' or $_POST['CodeExpense'] == '  ') {
		$InputError = 1;
		echo prnMsg(_('The Expense type  code cannot be an empty string or spaces'), 'error');
		...
		$InputError = 1;
		echo prnMsg(_('The expense code cannot contain any of the following characters ') . '" \' - &amp;', 'error');
	} elseif (ContainsIllegalCharacters($_POST['Description'])) {
		$InputError = 1;
		echo prnMsg(_('The expense description cannot contain any of the following characters ') . '" \' - &amp;', 'error');
	...
	if (isset($SelectedExpense) and $InputError != 1) {
		...
		if ($CheckRow[0] > 0) {
			$InputError = 1;
			echo prnMsg(_('The Expense type ') . $_POST['CodeExpense'] . _(' already exists.'), 'error');
		} else {
	...
	if ($MyRow[0] > 0) {
		echo prnMsg(_('Cannot delete this petty cash expense because it is used in some tab types') . '<br />' . _('There are') . ' ' . $MyRow[0] . ' ' . _('tab types using this expense code'), 'error');
	} else {
		$SQL = "DELETE FROM pcexpenses
				  WHERE codeexpense='" . $SelectedExpense . "'";
		$ErrMsg = _('The expense type record could not be deleted because');
		$Result = DB_query($SQL, $ErrMsg);
		echo prnMsg(_('Expense type') . ' ' . $SelectedExpense . ' ' . _('has been deleted'), 'success');
		unset($SelectedExpense);
		unset($_GET['delete']);
...
<table id="general-table" class="table table-bordered">
	...
	while ($MyRow = DB_fetch_array($Result)) {
		...
		echo '<tr class="striped_row">
				...
				<td><a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '?SelectedExpense=', $MyRow['codeexpense'], '" class="btn btn-info">', _('Edit'), '</a></td>
				<td><a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '?SelectedExpense=', $MyRow['codeexpense'], '&amp;delete=yes" onclick="return MakeConfirm(\'' . _('Are you sure you wish to delete this expense code and all the details it may have set up?') . '\', \'Confirm Delete\', this);" class="btn btn-danger">' . _('Delete') . '</a></td>
			</tr>';
	}
	//END WHILE LIST LOOP
...
if (isset($SelectedExpense)) {
	echo '<div class="row"><div class="col-xs-4">
			<a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '" class="btn btn-info">', _('Show All Petty Cash Expenses Defined'), '</a>
		</div></div><br />
';
}
...
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Expense Code'), '</label>
					<input type="text" class="form-control" name="CodeExpense" autofocus="autofocus" required="required" maxlength="20" /></div>
				</div>';
	}
...
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Description'), '</label>
			<input type="text" class="form-control" name="Description" size="50" required="required" maxlength="50" value="', $_POST['Description'], '" /></div>
		</div>';
	echo '<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Account Code'), '</label>
			<select required="required" class="form-control" name="GLAccount">';
...
<div class="form-group"> <label class="col-md-8 control-label">', _('Tag'), '</label>
	...
	$Result = DB_query($SQL);
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Tax Category'), '</label>
	...
	echo '<div class="row" align="center">
			<input type="submit" name="submit" class="btn btn-success" value="', _('Submit'), '" /></div></br>
