...
/*Ok Time To Post transactions to Inventory Transfers, and Update Posted variable & received Qty's  to LocTransfers */
	...
	foreach ($_SESSION['Transfer']->TransferItem AS $TrfLine) {
		...
		/*Update the quantity received from the inputs */
			...
			echo prnMsg(_('The quantity entered for'). ' ' . $TrfLine->StockID . ' '. _('is not numeric') . '. ' . _('All quantities must be numeric'),'error');
			$InputError = True;
		}
		if(filter_number_format($_POST['Qty' . $i])<0) {
			echo prnMsg(_('The quantity entered for'). ' ' . $TrfLine->StockID . ' '. _('is negative') . '. ' . _('All quantities must be for positive numbers greater than zero'),'error');
			$InputError = True;
		}
		if($TrfLine->PrevRecvQty + $TrfLine->Quantity > $TrfLine->ShipQty) {
			echo prnMsg( _('The Quantity entered plus the Quantity Previously Received can not be greater than the Total Quantity shipped for').' '. $TrfLine->StockID , 'error');
			$InputError = True;
		}
	...
	if($TotalQuantity < 0) {
		echo prnMsg( _('All quantities entered are less than zero') . '. ' . _('Please correct that and try again'), 'error');
		$InputError = True;
	}
//exit;
	if(!$InputError) {
	/*All inputs must be sensible so make the stock movement records and update the locations stocks */
		...
		/*Insert the StockSerialMovements and update the StockSerialItems  for controlled items*/
				...
				echo prnMsg(_('A stock transfer for item code'). ' - '  . $TrfLine->StockID . ' ' . $TrfLine->ItemDescription . ' '. _('has been created from').' ' . $_SESSION['Transfer']->StockLocationFromName . ' '. _('to'). ' ' . $_SESSION['Transfer']->StockLocationToName . ' ' . _('for a quantity of'). ' '. $TrfLine->Quantity,'success');
...
if(isset($_GET['Trf_ID'])) {
	...
	if(DB_num_rows($result) == 0) {
		echo '<h3 class="page-header">' . _('Transfer') . ' #' . $_GET['Trf_ID'] . ' '. _('Does Not Exist') . '</h3><br />';
...
if(isset($_SESSION['Transfer'])) {
	...
	echo '<div class="row"><div class="col-xs-4"><a href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8'). '?NewTransfer=true" class="btn btn-info">' .  _('Back to Selection') . '</a></div></div><br />';
	
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';

	echo prnMsg(_('Please Verify Shipment Quantities Received'),'info');
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Location Transfer Reference'). ' #' . $_SESSION['Transfer']->TrfID . ' '. _('from').' ' . $_SESSION['Transfer']->StockLocationFromName . ' '. _('to'). ' ' . $_SESSION['Transfer']->StockLocationToName . '</h3></div>
...
<table id="general-table" class="table table-bordered">';
	...
	foreach ($_SESSION['Transfer']->TransferItem AS $TrfLine) {
		...
		if($TrfLine->Controlled==1) {
			echo '<td class="number"><input type="hidden" name="Qty' . $i . '" value="' . locale_number_format($Qty,$TrfLine->DecimalPlaces) . '" /><a href="' . $RootPath .'/StockTransferControlled.php?TransferItem=' . $i . '" class="btn btn-info" />' . $Qty . '</a></td>';
		} else {
			echo '<td><input type="text" class="form-control" name="Qty' . $i . '" maxlength="10" size="auto" value="' . locale_number_format($Qty,$TrfLine->DecimalPlaces) . '" /></td>';
		}
		...
		if($TrfLine->Controlled==1) {
			if($TrfLine->Serialised==1) {
				echo '<td><a href="' . $RootPath .'/StockTransferControlled.php?TransferItem=' . $i . '" class="form-control">' . _('Enter Serial Numbers') . '</a></td>';
			} else {
				echo '<td><a href="' . $RootPath .'/StockTransferControlled.php?TransferItem=' . $i . '"class="form-control">' . _('Enter Batch Refs') . '</a></td>';
			}
		}
	...
	echo '</table>
		...
		<div>
			<input type="submit" class="btn btn-success" name="ProcessTransfer" value="'. _('Submit'). '" />
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' .  _(''). '</label>
			';
	echo '<select name="RecLocation" class="form-control" onchange="ReloadForm(form1.RefreshTransferList)">';
...
<div class="form-group"><br />
		<input type="submit" name="RefreshTransferList" class="btn btn-info" value="' . _('Select Location Receiving Into') . '" /></div>
		</div>
		</div>
		<br />';
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Pending Transfers Into').' '.$LocRow['locationname'] . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">';
		...
		while ($myrow=DB_fetch_array($TrfResult)) {
			...
			echo '<tr class="striped_row">
					...
					<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Trf_ID=' . $myrow['reference'] . '" class="btn btn-success">' .  _('Receive'). '</a></td>
					</tr>';
		}
		echo '</table></div></div></div></div>';
	} else if(!isset($_POST['ProcessTransfer'])) {
		echo prnMsg(_('There are no incoming transfers to this location'), 'info');
	}
	echo '
          </form>';
}
