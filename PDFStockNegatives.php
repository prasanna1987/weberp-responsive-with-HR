...
If (DB_num_rows($result)==0){
	...
	echo  prnMsg(_('There are no negative stocks to list'),'error');
...
if (DB_num_rows($result)>0){
	$pdf->OutputD('nERP' . '_NegativeStocks_' . date('Y-m-d') . '.pdf');
	$pdf->__destruct();
} else {
	$Title = _('Negative Stock Listing Problem');
	include('includes/header.php');
	echo    prnMsg(_('There are no negative stocks to list'),'info');
	include('includes/footer.php');
}
?>