...
if (!isset($WO) OR $WO==''){
	...
	echo prnMsg(_('This page must to be called from the work order entry screen'),'error');
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('For Work Order Number') . ' ' . $WO . ' ' . _('and output item') . ' ' . $StockID . ' - ' . $Description . '
	</h1></a></div>';
	
echo '<div class="row"><div class="col-xs-4">
<a href="' . $RootPath . '/WorkOrderEntry.php?WO=' . $WO . '" class="btn btn-default">' . _('Back To Work Order') . ' ' . $WO  . '</a>';
...
if (isset($_POST['AddControlledItems'])){
	if (isset($_POST['NumberToAdd'])){ // Must be adding serial numbers automatically
		if (!is_numeric(filter_number_format($_POST['NumberToAdd']))){
			echo prnMsg(_('The quantity of controlled items to add was not numeric - a number is expected'),'error');
		} else {
			DB_Txn_Begin();
			...
			for ($i=0;$i< filter_number_format($_POST['NumberToAdd']);$i++){
				...
				if (DB_num_rows($result)!=0){
					$InputError=true;
					echo prnMsg($NextItemNumber . ' ' . _('is already entered on this work order'),'error');
				}
				...
				if (DB_num_rows($result)!=0){
					$InputError=true;
					echo prnMsg($NextItemNumber . ' ' . _('has already been used for this item'),'error');
				}
				if (!$InputError){
					if ($i>0){
						$sql .= ',';
			...
			if (mb_strlen($_POST['Reference'])==0){
				echo prnMsg(_('The batch or serial number reference has not been entered - a reference is required'),'error');
				$InputError=true;
			}
			if (!is_numeric(filter_number_format($_POST['Quantity']))){
				echo prnMsg(_('The quantity for the batch must be numeric'),'error');
				$InputError=true;
			}
			$result = DB_query("SELECT serialno FROM woserialnos
								WHERE wo='" . $WO . "'
			...
			if (DB_num_rows($result)!=0){
				$InputError=true;
				echo prnMsg(_('The serial number or batch reference must be unique to the item - the reference entered is already entered on this work order'),'error');
			}
			$result = DB_query("SELECT serialno FROM stockserialitems
			...
			if (DB_num_rows($result)!=0){
				$InputError=true;
				echo prnMsg(_('The serial number or batch reference must be unique to the item. The serial number/batch entered already exists'),'error');
			}
			if (!$InputError){
				DB_Txn_Begin();
...
/*when serial numbers /lots received they are removed from the woserialnos table so no need to check if already received - they will only show here if they are in progress */
	...
	echo   prnMsg(_('The batch/serial number') . ' ' . $_GET['Reference'] . ' ' . _('has been deleted from this work order'),'info');
}
...
//update the serial numbers and quantities and notes for each serial number or batch
	...
	for ($i=0;$i<$_POST['CountOfItems'];$i++){
			...
			if (mb_strlen($_POST['Reference' . $i])==0){
				echo prnMsg($_POST['OldReference' .$i] . ': ' , _('The new batch or serial number reference has not been entered - a reference is required'),'error');
				$InputError=true;
			}
			...
			if ($_POST['Reference' .$i] != $_POST['OldReference' .$i]){
				...
				if (DB_num_rows($result)!=0){
					$InputError=true;
					echo prnMsg($_POST['Reference' .$i] . ': ' . _('The reference entered is already entered on this work order'),'error');
				}
				$result = DB_query("SELECT serialno FROM stockserialitems
				...
				if (DB_num_rows($result)!=0){
					$InputError=true;
					echo prnMsg($_POST['Reference' .$i] . ': ' . _('The serial number/batch entered already exists'),'error');
				}
			}
			if (!$InputError){
...
if ($Serialised==1 AND $NextSerialNo>0){
	...
	echo ':</td>
		<td class="has-error"><input type="text" name="NumberToAdd" required="required" size="10" class="form-control" maxlength="10" value="1" title="'._('The input must be in number format').'" placeholder="'._('Number of new serial numbers').'" /></td>
	...
	echo '</td><td><input type="text" name="Reference" class="form-control" maxlength="30" size="30" title="'._('The reference must be input').'"  /></td>';
	if ($Serialised==0){ //also need to add the quantity
		echo '<td class="has-error"><input type="text" required="required" name="Quantity" size="10" class="form-control" maxlength="10" value="1" title="'._('The quantity must be input').'" placeholder="'._('Quantity').'" /></td>';
	} else { //it will be 1 for a serial item
		echo '<input type="hidden" name="Quantity" value="1" />';
	}
}

echo '<td><input type="submit" name="AddControlledItems" class="btn btn-success" value="' . _('Add') . '" /></td>
	</tr>
	</table></div></div></div>
	<br />';
...
if (DB_num_rows($WOSerialNoResult)==0){
	echo   prnMsg(_('There are no serial items or batches yet defined for this work order item. Create new items first'),'info');
} else {
	echo '<br />
...
<table id="general-table" class="table table-bordered">';
	...
	while ($WOSNRow = DB_fetch_array($WOSerialNoResult)){
		...
		echo '<tr>
				<td class="has-error"><input required="required" type="text" class="form-control" name="Reference' . $i .'" value="' . $WOSNRow['serialno'] . '"/>';
		echo '<input type="hidden" name="OldReference' . $i . '" value="' . $WOSNRow['serialno'] . '"/></td>';
		if ($Serialised==0){
			echo '<td class="has-error"><input type="text" class="form-control" required="required" name="Quantity' . $i .'" value="' . locale_number_format($WOSNRow['quantity'],'Variable') . '" />';
		...
		echo '<td><textarea name="Notes' . $i .'" class="form-control">' . $WOSNRow['qualitytext']  . '</textarea></td>';
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Delete=1&Reference=' . $WOSNRow['serialno'] . '&Quantity=' . locale_number_format($WOSNRow['quantity'],'Variable') . '&WO=' . $WO . '&StockID=' . $StockID . '&Description=' . $Description . '&Serialised=' . $Serialised . '&NextSerialNo=' . $NextSerialNo . '" class="btn btn-danger">' . _('Delete') . '</a></td></tr>';
		$i++;
		$j++;
	}
	...
	echo '<input type="submit" name="UpdateItems" class="btn btn-info" value="' . _('Update') . '" /></td></tr>';
