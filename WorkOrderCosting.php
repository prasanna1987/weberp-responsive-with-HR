...
echo '
	...
	<div class="row" align="left"><a href="'. $RootPath . '/SelectWorkOrder.php" class="btn btn-default">' . _('Back to Work Orders'). '</a></div>
	<br />
	';
...
if (!isset($SelectedWO)) {
	...
	echo '<div class="row" align="center"><a href="' . $RootPath . '/SelectWorkOrder.php" class="btn btn-info">' .
		_('Select a work order') . '</a></div><br />';
	echo   prnMsg(_('This page can only be opened if a work order has been selected.'),'info');
...
while ($WORow = DB_fetch_array($WOItemsResult)){
	 ...
	 echo '<tr>
				<td>' . $WORow['stockid'] . '</td>
	 			...
	 			<td class="number"><a href="'. $RootPath . '/WorkOrderStatus.php?WO=' . $_POST['WO'] . '&amp;StockID=' . $WORow['stockid'] . '" class="btn btn-info">' . _('Status') . '</a></td>
	 			<td class="number"><a href="'. $RootPath . '/WorkOrderReceive.php?WO=' . $_POST['WO'] . '&amp;StockID=' . $WORow['stockid'] . '" class="btn btn-info">' . _('Receive') . '</a></td>
	 			<td class="number"><a href="'. $RootPath . '/WorkOrderIssue.php?WO=' . $_POST['WO'] . '&amp;StockID=' . $WORow['stockid'] . '" class="btn btn-info">' . _('Issue') . '</a></td>
 			</tr>';
...
echo '<tr>
		<td colspan="2" class="number"><strong>' . _('Totals') . '</strong></td>
		<td></td>
...
If (isset($_POST['Close'])) {
	...
	if ($_SESSION['CompanyRecord']['gllink_stock']==1){
		if ($_SESSION['WeightedAverageCosting']==1){
			echo  prnMsg(_('The item cost as calculated from the work order has been applied against the weighted average cost and the necessary GL journals created to update stock as a result of closing this work order'),'success');
		} else {
			echo  prnMsg(_('The work order has been closed and general ledger entries made for the variances on the work order'),'success');
		}
	} else {
		if ($_SESSION['WeightedAverageCosting']==1){
			echo  prnMsg(_('The item costs resulting from the work order have been applied against the weighted average stock value of the items on the work order, and the work order has been closed'),'success');
		} else {
			echo  prnMsg(_('The work order has been closed'),'success');
		}
	}
...
echo 	'<tr>
			<td colspan="9"><label><strong>Comments</strong></label>
					<textarea ' . $ReadOnly . ' class="form-control" name="CloseComments" >' . $_POST['CloseComments'] . '</textarea>
...
if ($WorkOrderRow['closed']==0){
	echo '<tr>
			<td colspan="9" align="center">
					...
					<input type="submit" class="btn btn-info" name="Close" value="' . _('Close This Work Order') . '" onclick="return confirm(\'' . _('Closing the work order takes the variances to the general ledger (if integrated). The work order will no longer be able to have manufactured goods received entered against it or materials issued to it.') . '  ' . _('Are You Sure?') . '\');" />
