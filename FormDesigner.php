...
function InputX($keyName, $keyValue) {
	echo '<td class="number">' . _('x') . ' = ' .
		'</td><td><input class="form-control" maxlength="4" name="' . $keyName . 'x" size="4" title="' .
...
function InputY($keyName, $keyValue) {
	echo '<td class="number">' . _('y') . ' = ' .
		'</td><td><input class="form-control" maxlength="4" name="' . $keyName . 'y" size="4" title="'.
...
function InputWidth($keyName, $keyValue) {
	echo '<td class="number">' . _('Width') . ' = ' .
		'</td><td><input class="form-control" maxlength="4" name="' . $keyName . 'Length" size="4" title="'.
...
function InputHeight($keyName, $keyValue) {
	echo '<td class="number">' . _('Height') . ' = ' .
		'</td><td><input class="form-control" maxlength="4" name="' . $keyName . 'height" size="4" title="'.
...
function SelectAlignment($keyName, $keyValue) {
	...
	echo '<td>' . _('Alignment'). ' = </td><td><select name="' . $keyName . 'Alignment" class="form-control">';
...
function SelectFontSize($keyName, $keyValue) {
	...
	echo '<td>' . _('Font Size'). ' = </td><td><select name="' . $keyName . 'FontSize" class="form-control">';
...
function SelectShowElement($keyName, $keyValue) {
	...
	echo '<td>' . _('Show'). ' = </td><td><select name="' . $keyName . 'Show" class="form-control">';
...
<table id="general-table" class="table table-bordered"><tr>';
	echo '<td>' .  _('Select the form to edit')  . '</td><td><select name="FormName" class="form-control">';
	...
	echo '</select></td></tr></table></div></div></div>';
	echo '<br /><div class="row" align="center"><input tabindex="6" type="submit" class="btn btn-info" name="submit" value="' . _('Edit Form Layout') . '" /></div><br />';
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Form Designer') . '<br />' .  _((string)$FormDesign['name']) . '</h1></a></div>';
...
<div class="col-xs-12">
<div class="table-responsive">
<table id="general-table" class="table table-bordered">'; //Start of outer table
...
echo '<th style="width:33%">' . _('Paper Size') . '<select name="PaperSize" class="form-control">';
...
echo '<th style="width:33%">' . _('Line Height') . '<input type="text" class="form-control" name="LineHeight" size="3" maxlength="3" value="'.$FormDesign->LineHeight.'" /></th></tr><tr>';
$counter=1; // Count how many sub tables are in the row
foreach ($FormDesign as $key) {
	switch ($key['type']) {
		case 'image':
			echo '<td colspan="1" valign="top"><table class="table"><tr><th colspan="4">' . _((string)$key['name']) . '</th>';
			echo '</tr><tr>'; // New table row.
				InputX($key['id'], $key->x);
				InputY($key['id'], $key->y);
			echo '</tr><tr>'; // New table row.
				echo '<td class="number">' . _('Width').' = ' . '</td><td><input type="text" class="form-control" name="'.$key['id'].'width" size="4" maxlength="4" value="'.$key->width.'" /></td>';
//				InputWidth($key['id'], $key->width);// Requires to standardize xml files from "width" to "Width"
			...
			break;
		case 'SimpleText':
			echo '<td colspan="1" valign="top"><table class="table">';
		...
		case 'ElementArray':
			...
			foreach ($key as $subkey) {
				...
				if ($subkey['type']=='SimpleText') {
					...
					echo '<td><input type="text" class="form-control" name="StartLine" size="4" maxlength="4" value="'.$key->y.'" /></td>';
				}
				echo '</tr>';
			}
			echo '</table></td>';
		...
		case 'CurvedRectangle':
			echo '<td colspan="1" valign="top"><table class="table"><tr><th colspan="6">' . _((string)$key['name']) . '</th>';
			echo '</tr><tr>'; // New table row.
				InputX($key['id'], $key->x);
				InputY($key['id'], $key->y);
				echo '<td class="number">' . _('Width').' = ' . '</td><td><input type="text" class="form-control" name="'.$key['id'].'width" size="4" maxlength="4" value="'.$key->width.'" /></td>';
//				InputWidth($key['id'], $key->width);// Requires to standardize xml files from "width" to "Width"
			echo '</tr><tr>'; // New table row.
				InputHeight($key['id'], $key->height);// Requires to standardize xml files from "height" to "Height"
				echo '<td class="number">' . _('Radius') . ' = ' . '</td><td><input class="form-control" maxlength="4" name="' . $key['id'] . 'radius" size="4" title="'. _('Radius of the rounded corners') . '" type="number" value="' . $key->radius . '" /></td>';  // Requires to standardize xml files from "radius" to "Radius" before changing the html name.
...
*/
		...
		case 'Rectangle': // This case is probably included in CurvedRectangle.
			echo '<td colspan="1" valign="top"><table class="table"><tr><th colspan="8">' . _((string)$key['name']) . '</th>';
			echo '</tr><tr>'; // New table row.
				InputX($key['id'], $key->x);
				InputY($key['id'], $key->y);
			echo '</tr><tr>'; // New table row.
...
//				InputWidth($key['id'], $key->width);// Requires to standardize xml files from "width" to "Width"
			...
			break;
		case 'Line':
			echo '<td colspan="1" valign="top"><table class="table"><tr><th colspan="6">' . _((string)$key['name']) . '</th></tr>';
            echo '<tr>';
			echo '<td class="number">' . _('Start x co-ordinate').' = ' . '</td><td><input type="text" class="form-control" name="'.$key['id'].'startx" size="4" maxlength="4" value="'.$key->startx.'" /></td>';
			echo '<td class="number">' . _('Start y co-ordinate').' = ' . '</td><td><input type="text" class="form-control" name="'.$key['id'].'starty" size="4" maxlength="4" value="'.$key->starty.'" /></td></tr><tr>';
			echo '<td class="number">' . _('End x co-ordinate').' = ' . '</td><td><input type="text" class="form-control" name="'.$key['id'].'endx" size="4" maxlength="4" value="'.$key->endx.'" /></td>';
			echo '<td class="number">' . _('End y co-ordinate').' = ' . '</td><td><input type="text" class="form-control" name="'.$key['id'].'endy" size="4" maxlength="4" value="'.$key->endy.'" /></td>';
            echo '</tr>';
			echo '</table></td>';
			$counter=$counter+1;
			break;
...
echo '</tr></table></div></div></div>'; //End of outer table
echo '<br /><div class="row"><div class="col-xs-4"><input tabindex="6" type="submit" class="btn btn-info" name="preview" value="' . _('Preview the Form Layout') . '" /></div>';
echo '<div class="col-xs-4"><input tabindex="7" type="submit" name="save" class="btn btn-info" value="' . _('Save the Form Layout') . '" /></div>';
