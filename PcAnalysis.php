...
function submit($TabToShow) {
	...
	if ($InputError == 0){
		...
		if (DB_num_rows($Result) != 0){
			...
			echo prnMsg('There is no data to analyse');
...
// the page is called.
	...
	include('includes/header.php');
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Excel file for Petty Cash Expenses Analysis') . '
		</h1></a></div>';
...
<input type="submit" name="submit" class="btn btn-success" value="' . _('Export to CSV') . '" /></div>
		</div>
		</div>
		<br />';
	echo '
