...
if (!isset($_GET['WO']) AND !isset($_POST['WO'])) {
	...
	echo  prnMsg(_('Select a Work Order Number to Print before calling this page'), 'error');
	echo '<br />
				<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/SelectWorkOrder.php" class="btn btn-info">' . _('Select Work Order') . '</a></div>
						</div>
	...
	echo prnMsg('This page must be called with a Work order number to print');
	echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> To Menu') . '</a></p><br />';
	exit;
}
if (isset($_GET['WO'])) {
	$SelectedWO = $_GET['WO'];
...
if (isset($SelectedWO) AND $SelectedWO != '' AND $SelectedWO > 0 AND $SelectedWO != 'Preview') {
	...
	if (DB_num_rows($result) == 0) {
		...
		echo  prnMsg(_('Unable to Locate Work Order Number') . ' : ' . $SelectedWO . ' ', 'error');
		echo '<br />
			<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/SelectWorkOrder.php" class="btn btn-info">' . _('Select Work Order') . '</a></div>
				</div>
...
if (isset($MakePDFThenDisplayIt) or isset($MakePDFThenEmailIt)) {
	...
	if ($MakePDFThenDisplayIt) {
		$pdf->OutputD('nERP' . '_WorkOrder_' . $SelectedWO . '_' . date('Y-m-d') . '.pdf');
		$pdf->__destruct();
	} else {
		$PdfFileName = 'nERP' . '_WorkOrder_' . $SelectedWO . '_' . date('Y-m-d') . '.pdf';
		...
		if($_SESSION['SmtpSetting'] == 0){//use the mail service provice by the server.
			...
			echo  prnMsg(_('The SMTP settings are wrong, please ask administrator for help'),'error'),'</div><br />';
			include('includes/footer.php');
			exit;
		}
		...
		if ($Success == 1) {
			...
			include('includes/header.php');
			echo  prnMsg(_('Work Order') . ' ' . $SelectedWO . ' ' . _('has been emailed to') . ' ' . $_POST['EmailTo'] . ' ' . _('as directed'), 'success');

		} else { //email failed
			$Title = _('Email a Work Order');
			include('includes/header.php');
			echo  prnMsg(_('Emailing Work order') . ' ' . $SelectedWO . ' ' . _('to') . ' ' . $_POST['EmailTo'] . ' ' . _('failed'), 'error');
		}
	}
	include('includes/footer.php');
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Print Labels') . '</label>
<select name="PrintLabels" class="form-control">';
	if ($PrintLabels=="Yes") {
	...
	if ($_POST['PrintOrEmail'] == 'Email') {
		...
		if (DB_num_rows($ContactsResult) > 0) {
			echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Email to') . '</label>
<input name="EmailTo" class="form-control" value="';
	...
	echo '
         <div class="row">
		 <div class="col-xs-4">
              <input type="submit" class="btn btn-info" name="DoIt" value="' . _('Paperwork') . '" />
         </div>
         </div><br />
         </form>';
	...
	if ($PrintLabels=="Yes") {
		...
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Label Item') . '</label>
					<input name="LabelItem" type="text" class="form-control" value="' .$LabelItem.'"/></div>
				</div>
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Label Description') . '</label>
					<input name="LabelDesc" type="text" class="form-control" value="' .$LabelDesc.'"/></div>
				</div>
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Label Lot') . '</label>
					<input name="LabelLot" type="text" class="form-control" value="' .$LabelLot.'"/></div>
				</div>
				</div>
				
				<div class="row">
					<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('No of Full Packages') . '</label>
					<input name="NoOfBoxes" class="form-control" value="' .$NoOfBoxes.'"/></div>
				</div>
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Labels/Package') . '</label>
					<input name="LabelsPerBox" class="form-control" value="' .$LabelsPerBox.'"/></div>
				</div>
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Weight/Package') . '</label>
					<input name="QtyPerBox" class="form-control" value="' .$QtyPerBox. '"/></div>
				</div>
				</div>
				
				<div class="row">
					<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('LeftOver Qty') . '</label>
					<input name="LeftOverQty" class="form-control" value="' .$LeftOverQty.'"/></div>
				</div>
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Print or Email the Order') . '</label>
					<select name="PrintOrEmail" class="form-control">';
...
<input name="EmailTo" class="form-control" value="';
		...
		echo '
			<div class="row">
			<div class="col-xs-4">
				<input type="submit" class="btn btn-info" name="DoIt" value="' . _('Labels') . '" />
			</div>
			</div><br />
			</form>';
	}
