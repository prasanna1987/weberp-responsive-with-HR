	...
	foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
		if ($POLine->Deleted == false) {
			if (!is_numeric(filter_number_format($_POST['ConversionFactor'.$POLine->LineNo]))){
				echo prnMsg(_('The conversion factor is expected to be numeric - the figure which converts from our units to the supplier units. e.g. if the supplier units is a tonne and our unit is a kilogram then the conversion factor that converts our unit to the suppliers unit is 1000'),'error');
			...
			if (!is_numeric(filter_number_format($_POST['SuppQty'.$POLine->LineNo]))){
				echo prnMsg(_('The quantity in the supplier units is expected to be numeric. Please re-enter as a number'),'error');
			...
			if (!is_numeric(filter_number_format($_POST['SuppPrice'.$POLine->LineNo]))){
				echo prnMsg(_('The supplier price is expected to be numeric. Please re-enter as a number'),'error');
			} else { //ok to update the PO object variables
 ...
 */
	...
	if ($_SESSION['PO'.$identifier]->DelAdd1=='' or mb_strlen($_SESSION['PO'.$identifier]->DelAdd1)<3){
		echo prnMsg( _('The purchase order cannot be committed to the database because there is no delivery street address specified'),'error');
		$InputError=1;
	} elseif ($_SESSION['PO'.$identifier]->Location=='' or ! isset($_SESSION['PO'.$identifier]->Location)){
		echo prnMsg( _('The purchase order can not be committed to the database because there is no location specified to book any stock items into'),'error');
		$InputError=1;
	} elseif ($_SESSION['PO'.$identifier]->LinesOnOrder <=0){
		echo prnMsg( _('The purchase order can not be committed to the database because there are no lines entered on this order'),'error');
		$InputError=1;
	}
	...
	if ($InputError!=1){
		...
		if ($_SESSION['AutoAuthorisePO']==1) {
			...
			if (DB_num_rows($AuthResult) > 0 AND $AuthRow['authlevel'] > $_SESSION['PO'.$identifier]->Order_Value()) { //user has authority to authrorise as well as create the order
				...
				echo prnMsg( _('You do not have permission to authorise this purchase order').'.<br />' .  _('This order is for').' '.
		...
		if ($_SESSION['ExistingOrder']==0){ /*its a new order to be inserted */
			...
			echo prnMsg(_('Purchase Order') . ' ' . $_SESSION['PO'.$identifier]->OrderNo . ' ' . _('on') . ' ' . $_SESSION['PO'.$identifier]->SupplierName . ' ' . _('has been created'),'success');
				...
				OR $_SESSION['PO'.$identifier]->Status=='Printed')){
			      ...
			      echo '<br /><div class="row"><div class="col-xs-4"><a target="_blank" href="'.$RootPath.'/PO_PDFPurchOrder.php?OrderNo=' . $_SESSION['PO'.$identifier]->OrderNo . '" class="btn btn-warning">' . _('Print') . '</a></div></div>';
			}
			...
			echo '<br /><br />';
			echo prnMsg(_('Purchase Order') . ' ' . $_SESSION['PO'.$identifier]->OrderNo . ' ' . _('has been updated'),'success');
			if ($_SESSION['PO'.$identifier]->AllowPrintPO==1
					AND ($_SESSION['PO'.$identifier]->Status=='Authorised'
					OR $_SESSION['PO'.$identifier]->Status=='Printed')){
		...
		if ($_SESSION['PO'.$identifier]->Status == 'Authorised'
                   AND in_array($_SESSION['PageSecurityArray']['GoodsReceived.php'], $_SESSION['AllowedPageSecurityTokens'])){
                	...
                	echo '<br /><div class="row"><div class="col-xs-4"><a href="SupplierInvoice.php?SupplierID=' . $_SESSION['PO'.$identifier]->SupplierID . '&amp;ReceivePO=' . $_SESSION['PO'.$identifier]->OrderNo . '&amp;DeliveryDate=' . $_SESSION['PO'.$identifier]->DeliveryDate . '" class="btn btn-info">' . _('Receive and Enter Purchase Invoice') . '</a></div></div><br />';
		}
...
if(isset($_GET['Delete'])){
	if($_SESSION['PO'.$identifier]->Some_Already_Received($_GET['Delete'])==0){
		...
		echo prnMsg( _('This item cannot be deleted because some of it has already been received'),'warn');
	}
}
...
if (isset($_POST['EnterLine'])){ /*Inputs from the form directly without selecting a stock item from the search */
	...
	if (!is_numeric(filter_number_format($_POST['Qty']))){
		$AllowUpdate = false;
		echo prnMsg( _('Cannot Enter this order line') . '<br />' . _('The quantity of the order item must be numeric'),'error');
	}
	if (filter_number_format($_POST['Qty'])<0){
		$AllowUpdate = false;
		echo prnMsg( _('Cannot Enter this order line') . '<br />' . _('The quantity of the ordered item entered must be a positive amount'),'error');
	}
	if (!is_numeric(filter_number_format($_POST['Price']))){
		$AllowUpdate = false;
		echo prnMsg( _('Cannot Enter this order line') . '<br />' . _('The price entered must be numeric'),'error');
	}
	if (!Is_Date($_POST['ReqDelDate'])){
		$AllowUpdate = False;
		echo prnMsg( _('Cannot Enter this order line') . '</strong><br />' . _('The date entered must be in the format') . ' ' . $_SESSION['DefaultDateFormat'], 'error');
	}
  ...
  * there will be an sql error  in SupplierInvoice.php without a valid GL Code
	*/
	if ($_SESSION['PO'.$identifier]->GLLink==1 OR $_SESSION['CompanyRecord']['gllink_creditors']==1){
		...
		if (DB_error_no() !=0) {
			$AllowUpdate = false;
			echo prnMsg( _('The validation process for the GL Code entered could not be executed because') . ' ' . DB_error_msg(), 'error');
			if ($debug==1){
				echo prnMsg (_('The SQL used to validate the code entered was') . ' ' . $sql,'error');
			}
			include('includes/footer.php');
			exit;
		}
		if (DB_num_rows($GLValidResult) == 0) { /*The GLCode entered does not exist */
			$AllowUpdate = false;
			echo prnMsg( _('Cannot enter this order line') . ':<br />' . _('The general ledger code') . ' - ' . $_POST['GLCode'] . ' ' . _('is not a general ledger code that is defined in the chart of accounts') . ' . ' . _('Please use a code that is already defined') . '. ' . _('See the Chart list from the link below'),'error');
		} else {
	...
	if ($_POST['AssetID'] !='Not an Asset'){
		...
		if (DB_num_rows($ValidAssetResult)==0){ // then the asset id entered doesn't exist
			$AllowUpdate = false;
			echo prnMsg(_('An asset code was entered but it does not yet exist. Only pre-existing asset ids can be entered when ordering a fixed asset'),'error');
		} else {
	...
	if (mb_strlen($_POST['ItemDescription'])<=3){
		$AllowUpdate = false;
		echo prnMsg(_('Cannot enter this order line') . ':<br />' . _('The description of the item being purchased is required where a non-stock item is being ordered'),'warn');
	}
				...
				if (count($_SESSION['PO'.$identifier]->LineItems)!=0){
					...
					/* do a loop round the items on the order to see that the item is not already on this order */
						if (($OrderItem->StockID == $ItemCode) AND ($OrderItem->Deleted==false)) {
							...
							echo prnMsg( _('The item') . ' ' . $ItemCode . ' ' . _('is already on this order') . '. ' . _('The system will not allow the same item on the order more than once') . '. ' . _('However you can change the quantity ordered of the existing line if necessary'),'error');
						}
				...
				if (DB_num_rows($ItemResult)==1){
					...
					if (DB_num_rows($PurchDataResult)>0){ //the purchasing data is set up
						...
						if ($ItemDiscountPercent != 0) {
							echo prnMsg(_('Taken accumulated supplier percentage discounts of') .  ' ' . locale_number_format($ItemDiscountPercent*100,2) . '%','info');
						}
						if ($ItemDiscountAmount != 0 ){
							echo prnMsg(_('Taken accumulated round sum supplier discount of') .  ' ' . $_SESSION['PO'.$identifier]->CurrCode . ' ' . locale_number_format($ItemDiscountAmount,$_SESSION['PO'.$identifier]->CurrDecimalPlaces) . ' (' . _('per supplier unit') . ')','info');
						}
					...
					echo prnMsg (_('The item code') . ' ' . $ItemCode . ' ' . _('does not exist in the database and therefore cannot be added to the order'),'error');
					if ($debug==1){
						echo '<br />' . $sql;
					}
					include('includes/footer.php');
...
if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_GET['Edit'])){
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>  '.$_SESSION['PO'.$identifier]->SupplierName;
echo '</h1></a></div>';
	if (isset($_SESSION['PO'.$identifier]->OrderNo)) {
		echo  '<div class="row"><div class="col-xs-10"><h3><strong> ' . _('Purchase Order Number:') .' '. $_SESSION['PO'.$identifier]->OrderNo.'</strong></h3></div>' ;
		echo '<div class="col-xs-2"><br /><a href="'.$RootPath.'/PO_Header.php?identifier=' . $identifier. '" class="btn btn-info">' ._('Back to PO Header') . '</a></div></div><br />';
	}
	...
	foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
		...
		if ($POLine->Deleted==False) {
			...
			echo '<tr class="striped_row">
                ...
                <td><input type="text" class="form-control" name="ItemDescription' . $POLine->LineNo.'" size="30" value="' . stripslashes($POLine->ItemDescription) . '" /></td>
				<td class="number">' . locale_number_format($POLine->Quantity,$POLine->DecimalPlaces) . '</td>
				<td>' . $POLine->Units . '</td>
				<td class="number">' . $DisplayPrice . '</td>
				<td><input type="text" class="form-control" name="ConversionFactor' . $POLine->LineNo .'" size="8" value="' . locale_number_format($POLine->ConversionFactor,'Variable') . '" /></td>
				<td><input type="text" class="form-control" name="SuppQty' . $POLine->LineNo .'" size="10" value="' . locale_number_format(round($POLine->Quantity/$POLine->ConversionFactor,$POLine->DecimalPlaces),$POLine->DecimalPlaces) . '" /></td>
				<td>' . $POLine->SuppliersUnit . '</td>
				<td><input type="text" class="form-control" name="SuppPrice' . $POLine->LineNo . '" size="10" value="' . $SuppPrice .'" /></td>
				<td class="number">' . $DisplayLineTotal . '</td>
				<td><input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="ReqDelDate' . $POLine->LineNo.'" size="10" value="' .$POLine->ReqDelDate .'" /></td>';
			if ($POLine->QtyReceived !=0 AND $POLine->Completed!=1){
				echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier .'&amp;Complete=' . $POLine->LineNo . '" class="btn btn-info">' . _('Complete') . '</a></td>';
			} elseif ($POLine->QtyReceived ==0) {
				echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier .'&amp;Delete=' . $POLine->LineNo . '" class="btn btn-danger">' . _('Delete'). '</a></td>';
			}
			echo '</tr>';
	...
	echo '</tbody>
			...
			<tr>
				<td colspan="9" class="number">' . _('TOTAL') . _(' excluding Tax') . '</td>
						<td class="number"><strong>' . $DisplayTotal . '</strong></td>
			</tr>
		...
		<br />
			<div class="row">
			<div class="col-xs-4">
			<input type="submit" class="btn btn-info" name="UpdateLines" value="' . _('Update Order Lines') . '" /></div>
			<div class="col-xs-4">
			<input type="submit" class="btn btn-success" name="Commit" value="' . _('Process Order') . '" />
			</div></div><br />
';
...
if (isset($_POST['NonStockOrder'])) {
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Item Description') . '</label>';
	echo '<input type="text" class="form-control" name="ItemDescription" size="40" /></div></div>';
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('General Ledger Code') . '</label>
			<select name="GLCode" class="form-control">';
	...
	echo '</select></div></div>';
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('OR Asset ID'). '</label>
	...
	echo'</select><a href="FixedAssetItems.php" class="btn btn-info" target=_blank>' .  _('New Fixed Asset') . '</a></div></div>
		</div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Quantity to purchase') . '</label>
			<input type="text" class="form-control" name="Qty" size="10" value="1" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Price per item') . '</label>
			<input type="text" class="form-control" name="Price" size="10" /></div>
		</div>
		
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Unit') . '</label>
			<input type="text" name="SuppliersUnit" class="form-control" size="10" value="' . _('each') . '" /></div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Delivery Date') . '</label>
			<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="ReqDelDate" size="11" value="'.$_SESSION['PO'.$identifier]->DeliveryDate .'" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"><br />
			<input type="submit" class="btn btn-success" name="EnterLine" value="' . _('Enter Item') . '" />
		</div></div></div><br />
';
}
...
<div class="block-title"><h3>' .  _('Search For Stock Items') . '</h3></div>';

	echo '
			<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">
' . _('Item Category') . '</label> <select name="StockCat" class="form-control">
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Description-part or full') . '</label>
		<input type="text" class="form-control" class="form-control" name="Keywords" size="20" maxlength="25" value="' . $_POST['Keywords'] . '" /></div></div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Show Only Items Defined for this Supplier?') . ' </label><input type="checkbox" ' . $Checked . ' name="SupplierItemsOnly" /></div></div></div>
<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Stock ID-part or full') . '</label>
		<input type="text" name="StockCode" class="form-control" size="15" maxlength="18" value="' . $_POST['StockCode'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label"></label><br />
<a target="_blank" class="btn btn-info" href="'.$RootPath.'/Stocks.php">' . _('Insert New Item') . '</a>
<input type="submit" name="NonStockOrder" class="btn btn-info" value="' . _('Order a non stock item') . '" />
</div></div>
		</div>
		<div class="row" align="center">
		
		<input type="submit" name="Search" class="btn btn-success" value="' . _('Search') . '" /></div><br />
		</div>';
...
if (isset($SearchResult)) {
	$PageBar = '<br /><br /><div class="row"><input type="hidden" name="currpage" value="'.$Offset.'">';
	if($Offset>0)
		$PageBar .= '<div class="col-xs-5"><input type="submit" class="btn btn-default" name="Prev" value="'._('Prev').'" /></div>';
	else
		$PageBar .= '<div class="col-xs-5"><input type="submit" class="btn btn-default" name="Prev" value="'._('Prev').'" disabled="disabled"/></div>';
	$PageBar .= '<div class="col-xs-6"><input type="submit" class="btn btn-success" value="'._('Add To Order').'" name="NewItem"/></div>';
	if($Offset<$ListPageMax)
		$PageBar .= '<div class="col-xs-0"><input type="submit" class="btn btn-default" name="Next" value="'._('Next').'" /></div>';
	else
		$PageBar .= '<div class="col-xs-0"><input type="submit" class="btn btn-default" name="Next" value="'._('Next').'" disabled="disabled"/>';
	$PageBar .= '</div> </div><br />';
...
';
	...
	while ($myrow=DB_fetch_array($SearchResult)) {
		...
		echo '<tr class="striped_row">
			...
			<td>' . $OrderUnits . '</td>
			<td>' . $ImageSource . '</td>
			<td><input class="form-control" type="text" size="6" value="0" name="NewQty' . $j . '" /></td>
