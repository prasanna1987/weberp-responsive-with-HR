...
if (isset($_POST['PrintPDF'])) {
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
	   echo prnMsg( _('The Reorder Level report could not be retrieved by the querry because') . ' '  . DB_error_msg(),'error');
...
/*Print out the grand totals */
	...
	if ($ListCount == 0){
			...
			include('includes/header.php');
			echo prnMsg(_('There were no items with demand greater than supply'),'error');
			echo '<br /><p align="right"><a href="' . $RootPath . '/index.php" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
	...
	include('includes/header.php');
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Inventory Reorder Level Report') . '<br /><small>';
	echo '' . '</small></h1></a></div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Stock Location') . '</label>
	...
	if (DB_num_rows($result1)==0){
		...
		echo '<p><a href="' . $RootPath . '/StockCategories.php" class="btn btn-info">' . _('Define Stock Categories') . '</a></p>';
		include ('includes/footer.php');
		exit;
	}
