...
if (!isset($Item) OR !isset($_SESSION['CustomerID']) OR $_SESSION['CustomerID']==''){
	...
	echo  prnMsg( _('A customer must be selected from the customer selection screen') . ', '
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . _('Special Customer Prices') . '</h1></a></div>';
echo '<p class="text-info"><strong>' . htmlspecialchars($myrow['name'], ENT_QUOTES, 'UTF-8') . ' ' . _('in') . ' ' . $myrow['currcode'] . '<br />' . ' ' . _('for') . ' ';
...
if (DB_num_rows($result)==0){
	echo  prnMsg( _('The part code entered does not exist in the system') . '. ' . _('Only valid parts can have prices entered against them'),'error');
	$InputError=1;
}
if ($myrow[1]=='K'){
...
//the link to delete a selected record was clicked instead of the submit button
	...
	$result = DB_query($sql);
	echo  prnMsg( _('This price has been deleted') . '!','success');
}
...
if (DB_num_rows($result) == 0) {
	echo '<tr><td>' . _('There are no default prices set up for this item') . '</td></tr>';
} else {
	echo '<thead><tr><th>' . _('Normal Price') . '</th></tr></thead>';
...
echo '</table></div></td><td valign="top">';
...
if (DB_num_rows($result) == 0) {
	echo '<tr><td>' . _('There are no special prices set up for this item') . '</td></tr>';
} else {
	...
	echo '<tr>
			...
			<td>' . $EndDateDisplay . '</td>
	 		<td><a href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8').'?Item='.$Item.'&amp;Price='.$myrow['price'].'&amp;Branch='.$myrow['branchcode'].
				'&amp;StartDate='.$myrow['startdate'].'&amp;EndDate='.$myrow['enddate'].'&amp;Edit=1" class="btn btn-info">' . _('Edit') . '</a></td>
			<td><a href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8').'?Item='.$Item.'&amp;Branch='.$myrow['branchcode'].'&amp;StartDate='.$myrow['startdate'] .'&amp;EndDate='.$myrow['enddate'].'&amp;delete=yes" onclick="return confirm(\'' . _('Are you sure you wish to delete this price?') . '\');" class="btn btn-danger">' . _('Delete') . '</a></td>
		</tr>';
...
echo '</table></div></td></tr></table></div></div></div><br />';
...
echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Branch') . '</label>
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Start Date') . '</label>
		<input type="text" name="StartDate" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" size="11" maxlength="10" value="' . $_POST['StartDate'] . '" /></div>
	</div>';
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('End Date') . '</label>
		<input type="text" name="EndDate" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" size="11" maxlength="10" value="' . $_POST['EndDate'] . '" /></div></div></div>';

echo '<div class="row"><div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Price') . '</label>
          <input type="text" class="form-control" name="Price" size="11" maxlength="10" value="' . locale_number_format($_POST['Price'],2) . '" /></div>
		</div>
	';
