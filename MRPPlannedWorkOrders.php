...
if ( isset($_POST['PrintPDF']) OR isset($_POST['Review']) ) {
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
	   echo prnMsg( _('The MRP planned work orders could not be retrieved by nERP because') . ' '  . DB_error_msg(),'error');
	  echo '<br /><p align="center"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('Menu') . '</a></p><br />';
	   if ($debug==1){
		  echo '<br />' . $sql;
	   }
	   include('includes/footer.php');
	...
	if (DB_num_rows($result)==0){ //then there is nothing to print
		...
		include('includes/header.php');
		echo prnMsg(_('There were no items with demand greater than supply'),'info');
		echo '<br /><p align="center"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('Menu') . '</a></p><br />';
		include('includes/footer.php');
		exit;
	}
	...
	if (isset($_POST['PrintPDF'])) { // Print planned work orders
		...
		$pdf->OutputD($_SESSION['DatabaseName'] . '_MRP_Planned_Work_Orders_' . Date('d-m-Y') . '.pdf');
		$pdf->__destruct();
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h5>
<div class="table-responsive">' . _('Consolidation') . ': ' . $_POST['Consolidation'] . '&nbsp;&nbsp;&nbsp;&nbsp;'
						  . _('Cutoff Date') . ': ' . $_POST['cutoffdate'] . '</h5></div>
<table id="general-table" class="table table-bordered">
		...
		while ($myrow = DB_fetch_array($result)){
			...
			echo '<tr class="striped_row">
				<td><a href="' . $RootPath . '/WorkOrderEntry.php?NewItem=' . urlencode($myrow['part']) . '&amp;ReqQty=' . urlencode($myrow['supplyquantity']) . '&amp;ReqDate=' . urlencode($myrow['duedate']) . '&amp;StartDate=' . urlencode($myrow['mrpdate']) . '" class="btn btn-success">' . _('Convert') . '</a></td>
		...
		// Print out the grand totals
		echo '<tr>
				<td colspan="3">' . _('<strong>Total Work Orders</strong>') .': ' . ($j-1) . '</td>
		...
		echo '<p align="right"><a class="noprint btn btn-default" href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '">', _('Back'), '</a></p>';
		include('includes/footer.php');
...
<div class="col-xs-12">';
	echo '
		<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">
			...
			<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Consolidation') . '</label>
				
					<select required="required" class="form-control" name="Consolidation">
...
<div class="form-group"> 
                  <input type="submit" class="btn btn-info" name="Review" value="' . _('Review') . '" /></div></div>
				  <div class="col-xs-4">
<div class="form-group">  <input type="submit" class="btn btn-warning" name="PrintPDF" value="' . _('Print PDF') . '" />
             </div>
             </div>
			 </div>
          </form></div></div>';
