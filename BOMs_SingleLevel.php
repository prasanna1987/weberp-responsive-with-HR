...
function DisplayBOMItems($UltimateParent, $Parent, $Component,$Level) {
		...
		while ($myrow=DB_fetch_array($result)) {
			...
			if( $myrow['mbflag']=='B'
				...
				$DrillText = '<a href="%s&amp;Select=%s" class="btn btn-info">' . _('Drill Down') . '</a>';
...
if (isset($Select)) { //Parent Stock Item selected so display BOM or edit Component
	...
	if (isset($SelectedParent) AND isset($_POST['Submit'])) {
		...
		if (!Is_Date($_POST['EffectiveAfter'])) {
			$InputError = 1;
			echo prnMsg(_('The effective after date field must be a date in the format') . ' ' .$_SESSION['DefaultDateFormat'],'error');
		...
		if (filter_number_format($_POST['Quantity'])==0) {
			$InputError = 1;
			echo prnMsg(_('The quantity entered cannot be zero'),'error');
			$Errors[$i] = 'Quantity';
			$i++;
		}
		if(!Date1GreaterThanDate2($_POST['EffectiveTo'], $_POST['EffectiveAfter'])){
			$InputError = 1;
			echo prnMsg(_('The effective to date must be a date after the effective after date') . '<br />' . _('The effective to date is') . ' ' . DateDiff($_POST['EffectiveTo'], $_POST['EffectiveAfter'], 'd') . ' ' . _('days before the effective after date') . '! ' . _('No updates have been performed') . '.<br />' . _('Effective after was') . ': ' . $_POST['EffectiveAfter'] . ' ' . _('and effective to was') . ': ' . $_POST['EffectiveTo'],'error');
			$Errors[$i] = 'EffectiveAfter';
			$i++;
			$Errors[$i] = 'EffectiveTo';
			$i++;
		...
		if($_POST['AutoIssue']==1 AND isset($_POST['Component'])){
			...
			if ($CheckControlledRow[0]==1){
				echo  prnMsg(_('Only non-serialised or non-lot controlled items can be set to auto issue. These items require the lot/serial numbers of items issued to the works orders to be specified so autoissue is not an option. Auto issue has been automatically set to off for this component'),'warn');
				$_POST['AutoIssue']=0;
			}
		}
		...
		//need to check not recursive BOM component of itself!
			...
			if (!CheckForRecursiveBOM ($SelectedParent, $_POST['Component'])) {
				...
				/*The component must already be on the BOM */
					...
					echo prnMsg( _('The component') . ' ' . $_POST['Component'] . ' ' . _('is already recorded as a component of') . ' ' . $SelectedParent . '.' . '<br />' . _('Whilst the quantity of the component required can be modified it is inappropriate for a component to appear more than once in a bill of material'),'error');
	...
	echo '<br /><div class="row"><div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Select a Different BOM') . '</a></div></div><br />';
...
<table id="general-table" class="table table-bordered">';
	 echo '<tr><td>' . _('Manufactured parent items').' : ';
	 while ($myrow = DB_fetch_array($result)){
...
<div class="table-responsive">
<table id="general-table" class="table table-bordered">';
		echo '<tr><td>' . _('Assembly parent items').' : ';
	 	$ix = 0;
	 	while ($myrow = DB_fetch_array($result)){
	 	   echo (($ix)?', ':'') . '<a href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Select='.$myrow['parent'].'">' .
			$myrow['description'].'&nbsp;('.$myrow['parent'].')</a>';
...
<div class="block-title"><h3>' . $SelectedParent .' - ' . $myrow[0] . ' ('. $MBdesc. ') </h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">';
    ...
    /* We do want to show the new component entry form in any case - it is a lot of work to get back to it otherwise if we need to add */
		...
		//editing a selected component from the link to the line item
			...
			echo '<div class="block-title"><h3>' .  ('Edit Component Details')  . '</h3></div>
					';
			echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Component') . '</label>
					' . $SelectedComponent . '
			...
			*/
				echo '<div class="block">';
			echo '<div class="block-title"><h3>' . _('New Component Details')  . '</h3></div>
				';
			echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Component code') . '</label>
					';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Sequence in BOM') . '</label>
                <input type="text" class="integer" required="required" size="5" name="Sequence" value="' . $_POST['Sequence'] . '" /></div>
            </div>';

		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Location') . '</label>
				<select tabindex="2" name="LocCode">';
		...
		echo '</select></div>
			...
			<div class="row">
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Work Centre Added') . '</label>';
		...
		if (DB_num_rows($result)==0){
			...
			echo '<a href="' . $RootPath . '/WorkCentres.php" class="btn btn-info">' . _('Work Centre Maintenance') . '</a></div></div></div>';
		...
		echo '</select></div>
				...
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Quantity') . '</label>
					<input ' . (in_array('Quantity',$Errors) ?  'class="inputerror"' : '' ) .' tabindex="4" type="text" class="number" required="required" name="Quantity" size="10" maxlength="8" title="' . _('Enter the quantity of this item required for the parent item') . '" value="';
		...
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Effective After') . ' (' . $_SESSION['DefaultDateFormat'] . ')</label>
				<input ' . (in_array('EffectiveAfter',$Errors) ?  'class="inputerror"' : '' ) . ' tabindex="5" type="text" required="required" name="EffectiveAfter" class="date" size="11" maxlength="10" value="' . $_POST['EffectiveAfter'] .'" /></div>
			</div>
			</div>
			
			<div class="row">
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Effective To') . ' (' . $_SESSION['DefaultDateFormat'] . ')</label>
				<input  ' . (in_array('EffectiveTo',$Errors) ?  'class="inputerror"' : '' ) . ' tabindex="6" type="text" name="EffectiveTo" class="date" size="11" maxlength="10" value="' . $_POST['EffectiveTo'] .'" /></div>
			</div>';

		if ($ParentMBflag=='M' OR $ParentMBflag=='G'){
			echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Auto Issue this Component to Work Orders') . '</label>
		...
		echo '</div>
			...
			<div class="row">
			<div class="col-xs-4">
				<input tabindex="8" class="btn btn-info" type="submit" name="Submit" value="' . _('Enter Information') . '" />
			</div>
            </div>
			</form>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Enter extract of the') . ' ' . _('Stock Code') . '</label>
	...
	<input tabindex="3" type="submit" class="btn btn-info" name="Search" value="' . _('Search Now') . '" /></div></div>';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
