...
<div class="col-xs-4">
<a href="' . $RootPath . '/StockCategories.php" class="btn btn-info">' . _('Define Stock Categories') . '</a></div></div>';
...
/*	echo '<p class="page_title_text"><img alt="" src="', $RootPath, '/css/', $Theme,
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>  ', // Icon title.
		_('Inventory Item'), ': ', $StockID, ' <br /><small> ', $myrow['description'], ' ', $ItemStatus, '</small></h1></a></div>';// Page title.
...
<table class="table">
		<tr>
			...
			<div class="table-responsive">
<table id="general-table" class="table table-bordered">'; //nested table
	...
	echo '</table></div>'; //end of first nested table
...
<table id="general-table" class="table table-bordered">';
	...
	echo '</table></div></td>'; //end of Item Category Property mod
	echo '<td style="width:15%; vertical-align:top">
				<div class="table-responsive">
<table id="general-table" class="table table-bordered">'; //nested table to show QOH/orders
	$QOH = 0;
	switch ($myrow['mbflag']) {
		case 'A':
		case 'D':
	...
	echo '<tr>
		...
		<tr>
			<th class="number" style="width:15%">' . _('Quantity On Demand') . ':</th>
...
<table id="general-table" class="table table-bordered">
		...
		while ($SuppRow = DB_fetch_array($SuppResult)) {
			...
			echo '<td class="select"><a href="' . $RootPath . '/PO_Header.php?NewOrder=Yes&amp;SelectedSupplier=' .
				$SuppRow['supplierid'] . '&amp;StockID=' . urlencode($StockID) . '&amp;Quantity='.$SuppRow['minorderqty'].'&amp;LeadTime='.$SuppRow['leadtime'] . '" class="btn btn-info">' . _('Order') . ' </a></td>';
			echo '</tr>';
		}
		echo '</table></div>';
	...
	echo '</td>
		...
		</table></div></div></div><br />',// End first item details table
		...
		$TableHead,
			...
			<div class="col-md-6">
		<div class="block"><div class="block-title"><h3>'. ('Operations'). '</h3></div>';
	/* Stock Transactions */
	echo '<ul class="list-unstyled">';
	if ($Its_A_Kitset_Assembly_Or_Dummy == false) {
		echo '<li><a href="' . $RootPath . '/StockAdjustments.php?StockID=' . urlencode($StockID) . '">' . _('Quantity Adjustments') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/StockTransfers.php?StockID=' . urlencode($StockID) . '&amp;NewTransfer=true">' . _('Location Transfers') . '</a></li>';
		...
		if (($myrow['mbflag'] == 'B')
			...
			while ($SuppRow = DB_fetch_array($SuppResult)) {
				if ($LastSupplierShown != $SuppRow['supplierid']){
					...
					echo '<li><a href="' . $RootPath . '/PO_Header.php?NewOrder=Yes' . '&amp;SelectedSupplier=' . $SuppRow['supplierid'] . '&amp;StockID=' . urlencode($StockID) . '&amp;Quantity='.$EOQ.'&amp;LeadTime='.$SuppRow['leadtime'].'">' .  _('Purchase from:') . ' ' . $SuppRow['suppname'] . '</a></li>
					';
	...
	echo '<li><a href="' . $RootPath . '/Stocks.php?">' . _('Add New Item') . '</a></li>';
	echo '<li><a href="' . $RootPath . '/Stocks.php?StockID=' . urlencode($StockID) . '">' . _('Modify Details') . '</a></li>';
	if ($Its_A_Kitset_Assembly_Or_Dummy == False) {
		echo '<li><a href="' . $RootPath . '/StockReorderLevel.php?StockID=' . urlencode($StockID) . '">' . _('Reorder Levels') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/StockCostUpdate.php?StockID=' . urlencode($StockID) . '">' . _('Standard Costings') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/PurchData.php?StockID=' . urlencode($StockID) . '">' . _('Purchasing Data') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/CustItem.php?StockID=' . urlencode($StockID) . '">' . _('Customer Item Data') . '</a></li>';
	}
	if ($Its_A_Labour_Item == True) {
		echo '<li><a href="' . $RootPath . '/StockCostUpdate.php?StockID=' . urlencode($StockID) . '">' . _('Standard Costings') . '</a></li>';
	}
	if (!$Its_A_Kitset) {
		echo '<li><a href="' . $RootPath . '/Prices.php?Item=' . urlencode($StockID) . '">' . _('Pricing') . '</a></li>';
		if (isset($_SESSION['CustomerID'])
			AND $_SESSION['CustomerID'] != ''
			AND mb_strlen($_SESSION['CustomerID']) > 0) {
			echo '<li><a href="' . $RootPath . '/Prices_Customer.php?Item=' . urlencode($StockID) . '">' . _('Special Prices for customer:') . ' - ' . $_SESSION['CustomerID'] . '</a></li>';
		}
		echo '<li><a href="' . $RootPath . '/DiscountCategories.php?StockID=' . urlencode($StockID) . '">' . _('Discount Category') . '</a></li>';
	    echo '<li><a href="' . $RootPath . '/StockClone.php?OldStockID=' . urlencode($StockID) . '">' . _('Clone This Item') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/RelatedItemsUpdate.php?Item=' . urlencode($StockID) . '">' . _('Related Items') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/PriceMatrix.php?StockID=' . urlencode($StockID) . '">' . _('Price Matrix') . '</a></li>';
	}
	
	//show the item image if it has been uploaded
		$SupportedImgExt = array('png','jpg','jpeg');
			$imagefile = reset((glob($_SESSION['part_pics_dir'] . '/' . urlencode($StockID) . '.{' . implode(",", $SupportedImgExt) . '}', GLOB_BRACE)));
		...
		echo '<li>' . $StockImgLink . '</li>';
	echo '</ul></div></div>';
			 
			 
		echo '<div class="col-md-6">
		<div class="block">
		<div class="block-title"><h3>' .('Inquiries and Reports') . '</h3></div>';
	/*Stock Inquiry Options */
	echo '<ul class="list-unstyled">
	<li><a href="' . $RootPath . '/StockMovements.php?StockID=' . urlencode($StockID) . '">' . _('Stock Movements') . '</a></li>';
	if ($Its_A_Kitset_Assembly_Or_Dummy == False) {
		echo '<li><a href="' . $RootPath . '/StockStatus.php?StockID=' . urlencode($StockID) . '">' . _('Stock Status') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/StockUsage.php?StockID=' . urlencode($StockID) . '">' . _('Stock Usage') . '</a></li>';
	}
	echo '<li><a href="' . $RootPath . '/SelectSalesOrder.php?SelectedStockItem=' . urlencode($StockID) . '">' . _('Outstanding Sales Orders') . '</a></li>';
	echo '<li><a href="' . $RootPath . '/SelectCompletedOrder.php?SelectedStockItem=' . urlencode($StockID) . '">' . _('Completed Sales Orders') . '</a></li>';
	if ($Its_A_Kitset_Assembly_Or_Dummy == False) {
		echo '<li><a href="' . $RootPath . '/PO_SelectOSPurchOrder.php?SelectedStockItem=' . urlencode($StockID) . '">' . _('Outstanding Purchase Orders') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/PO_SelectPurchOrder.php?SelectedStockItem=' . urlencode($StockID) . '">' . _('All Purchase Orders') . '</a></li>';

		$SupportedImgExt = array('png','jpg','jpeg');
		$imagefile = reset((glob($_SESSION['part_pics_dir'] . '/' . $StockID . '.{' . implode(",", $SupportedImgExt) . '}', GLOB_BRACE)));
		echo '<li><a href="' . $RootPath . '/' . $imagefile . '" target="_blank">' . _('Show Part Picture (if available)') . '</a></li>';
	}
	if ($Its_A_Dummy == False) {
		echo '<li><a href="' . $RootPath . '/BOMInquiry.php?StockID=' . urlencode($StockID) . '">' . _('Costed Bill Of Material') . '</a></li>';
		echo '<li><a href="' . $RootPath . '/WhereUsedInquiry.php?StockID=' . urlencode($StockID) . '">' . _('Where This Item Is Used') . '</a></li>';
	}
	if ($Its_A_Labour_Item == True) {
		echo '<li><a href="' . $RootPath . '/WhereUsedInquiry.php?StockID=' . urlencode($StockID) . '">' . _('Where This Labour Item Is Used') . '</a></li>';
	}
	echo '</ul></div></div>';
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>  ', // Icon title.
		_('Inventory Item'), '</h1></a></div>';
	// Inventory Item is not selected yet
	echo '
<div class="row" align="center"><a href="', $RootPath, '/Stocks.php?" class="btn btn-info">', _('Add New Item'), '</a></div><br />'
;
}
...
echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('In Stock Category') . '</label>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Description') . ' ' . _('-part or full') . '</label>';
if (isset($_POST['Keywords'])) {
	echo '<input type="text" class="form-control" autofocus="autofocus" name="Keywords" value="' . $_POST['Keywords'] . '" title="' . _('Enter text that you wish to search for in the item description') . '" size="20" maxlength="25" />';
} else {
	echo '<input type="text" class="form-control" autofocus="autofocus" name="Keywords" title="' . _('Enter text that you wish to search for in the item description') . '" size="20" maxlength="25" />';
}
echo '</div>
	</div>
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Stock ID') . ' ' . _('-part or full') . '</label>
		';
if (isset($_POST['StockCode'])) {
	echo '<input type="text" class="form-control" name="StockCode" value="' . $_POST['StockCode'] . '" title="' . _('Enter text that you wish to search for in the item code') . '" size="15" maxlength="18" />';
} else {
	echo '<input type="text" class="form-control" name="StockCode" title="' . _('Enter text that you wish to search for in the item code') . '" size="15" maxlength="18" />';
}
echo '</div>
		</div></div>
		<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Supplier Code') . '' . _('-part or full') . '</label>
		';
if (isset($_POST['SupplierStockCode'])) {
	echo '<input type="text" class="form-control" name="SupplierStockCode" value="' . $_POST['SupplierStockCode'] . '" title="' . _('Enter text that you wish to search for in the supplier\'s item code') . '" size="15" maxlength="18" />';
} else {
	echo '<input type="text" class="form-control" name="SupplierStockCode" title="' . _('Enter text that you wish to search for in the supplier\'s item code') . '" size="15" maxlength="18" />';
}
echo '</div></div>';
echo '<div class="col-xs-4">
<div class="form-group"><br /> <input type="submit" class="btn btn-success" name="Search" value="' . _('Search') . '" /></div></div>';
echo '</div><br />
      </form>';
	...
	if ($ListCount > 0) {
		...
		if ($ListPageMax > 1) {
			...
			<div class="col-xs-4">
				...
				<input type="submit" name="Previous" class="btn btn-default" value="' . _('Previous') . '" /></div></div>
			
<div class="col-xs-3"><div class="form-group"> <label class="col-md-12 control-label">' . $_POST['PageOffset'] . ' ' . _('of') . ' ' . $ListPageMax . ' ' . _('pages') . '. ' . _('Go to Page') . '</label> ';
			...
			<div class="col-xs-4"><div class="form-group"><br />
				<input type="submit" class="btn btn-info" name="Go" value="' . _('Go') . '" /></div></div>
				
				<div class="col-xs-1">
				<div class="form-group"><br />
				<input type="submit" name="Next" class="btn btn-default" value="' . _('Next') . '" /></div></div>
...
<div class="col-xs-12">
<div class="table-responsive">
		<table id="ItemSearchTable general-table" class="table table-bordered">
			<thead>
				<tr>
		...
		while (($myrow = DB_fetch_array($SearchResult)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			if ($myrow['discontinued']==1){
				$ItemStatus = '<p class="text-danger">' . _('Obsolete') . '</p>';
			} else {
				$ItemStatus ='';
			}
			...
			if (extension_loaded('gd') && function_exists('gd_info') && file_exists ($imagefile) ) {
				...
				$StockImgLink = '<img src="' . $imagefile . '" height="50" width="50" />';
			} else {
			...
			<td><input type="submit" class="btn  btn-info" name="Select" value="' . $myrow['stockid'] . '" /></td>
			<td>'.$StockImgLink.'</td>
			<td title="'. $myrow['longdescription'] . '">' . $myrow['description'] . '</td>
			<td class="number">' . $qoh . '</td>
			<td>' . $myrow['units'] . '</td>
			<td><a target="_blank" href="' . $RootPath . '/StockStatus.php?StockID=' . urlencode($myrow['stockid']).'" class="btn btn-info">' . _('View') . '</a></td>
			</tr>';
