...
if (isset($_POST['Process'])) {
	if ($_POST['SelectedTabs'] == '') {
		echo prnMsg(_('You have not selected a tab to claim the expenses from'), 'error');
		unset($SelectedTabs);
	}
}
if (isset($_POST['Go'])) {
...
if (isset($_POST['submit'])) {
	...
	if ($_POST['SelectedExpense'] == '') {
		$InputError = 1;
		echo prnMsg(_('You have not selected an expense to claim from this tab'), 'error');
	} elseif ($_POST['Amount'] == 0) {
		$InputError = 1;
		echo prnMsg(_('The amount must be greater than 0'), 'error');
	}
	if (!is_date($_POST['Date'])) {
		$InputError = 1;
		echo prnMsg(_('The date input is not in the correct format'), 'error');
	}
	if (isset($SelectedIndex) and $InputError != 1) { //Edit
		...
		if (isset($_FILES['Receipt']) and $_FILES['Receipt']['name'] != '') {
			...
			if ($UploadSize > ($_SESSION['MaxImageSize'] * 1024)) { //Server-side file size check. This will usually be caught by $UploadError == 2 (MAX_FILE_SIZE), but we must not trust the user.
				echo prnMsg(_('The uploaded file exceeds the maximum file size of') . ' ' . $_SESSION['MaxImageSize'] . 'KB', 'warn');
				$UploadTheFile = 'No';
				...
				echo prnMsg(_('File type not accepted. Only the following file types can be attached') . ': ' . implode(', ', $ReceiptSupportedExt), 'warn');
				$UploadTheFile = 'No';
			} elseif ($UploadError == 1 ) {  //upload_max_filesize error check
				echo prnMsg(_('The uploaded file exceeds the upload_max_filesize directive in php.ini. Please contact your system administrator.'), 'warn');
				$UploadTheFile ='No';
			} elseif ($UploadError == 2 ) {  //Client-side file size error check (MAX_FILE_SIZE)
				echo prnMsg(_('The uploaded file exceeds the maximum file size of') . ' ' . $_SESSION['MaxImageSize'] . 'KB', 'warn');
				$UploadTheFile ='No';
			} elseif ($UploadError == 3 ) {  //Partial upload error check
				echo prnMsg( _('The uploaded file was only partially uploaded. Please try again.'), 'warn');
				$UploadTheFile ='No';
			} elseif ($UploadError == 4 ) {  //No file uploaded error check
				echo prnMsg( _('No file was uploaded'), 'warn');
				$UploadTheFile ='No';
			} elseif ($UploadError == 5 ) {  //Undefined error check
				echo prnMsg( _('Undefined error'), 'warn');
				$UploadTheFile ='No';
			} elseif ($UploadError == 6 ) {  //Temp directory error check
				echo prnMsg( _('A necessary temporary folder is missing. Please contact your system administrator.'), 'warn');
				$UploadTheFile ='No';
			} elseif ($UploadError == 7 ) {  //Disk write failure error check
				echo prnMsg( _('Cannot Upload. Please contact your system administrator.'), 'warn');
				$UploadTheFile ='No';
			} elseif ($UploadError == 8 ) {  //Upload stopped by PHP extension error check
				echo prnMsg( _('The file upload was stopped by a PHP extension. Please contact your system administrator.'), 'warn');
				$UploadTheFile ='No';
			}
	...
	$Result = DB_query($SQL, $ErrMsg);
	echo prnMsg(_('The expense record on tab') . ' ' . $SelectedTabs . ' ' . _('has been deleted'), 'success');
	unset($_GET['delete']);
} //end of get delete
if (!isset($SelectedTabs)) {
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">', _('Petty Cash Tab'), '</label>
...
<div class="form-group"><br />
	...
	echo '<div class="row"><div class="col-xs-4">
			<a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '" class="btn btn-info">', _('Select another tab'), '</a>
		</div></div><br /><br />';
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>', ' ', $Title, '
		</h1></a></div>';
...
<div class="form-group"> <label class="col-md-8 control-label">', _('Detail of Tab Movements For Last (Days) '), ' </label>
						...
						<input type="text" class="form-control" name="Days" value="', $Days, '" required="required" maxlength="3" size="4" /> ', '</div></div><div class="col-xs-4">
<div class="form-group"><br />
...
<table id="general-table" class="table table-bordered">
		...
		while ($MyRow = DB_fetch_array($Result)) {
			...
			if (DB_num_rows($ReceiptResult) > 0) { //If receipt exists in database
				...
				$ReceiptFileName = $ReceiptHash . '.' . $ReceiptExt;
				$ReceiptPath = $ReceiptDir . $ReceiptFileName;
				$ReceiptText = '<a href="' . $ReceiptPath . '" download="ExpenseReceipt-' . mb_strtolower($SelectedTabs) . '-[' . $MyRow['date'] . ']-[' . $MyRow['counterindex'] . ']" class="btn btn-warning">' . _('Download') . '</a>';
			...
			if (($MyRow['authorized'] == '0000-00-00') and ($ExpenseCodeDes != 'ASSIGNCASH')) {
				...
				echo '<tr class="striped_row">
						...
						<td><a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?SelectedIndex=', $MyRow['counterindex'], '&amp;SelectedTabs=' . $SelectedTabs . '&amp;Days=' . $Days . '&amp;delete=yes" class="btn btn-danger" onclick=\'return confirm("' . _('Are you sure you wish to delete this expense?') . '");\'>' . _('Delete') . '</a></td>
					</tr>';
			} else {
				echo '<tr class="striped_row">
						<td>', ConvertSQLDate($MyRow['date']), '</td>
	...
	if (!isset($_GET['delete'])) {
		...
		if (isset($_GET['SelectedIndex'])) {
			echo '
<div class="block-title"><h3>', _('Update Expense'), '</h3></div>
				';
		} else {
			echo '
<div class="block-title"><h3>', _('New Expense'), '</h3></div>
				';
		}
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Date of Expense'), '</label>
			...
			<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Expense Code'), '</label>
				
						<select required="required" class="form-control" name="SelectedExpense">';
		...
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Gross Amount'), '</label>
				<input type="text" class="form-control" name="Amount" size="12" required="required" maxlength="11" value="', $_POST['Amount'], '" /></div>
			</div></div>
			<div class="row">';
		if (isset($_GET['edit'])) {
			$SQL = "SELECT counterindex,
			...
			while ($MyTaxRow = DB_fetch_array($TaxesResult)) {
				...
				echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', $MyTaxRow['description'], ' - ', ($MyTaxRow['taxrate'] * 100), '%</label>
						<input type="text" class="form-control" size="12" name="TaxAmount', $MyTaxRow['counterindex'], '" value="', -$MyTaxRow['amount'], '" /></div>
					</div>';
			}
		} else {
			...
			while ($MyTaxRow = DB_fetch_array($TaxResult)) {
				...
				echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', $MyTaxRow['description'], ' - ', ($MyTaxRow['taxrate'] * 100), '% </label>
						<input type="text" class="form-control" size="12" name="TaxAmount', $i, '" value="0" /></div>
					</div>';
				++$i;
			}
		}
		...
		//Select the tag
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Tag'), '</label>
		...
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Attach Receipt'), '</label>
				
				<input type="hidden" name="MAX_FILE_SIZE" value="' . $_SESSION['MaxImageSize'] * 1024 . '" />
				<input type="file" class="form-control" name="Receipt" id="Receipt" accept="' . implode(',', $ReceiptSupportedExtDotPrefix) . '" title="', _('Accepted file types'), ': ', implode(', ', $ReceiptSupportedExt), '" />
				</div>
			</div></div>';
		...
		echo '<div class="row">
				<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Purpose'), '</label>
				
					<input type="text" class="form-control" name="Purpose" size="50" maxlength="49" required="required" value="', $_POST['Purpose'], '" />
				</div>
			</div>';
...
<div class="form-group"> <label class="col-md-8 control-label">', _('Notes'), '</label>
					...
					<input type="text" class="form-control" name="Notes" size="50" maxlength="49" value="', $_POST['Notes'], '" />
				</div>
			</div></div>';
