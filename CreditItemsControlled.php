...
if (!isset($_GET['identifier'])){
	echo '<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/' . $CreditLink . '" class="btn btn-info">' .  _('Select Credit Items'). '</a></div></div><br />';
	echo prnMsg( _('This page must be called with the identifier to uniquely identify the credit note being entered. ') , 'error'),'
                 ';
	echo '</div>';
...
if (isset($_GET['LineNo'])){
	...
	echo '<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/' . $CreditLink . '" class="btn btn-info">' .  _('Select Credit Items'). '</a></div></div><br />';
	echo prnMsg( _('This can only be opened if a line item on a credit note has been selected.') . ' ' . _, 'error');
	echo '</div>';
	include('includes/footer.php');
	exit;
}
...
if (!isset($_SESSION['CreditItems' . $identifier])) {
	...
	echo '<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/' . $CreditLink . '" class="btn btn-info">' .  _('Select Credit Items'). '</a>
		</div></div>
		<br />';
	echo prnMsg( _('This can only be opened if a controlled credit note line item has been selected.') . ' ' . _,'error');
	echo '</div>';
	include('includes/footer.php');
	exit;
}
...
if ( $LineItem->Controlled != 1 ){
	echo '<br /><p align="right"><a href="' . $RootPath . '/' . $CreditLink . '">' .  _('<i class="fa fa-hand-o-left fa-fw"></i> To Credit Note Entry') . '</a></p><br />';
	
	echo prnMsg( _('Notice') . ' - ' . _('The line item must be defined as controlled to require input of the batch numbers or serial numbers being credited'),'warn');
	include('includes/footer.php');
	exit;
}
...
echo '<br /><p align="right"><a href="' . $RootPath . '/' . $CreditLink . '" class="btn btn-default">' .  _('<i class="fa fa-hand-o-left fa-fw"></i> To Credit Note Entry'). '</a></p><br />';

echo '<div class="row">' .  _('Credit of Controlled Item'). ' ' . $LineItem->StockID  . ' - ' . $LineItem->ItemDescription . ' '. _('from') .' '. $_SESSION['CreditItems' . $identifier]->CustomerName . '</div><br />';
