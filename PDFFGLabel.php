...
if ($NoOfLabels >0){
	...
	if ($MakePDFThenDisplayIt) {
		$pdf->OutputD('nERP' . '_FGLABEL_' . $SelectedWO . '_' . date('Y-m-d') . '.pdf');
		$pdf->__destruct();
	} else {
		$PdfFileName = 'nERP' . '__FGLABEL_' . $SelectedWO . '_' . date('Y-m-d') . '.pdf';
	...
	$Title = _('Label Error');
	include('includes/header.php');
	echo  prnMsg(_('There were no labels to print'),'warn');
	echo '<br /><p align="right"><a href="'.$RootPath.'/menu_data.php?Application=GL" class="btn btn-default">' .  _('<i class="fa fa-hand-o-left fa-fw"></i> to Menu') . '</a></p><br />';
	include('includes/footer.php');
}
?>
