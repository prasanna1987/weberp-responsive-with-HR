...
if (isset($_POST['UploadFile'])) {
	...
	if (sizeof($NewItemArray) == 0) {
		echo prnMsg(_('There are no items that can be imported'), 'error');
	} else {
		echo   prnMsg($InsertNum . ' ' . _('of') . ' ' . $Row . ' ' . _('rows have been added to the order'), 'info');
	}
}
...
/*read in all the guff from the selected order into the Items cart  */
	...
	if (DB_num_rows($GetOrdHdrResult)==1) {
		...
		if ($_SESSION['SalesmanLogin']!='' AND $_SESSION['SalesmanLogin']!=$myrow['salesman']){
			echo prnMsg(_('Your account is set up to see only a specific salespersons orders. You are not authorised to modify this order'),'error');
			include('includes/footer.php');
			exit;
		}
		if ($CustomerLogin == 1 AND $_SESSION['CustomerID'] != $myrow['debtorno']) {
			echo '<p class="text-danger">' . _('This transaction is addressed to another customer and cannot be displayed for privacy reasons') . '. ' . _('Please select only transactions relevant to your company').'</p>';
...
if (isset($_POST['ChangeCustomer']) AND $_POST['ChangeCustomer']!=''){
	...
	if ($_SESSION['Items'.$identifier]->Any_Already_Delivered()==0){
		...
		echo prnMsg(_('The customer the order is for cannot be modified once some of the order has been invoiced'),'warn');
	}
}
...
if (isset($_POST['SearchCust'])
	...
	if (DB_num_rows($result_CustSelect)==1){
		...
		echo prnMsg(_('No Customer Branch records contain the search criteria , please try again. Note a Customer Branch Name may be different to the Customer Name'),'info'); 
...
if (isset($SelectedCustomer)) {
	...
	if ($myrow[1] != 1){
		if ($myrow[1]==2){
			echo  prnMsg(_('The') . ' ' . htmlspecialchars($myrow[0], ENT_QUOTES, 'UTF-8', false) . ' ' . _('account is currently flagged as an account that needs to be watched. Please contact the credit control personnel to discuss'),'warn');
		}
...
# the branch was also selected from the customer selection so default the delivery details from the customer branches table CustBranch. The order process will ask for branch details later anyway
		...
		if (DB_num_rows($result)==0){
			...
			echo prnMsg(_('The branch details for branch code') . ': ' . $_SESSION['Items'.$identifier]->Branch . ' ' . _('against customer code') . ': ' . $_SESSION['Items'.$identifier]->DebtorNo . ' ' . _('could not be retrieved') . '. ' . _('Check the set up of the customer and branch'),'error');

			if ($debug==1){
				echo  prnMsg( _('The querry that failed to get the branch details was') . ':<br />' . $sql . 'warning');
			}
			include('includes/footer.php');
			exit;
		}
		...
		if ($_SESSION['SalesmanLogin']!=NULL AND $_SESSION['SalesmanLogin']!=$myrow['salesman']){
			echo prnMsg(_('Your login is only set up for a particular salesperson. This customer has a different salesperson.'),'error');
			include('includes/footer.php');
			exit;
		}
		...
		if ($_SESSION['Items'.$identifier]->SpecialInstructions)
		  echo  prnMsg($_SESSION['Items'.$identifier]->SpecialInstructions,'warn');
		...
		if ($_SESSION['CheckCreditLimits'] > 0){  /*Check credit limits is 1 for warn and 2 for prohibit sales */
			...
			if ($_SESSION['CheckCreditLimits']==1 AND $_SESSION['Items'.$identifier]->CreditAvailable <=0){
				echo  prnMsg(_('The') . ' ' . htmlspecialchars($myrow[0], ENT_QUOTES, 'UTF-8', false) . ' ' . _('account is currently at or over their credit limit'),'warn');
			} elseif ($_SESSION['CheckCreditLimits']==2 AND $_SESSION['Items'.$identifier]->CreditAvailable <=0){
				echo  prnMsg(_('No more orders can be placed by') . ' ' . htmlspecialchars($myrow[0], ENT_QUOTES, 'UTF-8', false) . ' ' . _(' their account is currently at or over their credit limit'),'warn');
				include('includes/footer.php');
				exit;
			}
		}
		...
		echo  prnMsg(_('The') . ' ' . htmlspecialchars($myrow[0], ENT_QUOTES, 'UTF-8', false) . ' ' . _('account is currently on hold please contact the credit control personnel to discuss'),'warn');
	}
...
# defaulted from the entry of the userid and password.
	...
	// the order process will ask for branch details later anyway
		...
		echo  prnMsg(_('Sorry, your account has been put on hold for some reason, please contact the credit control personnel.'),'warn');
		include('includes/footer.php');
		exit;
	}
}
...
if ($_SESSION['RequireCustomerSelection'] ==1
	...
	echo '<div class="block-header">
                            <a href="" class="header-title-link"><h1>' . _('Enter an Order or Quotation') . '</h1></a></div>';
	...
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier . '" method="post">
			 ...
			 <input name="FormID" type="hidden" value="' . $_SESSION['FormID'] . '" />
			   ...
			   <label class="col-md-8 control-label">' . _(' Branch Name-part or full') . '</label>
				
				<input type="text" class="form-control"  autofocus="autofocus" name="CustKeywords" size="20" maxlength="25" title="' . _('Enter a text extract of the customer\'s name, then click Search Now to find customers matching the entered name') . '" />
               </div>
			   </div>
				...
				<div class="col-xs-4">
               <div class="form-group"> 
			   <label class="col-md-8 control-label">' . _('Branch Code -part or full') . '</label>
				<input type="text" class="form-control"  name="CustCode" size="15" maxlength="18" title="' . _('Enter a part of a customer code that you wish to search for then click the Search Now button to find matching customers') . '" />
				</div>
			   </div>
				...
				<div class="col-xs-4">
               <div class="form-group"> 
			   <label class="col-md-8 control-label">' . _('Branch Phone Number-part or full') . '</label>
				<input type="text" name="CustPhone" class="form-control" size="15" maxlength="18" title="' . _('Enter a part of a customer\'s phone number that you wish to search for then click the Search Now button to find matching customers') . '"/>
				</div>
			   </div>
			   </div>
			...
			<div>
				<input type="submit" name="SearchCust" class="btn btn-success" value="' . _('Search') . '" />
				</div>
	...
	if (isset($result_CustSelect)) {
		...
		while ($myrow=DB_fetch_array($result_CustSelect)) {
			...
			echo '<tr>
					...
					<td><input type="submit" name="SubmitCustomerSelection' . $j .'" value="' . htmlspecialchars($myrow['brname'], ENT_QUOTES, 'UTF-8', false). '" class="btn btn-info" />
...
// everything below here only do if a customer is selected
 	...
 	if (isset($_POST['CancelOrder'])) {
		...
		if($_SESSION['ExistingOrder' . $identifier]!=0) { //need to check that not already dispatched
			...
			if (DB_num_rows($InvQties)>0){
				...
				echo   prnMsg( _('There are lines on this order that have already been invoiced. Please delete only the lines on the order that are no longer required') . '' . _('There is an option on confirming a dispatch/invoice to automatically cancel any balance on the order at the time of invoicing if you know the customer will not want the back order'),'warn');
			}
		}
		...
		if ($OK_to_delete==1){
			...
			//echo '<br /><br />';
			echo   prnMsg(_('This sales order has been cancelled as requested'),'success');
			include('includes/footer.php');
			exit;
		}
		...
		echo ': ' . $_SESSION['Items'.$identifier]->DebtorNo  . ' <br /><small>' . _('Customer Name') . ': ' . htmlspecialchars($_SESSION['Items'.$identifier]->CustomerName, ENT_QUOTES, 'UTF-8', false);
		echo '</small></h1></a></div>';
	...
	if (isset($_POST['Search']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
		...
		if ($_POST['Keywords']!='' AND $_POST['StockCode']=='') {
			$msg='<p class="text-info">' . _('Order Item description has been used in search') . '.</p>';
		} elseif ($_POST['StockCode']!='' AND $_POST['Keywords']=='') {
			$msg='<p class="text-info">' . _('Stock Code has been used in search') . '.</p>';
		} elseif ($_POST['Keywords']=='' AND $_POST['StockCode']=='') {
			$msg='<p class="text-info">' . _('Stock Category has been used in search') . '.</p>';
		}
		...
		if (DB_num_rows($SearchResult)==0 ){
			echo    prnMsg (_('There are no items available meeting the criteria specified'),'info');
		}
...
//Get The exchange rate used for GPPercent calculations on adding or amending items
		 ...
		 $i=1;
		  while ($i<=$_SESSION['QuickEntries'] AND isset($_POST['part_' . $i]) AND $_POST['part_' . $i]!='') {
			...
			if(!Is_Date($NewItemDue)) {
				echo  prnMsg(_('An invalid date entry was made for ') . ' ' . $NewItem . ' ' . _('The date entry') . ' ' . $NewItemDue . ' ' . _('must be in the format') . ' ' . $_SESSION['DefaultDateFormat'],'warn');
			...
			if (DB_num_rows($KitResult)==0){
				echo   prnMsg( _('The item code') . ' ' . $NewItem . ' ' . _('could not be retrieved from the system and has not been added to the order'),'warn');
				...
				if ($myrow['mbflag']=='K'){	/*It is a kit set item */
					...
					echo   prnMsg(_('Phantom assemblies cannot be sold, these items exist only as bills of materials used in other manufactured items. The following item has not been added to the order:') . ' ' . $NewItem, 'warn');
	...
	if (isset($_POST['AssetDisposalEntered'])){ //its an asset being disposed of
		if ($_POST['AssetToDisposeOf'] == 'NoAssetSelected'){ //don't do anything unless an asset is disposed of
			echo   prnMsg(_('No asset was selected to dispose of. No assets have been added to this customer order'),'warn');
		...
		foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
			...
			if (isset($_POST['Quantity_' . $OrderLine->LineNumber])){
				...
				if (ABS($OrderLine->Price - filter_number_format($_POST['Price_' . $OrderLine->LineNumber]))>0.01){
					...
					echo    prnMsg(_('Recalculated the price from the GP % entered - the GP % was') . ' ' . $OrderLine->GPPercent . '  the new GP % is ' . filter_number_format($_POST['GPPercent_' . $OrderLine->LineNumber]),'info');
				...
				if(!Is_Date($_POST['ItemDue_' . $OrderLine->LineNumber])) {
					echo   prnMsg(_('An invalid date entry was made for ') . ' ' . $NewItem . ' ' . _('The date entry') . ' ' . $ItemDue . ' ' . _('must be in the format') . ' ' . $_SESSION['DefaultDateFormat'],'warn');
					//Attempt to default the due date to something sensible?
				...
				if ($Quantity<0 OR $Price <0 OR $DiscountPercentage >100 OR $DiscountPercentage <0){
					echo   prnMsg(_('The item could not be updated because you are attempting to set the quantity ordered to less than 0 or the price less than 0 or the discount more than 100% or less than 0%'),'warn');
					...
					echo   prnMsg(_('The item you attempting to modify the price for has already had some quantity invoiced at the old price the items unit price cannot be modified retrospectively'),'warn');
					...
					echo   prnMsg(_('The item you attempting to modify has had some quantity invoiced at the old discount percent the items discount cannot be modified retrospectively'),'warn');

				} elseif ($_SESSION['Items'.$identifier]->LineItems[$OrderLine->LineNumber]->QtyInv > $Quantity){
					echo   prnMsg( _('You are attempting to make the quantity ordered a quantity less than has already been invoiced') . '. ' . _('The quantity delivered and invoiced cannot be modified retrospectively'),'warn');
					...
					if ($_SESSION['CheckCreditLimits'] > 0 AND $AlreadyWarnedAboutCredit==false){
						...
						if ($_SESSION['CheckCreditLimits']==1 AND $_SESSION['Items'.$identifier]->CreditAvailable <=0){
							echo   prnMsg(_('The customer account will breach their credit limit'),'warn');
							$AlreadyWarnedAboutCredit = true;
						} elseif ($_SESSION['CheckCreditLimits']==2 AND $_SESSION['Items'.$identifier]->CreditAvailable <=0){
							echo   prnMsg(_('This change would put the customer over their credit limit and is prohibited'),'warn');
	...
	if (isset($_POST['DeliveryDetails'])){
		...
		echo   prnMsg(_('You should automatically be forwarded to the entry of the delivery details page') . '. ' . _('if this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' .
		   '<a href="' . $RootPath . '/DeliveryDetails.php?identifier='.$identifier . '" class="btn btn-info">' . _('click here') . '</a> ' . _('to continue'), 'info');
	   	exit;
	}
...
/* This is where the order as selected should be displayed  reflecting any deletions or insertions*/
		...
		echo '
				<div class="col-xs-12">
				<div class="table-responsive">
		<table id="general-table" class="table table-bordered"><thead><tr>';
...
/*		if (in_array($_SESSION['PageSecurityArray']['OrderEntryDiscountPricing'], $_SESSION['AllowedPageSecurityTokens'])){*/
		...
		foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
			...
			if ($OrderLine->QOHatLoc < $OrderLine->Quantity AND ($OrderLine->MBflag=='B' OR $OrderLine->MBflag=='M')) {
				...
				$RowStarter = '<tr>'; //rows show red where stock deficiency
			} else {
				$RowStarter = '<tr>';
			}
...
/*			if($_SESSION['Items'.$identifier]->DefaultPOLine ==1){ //show the input field only if required*/
			if($ShowPOLine) {// Show the input field only if required.
				echo '<input maxlength="20" class="form-control" name="POLine_' . $OrderLine->LineNumber . '" size="20" title="' . _('Enter the customer\'s purchase order reference if required by the customer') . '" type="text" value="' . $OrderLine->POLine . '" /></td><td>';
			} else {
				echo '<input name="POLine_' . $OrderLine->LineNumber . '" type="hidden" value="" />';
			}

			echo '<a href="' . $RootPath . '/StockStatus.php?identifier='.$identifier . '&amp;StockID=' . $OrderLine->StockID . '&amp;DebtorNo=' . $_SESSION['Items'.$identifier]->DebtorNo . '" target="_blank" class="btn btn-info">' . $OrderLine->StockID . '</a></td>
				<td title="' . $OrderLine->LongDescription . '">' . $OrderLine->ItemDescription . '</td>';

			echo '<td><input class="form-control" maxlength="8" name="Quantity_' . $OrderLine->LineNumber . '" required="required" size="6" title="' . _('Enter the quantity of this item ordered by the customer') . '" type="text" value="' . locale_number_format($OrderLine->Quantity,$OrderLine->DecimalPlaces) . '" />';
...
/*			if (in_array($_SESSION['PageSecurityArray']['OrderEntryDiscountPricing'], $_SESSION['AllowedPageSecurityTokens'])){*/
			if ($ShowDiscountGP){
				echo '<td><input class="form-control" maxlength="16" name="Price_' . $OrderLine->LineNumber . '" required="required" size="16" title="' . _('Enter the price to charge the customer for this item') . '" type="text" value="' . locale_number_format($OrderLine->Price,$_SESSION['Items'.$identifier]->CurrDecimalPlaces)  . '" /></td>
					<td><input class="form-control" maxlength="4" name="Discount_' . $OrderLine->LineNumber . '" required="required" size="5" title="' . _('Enter the discount percentage to apply to the price for this item') . '" type="text" value="' . locale_number_format(($OrderLine->DiscountPercent * 100),2) . '" /></td>
					<td><input class="form-control" maxlength="40" name="GPPercent_' . $OrderLine->LineNumber . '" required="required" size="4" value="' . locale_number_format($OrderLine->GPPercent,2) . '" title="' . _('Enter a gross profit percentage to use as the basis to calculate the price to charge the customer for this line item') . '" type="text" /></td>';
			} else {
				...
				echo '<input class="form-control" maxlength="4" name="Discount_' . $OrderLine->LineNumber . '" required="required" size="5" title="' . _('Enter the discount percentage to apply to the price for this item') . '" type="hidden" value="' . locale_number_format(($OrderLine->DiscountPercent * 100),2) . '" />';
				echo '<input class="form-control" maxlength="40" name="GPPercent_' . $OrderLine->LineNumber . '" required="required" size="4" value="' . locale_number_format($OrderLine->GPPercent,2) . '" title="' . _('Enter a gross profit percentage to use as the basis to calculate the price to charge the customer for this line item') . '" type="hidden" />';
			}
			...
			echo '<td><input class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" maxlength="10" name="ItemDue_' . $OrderLine->LineNumber . '" size="10" type="text" value="' . $LineDueDate . '" /></td>';
			
			

			echo '<td rowspan="2"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier=' . $identifier . '&amp;Delete=' . $OrderLine->LineNumber . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');" class="btn btn-danger">' . $RemTxt . '</a></td></tr>';

			if ($_SESSION['AllowOrderLineItemNarrative'] == 1){
				$varColSpan=8+$ShowPOLine+$ShowDiscountGP;
				echo $RowStarter .
						'<td colspan="' . $varColSpan . '">' . _('Narrative') . ':<textarea name="Narrative_' . $OrderLine->LineNumber . '"  title="' . _('Enter any narrative to describe to the customer the nature of the charge for this line') . '" class="form-control" >' . stripslashes(AddCarriageReturns($OrderLine->Narrative)) . '</textarea><br /></td>
					</tr>';
			} else {
				echo '<tr>
...
/*		if (in_array($_SESSION['PageSecurityArray']['OrderEntryDiscountPricing'], $_SESSION['AllowedPageSecurityTokens'])){
		...
		<table id="general-table" class="table table-bordered">
				...
				<div class="row">
				<div class="col-xs-4">
					<input type="submit" name="Recalculate" class="btn btn-info" value="' . _('Update') . '" /></div>
					<div class="col-xs-4">
					<input type="submit" name="DeliveryDetails" class="btn btn-success" value="' . _('Enter Delivery Details and Confirm Order') . '" />
				</div>
				</div>
...
// Find the quantity in stock at location
				...
				printf('<tr class="striped_row">
						...
						<td class="number">%s</td>
						<td class="number">%s</td>
						<td><input class="number form-control" ' . ($j==0 ? 'autofocus="autofocus"':'') . ' type="text" required="required" size="6" name="OrderQty%s" value="0" />
			...
			echo '</tbody>
				<tr>
					<td class="centre" colspan="8"><input name="SelectingOrderItems" type="hidden" value="1" /><input type="submit" value="'._('Add to Sales Order').'" class="btn btn-success" /></td>
				</tr>
				</table></div></div>';
		...
		echo '</select></div></div>
			<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">'. _('Description-part or full') . '</label><input type="text" class="form-control" name="Keywords" size="20" maxlength="25" value="' ;
        ...
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Stock ID-part or full') . ':</label><input type="text" ' . (!isset($_POST['PartSearch']) ? 'autofocus="autofocus"' :'') . ' name="StockCode" size="15" class="form-control" maxlength="18" value="';
	...
	<div class="row">
		...
		<input type="checkbox" id="bordered-checkbox1"  name="RawMaterialFlag" value="M" />'._('Show Raw material?').'</label> </div></div></div>
		
		<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label"></label>
		<div class="checkbox">
		<label for="bordered-checkbox1">
		<input type="checkbox" id="bordered-checkbox1" name="CustItemFlag" value="C" />'._('Show Only Customer Items?').'</label> </div>
			</div></div></div>';

		echo '<div class="row">
		<div class="col-xs-3">
        <div class="form-group"> <br /><input type="submit" name="Search" class="btn btn-success" value="' . _('Search') . '" /></div></div>
			<div class="col-xs-3">
        <div class="form-group"> <br /><input type="submit" class="btn btn-success" name="QuickEntry" value="' .  _('Quick Entry') . '" /></div></div>';
        ...
        <div class="form-group"> <label class="col-md-8 control-label"></label><input type="submit" class="btn btn-info" name="ChangeCustomer" value="' . _('Change Customer') . '" /></div></div>
		
			<div class="col-xs-3">
        <div class="form-group"> <label class="col-md-8 control-label"></label><input type="submit" class="btn btn-info" name="SelectAsset" value="' . _('Fixed Asset Disposal') . '" /></div></div></div>';
		}
		echo '<div class="row">
		<div class="col-xs-3">
        <div class="form-group"> <label class="col-md-8 control-label">
						' . _('Upload items from csv file') . '</label><input id="example-file-input" type="file" name="CSVFile" />
						<input  type="submit" class="btn btn-info" name="UploadFile" value="' . _('Upload File') . '" />
					</div>
				</div>';

		echo '<div class="col-xs-9">
        <div class="form-group"><h4>' . _('The csv file should have exactly 2 columns, Stock ID and quantity.') . '</h4></div></div></div>';
		if (isset($SearchResult)) {
			

           
			echo '<input name="FormID" type="hidden" value="' . $_SESSION['FormID'] . '" />';
			echo '<div class="col-xs-12"><div class="table-responsive">
<table class="table">
			<tr>
					<td align="left"><input name="PreviousList" type="hidden" value="'.strval($Offset-1).'" /><input type="submit" name="Previous" class="btn btn-default" value="'._('Previous').'" /></td>
					<td align="center"><input name="SelectingOrderItems" type="hidden" value="1" /><input type="submit" value="'._('Add to Order').'" class="btn btn-success" /></td>
					<td align="right"><input name="NextList" type="hidden" value="'.strval($Offset+1).'" /><input name="Next" class="btn btn-default" type="submit" value="'._('Next').'" /></td>
					</tr></table></div></div>
...
<table id="general-table" class="table table-bordered">';
			...
			while ($myrow=DB_fetch_array($SearchResult)) {
				...
				printf('<tr class="striped_row">
						...
						<td class="number">%s</td>
						<td class="number">%s</td>
						<td><input class="number form-control" type="text" size="6" name="OrderQty%s"  ' . ($i==0 ? 'autofocus="autofocus"':'') . ' value="0" min="0"/>
						<input name="StockID%s" type="hidden" value="%s" />
						</td>
						</tr>',
						$myrow['stockid'],
...
<table class="table">
					...
					<tr>
					<td align="left"><input name="PreviousList" type="hidden" value="'. strval($Offset-1).'" /><input type="submit" name="Previous" value="'._('Previous').'" class="btn btn-default" /></td>
					<td align="center"><input name="SelectingOrderItems" type="hidden" value="1" /><input type="submit" value="'._('Add to Order').'" class="btn btn-success" /></td>
					<td align="right"><input name="NextList" type="hidden" value="'.strval($Offset+1).'" /><input name="Next" type="submit" value="'._('Next').'" class="btn btn-default" /></td>
					</tr></table></div>
...
<table id="general-table" class="table table-bordered">
			...
			for ($i=1;$i<=$_SESSION['QuickEntries'];$i++){
		 		...
		 		if($_SESSION['Items'.$identifier]->DefaultPOLine > 0){
					echo '<td><input type="text" class="form-control" name="poline_' . $i . '" size="21" maxlength="20" title="' . _('Enter the customer purchase order reference') . '" /></td>';
				}
				echo '<td><input type="text" class="form-control" name="part_' . $i . '" size="21" maxlength="20" title="' . _('Enter the item code ordered') . '" /></td>
						<td><input class="form-control" type="text" name="qty_' . $i . '" size="6" maxlength="6" title="' . _('Enter the quantity of the item ordered by the customer') . '" /></td>
						<td><input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="itemdue_' . $i . '" size="25" maxlength="25"
			...
			echo '</table></div>
					...
					<div class="row">
					<div class="col-xs-4">
						<input type="submit" name="PartSearch" class="btn btn-default" value="' . _('Back to Search') . '" />
					</div>
					<div class="col-xs-4">
						<input type="submit" class="btn btn-success" name="QuickEntry" value="' . _('Quick Entry') . '" /></div></div>
						<br />
';
		...
		';
			/*do not display colum unless customer requires po line number by sales order line*/
		 	if($_SESSION['Items'.$identifier]->DefaultPOLine ==1){
				echo	'<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('PO Line') . '</label>
							<input type="text" class="form-control" name="poline" size="21" maxlength="20" title="' . _('Enter the customer\'s purchase order reference') . '" /></div>
						</div>';
			}
			echo '<div class="col-xs-4">
<div class="form-group"> 
					<select name="AssetToDisposeOf" class="form-control">';
			...
			echo '</select></div>
				...
				<div class="row">
				<div class="col-xs-4">
					<input type="submit" class="btn btn-success" name="AssetDisposalEntered" value="' . _('Add To Order') . '" /></div>
					<div class="col-xs-4">
					<input type="submit" class="btn btn-info" name="PartSearch" value="' . _('Search Parts') . '" /></div>
			</div><br />';
