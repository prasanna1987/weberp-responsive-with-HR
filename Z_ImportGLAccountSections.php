...
$nERPUser = $_SESSION['UserID'];
$sql="SELECT password FROM www_users WHERE userid='" . $nERPUser ."'";
$result=DB_query($sql);
$myrow=DB_fetch_array($result);
$nERPpassword = $myrow[0];
...
if (isset($_POST['update'])) {
	$fp = fopen($_FILES['ImportFile']['tmp_name'], "r");
 	...
 	while (!feof ($fp)) {
    	...
    	if ($FieldValues[0]!='') {
			...
			$user = new xmlrpcval($nERPUser);
			$password = new xmlrpcval($nERPpassword);

			$msg = new xmlrpcmsg("nERP.xmlrpc_InsertGLAccountSection", array($accountsection, $user, $password));
	...
	prnMsg( _('Select a csv file containing the details of the account sections that you wish to import into nERP. '). '<br />' .
