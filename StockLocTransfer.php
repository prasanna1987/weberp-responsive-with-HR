...
$Title = _('Inventory Location Transfer');
	...
	if (DB_num_rows($result)!=0){
	  ...
	  if ($_FILES['SelectedTransferFile']['name']) { //start file processing
		...
		while ( ($myrow = fgetcsv($FileHandle, 10000, ',')) !== FALSE ) {
			...
			if (count($myrow) != 2){
				echo prnMsg (_('File contains') . ' '. count($myrow) . ' ' . _('columns, but only 2 columns are expected. The comma separated file should have just two columns the first for the item code and the second for the quantity to transfer'),'error');
				fclose($FileHandle);
			...
			for ($i=0; $i<count($myrow);$i++) {
				switch ($i) {
					case 0:
						...
						if ($StockIDCheck[0]==0){
							$InputError = True;
							$ErrorMessage .= _('The part code entered of'). ' ' . $StockID . ' '. _('is not set up in the system') . '. ' . _('Only valid parts can be entered for transfers'). '<br />';
						}
						break;
					case 1:
...
if(isset($_POST['Submit']) AND $InputError==False){
	...
	echo prnMsg( _('The inventory transfer records have been created successfully'),'success');
	echo '<p align="center"><a href="'.$RootPath.'/PDFStockLocTransfer.php?TransferNo=' . $_POST['Trf_ID'] . '" class="btn btn-info">' .  _('Print the Transfer Note'). '</a></p>';
	include('includes/footer.php');
	...
	if (isset($InputError) and $InputError==true){
		...
		echo prnMsg($ErrorMessage, 'error');
		echo '<br />';
...
<div class="col-xs-12">';
	...
	echo '<input type="hidden" name="Trf_ID" value="' . $Trf_ID . '" /><h3 class="page-header">' .  _('Inventory Location Transfer Shipment Reference').' # '. $Trf_ID. '</h3>
		';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('From Stock Location') . '</label>
			<select name="FromStockLocation" class="form-control">';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('To Stock Location').'</label>
			<select name="ToStockLocation" class="form-control">';
	while ($myrow=DB_fetch_array($resultStkLocs)){
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Upload CSV file') . '</label>
			<input name="SelectedTransferFile" type="file" class="form-cintrol" /></div>
		  </div>
		  </div>
...
<table id="general-table" class="table table-bordered">
	...
	if(isset($_POST['LinesCounter'])){
		...
		for ($i=0;$i < $_POST['LinesCounter'];$i++){
			...
			echo '<tr>
					<td><input type="text" class="form-control" name="StockID' . $j .'" size="21"  maxlength="20" value="' . $_POST['StockID' . $i] . '" /></td>
					<td><input class="form-control" type="text" name="StockQTY' . $j .'" size="10" maxlength="10" class="number" value="' . locale_number_format($_POST['StockQTY' . $i],'Variable') . '" /></td>
	...
	while($j < $z) {
		...
		echo '<tr>
				<td><input type="text" class="form-control" name="StockID' . $j .'" ' . ($j==0 OR $j==$z-9 ? 'autofocus="autofocus"' : '') . ' size="21"  maxlength="20" value="' . $_POST['StockID' . $j] . '" /></td>
				<td><input type="text" class="form-control" name="StockQTY' . $j .'" size="10" maxlength="10" class="number" value="' . locale_number_format($_POST['StockQTY' . $j]) . '" /></td>
			</tr>';
		$j++;
	}
	...
	echo '</table>
		...
		<div class="col-xs-4">
		<input type="submit" class="btn btn-info" name="EnterMoreItems" value="'. _('Add Line'). '" />
		</div>
		<div class="col-xs-4">
