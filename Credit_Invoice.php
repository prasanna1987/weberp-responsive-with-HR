...
if(!isset($_GET['InvoiceNumber']) AND !$_SESSION['ProcessingCredit']) {
	...
	echo prnMsg(_('This page can only be opened if an invoice has been selected for crediting') . '. ' . _('Please select an invoice first') . ' - ' . _('from the customer inquiry screen click the link to credit an invoice'),'info');
...
/*now populate the line items array with the stock movement records for the invoice*/
		...
		if(DB_num_rows($LineItemsResult)>0) {
			...
			echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=Sales" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i>') . '</a></p><br />';
			
			echo prnMsg( _('There are no line items that were retrieved for this invoice') . '. ' . _('The system can not create a credit note from this invoice'),'warn');
			include('includes/footer.php');
			exit;
		} //end of checks on returned data set
		DB_free_result($LineItemsResult);
	} else {
		echo prnMsg( _('This invoice can not be credited using the system') . '<br />' . _('CRITICAL ERROR') . ': ' . _('A duplicate DebtorTrans header record was found for invoice') . ' ' . $SESSION['ProcessingCredit'],'warn');
		include('includes/footer.php');
		exit;
...
if(isset($_POST['ChargeFreightCost']) AND abs($_POST['ChargeFreightCost'])>0) {
	...
	if (($TotalQtyCredited + abs($_POST['ChargeFreightCost']))<=0) {
		echo prnMsg(_('There are no item quantity or freight charge input'),'error');
	...
	foreach($_SESSION['CreditItems' . $identifier]->LineItems as $LineItem) {
		...
		if(isset($_POST['Quantity_' . $LineItem->LineNumber])) {
			...
			if($Quantity<0 OR $Price <0 OR $DiscountPercentage >100 OR $DiscountPercentage <0) {
				echo prnMsg(_('The item could not be updated because you are attempting to set the quantity credited to less than 0 or the price less than 0 or the discount more than 100% or less than 0%'),'error');
			} else {
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Credit Invoice') . ' ' . $_SESSION['ProcessingCredit'] . '
		' . ' - ' . $_SESSION['CreditItems' . $identifier]->CustomerName . '
		 - ' . _('Credit Note amounts stated in') . ' ' . $_SESSION['CreditItems' . $identifier]->DefaultCurrency . '</h3></div>
...
// Use of htmlspecialchars to convert ampersand, double_quote, single_quote, less_than, and greater_than to html entities to to preserve their meanings.
		...
		if($LnItm->Controlled==1) {
			...
			echo '<td><input class="form-cintrol" type="hidden" name="Quantity_' . $LnItm->LineNumber .'" value="' . locale_number_format($LnItm->QtyDispatched,$LnItm->DecimalPlaces) . '" /><a ' . ($j==1 ? 'autofocus="autofocus" ':'') . ' href="' . $RootPath . '/CreditItemsControlled.php?LineNo=' . $LnItm->LineNumber . '&amp;CreditInvoice=Yes&amp;identifier=' . $identifier . '" tabindex="'.$tabindex++.'">' . locale_number_format($LnItm->QtyDispatched,$LnItm->DecimalPlaces) . '</a></td>';

		} else {

			echo '<td><input class="form-control" ' .($j==1 ? 'autofocus="autofocus" ':'') . ' maxlength="6" name="Quantity_' . $LnItm->LineNumber .'" required="required" size="6" tabindex="'.$tabindex++.'" title="' . _('Enter the quantity of this item to credit') . '" type="text" value="' . locale_number_format($LnItm->QtyDispatched,$LnItm->DecimalPlaces) . '" /></td>';
		...
		$j++;
		echo '<td><input class="form-control" maxlength="12" name="Price_' . $LnItm->LineNumber . '" required="required" size="6" tabindex="'.$tabindex++.'" title="' . _('Enter the price at which to credit this item') . '" type="text" value="' . locale_number_format($LnItm->Price,$_SESSION['CreditItems' . $identifier]->CurrDecimalPlaces) . '" /></td>
			<td><input class="form-control" maxlength="3" name="Discount_' . $LnItm->LineNumber . '" required="required" size="3" tabindex="'.$tabindex++.'" type="text" value="' . locale_number_format(($LnItm->DiscountPercent * 100),2) . '" />%</td>
	...
	if(is_array($LnItm->Taxes) ) {
		foreach($LnItm->Taxes as $Tax) {
			...
			if(!isset($_POST['ProcessCredit'])) {
				echo '<input class="form-control" maxlength="4" name="' . $LnItm->LineNumber . $Tax->TaxCalculationOrder . '_TaxRate" required="required" size="4" tabindex="'.$tabindex++.'" type="text" value="' . locale_number_format($Tax->TaxRate*100,2) . '" />%';
			}
			$i++;
			if($Tax->TaxOnTax ==1) {
	...
	if(!isset($_POST['ProcessCredit'])) {
		...
		echo '<td class="number">' . $DisplayTaxAmount . '</td>
				...
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier=' . $identifier . '&Delete=' . $LnItm->LineNumber . '" onclick="return confirm(\'' . _('Are you sure you wish to delete this item from the credit?') . '\');" tabindex="'.$tabindex++.'" class="btn btn-danger">' . _('Delete') . '</a></td>
			</tr>';

		echo '<tr class="'.$RowClass.'">
	<td colspan="13"><textarea name="Narrative_' . $LnItm->LineNumber . '" class="form-control" tabindex="'.$tabindex++.'">' . $LnItm->Narrative . '</textarea>
				</td>
			</tr>';
		$j++;
	}
...
if(!isset($_POST['ProcessCredit'])) {
	echo '<tr>
		...
		<td><input class="form-control" maxlength="6" name="ChargeFreightCost" required="required" size="6" tabindex="'.$tabindex++.'" type="text" value="' . locale_number_format($_SESSION['CreditItems' . $identifier]->FreightCost,$_SESSION['CreditItems' . $identifier]->CurrDecimalPlaces) . '" /></td>
		<td>&nbsp;</td>
		<td>';
...
foreach($_SESSION['CreditItems' . $identifier]->FreightTaxes as $FreightTaxLine) {
	...
	if(!isset($_POST['ProcessCredit'])) {
		echo  '<input class="form-control" maxlength="4" name="FreightTaxRate' . $FreightTaxLine->TaxCalculationOrder . '" required="required" size="4" type="text" value="' . locale_number_format(($FreightTaxLine->TaxRate * 100),2) . '" tabindex="'.$tabindex++.'" />';
	}
...
if(!isset($_POST['ProcessCredit'])) {
	echo '<tr>
			...
			<td colspan="2">&nbsp;</td>
			<td class="number"><strong>' . locale_number_format($TaxTotal,$_SESSION['CreditItems' . $identifier]->CurrDecimalPlaces) . '</strong></td>
			<td class="number"><strong>' . locale_number_format($TaxTotal+($_SESSION['CreditItems' . $identifier]->total + $_SESSION['CreditItems' . $identifier]->FreightCost),$_SESSION['CreditItems' . $identifier]->CurrDecimalPlaces) . '</strong>
			</td>
			<td>&nbsp;</td>
		</tr>
...
if(isset($_POST['CreditType']) AND($_POST['CreditType']=='WriteOff' OR $_POST['CreditType'] == 'Return' OR $_POST['CreditType']=='ReverseOverCharge')) {
	foreach($_SESSION['CreditItems' . $identifier]->LineItems as $CreditLine) {
		...
		if($myrow1[0] == 0) {
			echo prnMsg(_('The credit quantity for the line for') . ' ' . $CreditLine->StockID . ' ' .('is more than the quantity invoiced. This check is made to ensure that the credit note is not duplicated.'),'error');
			$OKToProcess = false;
			include('includes/footer.php');
			exit;
		}
...
if((isset($_POST['CreditType']) AND $_POST['CreditType']=='WriteOff') AND !isset($_POST['WriteOffGLCode'])) {
	echo prnMsg(_('The GL code to write off the credit value to must be specified. Please select the appropriate GL code for the selection box'),'info');
	$OKToProcess = false;
}
...
/*company record is read in on login and has information on GL Links and debtors GL account*/
	...
	if($_SESSION['CompanyRecord']==0) {
		...
		echo prnMsg(_('The company information and preferences could not be retrieved') . ' - ' . _('Consult your system administrator'),'error');
		include('includes/footer.php');
		exit;
	}
...
/*Could do with setting up a more flexible freight posting schema that looks at the sales type and area of the customer branch to determine where to post the freight recovery */
	...
	echo '<div class="row">' . _('Credit Note number') . ' ' . $CreditNo . ' ' . _('has been processed'),'</div><div class="row">';
	if($_SESSION['InvoicePortraitFormat']==0) {
		echo '<div class="col-xs-4"><a href="' . $RootPath . '/PrintCustTrans.php?FromTransNo=' . $CreditNo . '&InvOrCredit=Credit&PrintPDF=True" class="btn btn-warning">' . _('Print this credit note') . '</a></div>';
	} else {
		echo '<div class="col-xs-4"><a href="' . $RootPath . '/PrintCustTransPortrait.php?FromTransNo=' . $CreditNo . '&InvOrCredit=Credit&PrintPDF=True" class="btn btn-warning">' . _('Print this credit note') . '</a></div>';
	}
	echo '</div>';
/*end of process credit note */
	...
	echo '<br />
		<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Credit Note Type') . '</label>
			<select name="CreditType" tabindex="'.$tabindex++.'" onchange="ReloadForm(Update)" class="form-control">';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Goods returned to location') . '</label>
				<select name="Location" tabindex="'.$tabindex++.'" class="form-control">';
		...
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Write off the cost of the goods to') . '</label>
			<select name="WriteOffGLCode" tabindex="'.$tabindex++.'" class="form-control">';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Sales person'). '</label>';
	...
	if($_SESSION['SalesmanLogin'] != '') {
		...
		echo '<select name="SalesPerson" tabindex="'.$tabindex++.'" class="form-control">';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Credit note text') . '</label>
			<textarea name="CreditText" class="form-control" tabindex="'.$tabindex++.'">' . $_POST['CreditText'] . '</textarea></div>
		</div></div>
		<div class="row">
		<div class="col-xs-4">

			<input name="Update" class="btn btn-info" tabindex="'.$tabindex++.'" type="submit" value="' . _('Update') . '" />
			</div>
			';
	$j++;
	if(!isset($_POST['CreditText'])) {
	...
	echo '<div class="col-xs-4"><input name="ProcessCredit" class="btn btn-success" type="submit" tabindex="'.$tabindex++.'" value="' . _('Process Credit') .'" />
		</div></div><br />
';
}
