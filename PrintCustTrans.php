...
// gather the invoice data
		...
		if (DB_error_no()!=0) {
			...
			echo prnMsg( _('There was a problem retrieving the invoice or credit note details for note number') . ' ' . $InvoiceToPrint . ' ' . _('from the system') . '. ' . _('To print an invoice, the sales order record, the customer transaction record and the branch record for the customer must not have been purged') . '. ' . _('To print a credit note only requires the customer, transaction, salesman and branch records be available'),'error');
			if ($debug==1) {
				echo prnMsg (_('The querry used to get this information that failed was') . '<br />' . $sql,'error');
			}
	...
	if (!isset($FromTransNo) OR $FromTransNo=='') {
		...
		echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Print Invoices or Credit Notes (Landscape Mode)') . '</h1></a></div>';
...
<div class="form-group">
		...
		echo '<p class="text-info"><strong>' . _('The last invoice created was number') . ' ' . $myrow[0] . '</strong><br />' .  '';
		...
		echo '<br /><strong>' . _('The last credit note created was number') . ' ' . $myrow[0] . '</strong><br />'  . '</p>';
		...
		while($FromTransNo <= filter_number_format($_POST['ToTransNo'])) {
			...
			if(DB_num_rows($result)==0 OR DB_error_no()!=0) {
				echo '<p class="text-info">' . _('There was a problem retrieving the invoice or credit note details for note number') . ' ' . $FromTransNo . ' ' . _('from the database') . '. ' . _('To print an invoice, the sales order record, the customer transaction record and the branch record for the customer must not have been purged') . '. ' . _('To print a credit note only requires the customer, transaction, salesman and branch records be available');
				if ($debug==1) {
					echo prnMsg( _('The querry used to get this information that failed was') . '<br />' . $sql, 'warn');
				}
				break;
				include('includes/footer.php');
				exit;
	...
	/* Then there's an invoice (or credit note) to print. So print out the invoice header and GST Number from the company record */
				...
				if( $CustomerLogin == 1 AND $myrow['debtorno'] != $_SESSION['CustomerID']) {
					echo '<p class="text-danger">' . _('This transaction is addressed to another customer and cannot be displayed for privacy reasons') . '. ' . _('Please select only transactions relevant to your company');
					include('includes/footer.php');
					exit;
				}
...
<table id="general-table" class="table table-bordered">
						<tr>
							<td>' . _('Your Order Ref') . '</td>
							<td>' . _('Our Order No') . '</td>
							<td>' . _('Order Date') . '</td>
							<td>' . _('Invoice Date') . '</td>
							<td>' . _('Sales Person') . '</td>
							<td>' . _('Shipper') . '</td>
							<td>' . _('Consignment Ref') . '</td>
						</tr>';
				   	echo '<tr>
