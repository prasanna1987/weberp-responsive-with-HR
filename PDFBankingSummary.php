...
' . _('Select the batch number of receipts to be printed') . '</label>
			<select required="required" class="form-control" autofocus="autofocus" name="BatchNo">';
	...
	echo '
        ...
        <div class="form-group"><br />
				<input type="submit" class="btn btn-info" name="EnterBatchNo" value="' . _('Create PDF') . '" />
			</div>
        </div>
		</div>
		</form>
...
prnMsg(_('An error occurred getting the GL receipts for batch number') . ' ' . $_POST['BatchNo'],'error');
	...
	$pdf->OutputD('nERP' . '_BankingSummary_' . date('Y-m-d').'.pdf');
	$pdf->__destruct();
}
