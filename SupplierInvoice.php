...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' .
	_('Enter Supplier Invoice') . ': ' . $SupplierName . ' ' . $_SESSION['SuppTrans']->SupplierID . '</h1></a></div>';
...
/*Now retrieve supplier information - name, currency, default ex rate, terms, tax rate etc */
	...
	echo prnMsg( _('To enter a supplier invoice the supplier must first be selected from the supplier selection screen'),'warn');
	echo '<br /><p align="center"><a href="' . $RootPath . '/SelectSupplier.php" class="btn btn-default">' . _('Select A Supplier to Enter an Invoice For') . '</a></p><br />';
	include('includes/footer.php');
	exit;
...
if (isset($_GET['ReceivePO']) AND $_GET['ReceivePO']!=''){
	...
	if (! in_array($_SESSION['PageSecurityArray']['GoodsReceived.php'], $_SESSION['AllowedPageSecurityTokens'])){
		echo prnMsg(_('Your permissions do not allow receiving of goods. Automatic receiving of purchase orders is restricted to those only users who are authorised to receive goods/services'),'error');
	} else {
		...
		/*Now Get the next GRN - function in SQL_CommonFunctions*/
			...
			foreach ($_SESSION['PO'.$identifier]->LineItems as $OrderLine) {
				...
				if ($OrderLine->Controlled ==1) { // it's a controlled item - we can't deal with auto receiving controlled items!!!
					echo prnMsg(_('Auto receiving of controlled stock items that require serial number or batch number entry is not currently catered for. Only orders with normal non-serial numbered items can be received automatically'),'error');
...
if (!isset($_POST['PostInvoice'])){
	...
	if (isset($_POST['Shipts']) AND $_POST['Shipts'] == _('Shipments')){
		...
		echo '<div class="row">' . _('You should automatically be forwarded to the entry of invoices against shipments page') .
			'. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh'). ') ' .
			'<a href="' . $RootPath . '/SuppShiptChgs.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.</div><br />';
		exit;
	}
	if (isset($_POST['GL']) AND $_POST['GL'] == _('General Ledger')){
		...
		echo '<div class="row">' . _('You should automatically be forwarded to the entry of invoices against the general ledger page') .
			'. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh'). ') ' .
			'<a href="' . $RootPath . '/SuppTransGLAnalysis.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.</div><br />';
		exit;
	}
	if (isset($_POST['Contracts']) AND $_POST['Contracts'] == _('Contracts')){
		...
		echo '<div class="row">' . _('You should automatically be forwarded to the entry of invoices against contracts page') .
			'. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh'). ') ' .
			'<a href="' . $RootPath . '/SuppContractChgs.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.</div>
			<br />';
		exit;
	}
	if (isset($_POST['FixedAssets'])
		...
		echo '<div class="row">' . _('You should automatically be forwarded to the entry of invoice amounts against fixed assets page') .
			'. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh'). ') ' .
			'<a href="' . $RootPath . '/SuppFixedAssetChgs.php" class="btn btn-default">' . _('click here') . '</a> ' . _('to continue') . '.</DIV><br />';
		exit;
	}
...
<table id="general-table" class="table table-bordered">
	...
	echo '<div class="sub-header"></div><form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post" id="form1">';
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Supplier Invoice Reference') . '</label>
			<input type="text" required="required" pattern=".{1,20}" title="'._('The input should not be blank and should be less than 20 characters').'" placeholder="'._('Within 20 characters needed').'" class="form-control" size="20" maxlength="20" name="SuppReference" value="' . $_SESSION['SuppTrans']->SuppReference . '" /></div></div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Invoice Date') . ' (' . _('in format') . ' ' . $_SESSION['DefaultDateFormat'] . ') </label>
		<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" size="11" maxlength="10" name="TranDate" value="' . $_SESSION['SuppTrans']->TranDate . '" /></div></div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Exchange Rate') . '</label>
		<input class="form-control" maxlength="12" name="ExRate" size="14" type="text" value="' . locale_number_format($_SESSION['SuppTrans']->ExRate,10) . '" /></div>
	</div>
	</div>';
...
<div class="col-xs-4"><input type="submit" name="GRNS" class="btn btn-info" value="' . _('Purchase Orders') . '" /></div>
<div class="col-xs-4"><input type="submit" name="Shipts" class="btn btn-info" value="' . _('Shipments') . '" /></div>
<div class="col-xs-4"><input type="submit" name="Contracts" class="btn btn-info" value="' . _('Contracts') . '" /></div></div><br />

<div class="row">';

	if ( $_SESSION['SuppTrans']->GLLink_Creditors == 1){
		echo '<div class="col-xs-4"><input type="submit" class="btn btn-info" name="GL" value="' . _('General Ledger') . '" /> </div>';
	}
	echo ' <div class="col-xs-4"><input type="submit" class="btn btn-info" name="FixedAssets" value="' . _('Fixed Assets') . '" />
		</div></div>';
...
<table id="general-table" class="table table-bordered">
		...
		echo '<tr>
				<td colspan="7" class="number" style="color:black">' . _('<strong>Total Value of Goods Charged</strong>') . ':</td>
				<td class="number" style="color:black">' . locale_number_format($TotalGRNValue,$_SESSION['SuppTrans']->CurrDecimalPlaces) . '</td>
			</tr>
			</table></div></div></div>';
	}
...
<div class="table-responsive">
<div class="block">
<div class="block-title"><h3>' . _('Shipment Charges') . '</h3></div>
<table id="general-table" class="table table-bordered">
				';
		$TableHeader = '<tr>
							<th>' . _('Shipment') . '</th>
		...
		echo '<tr>
				<td>' . _('Total shipment charges') . ':</td>
				<td>' .  locale_number_format($TotalShiptValue,$_SESSION['SuppTrans']->CurrDecimalPlaces) . '</td>
			</tr>
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Fixed Asset Additions') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
			';
		$TableHeader = '<tr>
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Contract Charges') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
			';
		$TableHeader = '<tr>
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('General Ledger Analysis') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
					';
			$TableHeader = '<tr>
			...
			echo '<tr>
					<td colspan="4">' . _('Total GL Analysis') .  ':</td>
					<td>' .  locale_number_format($TotalGLValue,$_SESSION['SuppTrans']->CurrDecimalPlaces) . '</td>
				</tr>
				</table></div></div></div></div>';
		}
...
<table id="general-table" class="table table-bordered">
				<tr>
					<th>' . _('Amount in supplier currency') . ':</th>
...
<table id="general-table" class="table table-bordered">
	...
	echo '<tr>
			<th>Tax Calulation</th>
			<td><select name="OverRideTax" class="form-control" onchange="ReloadForm(form1.ToggleTaxMethod)">';
	...
	foreach ($_SESSION['SuppTrans']->Taxes as $Tax) {
		...
		if (!isset($_POST['OverRideTax'])
			...
			echo  locale_number_format($_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * 100,$_SESSION['SuppTrans']->CurrDecimalPlaces)  .' % 
			<input type="hidden" name="TaxRate' . $Tax->TaxCalculationOrder . '"  value="' . locale_number_format($_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * 100,$_SESSION['SuppTrans']->CurrDecimalPlaces)  . '" />
			';
...
//		}
			...
			echo  ' 
			' . locale_number_format($_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * 100,$_SESSION['SuppTrans']->CurrDecimalPlaces) . '
			<input type="hidden" name="TaxRate' . $Tax->TaxCalculationOrder . '" value="' . locale_number_format($_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxRate * 100,$_SESSION['SuppTrans']->CurrDecimalPlaces) . '" />%';

			echo '</td>
				<td><input type="text" class="form-control" size="12" maxlength="12" name="TaxAmount'  . $Tax->TaxCalculationOrder . '"  value="' . locale_number_format(round($_SESSION['SuppTrans']->Taxes[$Tax->TaxCalculationOrder]->TaxOvAmount,$_SESSION['SuppTrans']->CurrDecimalPlaces),$_SESSION['SuppTrans']->CurrDecimalPlaces) . '" />';
		}
	...
	echo '<tr>
			<th>' . _('Invoice Total') . ':</th>
			<td colspan="2" class="number"><strong>' . $DisplayTotal . '</strong></td>
		</tr>
		</table></div></div></div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Comments') . '</label>
				<textarea name="Comments" class="form-control">' . $_SESSION['SuppTrans']->Comments . '</textarea></div>
			</div>
		';
	if ($CanSubmit) {
...
<div class="form-group"><br />		
	<input type="submit" class="btn btn-info" name="ToggleTaxMethod" value="' . _('Update Tax Calculation') . '" />
	</div></div>	
			<div class="col-xs-4">
<div class="form-group"><br />
				<input type="submit" name="PostInvoice" value="' . _('Enter Invoice') . '" class="btn btn-success" />
			</div></div>
		<br />
...
then do the updates and inserts to process the invoice entered */
	...
	if ( $TaxTotal + $_SESSION['SuppTrans']->OvAmount < 0){
		...
		$InputError = True;
		echo prnMsg(_('The invoice as entered cannot be processed because the total amount of the invoice is less than  0') . '. ' . _('Invoices are expected to have a positive charge'),'error');
		...
		echo prnMsg(_('The invoice as entered will be processed but be warned the amount of the invoice is  zero!') . '. ' . _('Invoices are normally expected to have a positive charge'),'warn');

	} elseif (mb_strlen( $_SESSION['SuppTrans']->SuppReference)<1){

		$InputError = True;
		echo prnMsg(_('The invoice as entered cannot be processed because the there is no suppliers invoice number or reference entered') . '. ' . _('The supplier invoice number must be entered'),'error');

	} elseif (!Is_date( $_SESSION['SuppTrans']->TranDate)){

		$InputError = True;
		echo prnMsg( _('The invoice as entered cannot be processed because the invoice date entered is not in the format') . ' ' . $_SESSION['DefaultDateFormat'],'error');

	} elseif (DateDiff(Date($_SESSION['DefaultDateFormat']), $_SESSION['SuppTrans']->TranDate, 'd') < 0){

		$InputError = True;
		echo prnMsg(_('The invoice as entered cannot be processed because the invoice date is after today') . '. ' . _('Purchase invoices are expected to have a date prior to or today'),'error');

	}elseif ( $_SESSION['SuppTrans']->ExRate <= 0){

		$InputError = True;
		echo prnMsg( _('The invoice as entered cannot be processed because the exchange rate for the invoice has been entered as a negative or zero number') . '. ' . _('The exchange rate is expected to show how many of the suppliers currency there are in 1 of the local currency'),'error');
		...
		echo prnMsg( _('The invoice total as entered is less than the sum of the shipment charges, the general ledger entries (if any), the charges for goods received, contract charges and fixed asset charges. There must be a mistake somewhere, the invoice as entered will not be processed'),'error');
		$InputError = True;
		...
		if ($myrow[0] == 1){ /*Transaction reference already entered */
			echo prnMsg( _('The invoice number') . ' : ' . $_POST['SuppReference'] . ' ' . _('has already been entered') . '. ' . _('It cannot be entered again'),'error');
			$InputError = True;
		}
	}
	...
	/*Add contract charges records as necessary */
		...
		echo prnMsg(_('Supplier invoice number') . ' ' . $InvoiceNo . ' ' . _('has been processed'),'success');
		echo '<br />
				<div class="row">
					<div class="col-xs-3"></div>
					<div class="col-xs-4"><a href="' . $RootPath . '/SupplierInvoice.php?&SupplierID=' .$_SESSION['SuppTrans']->SupplierID . '" class="btn btn-info">' . _('Enter another Invoice for this Supplier') . '</a></div>
					<div class="col-xs-4">
					<a href="' . $RootPath . '/Payments.php?&SupplierID=' .$_SESSION['SuppTrans']->SupplierID . '&amp;Amount=' . ($_SESSION['SuppTrans']->OvAmount+$TaxTotal) . '" class="btn btn-info">' . _('Enter payment') . '</a></div>
...
if(isset($InputError) AND $InputError==true){ //add a link to return if users make input errors.
	echo '<br /><p align="center"><a href="'.$RootPath.'/SupplierInvoice.php" class="btn btn-default">' . _('Back to Invoice Entry') . '</a></p>';
