...
if(!isset($_SESSION['Items'.$identifier]) OR !isset($_SESSION['Items'.$identifier]->DebtorNo)) {
	echo prnMsg(_('This page can only be read if an order has been entered') . '. ' . _('To enter an order select customer transactions then sales order entry'),'error');
	include('includes/footer.php');
	exit;
}

if($_SESSION['Items'.$identifier]->ItemsOrdered == 0) {
	echo prnMsg(_('This page can only be read if an there are items on the order') . '. ' . _('To enter an order select customer transactions then sales order entry'),'error');
	include('includes/footer.php');
	exit;
}
...
if(isset($_POST['Update'])
	...
	if(mb_strlen($_POST['DeliverTo'])<=1) {
		$InputErrors =1;
		echo prnMsg(_('You must enter the person or company to whom delivery should be made'),'error');
	}
	if(mb_strlen($_POST['BrAdd1'])<=1) {
		$InputErrors =1;
		echo prnMsg(_('You should enter the street address in the box provided') . '. ' . _('Orders cannot be accepted without a valid street address'),'error');
	}
...
//	}
	if(!is_numeric($_POST['FreightCost'])) {
		$InputErrors =1;
		echo prnMsg( _('The freight cost entered is expected to be numeric'),'error');
	}
	if(isset($_POST['MakeRecurringOrder']) AND $_POST['Quotation']==1) {
		$InputErrors =1;
		echo prnMsg( _('A recurring order cannot be made from a quotation'),'error');
	}
	if(($_POST['DeliverBlind'])<=0) {
		$InputErrors =1;
		echo prnMsg(_('You must select the type of packlist to print'),'error');
	}
...
// Check the date is OK
	if(isset($_POST['DeliveryDate']) and !Is_Date($_POST['DeliveryDate'])) {
		$InputErrors =1;
		echo prnMsg(_('An invalid date entry was made') . '. ' . _('The date entry must be in the format') . ' ' . $_SESSION['DefaultDateFormat'],'warn');
	}
// Check the date is OK
...
// Check the date is OK
	if(isset($_POST['ConfirmedDate']) and !Is_Date($_POST['ConfirmedDate'])) {
	 ...
	 elseif(Date1GreaterThanDate2(Date($_SESSION['DefaultDateFormat'],$EarliestDispatch), $_POST['DeliveryDate'])) {
		$InputErrors =1;
		echo '<br /><strong>' . _('The delivery details cannot be updated because you are attempting to set the date the order is to be dispatched earlier than is possible. No dispatches are made on Saturday and Sunday. Also, the dispatch cut off time is') . $_SESSION['DispatchCutOffTime'] . _(':00 hrs. Orders placed after this time will be dispatched the following working day.');
	}
	...
	if($InputErrors==0) {
		...
		if($_SESSION['DoFreightCalc']==True) {
			...
			if( !empty($BestShipper) ) {
				...
				echo prnMsg(_($_POST['FreightCost']),'warn');
			}
		}
		...
		if(DB_num_rows($result)==0) {
			...
			echo prnMsg(_('The branch details for branch code') . ': ' . $_SESSION['Items'.$identifier]->Branch . ' ' . _('against customer code') . ': ' . $_POST['Select'] . ' ' . _('could not be retrieved') . '. ' . _('Check the set up of the customer and branch'),'error');
		...
		if((isset($BestShipper) AND $BestShipper=='') AND ($_POST['ShipVia']=='' OR !isset($_POST['ShipVia']))) {
			...
			if(DB_num_rows($TestShipperExists)==1) {
				...
				if(DB_num_rows($TestShipperExists)>=1) {
					...
					echo prnMsg(_('We have a problem') . ' - ' . _('there are no shippers defined'). '. ' . _('Please use the link below to set up shipping or freight companies') . ', ' . _('the system expects the shipping company to be selected or a default freight company to be used'),'error');
					echo '<div class="row"><div class="col-xs-4"><a href="' . $RootPath . 'Shippers.php" class="btn btn-info">' . _('Enter') . '/' . _('Amend Freight Companies') . '</a></div></div><br />';
				}
			}
...
if(isset($_POST['MakeRecurringOrder']) AND ! $InputErrors) {
	...
	echo prnMsg(_('You should automatically be forwarded to the entry of recurring order details page') . '. ' . _('If this does not happen') . '(' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/RecurringOrders.php?identifier='.$identifier . '&amp;NewRecurringOrder=Yes">' . _('click here') . '</a> '. _('to continue'),'info');
	include('includes/footer.php');
	exit;
}
...
if(isset($_POST['BackToLineDetails']) and $_POST['BackToLineDetails']==_('Modify Order Lines')) {
	...
	echo prnMsg(_('You should automatically be forwarded to the entry of the order line details page') . '. ' . _('If this does not happen') . '(' . _('if the browser does not support META Refresh') . ') ' . '<a href="' . $RootPath . '/SelectOrderItems.php?identifier='.$identifier . '">' . _('click here') . '</a> '. _('to continue'),'info');
	include('includes/footer.php');
	exit;
...
if(isset($_POST['ProcessOrder'])) {
	...
	if($_POST['FreightCost'] != $OldFreightCost AND $_SESSION['DoFreightCalc']==True) {
		$OK_to_PROCESS = 0;
		echo prnMsg(_('The freight charge has been updated') . '. ' . _('Please reconfirm that the order and the freight charges are acceptable and then confirm the order again if OK') .' <br /> '. _('The new freight cost is') .' ' . $_POST['FreightCost'] . ' ' . _('and the previously calculated freight cost was') .' '. $OldFreightCost,'warn');
	} else {
...
/* finally write the order header to the database and then the order line details */
	...
	if($_SESSION['Items'.$identifier]->Quotation==1) {
		echo prnMsg(_('Quotation Number') . ' ' . $OrderNo . ' ' . _('has been entered'),'success');
	} else {
...
echo '<br /><div class="row">';
		if($_POST['Quotation']==0) { /*then its not a quotation its a real order */
			...
			echo '
					<div class="col-xs-4">
					' . ' ' . '<a target="_blank" href="' . $RootPath . '/PrintCustOrder_generic.php?identifier='.$identifier . '&amp;TransNo=' . $OrderNo . '" class="btn btn-warning">' . _('Print packing slip') . '' . '</a></div>
				';

			echo '<div class="col-xs-4">' . ' ' . '<a href="' . $RootPath . '/ConfirmDispatch_Invoice.php?identifier='.$identifier . '&amp;OrderNumber=' . $OrderNo .'" class="btn btn-success">' . _('Invoice') . '</a></div>
				';

		} else {
			/*link to print the quotation */
			
			echo '<div class="col-xs-4">' . ' ' . '<a href="' . $RootPath . '/PDFQuotationPortrait.php?identifier='.$identifier . '&amp;QuotationNo=' . $OrderNo . '" target="_blank" class="btn btn-warning">' .  _('Print Quotation')  . '</a></div><br />
					';
		}
		echo '<div class="col-xs-4">' . ' ' . '<a href="'. $RootPath .'/SelectOrderItems.php?identifier='.$identifier . '&amp;NewOrder=Yes" class="btn btn-info">' .  _('Back to Sales Orders')  . '</a></div>
				</div>
				<br />
';
	} else {
		/*its a customer logon so thank them */
		echo prnMsg(_('Thank you for your business'),'success');
	}
...
/* update the order header then update the old order line details and insert the new lines */
	...
	if($_SESSION['Items'.$identifier]->Quotation==0) {//now its being changed? to an order
		...
		if(DB_num_rows($ContractResult)==1) {//then it is a contract quotation being changed to an order
			...
			if(is_null($CostRow[0]) OR $CostRow[0]==0) {
				$Cost =0;
				echo prnMsg(_('In automatically creating a work order for') . ' ' . $ContractRow['contractref'] . ' ' . _('an item on this sales order, the cost of this item as accumulated from the sum of the component costs is nil. This could be because there is no bill of material set up ... you may wish to double check this'),'warn');
			} else {
	...
	if($Quotation) {//handle Quotations and Orders print after modification
		echo prnMsg(_('Quotation Number') .' ' . $_SESSION['ExistingOrder'.$identifier] . ' ' . _('has been updated'),'success');

		/*link to print the quotation */
		echo '<br /><div class="row">';
		echo '<div class="col-xs-4">' . ' ' . '<a href="' . $RootPath . '/PDFQuotationPortrait.php?identifier='.$identifier . '&amp;QuotationNo=' . $_SESSION['ExistingOrder'.$identifier] . '" target="_blank" class="btn btn-warning">' .  _('Print Quotation')  . '</a></div>
				</div>';
	} else {

	echo prnMsg(_('Order Number') .' ' . $_SESSION['ExistingOrder'.$identifier] . ' ' . _('has been updated'),'success');

	echo '<br />
			<div class="row">

			';
	echo '<div class="col-xs-3"><a  target="_blank" href="' . $RootPath . '/PrintCustOrder_generic.php?identifier='.$identifier  . '&amp;TransNo=' . $_SESSION['ExistingOrder'.$identifier] . '" class="btn btn-warning">' .  _('Print packing slip') . ''  . '</a></div>
		';
	echo '<div class="col-xs-3"><a href="' . $RootPath .'/ConfirmDispatch_Invoice.php?identifier='.$identifier  . '&amp;OrderNumber=' . $_SESSION['ExistingOrder'.$identifier] . '" class="btn btn-success">' .  _('Invoice')  . '</a></div>
		';
	echo '<div class="col-xs-3"><a href="' . $RootPath .'/SelectSalesOrder.php?identifier='.$identifier   . '" class="btn btn-info">' .  _('Back to Sales Orders')  . '</a></div>
		</div><br />
...
if(isset($_SESSION['Items'.$identifier]->SpecialInstructions) and mb_strlen($_SESSION['Items'.$identifier]->SpecialInstructions)>0) {
	echo prnMsg($_SESSION['Items'.$identifier]->SpecialInstructions,'info');
}
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Delivery Details') . '';

echo '<br /><small>' . ' ' . _('Customer Code') . ' :<strong> ' . $_SESSION['Items'.$identifier]->DebtorNo . '<br />';
echo '</strong>' . _('Customer Name') . ' :<strong> ' . $_SESSION['Items'.$identifier]->CustomerName . '</strong></small></h1></a></div>
';
...
<table id="general-table" class="table table-bordered">';
	...
	echo '<tr class="striped_row">
			<td colspan="6" class="number"><strong>' .  _('TOTAL Excl Tax/Freight')  . '</strong></td>
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Order Summary') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
	<tr>
		<th>' .  _('Item Description')  . '</th>
...
echo '<br />
	<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' .  _('Deliver To') .'</label>
		<input type="text" autofocus="autofocus" class="form-control" required="required" size="42" maxlength="40" name="DeliverTo" value="' .  stripslashes($_SESSION['Items' . $identifier]->DeliverTo) . '" title="' . _('Enter the name of the customer to deliver this order to') . '" /></div>
	</div>';

echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">', _('Deliver from'), '</label>
...
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Estimated Delivery Date') .'</label>
		<input class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" type="text" size="11" maxlength="10" name="DeliveryDate" value="' . $_SESSION['Items'.$identifier]->DeliveryDate . '" title="' . _('Enter the estimated delivery date requested by the customer') . '"/></div>
	</div></div>';
// The date when a quote was issued to the customer
echo '<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Quote Date') .'</label>
		<input class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" type="text" size="11" maxlength="10" name="QuoteDate" value="' . $_SESSION['Items'.$identifier]->QuoteDate . '" /></div>
	</div>';
// The date when the customer confirmed their order
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Confirmed Order Date') .'</label>
		<input class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" type="text" size="11" maxlength="10" name="ConfirmedDate" value="' . $_SESSION['Items'.$identifier]->ConfirmedDate . '" /></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Delivery Address 1 (Street)') . '</label>
		<input type="text" size="42" class="form-control" maxlength="40" name="BrAdd1" value="' . $_SESSION['Items'.$identifier]->DelAdd1 . '" /></div>
	</div>
	</div>
	<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Delivery Address 2 (Street)') . '</label>
		<input type="text" size="42" class="form-control" maxlength="40" name="BrAdd2" value="' . $_SESSION['Items'.$identifier]->DelAdd2 . '" /></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Delivery Address 3 (Suburb/City)') . '</label>
		<input type="text" size="42" class="form-control" maxlength="40" name="BrAdd3" value="' . $_SESSION['Items'.$identifier]->DelAdd3 . '" /></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Delivery Address 4 (State/Province)') . '</label>
		<input type="text" class="form-control" size="42" maxlength="40" name="BrAdd4" value="' . $_SESSION['Items'.$identifier]->DelAdd4 . '" /></div>
	</div>
	</div>
	<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Delivery Address 5 (Postal Code)') . '</label>
		<input type="text" size="42" class="form-control" maxlength="40" name="BrAdd5" value="' . $_SESSION['Items'.$identifier]->DelAdd5 . '" /></div>
	</div>';
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Country') . '</label>
		<select name="BrAdd6" class="form-control">';
...
echo'	<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' .  _('Contact Phone Number') .'</label>
		<input type="tel" size="25" maxlength="25" class="form-control" required="required" name="PhoneNo" value="' . $_SESSION['Items'.$identifier]->PhoneNo . '" title="' . _('Enter the telephone number of the contact at the delivery address.') . '" /></div>
	</div>
	</div>
	<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Contact Email') . '</label>
		<input type="email" class="form-control" size="40" maxlength="38" name="Email" value="' . $_SESSION['Items'.$identifier]->Email . '" title="' . _('Enter the email address of the contact at the delivery address') . '" /></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Customer Reference') .'</label>
		<input type="text" size="25" class="form-control"  maxlength="50" name="CustRef" value="' . $_SESSION['Items'.$identifier]->CustRef . '" title="' . _('Enter the customer\'s purchase order reference relevant to this order') . '" /></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Comments') .'</label>
		<textarea name="Comments" class="form-control">' . $_SESSION['Items'.$identifier]->Comments  . '</textarea></div>
	...
	if($CustomerLogin  == 1) {
		...
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Sales person'). '</label>
		...
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Packinglist Type') . '</label>
				<select name="DeliverBlind" class="form-control">';
		...
		if(isset($_SESSION['PrintedPackingSlip']) AND $_SESSION['PrintedPackingSlip']==1) {
			...
			echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Reprint packing slip') .'</label>
				<select name="ReprintPackingSlip" class="form-control">';
			echo '<option value="0">' . _('Yes') . '</option>';
			echo '<option selected="selected" value="1">' . _('No') . '</option>';
			echo '</select>	'. _('Last printed') .': ' . ConvertSQLDate($_SESSION['DatePackingSlipPrinted']) . '</div></div>';
		} else {
			echo '<div class="col-xs-4">
<div class="form-group"> <input type="hidden" name="ReprintPackingSlip" value="0" /></div></div>';
		}
		...
		//echo '
//				<div class="col-xs-4">
//<div class="form-group"> <label class="col-md-12 control-label">' .  _('Freight Cost(Excl.Tax)') .'</label>
//				<input type="text" class="form-control" size="10" maxlength="12" name="FreightCost" value="' . $_SESSION['Items'.$identifier]->FreightCost . '" /></div>';
//
//		if($_SESSION['DoFreightCalc']==true) {
//			echo '<input type="submit" class="btn btn-info" name="Update" value="' . _('Recalc Freight Cost') . '" />';
//		}
//		echo '</div>';
		...
		echo '<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Shipper') .'</label>
				<select name="ShipVia" class="form-control">';
		...
		echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' .  _('Quotation Only') .'</label>
				<select name="Quotation" class="form-control">';
		if($_SESSION['Items'.$identifier]->Quotation==1) {
...
echo '<div class="row">
<div class="col-xs-4">
<input type="submit" name="BackToLineDetails" value="' . _('Modify Order Lines') . '" class="btn btn-info" /></div>';

if($_SESSION['ExistingOrder'.$identifier]==0) {
	echo '
	<div class="col-xs-4"><input type="submit" name="ProcessOrder" class="btn btn-success" value="' . _('Submit') . '" /></div>';
	echo '<div class="col-xs-4"><input type="submit" name="MakeRecurringOrder" class="btn btn-success" value="' . _('Create Recurring Order') . '" /></div>
';
} else {
	echo '
	<div class="col-xs-4"><input type="submit" name="ProcessOrder" class="btn btn-success" value="' . _('Submit') . '" /></div>
