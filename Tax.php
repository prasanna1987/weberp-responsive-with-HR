...
if (isset($_POST['TaxAuthority']) and isset($_POST['PrintPDF']) and isset($_POST['NoOfPeriods']) and isset($_POST['ToPeriod'])) {
	...
	if (DB_error_no() != 0) {
		...
		echo prnMsg(_('The accounts receivable transaction details could not be retrieved because') . ' ' . DB_error_msg(), 'error');
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=GL" class="btn btn-default">' . _('Back to Menu') . '</a></p>';
		if ($debug == 1) {
			echo '<br />' . $SQL;
		}
	...
	if ($SalesCount + $PurchasesCount + $PettyCashCount == 0) {
		$Title = _('Taxation Reporting Error');
		include('includes/header.php');
		echo prnMsg(_('There are no tax entries to list'), 'info');
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=GL" class="btn btn-default">' . _('Back to Menu') . '</a></p>';
		include('includes/footer.php');
		exit;
	} else {
	...
	include('includes/header.php');
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Tax Reporting') . '</h1></a></div>';
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '" method="post">';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Tax Authority') . '</label>
			<select name="TaxAuthority" class="form-control">';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Period') . '</label>
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Ending On') . '</label>
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Report Type') . '</label>
				...
				<select name="DetailOrSummary" class="form-control">
					<option value="Detail">' . _('Detailed') . '</option>
