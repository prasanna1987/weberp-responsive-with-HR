...
if (isset($_POST['PrintPDF'])
	...
	if (DB_error_no() !=0) {
		...
		echo prnMsg(_('The Supplier details could not be retrieved by nERP because') .  ' ' . DB_error_msg(),'error');
		echo '<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/menu_data.php?Application=PO" class="btn btn-default">' . _('Back to Menu') . '</a></div></div><br />';
	...
	while ($AgedAnalysis = DB_fetch_array($SupplierResult)){
		...
		if ($_POST['DetailedReport']=='Yes'){
		   ...
		   $sql = "SELECT systypes.typename,
			...
			include('includes/header.php');
			echo prnMsg(_('The details of outstanding transactions for Supplier') . ' - ' . $AgedAnalysis['supplierid'] . ' ' . _('could not be retrieved because') . ' - ' . DB_error_msg(),'error');
			echo '<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/menu_data.php?Application=PO" class="btn btn-default">' . _('Back to Menu') . '</a></div></div><br />';
	...
	if ($ListCount == 0) {
		...
		include('includes/header.php');
		echo prnMsg(_('There are no results so the PDF is empty'),'info');
		include('includes/footer.php');
	} else {
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1> ' . ' ' . $Title . '</h1></a></div><br />';
...
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('From Supplier Code') . '</label>
				<input tabindex="1" class="form-control" type="text" required="required"  autofocus="autofocus" maxlength="6" size="7" name="FromCriteria" value="1" title+"' . _('Enter the first supplier code alphabetially to include in the report') . '" /></div>
			</div>
			<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('To Supplier Code') . '</label>
				<input tabindex="2" type="text" class="form-control" maxlength="6" size="7" name="ToCriteria" required="required" value="zzzzzz" title="' . _('Enter the last supplier code alphabetically to include in the report') . '" /></div>
			</div>
			<div class="col-xs-4">
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Currency') . '' . '</label>
				<select tabindex="4" name="Currency" class="form-control">';
...
<div class="form-group "> <label class="col-md-8 control-label">' . _('Report Type') . '' . '</label>
				<select tabindex="5" name="DetailedReport" class="form-control">
...
<div class="form-group"> <br />
				<input tabindex="6" class="btn btn-warning" type="submit" name="PrintPDF" value="' . _('Print PDF') . '" />
			</div>
            </div>
			</div>
            </form>
