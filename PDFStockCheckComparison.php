...
/*First off do the Inventory Comparison file stuff */
	if ($_POST['ReportOrClose']=='ReportAndClose'){
		...
		if (DB_error_no() !=0) {
			...
			echo '<br />';
			echo prnMsg( _('The inventory check file could not be retrieved because'). ' - ' . DB_error_msg(),'error');
		...
		while ($myrow = DB_fetch_array($StockChecks)){
			...
			if (DB_error_no() !=0) {
				...
				include('includes/header.php');
				echo '<br />';
				echo prnMsg( _('The inventory counts file could not be retrieved because'). ' - ' . DB_error_msg(). 'error');
				echo '<br /><p align="right"><a href="' . $RootPath . '/index.php" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
	...
	if (DB_num_rows($CheckedItems)==0){
		...
		echo prnMsg(_('There is no inventory check data to report on'), 'warn');
		echo '<p class="text-info">' .  _('To start an inventory check first run the'). ' <a href="' . $RootPath . '/StockCheck.php">' .  _('inventory check sheets') . '</a> - '. _('and select the option to create new Inventory Comparison data file');
		echo '</p>';
		include('includes/footer.php');
		exit;
	}
	...
	While ($CheckItemRow = DB_fetch_array($CheckedItems)){
		...
		if (DB_error_no() !=0) {
			...
			echo prnMsg( _('The inventory counts could not be retrieved by the SQL because').' - ' . DB_error_msg(), 'error');
	   		echo '<br /><p align="right"><a href="' . $RootPath . '/index.php" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
    ...
    $pdf->OutputD('nERP' . '_StockComparison_' . date('Y-m-d') . '.pdf');
    $pdf->__destruct();
...
<div class="col-xs-12">';
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Choose Option'). '</label>
             <select name="ReportOrClose" class="form-control">';
...
<div class="form-group"> <label class="col-md-8 control-label">' .  _('Action for Zero Counts') . '</label>
             <select name="ZeroCounts" class="form-control">';
...
<input type="submit" name="PrintPDF" class="btn btn-warning" value="' . _('Print PDF'). '" /></div>';
    echo '</div>
	</div>
          </form></div></div>';
