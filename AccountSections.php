...
if(isset($_POST['submit'])) {
	...
	if(isset($_POST['SectionID'])) {
		...
		if((DB_num_rows($result)!=0 AND !isset($_POST['SelectedSectionID']))) {
			$InputError = 1;
			echo prnMsg( _('The account section already exists in the System'),'error');
			$Errors[$i] = 'SectionID';
			$i++;
		}
	}
	if(ContainsIllegalCharacters($_POST['SectionName'])) {
		$InputError = 1;
		echo prnMsg( _('The account section name cannot contain any special characters') ,'error');
		$Errors[$i] = 'SectionName';
		$i++;
	}
	if(mb_strlen($_POST['SectionName'])==0) {
		$InputError = 1;
		echo prnMsg( _('The account section name must contain at least one character') ,'error');
		$Errors[$i] = 'SectionName';
		$i++;
	}
	if(isset($_POST['SectionID']) AND (!is_numeric($_POST['SectionID']))) {
		$InputError = 1;
		echo prnMsg( _('The section number must be an integer'),'error');
		$Errors[$i] = 'SectionID';
		$i++;
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'accountgroups'
	...
	if($myrow['sections']>0) {
		echo prnMsg( _('Cannot delete this account section because general ledger accounts groups have been created using this section'),'warn');
		echo '<div>',
		...
		$result = DB_query($sql);
		echo prnMsg( $SectionName . ' ' . _('section has been deleted') . '!','success');
...
/*	An account section could be posted when one has been edited and is being updated
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1> ', // Icon title.
		_('Account Sections'), '</h1></a></div>';// Page title.
...
<table id="general-table" class="table table-bordered">
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		echo '<tr class="striped_row">
				...
				<td class="noprint"><a href="', htmlspecialchars($_SERVER['PHP_SELF'].'?SelectedSectionID='.urlencode($myrow['sectionid']), ENT_QUOTES, 'UTF-8'), '" class="btn btn-info">', _('Edit'), '</a></td>
				<td class="noprint">';
		if( $myrow['sectionid'] == '1' or $myrow['sectionid'] == '2' ) {
			echo '<strong><span class="text-danger">', _('Restricted'), '</span></strong>';
		} else {
			echo '<a href="', htmlspecialchars($_SERVER['PHP_SELF'].'?SelectedSectionID='.urlencode($myrow['sectionid']).'&delete=1', ENT_QUOTES, 'UTF-8'), '" class="btn btn-danger">', _('Delete'), '</a>';
		}
		echo '</td>
			</tr>';
	} //END WHILE LIST LOOP
...
if(isset($_POST['SelectedSectionID']) or isset($_GET['SelectedSectionID'])) {
	echo '<br /><div class="row"><div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '" class="btn btn-info">' . _('Review Account Sections') . '</a></div></div><br />';
}
...
if(! isset($_GET['delete'])) {
	...
	if(isset($_GET['SelectedSectionID'])) {
		...
		$result = DB_query($sql);
		if( DB_num_rows($result) == 0 ) {
			echo prnMsg( _('Could not retrieve the requested section please try again.'),'warn');
			unset($_GET['SelectedSectionID']);
		} else {
			$myrow = DB_fetch_array($result);
			...
			echo '<input name="SelectedSectionID" type="hidden" value="', $_POST['SectionID'], '" />
				<div class="block">
						...
						<div class="block-title"><h3>', _('Edit Account Section Details'), '</h3></div>
					<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">', _('Section Number'), '</label>
						', $_POST['SectionID'], '</div>
					</div>';
		}
		...
		echo '<div class="block">
						...
						<div class="block-title"><h3>', _('New Account Section Details'), '</h3>
				</div>
			<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Section Number'), '</label>
					<input autofocus="autofocus" ',
						( in_array('SectionID',$Errors) ? 'class="inputerror number"' : 'class="form-control" ' ),
						'maxlength="4" name="SectionID" required="required" size="4" tabindex="1" type="text" value="', $_POST['SectionID'], '" /></div>
				</div>';
	}
	echo	'<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">', _('Section Description'), '</label>
				<input ',
					( in_array('SectionName',$Errors) ? 'class="inputerror text" ' : 'class="form-control" ' ),
					'maxlength="30" name="SectionName" required="required" size="30" tabindex="2" type="text" value="', $_POST['SectionName'], '" /></div>
			</div>
			<div class="col-xs-4">
<div class="form-group"> <br /><input name="submit" class="btn btn-info" tabindex="3" type="submit" value="', _('Enter Information'), '" /></div>
			</div>
		</div>
		</div>
		</form>';
