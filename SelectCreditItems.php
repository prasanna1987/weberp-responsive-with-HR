...
prnMsg(_('This credit note has already been processed. Refreshing the page will not enter the credit note again') . '<br />' . _('Please use the navigation links provided rather than using the browser back button and then having to refresh'),'info');
	//echo '<p align="right"><a href="' . $RootPath . '/menu_data.php?" class="btn btn-default">' . _('Back to Menu') . '</a></p>';
...
if (isset($_POST['SearchCust']) AND $_SESSION['RequireCustomerSelection']==1){
	...
	if ($_POST['Keywords'] AND $_POST['CustCode']) {
		  echo    
prnMsg( _('Customer name keywords have been used in preference to the customer code extract entered'), 'info' ),'</div>';
	}
	if ($_POST['Keywords']=='' AND $_POST['CustCode']=='') {
		  echo    
prnMsg( _('At least one Customer Name keyword OR an extract of a Customer Code must be entered for the search'), 'info' ),'</div>';
	} else {
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Select Customer For Credit Note') . '</h1></a></div>';
...
<div class="col-xs-12">';
	...
	echo '<div class="row">
		<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Customer name-part or full') . '</label>
			<input type="text" class="form-control" name="Keywords" size="20" maxlength="25" /></div>
			</div>
			<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Customer code-part or full') . '</label>
			<input type="text" class="form-control" name="CustCode" size="15" maxlength="18" /></div>
		</div>';
	echo '<div class="col-xs-4">
        <div class="form-group"> <br />
			<input type="submit" class="btn btn-success" name="SearchCust" value="' . _('Search') . '" />
		</div></div>
		</div>
		';
...
<table id="general-table" class="table table-bordered">';
		...
		while ($myrow=DB_fetch_array($result_CustSelect)) {
			...
			if ($LastCustomer != $myrow['name']) {
				echo '<tr><td colspan="4"><h4 align="center">For Customer : <strong>' . $myrow['name'] . '</strong></h4></td></tr>';
			} else {
				echo '<tr><td colspan="4"></td></tr>';
			}
			echo '<tr>
				<td><input tabindex="'.($j+5).'" type="submit" class="btn btn-info" name="SubmitCustomerSelection' . $j .'" value="' . htmlspecialchars($myrow['brname'], ENT_QUOTES,'UTF-8'). '" />
		...
		echo '</table></div></div></div><input type="hidden" name="JustSelectedACustomer" value="Yes" />';
...
/* everything below here only do if a customer is selected
   first add a header to show who we are making a credit note for */
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . $_SESSION['CreditItems'.$identifier]->CustomerName  . ' - ' . $_SESSION['CreditItems'.$identifier]->DeliverTo . '</h1></a></div>';
...
prnMsg(_('There are no products available that match the criteria specified'),'info');
			   if ($debug==1){
				    echo    
prnMsg(_('The querry statement used was') . ':<br />' . $SQL,'info');
			   }
		}
	     ...
	     do {
		   ...
		   is not already on this credit note */
			    ...
			    if ($_SESSION['SO_AllowSameItemMultipleTimes']==0 AND strcasecmp($OrderItem->StockID, $_POST['NewItem']) == 0) {
				     ...
				     echo   prnMsg($_POST['NewItem'] . ' ' . _('is already on this credit - the system will not allow the same item on the credit note more than once. However you can change the quantity credited of the existing line if necessary'),'warn');
			    }
		   ...
		   if ($AlreadyOnThisCredit!=1){
				...
				$result1 = DB_query($sql,$ErrMsg);
		   		...
		   		if ($myrow = DB_fetch_array($result1)){
					...
					echo   prnMsg( $_POST['NewItem'] . ' ' . _('does not exist in the system and cannot therefore be added to the credit note'),'warn');
			   	}
		...
		foreach ($_SESSION['CreditItems'.$identifier]->LineItems as $LineItem) {
			...
			if (isset($_POST['Quantity_' . $LineItem->LineNumber])){
				...
				if ($Quantity<0 OR $Price <0 OR $DiscountPercentage >100 OR $DiscountPercentage <0){
					echo   prnMsg(_('The item could not be updated because you are attempting to set the quantity credited to less than 0 or the price less than 0 or the discount more than 100% or less than 0%'),'warn');
...
/* get the item details from the database and hold them in the cart object make the quantity 1 by default then add it to the cart */
			   ...
			   is not already on this credit note */
					...
					if ($_SESSION['SO_AllowSameItemMultipleTimes']==0 AND strcasecmp($OrderItem->StockID, $_POST['NewItem']) == 0) {
					     $AlreadyOnThisCredit = 1;
					     echo   prnMsg(_('The item selected is already on this credit the system will not allow the same item on the credit note more than once. However you can change the quantity credited of the existing line if necessary.'),'warn');
				    }
			   } /* end of the foreach loop to look for preexisting items of the same code */
...
<table id="general-table" class="table table-bordered">
		  ...
		  foreach ($_SESSION['CreditItems'.$identifier]->LineItems as $LineItem) {
			   ...
			   if ($LineItem->Controlled==0){
			   	echo '<td><input type="text" class="form-control" name="Quantity_' . $LineItem->LineNumber . '" maxlength="8" size="6" value="' . locale_number_format(round($LineItem->Quantity,$LineItem->DecimalPlaces),$LineItem->DecimalPlaces) . '" /></td>';
			   } else {
				echo '<td><a href="' . $RootPath . '/CreditItemsControlled.php?LineNo=' . $LineItem->LineNumber . '&identifier=' . $identifier . '">' . locale_number_format($LineItem->Quantity,$LineItem->DecimalPlaces) . '</a>
			...
			<td><input type="text" class="form-control" name="Price_' . $LineItem->LineNumber . '" size="10" maxlength="12" value="' . locale_number_format($LineItem->Price,$_SESSION['CreditItems'.$identifier]->CurrDecimalPlaces) . '" /></td>
			<td><input type="checkbox" name="Gross" value="false" /></td>
			<td><input type="text" class="form-control" name="Discount_' . $LineItem->LineNumber . '" size="3" maxlength="3" value="' . locale_number_format(($LineItem->DiscountPercent * 100),'Variable') . '" />%</td>
			...
			foreach ($LineItem->Taxes AS $TaxKey=>$Tax) {
				...
				echo '<input type="text" class="form-control" name="' . $LineItem->LineNumber . $TaxKey . '_TaxRate" maxlength="4" size="4" value="' . locale_number_format($Tax->TaxRate*100,'Variable') . '" />';
				$i++;
				if ($Tax->TaxOnTax ==1){
			...
			echo '<td class="number">' . $DisplayTaxAmount . '</td>
				...
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier=' . $identifier . '&Delete=' . $LineItem->LineNumber . '" class="btn btn-danger" onclick="return confirm(\'' . _('Are you sure you wish to delete this line item from the credit note?') . '\');">' . _('Delete') . '</a></td>
				</tr>';

			echo '<tr class="striped_row">
					<td colspan="11"><textarea class="form-control"  name="Narrative_' . $LineItem->LineNumber . '">' . $LineItem->Narrative . '</textarea></td>
				</tr>';
		...
		echo '<td colspan="2" class="number">' .  _('Credit Freight') . '</td>
			<td><input type="text" class="form-control" size="6" maxlength="6" name="ChargeFreightCost" value="' . locale_number_format($_SESSION['CreditItems'.$identifier]->FreightCost,$_SESSION['CreditItems'.$identifier]->CurrDecimalPlaces) . '" /></td>';
		...
		foreach ($_SESSION['CreditItems'.$identifier]->FreightTaxes as $FreightTaxLine) {
			...
			echo  '<input type="text" class="form-control" name=FreightTaxRate' . $FreightTaxLine->TaxCalculationOrder . ' maxlength="4" size="4" value="' . locale_number_format(($FreightTaxLine->TaxRate * 100),'Variable') . '" />';
...
<table id="general-table" class="table table-bordered">
				...
				<tr>
					...
					<td><select name="CreditType" class="form-control" onchange="ReloadForm(MainForm.Update)" >';
...
/*if the credit note is a return of goods then need to know which location to receive them into */
			...
			echo '<tr><td>' . _('Write off the cost of the goods to') . '</td>
					<td><select name="WriteOffGLCode" class="form-control">';
		...
		echo '<tr>
				...
				<td><select name="SalesPerson" class="form-control">';
		...
		echo '</select></td>
		  ...
		  echo '<tr><td>' . _('Credit Note Text') . ' :</td>
		  		<td><textarea name="CreditText" class="form-control">' . $_POST['CreditText'] . '</textarea></td>
			</tr>
			</table></div></div></div>';
...
prnMsg (_('The GL code to write off the credit value to must be specified. Please select the appropriate GL code for the selection box'),'info');
		  ...
		  echo '<div class="row">
		  <div class="col-xs-4">
				<input type="submit" class="btn btn-info" name="Update" value="' . _('Update') . '" /></div>
				';
		  if (!isset($_POST['ProcessCredit']) AND $OKToProcess == true){
			echo '<div class="col-xs-4"><input type="submit" class="btn btn-success" name="ProcessCredit" value="' . _('Process Credit Note') . '" />
					</div>';
		  }
		  echo '<div class="col-xs-4">
				<input type="submit" class="btn btn-danger" name="CancelCredit" value="' . _('Cancel') . '" onclick="return confirm(\'' . _('Are you sure you wish to cancel the whole of this credit note?') . '\');" /></div>
...
/* Now show the stock item selection search stuff below */
	 ...
	 if (isset($_POST['PartSearch']) AND $_POST['PartSearch']!='' AND !isset($_POST['ProcessCredit'])){
        ...
        <div class="form-group"> <label class="col-md-8 control-label">
				...
				' . _('Select a stock category') . '</label><select name="StockCat" class="form-control">';
		 ...
		 echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Enter text extracts in the description') . '</label>';
		 echo '<input type="text" class="form-control" name="Keywords" size="20" maxlength="25" value="' . $_POST['Keywords'] . '" /></div></div>';
		
		 echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Enter extract of the Stock Code') . '</label>';
		 echo '<input type="text" class="form-control" name="StockCode" size="15" maxlength="18" value="' . $_POST['StockCode'] . '" /></div>';
		 echo '</div>';
		 echo '</div>
        ...
        <div class="form-group"> 
		 ...
		 echo '<input type="submit" name="Search" class="btn btn-success" value="' . _('Search') .'" /></div></div>
				<div class="col-xs-4">
        <div class="form-group"> <input type="submit" class="btn btn-info" name="ChangeCustomer" value="' . _('Change Customer') . '" />
		</div>
		</div>
		<div class="col-xs-4">
        <div class="form-group"> 
				<input type="submit" class="btn btn-info" name="Quick" value="' . _('Quick Entry') . '" />
				</div>
				</div>
				</div>
				';
			  ...
			  while ($myrow=DB_fetch_array($SearchResult)) {
				...
				if (extension_loaded('gd') && function_exists('gd_info') && file_exists ($imagefile) ) {
					...
					printf('<tr class="striped_row">
							<td><input type="submit" name="NewItem" class="btn btn-info" value="%s" /></td>
							<td>%s</td>
							<td>%s</td>
							<td>' . $ImageSource . '</td>
							</tr>',
...
/*FORM VARIABLES TO POST TO THE CREDIT NOTE 10 AT A TIME WITH PART CODE AND QUANTITY */
		 ...
		 <div class="col-xs-8">
		 <div class="block">
		 <div class="block-title"><h2>Quick Entry</h2></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
';
	     ...
	     echo '<tr>
	      ...
	      for ($i=1;$i<=$_SESSION['QuickEntries'];$i++){
	     	...
	     	echo '<tr class="striped_row">
					<td><input type="text" class="form-control" name="part_' . $i . '" size="21" maxlength="20" /></td>
					<td><input type="text" class="form-control" name="qty_' . $i . '" size="6" maxlength="6" /></td>
				</tr>';
	     }
	     ...
	     echo '</table>
				...
				<div class="row">
				<div class="col-xs-4">
				<input type="submit" class="btn btn-success" name="QuickEntry" value="' . _('Submit') . '" />
				</div>
				<div class="col-xs-4">
				<input type="submit" class="btn btn-info" name="PartSearch" value="' . _('Search') . '" />
				</div>
				</div>';
...
echo '<div class="col-xs-12">';
	 echo '<p><span class="text-success"><strong>'. _('Credit Note number') . ' ' . $CreditNo . ' ' . _('processed') . '</strong></span>  -  ';
	 echo '<a target="_blank" href="' . $RootPath . '/PrintCustTrans.php?FromTransNo=' . $CreditNo . '&InvOrCredit=Credit" class="btn btn-info">' . _('Show this Credit Note') . '</a></p></div><br />';
	if ($_SESSION['InvoicePortraitFormat']==0){
		echo '<div class="col-xs-4">';
	 	echo '<a href="' . $RootPath . '/PrintCustTrans.php?FromTransNo=' . $CreditNo . '&InvOrCredit=Credit&PrintPDF=True" class="btn btn-info">' . _('Print this Credit Note') . '</a></div>';
	} else {
		echo '<div class="col-xs-4">';
		echo '<a href="' . $RootPath . '/PrintCustTransPortrait.php?FromTransNo=' . $CreditNo . '&InvOrCredit=Credit&PrintPDF=True" class="btn btn-info">' . _('Print this Credit Note') . '</a></div>';
	}
	echo '<div class="col-xs-4">';
	 echo '<a href="' . $RootPath . '/SelectCreditItems.php" class="btn btn-success">' . _('Enter Another Credit Note') . '</a></div>';
	 echo '</div></div>';
