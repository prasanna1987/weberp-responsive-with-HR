...
if (empty($JournalNo)) {
	...
	echo  prnMsg(_('This page should be called with Journal No'),'error');
...
if ($JournalNo=='Preview') {
	...
	$sql="SELECT gltrans.typeno,
			...
			WHERE gltrans.typeno='" . $JournalNo . "'";
...
if ($LineCount == 0) {   //UldisN
	...
	include('includes/header.php');
	echo   prnMsg(_('There were no Journals to print'),'warn');
    ...
    $pdf->OutputD('nERP' . '_Journal_' . date('Y-m-d').'.pdf');//UldisN
