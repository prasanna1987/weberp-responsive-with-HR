...
/* Get this puppy to run from cron (cd nERP && php -f RecurringSalesOrdersProcess.php "nERPdemo") or direct URL (RecurringSalesOrdersProcess.php?Database=nERPdemo) */
...
if (DB_num_rows($RecurrOrdersDueResult)==0){
	echo   prnMsg(_('There are no recurring order templates that are due to have another recurring order created'),'warn');
	include('includes/footer.php');
	exit;
}

echo    prnMsg(_('The number of recurring orders to process is') .' : ' . DB_num_rows($RecurrOrdersDueResult),'info');
...
while ($RecurrOrderRow = DB_fetch_array($RecurrOrdersDueResult)){
	...
	echo  prnMsg(_('Recurring order was created for') . ' ' . $RecurrOrderRow['name'] . ' ' . _('with order Number') . ' ' . $OrderNo, 'success');
...
//		$CustTaxAuth = $myrow[2];
	...
	if (IsEmailAddress($RecurrOrderRow['email'])){
		...
		echo   prnMsg(_('No email was sent for this order because the location has no email contact defined with a valid email address'),'warn');
	}
