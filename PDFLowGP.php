...
if (isset($_POST['PrintPDF'])) {
	...
	if (! Is_Date($_POST['FromDate']) OR ! Is_Date($_POST['ToDate'])){
		...
		echo prnMsg(_('The dates entered must be in the format') . ' '  . $_SESSION['DefaultDateFormat'],'error');
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
		echo prnMsg(_('The low GP items could not be retrieved by the SQL because') . ' - ' . DB_error_msg(),'error');
		echo '<p align="right"><a href="' .$RootPath .'/menu_data.php?Application=Sales" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
		if ($debug==1){
		  echo '<br />' . $SQL;
		}
		include('includes/footer.php');
	...
	if (DB_num_rows($LowGPSalesResult) == 0) {
		...
		include('includes/header.php');
		echo  prnMsg(_('No low GP items retrieved'), 'warn');
		echo '<p align="right"><a href="'  . $RootPath . '/menu_data.php?Application=Sales" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
		if ($debug==1){
		  echo '<br />' .  $SQL;
		}
	...
	$YPos -= (2*$line_height);
	$pdf->OutputD('nERP' . '_LowGPSales_' . date('Y-m-d') . '.pdf');
	$pdf->__destruct();
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' '
		. _('Low Gross Profit Report') . '</h1></a></div>';
...
<div class="col-xs-12">';
		...
		echo '<div class="row">
	<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Sales Made From') . ' (' . _('in the format') . ' ' . $_SESSION['DefaultDateFormat'] . ')</label>
				<input type="text" required="required" autofocus="autofocus" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="FromDate" size="11" maxlength="10" value="' . $_POST['FromDate'] . '" /></div>
			</div>
			<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Sales Made To') . ' (' . _('in the format') . ' ' . $_SESSION['DefaultDateFormat'] . ')</label>
				<input type="text" required="required" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="ToDate" size="11" maxlength="10" value="' . $_POST['ToDate'] . '" /></div>
			</div>
			<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Show sales with GP % below') . '</label>
				<input type="text" class="form-control" name="GPMin" maxlength="3" size="3" value="' . $_POST['GPMin'] . '" /></div>
			</div>
			</div>
        ...
        <div class="form-group"> 
				<input type="submit" class="btn btn-info" name="PrintPDF" value="' . _('Print PDF') . '" />
			</div>
			</div>
			</div>
        </form>
