...
if (!isset($_SESSION['Items'.$identifier])){
	...
	if (DB_num_rows($result)==0) {
		echo prnMsg(_('Your user account does not have a valid default inventory location set up. Please consult your system administrator to modify your user account.'),'error');
		...
		if ($myrow['cashsalecustomer']=='' OR $myrow['cashsalebranch']==''){
			echo prnMsg(_('It is first necessary to define a cash sales customer for the location that is your default location.'),'error');
			include('includes/footer.php');
			exit;
		}
		...
		if ($myrow['dissallowinvoices'] != 1){
			if ($myrow['dissallowinvoices']==2){
				echo prnMsg($myrow['name'] . ' ' . _('Although this account is defined as the cash sale account for the location.  The account is currently flagged as an account that needs to be watched. Please contact the credit control personnel '),'warn');
			}
			...
			if ($_SESSION['CheckCreditLimits'] > 0 AND $AlreadyWarnedAboutCredit==false) {  /*Check credit limits is 1 for warn and 2 for prohibit sales */
				...
				if ($_SESSION['CheckCreditLimits']==1 AND $_SESSION['Items'.$identifier]->CreditAvailable <=0){
					echo prnMsg(_('The') . ' ' . $myrow['brname'] . ' ' . _('account is currently at or over their credit limit'),'warn');
					$AlreadyWarnedAboutCredit = true;
				} elseif ($_SESSION['CheckCreditLimits']==2 AND $_SESSION['Items'.$identifier]->CreditAvailable <=0){
					echo prnMsg(_('No more orders can be placed by') . ' ' . $myrow[0] . ' ' . _(' their account is currently at or over their credit limit'),'warn');
					$AlreadyWarnedAboutCredit = true;
					include('includes/footer.php');
					exit;
				}
			...
			echo prnMsg($myrow['brname'] . ' ' . _('Although the account is defined as the cash sale account for the location  the account is currently on hold. Please contact the credit control personnel'),'warn');
		}
...
if (isset($_POST['CancelOrder'])) {
	...
	echo prnMsg(_('This sale has been cancelled as requested'),'success');
	echo '<br /><div class="row" align="center"><a href="' .htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Back to Counter Sales') . '</a></div><br />';
	include('includes/footer.php');
	exit;
	...
	while ($i<=$_SESSION['QuickEntries']
		...
		if (DB_num_rows($KitResult)==0){
			...
			if ($myrow['mbflag']=='K'){	/*It is a kit set item */
				...
				echo prnMsg(_('The system does not currently allow for counter sales of controlled or serialised items'),'warn');
			} else if ($NewItemQty<=0) {
				echo prnMsg(_('Only items entered with a positive quantity can be added to the sale'),'warn');
...
<table id="general-table" class="table table-bordered">
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
		...
		echo '<a target="_blank" href="' . $RootPath . '/StockStatus.php?identifier='.$identifier . '&amp;StockID=' . $OrderLine->StockID . '&amp;DebtorNo=' . $_SESSION['Items'.$identifier]->DebtorNo . '" class="btn btn-info">' . $OrderLine->StockID . '</a></td>
			<td title="' . $OrderLine->LongDescription . '">' . $OrderLine->ItemDescription . '</td>';

		echo '<td class="has-error"><input class="form-control" tabindex="2" type="text" name="Quantity_' . $OrderLine->LineNumber . '" required="required" size="6" maxlength="6" value="' . locale_number_format($OrderLine->Quantity,$OrderLine->DecimalPlaces) . '" />';

		echo '</td>
			<td class="number">' . locale_number_format($OrderLine->QOHatLoc,$OrderLine->DecimalPlaces) . '</td>
			<td>' . $OrderLine->Units . '</td>';
		if (in_array($_SESSION['PageSecurityArray']['OrderEntryDiscountPricing'], $_SESSION['AllowedPageSecurityTokens'])){
			echo '<td class="has-error"><input class="form-control" type="text" name="Price_' . $OrderLine->LineNumber . '" required="required" size="16" maxlength="16" value="' . locale_number_format($OrderLine->Price,$_SESSION['Items'.$identifier]->CurrDecimalPlaces) . '" /></td>
				<td class="has-error"><input class="form-control" type="text" name="Discount_' . $OrderLine->LineNumber . '" required="required" size="5" maxlength="4" value="' . locale_number_format(($OrderLine->DiscountPercent * 100),2) . '" /></td>
				<td><input class="form-control" type="text" name="GPPercent_' . $OrderLine->LineNumber . '" required="required" size="3" maxlength="40" value="' . locale_number_format($OrderLine->GPPercent,2) . '" /></td>';
		} else {
		...
		echo '<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier . '&amp;Delete=' . $OrderLine->LineNumber . '" class="btn btn-danger" onclick="return confirm(\'' . _('Are You Sure?') . '\');">' . _('Delete') . '</a></td></tr>';

		if ($_SESSION['AllowOrderLineItemNarrative'] == 1){
			echo $RowStarter;
			echo '<tr><td colspan="11">' . _('Narrative') . ':<textarea name="Narrative_' . $OrderLine->LineNumber . '" class="form-control">' . stripslashes(AddCarriageReturns($OrderLine->Narrative)) . '</textarea></td></tr>';
		} else {
...
<table id="general-table" class="table table-bordered"><tr>
		...
		<td><input type="text" class="form-control" size="25" maxlength="25" name="DeliverTo" value="' . stripslashes($_SESSION['Items'.$identifier]->DeliverTo) . '" /></td>
	</tr>';
	echo '<tr>
		<td>' .  _('Contact Phone Number') .':</td>
		<td><input type="te1" size="25" class="form-control" maxlength="25" name="PhoneNo" value="' . stripslashes($_SESSION['Items'.$identifier]->PhoneNo) . '" /></td>
	</tr>';

	echo '<tr>
			<td>' . _('Contact Email') . ':</td>
			<td><input type="email" class="form-control" placeholder="" size="25" maxlength="30" name="Email" value="' . stripslashes($_SESSION['Items'.$identifier]->Email) . '" /></td></tr>';

	echo '<tr>
			<td>' .  _('Customer Reference') .':</td>
			<td><input type="text" class="form-control" size="25" maxlength="25" name="CustRef" value="' . stripcslashes($_SESSION['Items'.$identifier]->CustRef) . '" /></td>
		</tr>';
	...
	if ($_SESSION['SalesmanLogin'] != '') {
		...
		echo '<td><select name="SalesPerson" class="form-control">';
	...
	echo '<tr>
			<td>' .  _('Comments') .':</td>
			<td><textarea name="Comments" class="form-control">' . stripcslashes($_SESSION['Items'.$identifier]->Comments)  . '</textarea></td>
		</tr>';
	echo '</table></div>'; //end the sub table in the first column of master table
	echo '</td><th valign="bottom">'; //for the master table
	echo '<div class="table-responsive">
<table id="general-table" class="table table-bordered">'; // a new nested table in the second column of master table
	...
	echo '<tr>
			<td>' . _('Bank') . ':</td>
			<td><select name="BankAccount" class="form-control">';
	...
	echo '<tr>
			<td>' . _('Amount Paid') . ':</td>
			<td><input type="text" class="form-control" name="AmountPaid" required="required"  title="' . _('Enter the amount paid by the customer, this must equal the amount of the sale') . '" maxlength="12" size="12" value="' . $_POST['AmountPaid'] . '" /></td>
		</tr>';

	echo '</table></div>'; //end the sub table in the second column of master table
	echo '</th>
		</tr>
		</table></div></div></div>';	//end of column/row/master table
}
	if (!isset($_POST['ProcessSale'])){
		echo '<br />
				<div class="row">
				<div class="col-xs-4">
					<input type="submit" name="Recalculate" value="' . _('Re-Calculate') . '" class="btn btn-info"  />
					</div>
					<div class="col-xs-4">
					<input type="submit" name="ProcessSale" value="' . _('Submit') . '" class="btn btn-success" />
					</div>
					<div class="col-xs-4"><input type="submit" class="btn btn-danger" name="CancelOrder" value="' . _('Cancel Sale') . '" onclick="return confirm(\'' . _('Are you sure you wish to cancel this sale?') . '\');" /></div>
...
if (isset($_POST['ProcessSale']) AND $_POST['ProcessSale'] != ''){
	...
	if ($_SESSION['Items'.$identifier]->LineCounter == 0){
		echo prnMsg(_('There are no line Items on this sale. Please enter line Items to invoice'),'error');
		$InputError = true;
	}
	if (abs(filter_number_format($_POST['AmountPaid']) -(round($_SESSION['Items'.$identifier]->total+filter_number_format($_POST['TaxTotal']),$_SESSION['Items'.$identifier]->CurrDecimalPlaces)))>=0.01) {
		echo prnMsg(_('The amount entered as payment does not equal the amount of the invoice. Please ensure the customer has paid the correct amount and re-enter'),'error');
		$InputError = true;
	}
	...
	//sadly this check does not combine quantities occuring twice on and order and each line is considered individually :-(
		...
		foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
			...
			if ($CheckNegRow['mbflag']=='B' OR $CheckNegRow['mbflag']=='M'){
				if ($CheckNegRow['quantity'] < $OrderLine->Quantity){
					echo prnMsg( _('Invoicing the selected order would result in negative stock. The system parameters are set to prohibit negative stocks from occurring. This invoice cannot be created until the stock on hand is corrected.'),'error',$OrderLine->StockID . ' ' . $CheckNegRow['description'] . ' - ' . _('Negative Stock Prohibited'));
					$NegativesFound = true;
				}
				...
				while ($NegRow = DB_fetch_array($Result)){
					if ($NegRow['qtyleft']<0){
						echo prnMsg(_('Invoicing the selected order would result in negative stock for a component of an assembly item on the order. The system parameters are set to prohibit negative stocks from occurring. This invoice cannot be created until the stock on hand is corrected.'),'error',$NegRow['component'] . ' ' . $NegRow['description'] . ' - ' . _('Negative Stock Prohibited'));
						$NegativesFound = true;
		...
		if ($NegativesFound){
			echo prnMsg(_('Invoicing this sale would result in negative stock. No futher processing can be performed. Alter the sale first changing quantities or deleting lines which do not have sufficient stock.'),'error');
			$InputError = true;
		}
	...
	if ($InputError == true ) { //allow the error to be fixed and then resubmit buttone needs to show
		echo '<br />
				...
				<div class="col-xs-4">
					...
					<div class="col-xs-4">
					<input type="submit" name="ProcessSale" value="' . _('Process The Sale') . '" class="btn btn-info" />
				</div>
				</div>
				';
	...
	/*company record read in on login with info on GL Links and debtors GL account*/
		...
		if ($_SESSION['CompanyRecord']==0){
			...
			echo prnMsg( _('The company information and preferences could not be retrieved. Consult your system administrator'), 'error');
			include('includes/footer.php');
			exit;
		}
	...
	/*First add the order to the database - it only exists in the session currently! */
			...
			if ($StockItem->MBflag=='M'
				...
				if ($ShortfallQuantity < 0) { //then we need to make a work order
					...
					if (is_null($CostRow[0]) OR $CostRow[0]==0){
						$Cost =0;
						echo prnMsg(_('In automatically creating a work order for') . ' ' . $StockItem->StockID . ' ' . _('an item on this sales order, the cost of this item as accumulated from the sum of the component costs is nil. This could be because there is no bill of material set up ... you may wish to double check this'),'warn');
					} else {
						$Cost = $CostRow[0];
					}
					...
					if ($StockItem->Serialised AND $StockItem->NextSerialNo>0){
						...
						for ($i=0;$i<$WOQuantity;$i++){
							...
							if (DB_num_rows($result)!=0){
								$WOQuantity++;
								echo prnMsg(($StockItem->NextSerialNo + $i) . ': ' . _('This automatically generated serial number already exists - it cannot be added to the work order'),'error');
							} else {
		...
		echo prnMsg(_('Order Number') . ' ' . $OrderNo . ' ' . _('has been entered'),'success');
	...
	// *************************************************************************
		...
		if ($_SESSION['InvoicePortraitFormat']==0){
			echo '<div class="col-xs-4"><a target="_blank" href="' . $RootPath.'/PrintCustTrans.php?FromTransNo='.$InvoiceNo.'&amp;InvOrCredit=Invoice&amp;PrintPDF=True" class="btn btn-warning">Print this Invoice</a></div>';
			...
			echo '<div class="col-xs-4"><a href="' . $RootPath.'/PrintCustTransPortrait.php?FromTransNo='.$InvoiceNo.'&amp;InvOrCredit=Invoice&amp;PrintPDF=True" class="btn btn-warning" target="_blank">Print this Invoice</a></div>';
		}

		echo '<div class="col-xs-4"><a href="' .htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Back to Counter Sales') . '</a></div></div><br />';
...
if (!isset($_POST['ProcessSale'])){
	...
	// Select the most recently ordered items for quick select
			...
			echo '<div class="block-header"><a href="" class="header-title-link"><h1>'.' ';
			echo _('Frequently Ordered Items') . '<small>';
			echo '' . _('Frequently Ordered Items') . _(', shows the most frequently ordered items in the last 6 months.  You can choose from this list, or search for other items') . '.</small></h1></a></div>';
			echo '
...
';
	...
	// Find the quantity in stock at location
				...
				printf('<tr class="striped_row">
						...
						<td class="number">%s</td>
						<td class="number">%s</td>
						<td class="has-error"><input class="form-control" tabindex="'.strval($j+7).'" type="text" ' . ($i==0?'autofocus="autofocus"':'') . ' size="6" required="required" name="OrderQty%s" value="0" />
			...
			echo '<td colspan="8"><input type="hidden" name="SelectingOrderItems" value="1" /><input tabindex="'.strval($j+8).'" type="submit" value="'._('Add to Sale').'" /></td>';
			echo '</table></div></di></div>';
		} //end of if Frequently Ordered Items > 0
		if (isset($msg)){
			echo '<p class="text-info"><strong>' . $msg . '</strong></p>';
		}
		echo '
...
' . _('Select a Stock Category') . ': </label><select tabindex="1" name="StockCat" class="form-control">';
		...
		echo '</select></div>
				...
				<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">
' . _('Description-part or full') . '</label>
				<input tabindex="2" type="text" class="form-control" autofocus="autofocus" name="Keywords" size="20" maxlength="25" value="';
        if (isset($_POST['Keywords'])) echo $_POST['Keywords'];
        echo '" /></div></div>
				<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">
' . _('Stock ID-part or full') . '</label>
				<input tabindex="3" type="text" class="form-control" name="StockCode" size="15" maxlength="18" value="';
        ...
        echo '" /></div>
			...
			<div class="row">
			<div class="col-xs-4">
				<input tabindex="4" type="submit" class="btn btn-success" name="Search" value="' . _('Search') . '" />
				</div>
				<div class="col-xs-4">
       <input tabindex="5" class="btn btn-info" type="submit" name="QuickEntry" value="' . _('Quick Entry') . '" /></div>
...
<table class="table">
			<tr>
					<td align="left"><input type="hidden" name="PreviousList" value="'.strval($Offset-1).'" /><input tabindex="'.strval($j+7).'" type="submit" name="Previous" value="'._('Previous').'" class="btn btn-default" /></td>
					<td align="center"><input type="hidden" name="SelectingOrderItems" value="1" /><input tabindex="'.strval($j+8).'" type="submit" value="'._('Add to Sales').'" class="btn btn-success" /></td>
					<td align="right"><input type="hidden" name="NextList" value="'.strval($Offset+1).'" /><input tabindex="'.strval($j+9).'" type="submit" name="Next" class="btn btn-default" value="'._('Next').'" /></td>
				</tr></table></div></div>';
...
<thead>
			...
			while ($myrow=DB_fetch_array($SearchResult)) {
				...
				printf('<tr class="striped_row">
						...
						<td class="number">%s</td>
						<td class="number">%s</td>
						<td class="has-error"><input class="form-control"  tabindex="'.strval($j+7).'" type="text" size="6" required="required" ' . ($i==0?'autofocus="autofocus"':'') . ' name="OrderQty%s" value="0" /><input type="hidden" name="StockID%s" value="%s" /></td>
						</tr>',
						$myrow['stockid'],
						$myrow['description'],
						$myrow['units'],
...
<table id="general-table" class="table table-bordered">
		...
		for ($i=1;$i<=$_SESSION['QuickEntries'];$i++){
	 		...
	 		echo '<td><input type="text" class="form-control" name="part_' . $i . '"' . ($i==1 ? ' autofocus="autofocus"':'') . ' data-type="no-illegal-chars" title="' . _('Enter a part code to be sold. Part codes can contain any alpha-numeric characters underscore or hyphen.') . '"size="21" maxlength="20" /></td>
					<td><input type="text" class="form-control" name="qty_' . $i . '" size="6" maxlength="6" />
						<input type="hidden" class="date" name="ItemDue_' . $i . '" value="' . $DefaultDeliveryDate . '" /></td></tr>';
   		}
	 	echo '</table>
		</div></div>
		<div class="col-xs-4">
					<input type="submit" name="PartSearch" value="' . _('Search Items') . '" class="btn btn-info" />
				</div>
		</div>
				<br />
				<div class="row" align="center">
				
					<input type="submit" name="QuickEntry" value="' . _('Quick Entry') . '" class="btn btn-success" /></div>
