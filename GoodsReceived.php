...
if (isset($_GET['PONumber']) AND $_GET['PONumber']<=0 AND !isset($_SESSION['PO'.$identifier])) {
	...
	echo '<div class="row"><div class="col-xs-4">
			<a href= "' . $RootPath . '/PO_SelectOSPurchOrder.php" class="btn btn-default">' . _('Select a purchase order to receive') . '</a>
		</div></div>
		<br /><p class="text-info">' .  _('This page can only be opened if a purchase order has been selected. Please select a purchase order first');
echo '</p>';
...
if ($_SESSION['PO'.$identifier]->Status != 'Printed') {
	echo  prnMsg( _('Purchase orders must have a status of Printed before they can be received').'.<br />' .
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Receive Purchase Order') . ' : '. $_SESSION['PO'.$identifier]->OrderNo .' '. _('from'). ' ' . $_SESSION['PO'.$identifier]->SupplierName . '</h1></a></div>';
...
if (!isset($_POST['ProcessGoodsReceived'])) {
	if (!isset($_POST['DefaultReceivedDate']) AND !isset($_SESSION['PO' . $identifier]->DefaultReceivedDate)){
		...
		if (isset($_POST['DefaultReceivedDate']) AND is_date($_POST['DefaultReceivedDate'])) {
			...
			echo prnMsg(_('The default received date is not in format'),'error');
			$_POST['DefaultReceivedDate'] = Date($_SESSION['DefaultDateFormat']);
		}
	}
	if (!isset($_POST['SupplierReference'])) {
		...
		if (isset($_POST['SupplierReference']) AND mb_strlen(trim($_POST['SupplierReference']))>30) {
			echo prnMsg(_('The supplier\'s delivery note no. should not be more than 30 characters'),'error');
		} else {
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' .  _('Date Goods/Service Received'). '</label>
				<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" maxlength="10" size="11" onchange="return isDate(this, this.value, '."'".
			$_SESSION['DefaultDateFormat']."'".')" name="DefaultReceivedDate" value="' . $_SESSION['PO' . $identifier]->DefaultReceivedDate . '" /></div></div>
				<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _("Supplier's Reference") . '</label>
				<input type="text" class="form-control" name="SupplierReference" value="' . $SupplierReference. '" maxlength="30" size="20"  onchange="ReloadForm(form1.Update)"/></div>
			</div>
		</div>
		<br />';
...
<table id="general-table" class="table table-bordered">
			...
			<tr>
				<th align="center" colspan="6">' . _('Supplier References') . '</th>
				<th></th>
				<th align="center" colspan="9">' . _('Our References') . '</th>
			</tr>
			<tr>
				<th>' . _('Item Code') . '</th>
...
if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_POST['ProcessGoodsReceived'])){
	...
	*/
	...
	//Setup & Format values for LineItem display
		...
		if ($imagefile) {
			$ImageLink = '<a href="' . $imagefile . '" target="_blank" class="btn btn-info">' .  $LnItm->StockID . '</a>';
		} else {
		...
		if ($LnItm->Controlled == 1) {
			...
			echo '<input type="hidden" name="RecvQty_' . $LnItm->LineNo . '" autofocus="autofocus" value="' . locale_number_format($LnItm->ReceiveQty,$LnItm->DecimalPlaces) . '" /><a href="GoodsReceivedControlled.php?identifier=' . $identifier . '&amp;LineNo=' . $LnItm->LineNo . '" class="btn btn-info">' . locale_number_format($LnItm->ReceiveQty,$LnItm->DecimalPlaces) . '</a></td>';

		} else {
			echo '<input type="text" class="form-control" name="RecvQty_' . $LnItm->LineNo . '" pattern="(?:^\d{1,3}(?:\.?\d{3})*(?:,\d{1,})?$)|(?:^\d{1,3}(?:,?\d{3})*(?:\.\d{1,})?$)|(?:^\d{1,3}(?:\s?\d{3})*(?:\.\d{1,})?$)|(?:^\d{1,3}(?:\s?\d{3})*(?:,\d{1,})?$)|(?:^(\d{1,2},)?(\d{2},)*(\d{3})(\.\d+)?|(\d{1,3})(\.\d+)?$)" title="' . _('Enter the quantity to receive against this order line as a number') . '"
maxlength="10" size="10" value="' . locale_number_format(round($LnItm->ReceiveQty,$LnItm->DecimalPlaces),$LnItm->DecimalPlaces) . '" /></td>';
		...
		if ($LnItm->Controlled == 1) {
			if ($LnItm->Serialised==1){
				echo '<td><a href="GoodsReceivedControlled.php?identifier=' . $identifier . '&amp;LineNo=' . $LnItm->LineNo . '" class="btn btn-info">' .
...
if (isset($_POST['DefaultReceivedDate']) AND !is_date($_POST['DefaultReceivedDate'])) {
	$InputError = true;
	echo prnMsg(_('The goods received date is not in format'),'error');

}

if (isset($_POST['SupplierReference']) AND mb_strlen(trim($_POST['SupplierReference']))>30) {
	$InputError = true;
	echo prnMsg(_('The delivery note of suppliers should not be more than 30 characters'),'error');
}
	...
	foreach ($_SESSION['PO'.$identifier]->LineItems as $OrderLine) {
		...
		if ($OrderLine->ReceiveQty < 0 AND $_SESSION['ProhibitNegativeStock']==1){
			...
			if ($CheckNegRow[0]+$OrderLine->ReceiveQty<0){
				$NegativesFound=true;
				echo prnMsg(_('Receiving a negative quantity that results in negative stock is prohibited by the system. This delivery of stock cannot be processed until the stock of the item is corrected.'),'error',$OrderLine->StockID . ' Cannot Go Negative');
			}
...
if ($_SESSION['PO'.$identifier]->SomethingReceived()==0 AND isset($_POST['ProcessGoodsReceived'])){ /*Then dont bother proceeding cos nothing to do ! */
	...
	echo prnMsg(_('There is nothing to process') . '. ' . _('Please enter valid quantities greater than zero'),'warn');
	echo '<div class="row"><div class="col-xs-4"><input type="submit" name="Update" class="btn btn-info" value="' . _('Update') . '" /></div></div><br />
';

} elseif ($NegativesFound){

	echo prnMsg(_('Negative stocks would result by processing a negative delivery - quantities must be changed or the stock quantity of the item going negative corrected before this delivery will be processed.'),'error');
...
';
	...
	echo prnMsg(_('Entered quantities cannot be greater than the quantity entered on the purchase invoice including the allowed over-receive percentage'). ' ' . '(' . $_SESSION['OverReceiveProportion'] .'%)','error');
	echo '<br />';
	echo prnMsg(_('Modify the ordered items on the purchase order if you wish to increase the quantities'),'info');
	echo '<div class="row"><div class="col-xs-4"><input type="submit" name="Update" class="btn btn-info" value="' . _('Update') . '" /></div></div>';
...
// Otherwise if you try to fullfill item quantities separately will give error.
	...
	if(DB_num_rows($Result)==0){//Those goods must have been received by another user. So should destroy the session data and show warning to users
		echo prnMsg(_('This order has been changed or invoiced since this delivery was started to be processed').' . '._('Processing halted'),'error');
	...
	while ($myrow = DB_fetch_array($Result)) {
		...
		if ($_SESSION['PO'.$identifier]->LineItems[$LineNo]->GLCode != $myrow['glcode'] OR
			...
			echo prnMsg(_('This order has been changed or invoiced since this delivery was started to be processed') . '. ' . _('Processing halted') . '. ' . _('To enter a delivery against this purchase order') . ', ' . _('it must be re-selected and re-read again to update the changes made by the other user'),'warn');
...
<table id="general-table" class="table table-bordered">
			...
			echo '<div class="row"><div class="col-xs-4"><a href="' . $RootPath . '/PO_SelectOSPurchOrder.php" class="btn btn-info">' .
				_('Select a different purchase order for receiving goods against') . '</a></div>
';
			echo '<div class="col-xs-4"><a href="' . $RootPath . '/GoodsReceived.php?PONumber=' . $_SESSION['PO'.$identifier]->OrderNo . '" class="btn btn-info">' .  _('Re-read the updated purchase order for receiving goods against'). '</a></div>';
			unset($_SESSION['PO'.$identifier]->LineItems);
			unset($_SESSION['PO'.$identifier]);
			unset($_POST['ProcessGoodsReceived']);
            echo '</div>';
...
/* Need to get the current location quantity will need it later for the stock movement */
	...
	echo prnMsg(_('GRN number'). ' '. $GRN .' '. _('has been processed'),'success');
                 
			echo '<div class="row">
<div class="col-xs-4">
			<a href="PDFGrn.php?GRNNo='.$GRN .'&amp;PONo='.$PONo.'" class="btn btn-info">' .  _('Print this GRN') . '</a>
			</div>
			<div class="col-xs-4">
			<a href="PDFQALabel.php?GRNNo='.$GRN .'&amp;PONo='.$PONo.'" class="btn btn-info">' .  _('Print QA Labels') . '</a>
			</div>
			<div class="col-xs-4">
			<a href="' . $RootPath . '/PO_SelectOSPurchOrder.php" class="btn btn-info">' . _('Select a different PO for GRN'). '</a>
		</div>
                 </div><br />';
...
/*end of process goods received entry */
	...
	echo '<br />
		<div class="row">
		<div class="col-xs-4">
			<a href="' . $RootPath . '/PO_Header.php?ModifyOrderNumber=' .$_SESSION['PO'.$identifier]->OrderNo . '" class="btn btn-info">' . _('Modify Order Items'). '</a>
		</div>
		
		<div class="col-xs-4">
			<input type="submit" name="Update" value="' . _('Update') . '" class="btn btn-info" />
			</div>
			<div class="col-xs-4">
			<input type="submit" name="ProcessGoodsReceived" value="' . _('Process Goods Received') . '" class="btn btn-info" />
		</div></div><br />
';
}
echo '';
