...
if (isset($_POST['FromDate']) AND !Is_Date($_POST['FromDate'])){
	echo   
$msg = _('The date from must be specified in the format') . ' ' . $_SESSION['DefaultDateFormat'],'</div>';
	$InputError=1;
}
if (isset($_POST['ToDate']) AND !Is_Date($_POST['ToDate'])){
	echo   
$msg =  _('The date to must be specified in the format') . ' ' .  $_SESSION['DefaultDateFormat'],'</div>';
	$InputError=1;
}
...
<div class="col-xs-12">';
	 ...
	 echo '<div class="row">
		<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Enter the date from which variances between orders and deliveries are to be listed') . '</label>
				<input type="text" required="required" autofocus="autofocus" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="FromDate" maxlength="10" size="11" value="' . Date($_SESSION['DefaultDateFormat'], Mktime(0,0,0,Date('m')-1,0,Date('y'))) . '" /></div>
			</div>
			<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Enter the date to which variances between orders and deliveries are to be listed') . '</label>
	 ...
	 echo '<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Enter the number of days considered acceptable between delivery requested date and invoice date(ie the date dispatched)') . '</label>
				<input type="text" class="integer form-control" name="DaysAcceptable" maxlength="2" size="2" value="' . $_POST['DaysAcceptable'] . '" /></div>
			</div>
			</div>
			<div class="row">
			<div class="col-xs-4">
	...
	while ($myrow=DB_fetch_array($result)){
	 ...
	 echo '<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Inventory Location') . '</label>
	...
	while ($myrow=DB_fetch_array($result)){
	 ...
	 echo '<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Email the report off') . '</label>
		...
		<input type="submit" name="Go" class="btn btn-info" value="' . _('Create PDF') . '" />
		</div>
		</div>
		</div>
	</form>
...
if (DB_error_no()!=0){
	...
	include('includes/header.php');
	echo  prnMsg( _
('An error occurred getting the days between delivery requested and actual invoice'),'error');
	if ($debug==1){
		echo  prnMsg( _
('The SQL used to get the days between requested delivery and actual invoice dates was') . "<br />$sql",'error');
	}
	include ('includes/footer.php');
	exit;
	...
	$Title = _('DIFOT Report Error');
  	include('includes/header.php');
	echo    prnMsg( _
('There were no variances between deliveries and orders found in the database within the period from') . ' ' . $_POST['FromDate'] . ' ' . _('to') . ' ' . $_POST['ToDate'] . '. ' . _('Please try again selecting a different date range'), 'info');
	if ($debug==1) {
		echo  prnMsg( _
('The SQL that returned no rows was') . '<br />' . $sql,'error');
	}
	include('includes/footer.php');
	exit;
}
...
$ReportFileName = 'nERP' . '_DIFOT_' . date('Y-m-d').'.pdf';
