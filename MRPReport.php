...
if (isset($_POST['PrintPDF']) AND $_POST['Part']!='') {
	...
	if (DB_error_no() !=0) {
		...
		echo prnMsg(_('The MRP calculation must be run before this report will have any output. MRP requires set up of many parameters, including, EOQ, lead times, minimums, bills of materials, demand types, master schedule etc'),'error');
	...
	if (DB_num_rows($result) == 0) {
		...
		include('includes/header.php');
		echo prnMsg(_('The MRP calculation must be run before this report will have any output. MRP requires set up of many parameters, including, EOQ, lead times, minimums, bills of materials, demand types, master schedule, etc'), 'warn');
		echo '<br /><p align="center"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('Menu') . '</a></p><br />';
		include('includes/footer.php');
		exit;
	}
	...
	if (isset($errors)) {
		...
		include('includes/header.php');
		echo prnMsg( _('The MRP Report could not be retrieved by nERP because') . ' '  . DB_error_msg(),'error');
		echo '<br /><p align="center"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('Menu') . '</a></p><br />';
		if ($debug==1){
			echo '<br />' . $sql;
		}
	...
	if (isset($_POST['PrintPDF'])) {
		echo prnMsg(_('This report shows the MRP calculation for a specific item - a part code must be selected'),'warn');
	}
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Description-part or full') . '</label>';
	if (isset($_POST['Keywords'])) {
		echo '<input type="text" class="form-control" autofocus="autofocus" name="Keywords" value="' . $_POST['Keywords'] . '" size="20" maxlength="25" />';
	} else {
		echo '<input type="text" class="form-control" autofocus="autofocus" name="Keywords" size="20" maxlength="25" />';
	}
	echo '</div>
		</div>
		<div class="col-xs-4">
...
<div class="col-xs-12">';
	...
	if ($ListCount > 0) {
		...
		if ($ListPageMax > 1) {
			echo '<div class="row">
					<p class="text-info">' . $_POST['PageOffset'] . ' ' . _('of') . ' ' . $ListPageMax . ' ' . _('pages') . '. ' . _('Go to Page') . ':</p></div> ';
			echo '<div class="row">
<div class="col-xs-3">
<div class="form-group"> <select name="PageOffset" class="form-control">';
			$ListPage = 1;
			...
			<div class="col-xs-3">
<div class="form-group">
				<input type="submit" name="Go" class="btn btn-default" value="' . _('Go') . '" />
				</div></div>
				<div class="col-xs-3">
<div class="form-group">
				...
				<div class="col-xs-3">
<div class="form-group">
				<input type="submit" name="Next" class="btn btn-default" value="' . _('Next') . '" />
				</div></div>
...
<table id="general-table" class="table table-bordered">';
		...
		while (($myrow = DB_fetch_array($searchresult)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			echo '<tr class="striped_row">
				<td><input type="submit" name="Select" value="'.$myrow['stockid']. '" class="btn btn-success" /></td>
				<td>' . $myrow['description'] . '</td>
				<td class="number">' . $qoh . '</td>
				<td>' . $myrow['units'] . '</td>
				<td><a target="_blank" href="' . $RootPath . '/StockStatus.php?StockID=' . $myrow['stockid'] .'" class="btn btn-info">' . _('View') . '</a></td>
				</tr>';
			$j++;
