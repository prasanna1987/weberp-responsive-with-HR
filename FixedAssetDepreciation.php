...
if (DB_num_rows($result)==0) { //then depn has never been run yet?
	...
	if (!isset($_POST['ProcessDate'])) {
		...
		if (!Is_Date($_POST['ProcessDate'])){
			echo prnMsg(_('The date is expected to be in the format') . ' ' . $_SESSION['DefaultDateFormat'], 'error');
			$InputError =true;
		}else {
...
if (Date1GreaterThanDate2($_POST['ProcessDate'],Date($_SESSION['DefaultDateFormat']))){
	echo prnMsg(_('No depreciation will be committed as the processing date is a future date.'),'warn');
	$InputError =true;
}
...
$Heading = '<tr>
				...
				<th>' . _('Cost') . '</th>
				<th>' . _('Accum Depreciation') . '</th>
				<th>' . _('B/fwd Book Value') . '</th>
				<th>' .  _('Depreciation Type') . '</th>
				<th>' .  _('Depreciation Rate') . '</th>
				<th>' . _('New Depreciation') . '</th>
			</tr>';
echo $Heading;
...
if (isset($_POST['CommitDepreciation']) AND $InputError==false){
	...
	echo prnMsg(_('Depreciation') . ' ' . $TransNo . ' ' . _('has been successfully entered'),'success');
	unset($_POST['ProcessDate']);
	echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=FA" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
	...
	if ($AllowUserEnteredProcessDate){
		echo '
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Date to Process Depreciation'). '</label>
			<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" required="required" name="ProcessDate" maxlength="10" size="11" value="' . $_POST['ProcessDate'] . '" /></div></div>';
	} else {
		echo '
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Date to Process Depreciation'). '</label>
			' . $_POST['ProcessDate']  . '</div></div>';
	}
	echo '
<div class="col-xs-4">
<div class="form-group"><br />
<input type="submit" name="CommitDepreciation" class="btn btn-info" value="'._('Commit Depreciation').'" /></div></div>
		</div>
