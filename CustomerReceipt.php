...
if ((isset($_POST['BatchInput'])
	...
	echo '<br />';
	echo prnMsg(_('A bank account must be selected for this receipt'), 'warn');
...
if (!isset($_GET['Delete']) AND isset($_SESSION['ReceiptBatch' . $identifier])){
	...
	if (DB_num_rows($result)==1){
		...
		echo prnMsg( _('The bank account number') . ' ' . $_POST['BankAccount'] . ' ' . _('is not set up as a bank account'),'error');
	...
	if (isset($_POST['ExRate']) AND $_POST['ExRate']!=''){
		if (is_numeric(filter_number_format($_POST['ExRate']))){
			...
			echo prnMsg(_('The exchange rate entered should be numeric'),'warn');
		}
	}
	if (isset($_POST['FunctionalExRate']) AND $_POST['FunctionalExRate']!=''){
		if (is_numeric(filter_number_format($_POST['FunctionalExRate']))){
			...
			echo prnMsg(_('The default exchange rate entered should be numeric'),'warn');
		}
	}
	...
	if ($_SESSION['ReceiptBatch' . $identifier]->Currency!=$_POST['Currency']){
		...
		if (count($_SESSION['ReceiptBatch' . $identifier]->Items)>0){
			unset($_SESSION['ReceiptBatch' . $identifier]->Items);
			echo prnMsg(_('Changing the currency of the receipt means that existing entries need to be re-done - only customers trading in the selected currency can be selected'),'warn');
		}
		...
		$AllowThisPosting = true;
 		if ($_SESSION['ProhibitJournalsToControlAccounts'] == 1) {
 			if ($_SESSION['CompanyRecord']['gllink_debtors'] == '1' AND $_POST['GLCode'] == $_SESSION['CompanyRecord']['debtorsact']) {
 				echo prnMsg(_('Payments involving the debtors control account cannot be entered. The general ledger debtors ledger (AR) integration is enabled so control accounts are automatically maintained. This setting can be disabled in System settings'), 'warn');
 			...
 			if ($_SESSION['CompanyRecord']['gllink_creditors'] == '1' AND
				($_POST['GLCode'] == $_SESSION['CompanyRecord']['creditorsact'] OR $_POST['GLCode'] == $_SESSION['CompanyRecord']['grnact'])) {
 				echo prnMsg(_('Payments involving the creditors control account or the GRN suspense account cannot be entered. The general ledger creditors ledger (AP) integration is enabled so control accounts are automatically maintained. This setting can be disabled in System settings'), 'warn');
 				$AllowThisPosting = false;
 			}
 ...
 /* once all receipts items entered, process all the data in the
  ...
  also add the total discount total receipts*/
	...
	if ($_SESSION['CompanyRecord']==0){
		echo prnMsg(_('The system has not yet been set up properly') . ' - ' . _('this information is needed to process the batch') . '. ' . _('Processing has been cancelled'),'error');
		include('includes/footer.php');
		exit;
	}
	...
	echo '<br />
		<div class="block-header"><a href="" class="header-title-link"><h1> ',// Icon title.
			_('Summary of Receipt Batch'), '</h1></a></div>',// Page title.
		'<br />
...
<table id="general-table" class="table table-bordered">
	...
	foreach ($_SESSION['ReceiptBatch' . $identifier]->Items as $ReceiptItem) {
		...
		echo '<tr class="striped_row">
			...
			<td>' . locale_number_format($ReceiptItem->Amount/$_SESSION['ReceiptBatch' . $identifier]->ExRate/$_SESSION['ReceiptBatch' . $identifier]->FunctionalExRate,$_SESSION['ReceiptBatch' . $identifier]->CurrDecimalPlaces)  . '</td>';

		if ($ReceiptItem->GLCode ==''){
			echo '<td class="noprint"><a target="_blank" href="', $RootPath, '/PDFReceipt.php?BatchNumber=', $_SESSION['ReceiptBatch' . $identifier]->BatchNo, '&ReceiptNumber=', $CustomerReceiptCounter, '" class="btn btn-warning">', _('Print a Customer Receipt'), '</a></td></tr>';
	...
	echo '<br />';
	echo prnMsg( _('Receipt batch') . ' ' . $_SESSION['ReceiptBatch' . $identifier]->BatchNo . ' ' . _('has been successfully entered into the system'),'success');

	echo '<div class="row noprint">',
		'<div class="col-xs-4"><a href="' . $RootPath . '/PDFBankingSummary.php?BatchNo=' . $_SESSION['ReceiptBatch' . $identifier]->BatchNo . '" class="btn btn-info">' . _('Print PDF Batch Summary') . '</a></div>';
	echo '<div class="col-xs-4" class="btn btn-info">' . ' ' . '<a href="' . $RootPath . '/CustomerAllocations.php" class="btn btn-info">' . _('Allocate Receipts') . '</a></div>';
	echo '<div class="col-xs-4">', '<a href="', $RootPath, '/CustomerReceipt.php?NewReceipt=Yes&Type=', urlencode($_GET['Type']), '" class="btn btn-info">', _('Enter Receipts'), '</a></div>',
		'</div><br />';
...
/*Will only be true if clicked to search for a customer code */
	...
	if ($_POST['Keywords']==''
		...
		if (DB_error_no() !=0) {
			echo prnMsg(_('The searched customer records cannot be retrieved because') . ' - ' . DB_error_msg(),'error');
			if ($debug==1){
				echo prnMsg(_('Querry used to retrieve the customer details was') . '<br />' . $sql,'error');
			}
			...
			echo prnMsg( _('No customer records contain the input text') . ' - ' . _('please alter your search criteria and try again'),'info');
		}
...
$SQL = "SELECT DISTINCT bankaccounts.bankaccountname,
...
if (isset($_POST['GLEntry'])) {
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Bank Account Receipts Entry') . '</h1></a></div>';
} else {
	echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Enter Customer Receipt') . '<br />';
	echo '<small>' . _('Enter a negative amount for refund to a customer (i.e. to pay a credit note)') . '</small></h1></a></div>';
}
echo '<br />
	...
	<div class="row">
		...
		<select tabindex="1" class="form-control" autofocus="autofocus" name="BankAccount" onchange="ReloadForm(form1.BatchInput)">';
...
if (DB_num_rows($AccountsResults)==0){
	echo '</select></div>
		...
		';
	echo prnMsg(_('Bank Accounts have not yet been defined') . '. ' . _('You must first') . ' ' . '<a href="' . $RootPath . '/BankAccounts.php">' . _('define the bank accounts') . '</a>' . _('and general ledger accounts to be affected'),'info');
	include('includes/footer.php');
	 exit;
} else {
...
echo '<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Deposit Date') . '</label>
		<input tabindex="2" type="text" required="required" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker"
 name="DateBanked" maxlength="10" size="11" onchange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')" value="' . $_SESSION['ReceiptBatch' . $identifier]->DateBanked . '" /></div>
	</div>
	<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Currency') . '</label>
		<select tabindex="3" name="Currency" class="form-control" onchange="ReloadForm(form1.BatchInput)">';
...
if (DB_num_rows($result)==0){
	echo '</select></div></div>';
	echo prnMsg(_('No currencies are defined yet') . '. ' . _('Receipts cannot be entered until a currency is defined'),'warn'),'
                 </div></div>';
...
if ($_SESSION['ReceiptBatch' . $identifier]->AccountCurrency != $_SESSION['ReceiptBatch' . $identifier]->Currency AND isset($_SESSION['ReceiptBatch' . $identifier]->AccountCurrency)) {
	...
	if(isset($SuggestedExRate)) {
		$SuggestedExRateText = '<strong>' . _('Suggested rate:') . ' 1 ' . $_SESSION['ReceiptBatch' . $identifier]->AccountCurrency . ' = ' . locale_number_format($SuggestedExRate,8) . ' ' . $_SESSION['ReceiptBatch']->Currency . '</strong>';
	} else {
		$SuggestedExRateText = '<strong>1 ' . $_SESSION['ReceiptBatch' . $identifier]->AccountCurrency . ' = ? ' . $_SESSION['ReceiptBatch' . $identifier]->Currency . '</strong>';
	}
	echo '
			<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">', _('Receipt Exchange Rate'), '</label>
			<input class="form-control" maxlength="12" name="ExRate" required="required" size="14" tabindex="4" type="text" value="', locale_number_format($_SESSION['ReceiptBatch' . $identifier]->ExRate,8), '" /> ', $SuggestedExRateText, ' <i>', _('The exchange rate between the currency of the bank account and the currency of the receipt'), '.</i></div>
		</div>';
}
...
if($_SESSION['ReceiptBatch' . $identifier]->AccountCurrency != $_SESSION['CompanyRecord']['currencydefault'] AND isset($_SESSION['ReceiptBatch' . $identifier]->AccountCurrency)) {
	...
	if(isset($SuggestedFunctionalExRate)) {
		$SuggestedFunctionalExRateText = '<strong>' . _('Suggested rate:') . ' 1 ' . $_SESSION['CompanyRecord']['currencydefault'] . ' = ' . locale_number_format($SuggestedFunctionalExRate,8) . ' ' . $_SESSION['ReceiptBatch' . $identifier]->AccountCurrency . '</strong>';
	} else {
		$SuggestedFunctionalExRateText = '<strong>1 ' . $_SESSION['CompanyRecord']['currencydefault'] . ' = ? ' . $_SESSION['ReceiptBatch']->AccountCurrency . '</strong>';
	}
	echo '<div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">', _('Functional Exchange Rate'), '</label>
			<input class="form-control" maxlength="12" name="FunctionalExRate" pattern="[0-9\.,]*" required="required" size="14" tabindex="5" type="text" value="', $_SESSION['ReceiptBatch' . $identifier]->FunctionalExRate, '" /> ', $SuggestedFunctionalExRateText, ' <i>', _('The exchange rate between the default currency and the currency of the bank account'),  '.</i></div>
		</div>';
}

echo '	
<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Receipt Type') . '</label>
		<select name="ReceiptType" tabindex="6" class="form-control" onchange="ReloadForm(form1.BatchInput)">';
...
<div class="col-xs-4">
		...
		<input maxlength="50" name="BankTransRef" class="form-control" size="52" tabindex="7" type="text" value="', $_SESSION['ReceiptBatch' . $identifier]->BankTransRef,'" /> <i>', _('Reference for Bank Transactions search'), '.</i></div>
	</div>';
...
echo '<div class="col-xs-4">
		...
		<input maxlength="200" name="BatchNarrative" size="52" class="form-control" tabindex="8" type="text" value="', $_SESSION['ReceiptBatch' . $identifier]->Narrative, '" /> <i>', _('Narrative for General Ledger Account search'), '.</i></div>
	</div></div>
	<input name="PreviousCurrency" type="hidden" value="', $_POST['Currency'], '" />
	<div class="row">
	<div class="col-xs-4">
        <div class="form-group">
			<input name="BatchInput" tabindex="9" type="submit" class="btn btn-danger" value="Please full-fill above parameters before proceeding" />
		</div>
		</div>
	</div>	
	';
...
if (isset($_SESSION['CustomerRecord' . $identifier])
	...
	echo prnMsg(_('The selected customer does not trade in the currency of the receipt being entered - either the currency of the receipt needs to be changed or a different customer shoulld be selected'),'warn');
	unset($_SESSION['CustomerRecord' . $identifier]);
}
...
/*a customer is selected  */
	...
	echo '<h3 class="page-header">' . ' ' . $_SESSION['CustomerRecord' . $identifier]['name'] . ' - (' . _('All amounts stated in') . ' ' . $_SESSION['CustomerRecord' . $identifier]['currency'] . ')<br /><small>' . _('Terms ') . ':  ' . $_SESSION['CustomerRecord' . $identifier]['terms'] . '<br />' . _('Credit Limit') . ': ' . locale_number_format($_SESSION['CustomerRecord'.$identifier]['creditlimit'],0) . '<br/>' . _('Credit Status') . ': ' . $_SESSION['CustomerRecord'.$identifier]['reasondescription'].'</small></h3><br />';

	if ($_SESSION['CustomerRecord' . $identifier]['dissallowinvoices']!=0){
	   echo '<br />
			<h2 class="text-danger">' . _('ACCOUNT ON HOLD') . '</h2>
			<br/>';
	}
...
<table id="general-table" class="table table-bordered">
	...
	echo '<tr>
		...
		<td>' . locale_number_format($_SESSION['CustomerRecord' . $identifier]['overdue2'],$_SESSION['CustomerRecord' . $identifier]['currdecimalplaces']) . '</td>
		<td><a href="CustomerInquiry.php?CustomerID=' . $_POST['CustomerID'] . '&Status=0" target="_blank" class="btn btn-info">' . _('Inquiry') . '</td>
		</tr>
		</table>
		</div>
		</div>
...
<div class="block"><div class="block-title"><h2>
			' . _('General Ledger Receipt Entry') . '</h2>
			</div>';
...
// End select tag
	...
	echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('GL Account') . '</label>
			<select tabindex="8" name="GLCode" class="form-control">';
	...
	$result=DB_query($SQL);
	if (DB_num_rows($result)==0){
		echo '</select>' . _('No General ledger accounts have been set up yet') . ' - ' . _('receipts cannot be entered against GL accounts until the GL accounts are set up') . '</div>
			</div>';
	} else {
		echo '<option value=""></option>';
		while ($myrow=DB_fetch_array($result)){
...
if (((isset($_SESSION['CustomerRecord' . $identifier])
	...
	echo '<div class="row"><div class="col-xs-4">
        <div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Amount of Receipt') . '</label>
		<input tabindex="9" type="text" class="form-control" name="Amount" required="required" maxlength="12" size="13" value="' . $_POST['Amount'] . '" /></div>
		</div>
		';

	if (!isset($_POST['GLEntry'])){
		echo '<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Amount of Discount') . '</label>
				<input tabindex="10" type="text" name="Discount" maxlength="12" size="13" class="form-control" value="' . $_POST['Discount'] . '" /> ' . _('agreed prompt payment discount is') . ' ' . $DisplayDiscountPercent . '</div>
			</div>';
	} else {
	...
	echo '
	<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Payee Bank Details') . '</label>
		<input tabindex="12" type="text" class="form-control" name="PayeeBankDetail" maxlength="22" size="20" value="' . $_POST['PayeeBankDetail'] . '" /></div>
		</div></div>
		<div class="row">
		<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Narrative') . '</label>
		<textarea name="Narrative" cols="40" class="form-control" rows="1"></textarea></div>
		</div>
        ...
        <div class="form-group"><br />
			...
			<input tabindex="14" type="submit" class="btn btn-success" name="Process" value="' . _('Accept') . '" />
			</div>
			</div>
		<div class="col-xs-4">
        <div class="form-group"><br />	
			<input tabindex="15" type="submit" class="btn btn-danger" name="Cancel" value="' . _('Cancel') . '" />
		</div>
		</div>
		</div>
		</div>
	...
	/*Show the form to select a customer */
		echo '<div class="row col-md-12"><h3><strong>' . ' ' . _('Select a Customer for entering receipt or enter receipt directly to GL') . '</strong></h3></div>
		
		<div class="row">
		<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Customer Name') . ' ' . '' . _('-part or full') . '</label>
			<input tabindex="9" class="form-control" type="text" name="Keywords" size="15" maxlength="25" /></div>
			</div>
			
			<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Customer') . ' ' . '' . _('code-part or full') . '</label>
			<input tabindex="10" class="form-control" type="text" name="CustCode" data-type="no-illegal-chars" title="' . _('Enter an extract of the customer code to search for. Customer codes can contain any alpha-numeric character or underscore') . '" size="10" maxlength="18" /></div></div>
			
			<div class="col-xs-4">
        <div class="form-group"> <label class="col-md-8 control-label">' . _('Customer invoice number') . '</label>
			<input tabindex="11" class="form-control" type="text" name="CustInvNo" class="integer" size="8" maxlength="8" /></div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-4">
        <div class="form-group"> 
			<input tabindex="11" type="submit" class="btn btn-info" name="Search" value="' . _('Search') . '" />
			</div></div>
			<div class="col-xs-4">
        <div class="form-group"> 
			<input tabindex="12" class="btn btn-info" type="submit" name="GLEntry" value="' . _('Enter A GL Receipt') . '" />
			</div>
		</div>
		</div>
		';
...
';
		...
		while ($myrow=DB_fetch_array($CustomerSearchResult)) {
			...
			printf('<tr class="striped_row">
					<td><input tabindex="'. strval(12+$j).'" type="submit" name="Select" value="%s" class="btn btn-info" /></td>
					<td>%s</td>
					</tr>',
					$myrow['debtorno'],
					$myrow['name']);
...
if (isset($_SESSION['ReceiptBatch' . $identifier])){
	...
	if (!$BankAccountEmpty) {
		...
		echo '<div class="row col-md-12"><h2><strong>
             ' . ' ' . $PaymentTypeString . ' - ' . _('Deposited to') . " " .
				$_SESSION['ReceiptBatch' . $identifier]->BankAccountName . ' ' . _('on') . ' ' . $_SESSION['ReceiptBatch' . $identifier]->DateBanked . '</strong><br /><br /></h2></div>';
	}
...
<table id="general-table" class="table table-bordered">
	...
	foreach ($_SESSION['ReceiptBatch' . $identifier]->Items as $ReceiptItem) {
		...
		echo '<tr>
				...
				<td>' . locale_number_format($ReceiptItem->Discount,$_SESSION['ReceiptBatch' . $identifier]->CurrDecimalPlaces) . '</td>
				...
				<td><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Delete='     . urlencode($ReceiptItem->ID)
                                                                                           ...
                                                                                           . '&identifier=' . urlencode($identifier) . '" class="btn btn-danger">'
                                                                                           . _('Delete') . '</a></td>
			</tr>';
	...
	echo '<tr><td><strong>Total</strong></td>
			<td colspan="5" align="right"><strong>' . locale_number_format($BatchTotal,$_SESSION['ReceiptBatch' . $identifier]->CurrDecimalPlaces) . '</strong></td>
		</tr>
...
if (isset($_SESSION['ReceiptBatch' . $identifier]->Items) AND count($_SESSION['ReceiptBatch' . $identifier]->Items) > 0){
	echo '<div class="row">
			<div class="col-xs-4">
			<div class="form-group">
			<input tabindex="13" type="submit" class="btn btn-info" name="CommitBatch" value="' . _('Accept and Process Batch') . '" />
		</div>
		</div>
		</div>
		';
