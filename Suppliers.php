...
Function Is_ValidAccount ($ActNo) {
	...
	echo _('The prefix') . ' - ' . $BankPrefix . ' ' . _('is not a valid New Zealand Bank') . '.<br />' .
			_('If you are using nERP outside New Zealand error trapping relevant to your country should be used');
	return False;
	exit;
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Suppliers') . '</h1></a></div>';
if (isset($SupplierID)) {
	echo '<br /><p align="left">
			<a href="' . $RootPath . '/SupplierContacts.php?SupplierID=' . $SupplierID . '" class="btn btn-default">' . _('Review Supplier Contact Details') . '</a>
		</p><br />';
}
$InputError = 0;
...
if (isset($_POST['submit'])) {
	...
	if ($myrow[0]>0 and isset($_POST['New'])) {
		$InputError = 1;
		echo prnMsg( _('The supplier number already exists in the system'),'error');
		$Errors[$i] = 'ID';
		$i++;
	}
	if (mb_strlen(trim($_POST['SuppName'])) > 40
		...
		$InputError = 1;
		echo prnMsg(_('The supplier name must be entered and be forty characters or less'),'error');
		$Errors[$i]='Name';
		$i++;
	}
	if ($_SESSION['AutoSupplierNo']==0 AND mb_strlen($SupplierID) == 0) {
		$InputError = 1;
		echo prnMsg(_('The Supplier Code cannot be empty'),'error');
		$Errors[$i]='ID';
		$i++;
	}
	if (ContainsIllegalCharacters($SupplierID)) {
		$InputError = 1;
		echo prnMsg(_('The supplier code cannot contain any of the illegal characters') ,'error');
		$Errors[$i]='ID';
		$i++;
	}
	if (mb_strlen($_POST['Phone']) >25) {
		$InputError = 1;
		echo prnMsg(_('The telephone number must be 25 characters or less'),'error');
		$Errors[$i] = 'Telephone';
		$i++;
	}
	if (mb_strlen($_POST['Fax']) >25) {
		$InputError = 1;
		echo prnMsg(_('The fax number must be 25 characters or less'),'error');
		$Errors[$i] = 'Fax';
		$i++;
	}
	if (mb_strlen($_POST['Email']) >55) {
		$InputError = 1;
		echo prnMsg(_('The email address must be 55 characters or less'),'error');
		$Errors[$i] = 'Email';
		$i++;
	}
	if (mb_strlen($_POST['Email'])>0 AND !IsEmailAddress($_POST['Email'])) {
		...
		$InputError = 1;
		echo prnMsg(_('The URL address must be 50 characters or less'),'error');
		$Errors[$i] = 'URL';
		$i++;
	}
	if (mb_strlen($_POST['BankRef']) > 12) {
		$InputError = 1;
		echo prnMsg(_('The bank reference text must be less than 12 characters long'),'error');
		$Errors[$i]='BankRef';
		$i++;
	}
	if (!Is_Date($_POST['SupplierSince'])) {
		$InputError = 1;
		echo prnMsg(_('The supplier since field must be a date in the format') . ' ' . $_SESSION['DefaultDateFormat'],'error');
	...
	*/
	...
	if ($InputError != 1) {
		...
		if ($_SESSION['geocode_integration']==1 ) {
			...
			if ($map_host=="") {
			echo prnMsg(_('Geocode Integration is enabled, but no hosts are setup.  Go to Geocode Setup'), 'warn');
			} else {
...
//			$xml = simplexml_load_file($request_url) or die("url not loading");
			...
			// failure to geocode
				$geocode_pending = false;
				echo '<p class="text-danger">' . _('Address') . ': ' . $address . ' ' . _('failed to geocode') ."\n";
		...
		if (!isset($_POST['New'])) {
			...
			if ($supptrans == 0) {
				...
				if ($suppcurr[0] != $_POST['CurrCode']) {
					echo prnMsg( _('Cannot change currency code as transactions already exist'), 'info');
				}
				$sql = "UPDATE suppliers SET suppname='" . $_POST['SuppName'] . "',
							address1='" . $_POST['Address1'] . "',
							address2='" . $_POST['Address2'] . "',
			...
			echo prnMsg(_('The supplier master record for') . ' ' . $SupplierID . ' ' . _('has been updated'),'success');
			...
			echo prnMsg(_('A new supplier for') . ' ' . $_POST['SuppName'] . ' ' . _('has been added to the system'),'success');
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'SuppTrans' , PurchOrders, SupplierContacts
	...
	if ($myrow[0] > 0) {
		$CancelDelete = 1;
		echo prnMsg(_('Cannot delete this supplier because there are transactions that refer to this supplier'),'warn');
		...
		if ($myrow[0] > 0) {
			$CancelDelete = 1;
			echo prnMsg(_('Cannot delete the supplier record because purchase orders have been created against this supplier'),'warn');
			...
			if ($myrow[0] > 0) {
				$CancelDelete = 1;
				echo prnMsg(_('Cannot delete this supplier because there are supplier contacts set up against it') . ' - ' . _('delete these first'),'warn');
	...
	if ($CancelDelete == 0) {
		...
		$result = DB_query($sql);
		echo prnMsg(_('Supplier record for') . ' ' . $SupplierID . ' ' . _('has been deleted'),'success');
		unset($SupplierID);
...
<div class="col-xs-12">';
	...
	if ($_SESSION['AutoSupplierNo']== 0 ) {
		echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Supplier Code') . '</label>
			<input type="text" data-type="no-illegal-chars" title="'._('The supplier id should not be within 10 legal characters and cannot be blank').'" required="required" class="form-control" name="SupplierID" placeholder="'._('within 10 characters').'" size="11" maxlength="10" /></div>
			</div></div>';
	}
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Supplier Name') . '</label>
			<td><input type="text" pattern="(?!^\s+$)[^<>+]{1,40}" required="required" title="'._('The supplier name should not be blank and should be less than 40 legal characters').'" class="form-control" name="SuppName" size="42" placeholder="'._('Within 40 legal characters').'" maxlength="40" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 1 (Street)') . '</label>
			<input type="text" pattern=".{1,40}" title="'._('The input should be less than 40 characters').'" placeholder="'._('Less than 40 characters').'" name="Address1" class="form-control" size="42" maxlength="40" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 2 (Street)') . '</label>
			<input type="text" name="Address2" pattern=".{1,40}" title="'._('The input should be less than 40 characters').'" placeholder="'._('Less than 40 characters').'" size="42" class="form-control" maxlength="40" /></div>
		</div></div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 3 (Suburb/City)') . '</label>
			<input type="text" title="'._('The input should be less than 40 characters').'" placeholder="'._('Less than 40 characters').'" name="Address3" size="42" class="form-control" maxlength="40" /></div>
		</div>
		
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 4 (State/Province)') . '</label>
			<input type="text" name="Address4" class="form-control" placeholder="'._('Less than 50 characters').'" size="42" maxlength="50" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 5 (Postal Code)') . '</label>
			<input type="text" class="form-control" name="Address5" size="42" placeholder="'._('Less than 40 characters').'" maxlength="40" /></div></div>
		</div>
		<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Country') . '</label>
	...
	echo '</select>
			...
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Telephone') . '</label>
			<input type="tel" pattern="[\s\d+)(-]{1,40}" title="'._('The input should be phone number').'" placeholder="'._('only number + - ( and ) allowed').'" class="form-control" name="Phone" size="30" maxlength="40" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Facsimile') . '</label>
			<input type="tel" pattern="[\s\d+)(-]{1,40}" title="'._('The input should be fax number').'" placeholder="'._('only number + - ( and ) allowed').'" name="Fax" class="form-control" size="30" maxlength="40" /></div>
		</div></div>
		<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Email Address') . '</label>
			<input type="email" class="form-control" name="Email" title="'._('Only email address are allowed').'" placeholder="'._('email format such as xx@mail.cn').'" size="30" maxlength="50" pattern="[a-z0-9!#$%&\'*+/=?^_` {|}~.-]+@[a-z0-9-]+(\.[a-z0-9-]+)*" /></div>
		</div>
		
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('URL') . '</label>
			<input type="url" class="form-control" name="URL" title="'._('Only URL address are allowed').'" placeholder="'._('URL format such as www.example.com').'" size="30" maxlength="50" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier Type') . '</label>
	...
	echo '<div class="row">
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier Since') . ' (' . $_SESSION['DefaultDateFormat'] . ')</label>
			<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="SupplierSince" value="' . $DateString . '" size="11" maxlength="10" /></div>
		</div>
		
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Bank Name') . '</label>
			<input type="text" class="form-control" name="BankPartics" size="13" maxlength="12" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('IFSC Code') . '</label>
			<input type="text" class="form-control" name="BankRef" value="0" size="13" maxlength="12" /></div>
		</div></div>
		<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Bank Account No') . '</label>
			<input type="text" class="form-control" placeholder="'._('Less than 30 characters').'" name="BankAct" size="31" maxlength="30" /></div></div>';

	$result=DB_query("SELECT terms, termsindicator FROM paymentterms");

	echo '
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Payment Terms') . '</label>
...
//<div class="form-group"> <label class="col-md-8 control-label">' . _('Factor Company') . '</label>
...
//		</div>';
		echo '
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('GST #') . '</label>
			<input type="text" class="form-control" name="TaxRef" placehoder="'._('Within 20 characters').'" size="21" maxlength="20" /></div></div></div>';
	...
	echo '<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier Currency') . '</label>
	...
	echo '</select></div>
		...
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Remittance Advice') . '</label>
			<select name="Remittance" class="form-control">
				<option value="0">' . _('Not Required') . '</option>
				<option value="1">' . _('Required') . '</option>
				</select></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Tax Group') . '</label>
...
<input type="submit" class="btn btn-success" name="submit" value="' . _('Add New Supplier') . '" />';
	echo '</div><br />
		</form></div></div>';
}
...
<div class="form-group"> <label class="col-md-8 control-label">';
			echo _('Supplier Code') . '</label>
					<input '.(in_array('ID',$Errors) ? 'class="inputerror"' : '').' type="text" class="form-control" name="SupplierID" value="' . $SupplierID . '" size="12" maxlength="10" /></div></div></div>';
		}
	}

	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier Name') . '</label>
			<input '.(in_array('Name',$Errors) ? 'class="inputerror"' : '').' type="text" class="form-control" name="SuppName" value="' . $_POST['SuppName'] . '" size="42" maxlength="40" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 1 (Street)') . '</label>
			<input type="text" name="Address1" class="form-control" value="' . $_POST['Address1'] . '" size="42" maxlength="40" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 2 (Street)') . '</label>
			<input type="text" name="Address2" class="form-control" value="' . $_POST['Address2'] . '" size="42" maxlength="40" /></div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 3 (Suburb/City)') . '</label>
			<input type="text" name="Address3" class="form-control" placeholder="'._('Within 40 characters').'" value="' . $_POST['Address3'] . '" size="42" maxlength="40" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 4 (State/Province)') . '</label>
			<input type="text" name="Address4" class="form-control" value="' . $_POST['Address4'] . '" placeholder="'._('Within 40 characters').'" size="42" maxlength="40" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Address Line 5 (Postal Code)') . '</label>
			<input type="text" name="Address5" class="form-control" value="' . $_POST['Address5'] . '" size="42" placeholder="'._('Within 40 characters').'" maxlength="40" /></div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Country') . '</label>
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Telephone') . '</label>
			<input '.(in_array('Name',$Errors) ? 'class="inputerror"' : '').' type="tel" class="form-control" pattern="[\s\d+()-]{1,40}" placeholder="'._('Only digit blank ( ) and - allowed').'" name="Phone" value="' . $_POST['Phone'] . '" size="42" maxlength="40" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Facsimile') . '</label>
			<input '.(in_array('Name',$Errors) ? 'class="inputerror"' : '').' type="tel" class="form-control" pattern="[\s\d+()-]{1,40}" placeholder="'._('Only digit blank ( ) and - allowed').'" name="Fax" value="' . $_POST['Fax'] . '" size="42" maxlength="40" /></div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Email Address') . '</label>
			<input '.(in_array('Name',$Errors) ? 'class="inputerror"' : '').' type="email" title="'._('The input must be in email format').'" name="Email" value="' . $_POST['Email'] . '" size="42" maxlength="40" class="form-control" placeholder="'._('email format such as xx@mail.cn').'" pattern="[a-z0-9!#$%&\'*+/=?^_` {|}~.-]+@[a-z0-9-]+(\.[a-z0-9-]+)*" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('URL') . '</label>
			<input '.(in_array('Name',$Errors) ? 'class="inputerror"' : '').' type="url" title="'._('The input must be in url format').'" name="URL" value="' . $_POST['URL'] . '" size="42" maxlength="40" class="form-control" placeholder="'._('url format such as www.example.com').'" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier Type') . '</label>
			<select name="SupplierType" class="form-control">';
	...
	echo '</select></div>
		...
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Supplier Since') . ' (' . $_SESSION['DefaultDateFormat'] .')</label>
			<input '.(in_array('SupplierSince',$Errors) ? 'class="inputerror"' : '').' size="11" maxlength="10" type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="SupplierSince" value="' . $_POST['SupplierSince'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Bank Particulars') . '</label>
			<input type="text" name="BankPartics" size="13" class="form-control" maxlength="12" value="' . $_POST['BankPartics'] . '" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Bank Reference') . '</label>
			<input '.(in_array('BankRef',$Errors) ? 'class="inputerror"' : '').' type="text" name="BankRef" class="form-control" size="13" maxlength="12" value="' . $_POST['BankRef'] . '" /></div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Bank Account No') . '</label>
			<input type="text" class="form-control" name="BankAct" size="31" maxlength="30" value="' . $_POST['BankAct'] . '" /></div>
		</div>';
...
//	echo '</select></div></div></div>';
	echo '
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Tax Reference') . '</label>
			<input type="text" class="form-control" name="TaxRef" size="21" maxlength="20" value="' . $_POST['TaxRef'] .'" /></div>
		</div></div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Tax Group') . '</label>
	...
	if (isset($_POST['New'])) {
		...
		echo '<div class="row"><div class="col-xs-4"></div><div class="col-xs-4">

					<input type="submit" class="btn btn-success" name="submit" value="' . _('Update Supplier') . '" />
				</div>
				<div class="col-xs-4">

				<input type="submit" class="btn btn-danger" name="delete" value="' . _('Delete Supplier') . '" onclick="return confirm(\'' . _('Are you sure you wish to delete this supplier?') . '\');" />
			</div></div>
			<br />';
//		echo '<p><font color=red><b>' . _('WARNING') . ': ' . _('There is no second warning if you hit the delete button below') . '. ' . _('However checks will be made to ensure there are no outstanding purchase orders or existing accounts payable transactions before the deletion is processed') . '<br /></font></b>';
		echo prnMsg(_('WARNING') . ': ' . _('There is no second warning if you hit the delete button above') . '. ' . _('However checks will be made to ensure there are no outstanding purchase orders or existing accounts payable transactions before the deletion is processed'), 'warn');
