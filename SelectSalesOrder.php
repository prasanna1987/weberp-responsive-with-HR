...
if (isset($_POST['PlacePO'])){ /*user hit button to place PO for selected orders */
	...
	if (mb_strlen($OrdersToPlacePOFor)==''){
		echo   prnMsg(_('There were no sales orders checked to place purchase orders for. No purchase orders will be created.'),'info');
	} else {
   /*  Now build SQL of items to purchase with purchasing data and preferred suppliers - sorted by preferred supplier */
		...
		*/
		...
		if (count($ItemArray)==0){
			echo  prnMsg(_('There might be no supplier purchasing data set up for any items on the selected sales order(s). No purchase orders have been created'),'warn');
		} else {
			...
			foreach ($ItemArray as $ItemRow) {
				...
				/* This order item is purchased from a different supplier so need to finish off the authorisation of the previous order and start a new order */
					...
					if ($SupplierID !=''){ //then we have just added a purchase order
						echo '<br />';
						echo prnMsg(_('Purchase Order') . ' ' . $PO_OrderNo . ' ' . _('on') . ' ' . $SupplierID . ' ' . _('has been created'),'success');
						DB_Txn_Commit();
					}
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Outstanding Sales Orders/Quotations') . '</h1></a></div> ';
echo '
...
if (isset($OrderNumber) AND $OrderNumber!='') {
	...
	if (!is_numeric($OrderNumber)){
		echo '<br />
			<strong>' . _('The Order Number entered MUST be numeric') . '</strong>
			<br />';
		unset ($OrderNumber);
...
if (!isset($StockID)) {
	...
	if (!isset($OrderNumber) OR $OrderNumber==''){
        ...
        <label class="col-md-8 control-label">' . _('Order number') . ': </label>
				<input type="text" class="form-control" name="OrderNumber" maxlength="8" size="9" /></div></div>
				<div class="col-xs-2">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Location') . '</label>
		...
		echo '</select></div></div>
			<div class="col-xs-2">
        <div class="form-group"> 
        <label class="col-md-8 control-label">Type</label><select name="Quotations" class="form-control">';

		if ( $_POST['Quotations'] == 'Quotes_Only' ){
			echo '<option selected="selected" value="Quotes_Only">' . _('Quotations') . '</option>';
			echo '<option value="Orders_Only">' . _('Orders')  . '</option>';
			echo '<option value="Overdue_Only">' . _('Overdue') . '</option>';
		} elseif ( $_POST['Quotations'] == 'Overdue_Only' ){
			echo '<option selected="selected" value="Overdue_Only">' . _('Overdue') . '</option>';
			echo '<option value="Quotes_Only">' . _('Quotations') . '</option>';
			echo '<option value="Orders_Only">' . _('Orders') . '</option>';
		} else {
			echo '<option selected="selected" value="Orders_Only">' . _('Orders') . '</option>';
			echo '<option value="Quotes_Only">' . _('Quotations') . '</option>';
			echo '<option value="Overdue_Only">' . _('Overdue') . '</option>';
		}
		...
		echo '</select>
			...
			<div class="col-xs-2">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Customer Ref') . '</label>
				<input type="text" class="form-control" name="CustomerRef" value="' . $_POST['CustomerRef'] . '" size="12" /></div>
			</div>
				...
				<div class="col-xs-2">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Due Date From') . '</label>
				<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="DueDateFrom" value="' . $_POST['DueDateFrom'] . '" size="10" /></div>
				</div>
				
				
				<div class="col-xs-2">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Due Date To') . '</label>
				<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="DueDateTo" value="' . $_POST['DueDateTo'] . '" size="10" /></div>
			</div>
			</div>
			...
			<div class="row">
				<div class="col-xs-2">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Order Date From') . '</label>
				<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="OrderDateFrom" value="' . $_POST['OrderDateFrom'] . '" size="10" class="date form-control" />
				</div>
				</div>
				<div class="col-xs-2">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Order Date To') . '</label>
		<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="OrderDateTo" value="' . $_POST['OrderDateTo'] . '" size="10" class="date form-control" />
			</div>
			</div>
        ...
        <div class="form-group"> 
        <br>
				<input type="submit" name="SearchOrders" value="' . _('Search') . '" class="btn btn-success" /></div>
				</div>
        ...
        <div class="form-group">
		 <br> 
       <a href="' . $RootPath . '/SelectOrderItems.php?NewOrder=Yes" class="btn btn-info">' . _('New Sales Order') . '</a></div>
				</div>
			</div>
		';
	}
	...
	echo '<br /><div class="row">
		<h3>' . _('To search for sales orders for a specific part use the part selection facilities below') . '</h3>
		<br />
		<div class="col-xs-2">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Stock category') . '</label>
	...
	echo '</select></div></div>
			<div class="col-xs-3">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Description-part or full') . '</label>
			<input type="text" class="form-control" name="Keywords" size="20" maxlength="25" /></div>
      		...
      		<div class="col-xs-3">
        <div class="form-group"> 
        <label class="col-md-8 control-label">' . _('Stock ID-part or full') . '</label>
      		<input type="text" class="form-control" name="StockCode" size="15" maxlength="18"  value="' . $_POST['StockCode'] . '" /></div>
      	</div>
      ';
	echo '<div class="col-xs-2">
        <div class="form-group"> 
        <br />
			<input type="submit" name="SearchParts" class="btn btn-success" value="' . _('Search') . '" /></div></div>
		<div class="col-xs-2">
        <div class="form-group"> 
        <br />
...
//end if stock search results to show
	...
	//Harmonize the ordervalue with SUM function since nERP allowed same items appeared several times in one sales orders. If there is no sum value, this situation not inclued.
	...
	if (DB_num_rows($SalesOrdersResult)>0) {
		...
		if ( $_POST['Quotations'] == 'Orders_Only' OR $_POST['Quotations'] == 'Overdue_Only' ){
			...
			echo '<thead><tr>
								...
								<th>' . _('Required Date') . '</th>
								<th>' . _('Delivery To') . '</th>
								<th colspan="3">' . _('Quote Total') .  '<br />' . $_SESSION['CompanyRecord']['currencydefault'] . '</th>
		...
		while ($myrow=DB_fetch_array($SalesOrdersResult)) {
			...
			if ($_SESSION['RequirePickingNote'] == 1) {
				...
				if (isset($myrow['prid']) and $myrow['prid'] > '') {
					$PrintPickLabel = '<td><a href="' . $RootPath . '/GeneratePickingList.php?TransNo=' . $myrow['orderno'] . '" class="btn btn-info">' . str_pad($myrow['prid'], 10, '0', STR_PAD_LEFT) . '</a></td>';
				} else {
					$PrintPickLabel = '<td><a href="' . $RootPath . '/GeneratePickingList.php?TransNo=' . $myrow['orderno'] . '" class="btn btn-info">' . _('Pick') . '</a></td>';
				}
				$PrintDummyFlag = '';
			}
			...
			if ($myrow['customerref'] !== '') {
				$CustomerRef = '<a href="' . $RootPath . '/SelectCompletedOrder.php?CustomerRef=' . $myrow['customerref'] . '" target="_blank" class="btn btn-info">' . $myrow['customerref'] . '</a>';
			} else {
				$CustomerRef = '';
			}
			 ...
			 /*Check authority to create POs if user has authority then show the check boxes to select sales orders to place POs for otherwise don't provide this option */
				if ($AuthRow['cancreate']==0 AND $myrow['poplaced']==0){ //cancreate==0 if the user can create POs and not already placed
					echo '<tr class="striped_row">
							<td><a href="' . $ModifyPage . '" class="btn btn-info">' . $myrow['orderno'] . '</a></td>
							<td><a href="' . $PrintAck . '" class="btn btn-info">' . _('Print') . '</a>' . $PrintDummyFlag . '</td>
							' . $PrintPickLabel . '
							<td><a href="' . $Confirm_Invoice . '" class="btn btn-success">' . _('Invoice') . '</a></td>
			 				<td><a target="_blank" href="' . $PrintDispatchNote . '" class="btn btn-info">' . $PrintText . ' </a></td>
					...
					printf('<tr class="striped_row">
							...
							<td><a href="%s" class="btn btn-info">' . _('Print') . '</a></td>
							' . $PrintPickLabel . '
							<td><a href="%s" class="btn btn-success">' . _('Invoice') . '</a></td>
							<td><a target="_blank" href="%s" class="btn btn-info">' . $PrintText . ' </a></td>
				...
				printf('<tr class="striped_row">
						<td><a href="%s" class="btn btn-info">%s</a></td>
						<td><a target="_blank" href="%s" class="btn btn-warning">' . _('Print') . '</a></td>
						<td>%s</td>
						<td>%s</td>
						<td>%s</td>
						<td>%s</td>
						<td>%s</td>
						<td>%s</td>
						<td class="number" colspan="3">%s</td>
						</tr>',
						$ModifyPage,
						$myrow['orderno'],
		...
		echo '</tbody>
				...
				<tr>
					<td colspan="10" class="number"><strong>';
		...
		echo ' ' . $_SESSION['CompanyRecord']['currencydefault'] . ':	</strong></td>
		...
		if ($_POST['Quotations']=='Orders_Only' AND $AuthRow['cancreate']==0){ //cancreate==0 means can create POs
			echo '<td>
					<input type="submit" name="PlacePO" value="' . _('Place') . " " . _('PO') . '" onclick="return confirm(\'' . _('This will create purchase orders for all the items on the checked sales orders above, based on the preferred supplier purchasing data held in the system. Are You Absolutely Sure?') . '\');" class="btn btn-info" />
				</td>';
		}
