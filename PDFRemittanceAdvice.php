...
If ((isset($_POST['PrintPDF']))
	...
	if (DB_num_rows($SuppliersResult)==0){
		...
		echo   prnMsg(_('There were no remittance advices to print out for the supplier range and payment date specified'),'warn');
		echo '<br /><p align="right"><a href="'.htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-default">' .  _('<i class="fa fa-hand-o-left fa-fw"></i> Back') . '</a>';
...
/*then print the report */
	...
	while ($SuppliersPaid = DB_fetch_array($SuppliersResult)){
		...
		if (DB_error_no() !=0) {
			...
			include('includes/header.php');
			echo   prnMsg(_('The details of the payment to the supplier could not be retrieved because') . ' - ' . DB_error_msg(),'error');
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('From Supplier Code') . '</label>
			<input type="text" maxlength="6" class="form-control" size="7" name="FromCriteria" value="' . $DefaultFromCriteria . '" /></div>
		</div>';
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('To Supplier Code') . '</label>
			<input type="text" class="form-control" maxlength="6" size="7" name="ToCriteria" value="' . $DefaultToCriteria . '" /></div>
		</div>';
	...
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Date Of Payment') . '</label>
			<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="PaymentDate" maxlength="10" size="11" value="' . $DefaultDate . '" /></div>
		</div></div>';
