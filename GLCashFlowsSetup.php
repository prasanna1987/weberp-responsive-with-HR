	...
	case 'Update':
		...
		if($_SESSION['PeriodProfitAccount'] != $_POST['PeriodProfitAccount'] ) {
			if(DB_query(
				...
				echo prnMsg(_('The net profit of the period GL account was updated'), 'success');
			}
		}
		if($_SESSION['RetainedEarningsAccount'] != $_POST['RetainedEarningsAccount'] ) {
			if(DB_query(
				...
				echo prnMsg(_('The retained earnings GL account was updated'), 'success');
			}
		}
		break;// END Update.
	case 'Reset':
		...
		if($Result) {
			echo prnMsg(_('The cash flow activity was reset in all accounts'), 'success');
		}
		break;// END Reset.
	case 'Automatic':
		...
		if($Result) {
			echo prnMsg(_('The cash flow activity was updated in some accounts'), 'success');// RChacon: Show replacements done.
		}
		break;// END Automatic.
	case 'Manual':
...
echo '<br />',
	...
	'<input name="FormID" type="hidden" value="', $_SESSION['FormID'], '" />', // Form's head.
		...
		'<div class="col-xs-2">',
						'<button name="Action" type="submit" value="Update" class="btn btn-info"> ', _('Update'), '</button></div>', // "Update" button.
						'<div class="col-xs-3"><button name="Action" type="submit" value="Reset" class="btn btn-danger">', _('Reset values'), '</button></div>', // "Reset values" button.
						'<div class="col-xs-3"><button name="Action" type="submit" value="Automatic" class="btn btn-info">', _('Automatic setup'), '</button></div>', // "Automatic setup" button.
						'
						<div class="col-xs-2"><button name="Action" type="submit" value="Manual" class="btn btn-info">', _('Manual setup'), '</button></div>', // "Manual setup" button.
						'<div class="col-xs-2"><button onclick="window.location=\'menu_data.php?Application=GL\'" type="button" class="btn btn-default">', _('Return'), '</button>', // "Return" button.
					'</div></div><br />',
				'';
...
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">', _('GL account to post the net profit for the period'), '</label>
	 			<select id="PeriodProfitAccount" name="PeriodProfitAccount" required="required" class="form-control">';
...
echo				'</select>',
				 // If it is not set the $field_help parameter OR it is TRUE, shows the page help text.*/
		 		'</div>
			</div>';
// Setups the retained earnings GL account:
echo		'<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">', _('GL account to post the retained earnings'), '</label>
<select id="RetainedEarningsAccount" name="RetainedEarningsAccount" required="required" class="form-control">';
