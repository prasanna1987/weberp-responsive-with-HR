...
echo '<div class="block-header"><a href="" class="header-title-link"><h1> ',// Icon title.
	_('General Ledger Account Inquiry'), '</h1></a></div>';// Page title.
...
echo '<div class="text-info"><strong>' . _('Use Ctrl/Shift key to select multiple periods') . '</strong></div><br />';
...
// End select tag
echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('For Period range').'</label>
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>', _('Transactions for account:'), ' ', $SelectedAccount, ' - ', $SelectedAccountName, '</h3></div>
...
<table id="general-table" class="table table-bordered">
	...
	if($PandLAccount==True) {
		...
		if($RunningTotal < 0 ) {// It is a credit balance b/fwd
			echo	'<td>&nbsp;</td>
					<td><b>', locale_number_format(-$RunningTotal,$_SESSION['CompanyRecord']['decimalplaces']), '</b></td>';
		} else {// It is a debit balance b/fwd
			echo	'<td><b>', locale_number_format($RunningTotal,$_SESSION['CompanyRecord']['decimalplaces']), '</b></td>
					<td>&nbsp;</td>';
		}
	...
	while($myrow=DB_fetch_array($TransResult)) {
		if($myrow['periodno']!=$PeriodNo) {
			if($PeriodNo!=-9999) { //ie its not the first time around
				...
				if($PeriodTotal < 0 ) {// It is a credit balance b/fwd
					echo	'<td>&nbsp;</td>
							<td><b>', locale_number_format(-$PeriodTotal,$_SESSION['CompanyRecord']['decimalplaces']), '</b></td>';
				} else {// It is a debit balance b/fwd
					echo	'<td><b>', locale_number_format($PeriodTotal,$_SESSION['CompanyRecord']['decimalplaces']), '</b></td>
							<td>&nbsp;</td>';
				}
				echo		'<td colspan="5">&nbsp;</td>
						</tr>';
		...
		echo	'<tr class="striped_row">
				...
				<td><a href="', $URL_to_TransDetail, '" class="btn btn-info">', $myrow['typeno'], '</a></td>
				<td>', $FormatedTranDate, '</td>
				<td>', $myrow['narrative'], '</td>
				<td>', $DebitAmount, '</td>
				<td>', $CreditAmount, '</td>
				<td>', locale_number_format($RunningTotal, $_SESSION['CompanyRecord']['decimalplaces']), '</td>
				<td>', $myrow['tagdescription'], '</td>';
		if(isset($BankAccount)) {
			echo '<td>', $Currency, '</td>
				<td><strong>', locale_number_format($OrgAmt, $_SESSION['CompanyRecord']['decimalplaces']), '</strong></td>
				<td>', $BankRef, '</td>';
		}
		echo '</tr>';
	}
...
if(isset($ShowIntegrityReport) AND $ShowIntegrityReport==True AND $_POST['tag']=='0') {
	...
	echo prnMsg( _('There are differences between the sum of the transactions and the recorded movements in the ChartDetails table') . '. ' . _('A log of the account differences for the periods report shows below'),'warn');
	echo '<p>' . $IntegrityReport.'</p>';
}
include('includes/footer.php');
?>
