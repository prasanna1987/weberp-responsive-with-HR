...
if(!isset($_POST['CreateCSV'])) {
	...
	echo '<div class="block-header"><a href="" class="header-title-link"><h1> ' . _('Customer Activity and Balances') . '</h1></a></div>';
}
...
if (!isset($_POST['RunReport'])){
	...
	echo '<form id="Form1" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">
			...
			<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Customer') . '</label>
...
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Sales Area') . '</label>
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Date From') . '</label>
					<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker"  name="FromDate" maxlength="10" size="11" value="' . Date($_SESSION['DefaultDateFormat'], Mktime(0, 0, 0, Date('m') - $_SESSION['NumberOfMonthMustBeShown'], Date('d'), Date('Y'))) . '" /></div>
				</div>
				
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Date To') . '</label>
					<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="ToDate" maxlength="10" size="11" value="' . Date($_SESSION['DefaultDateFormat']) . '" /></div>
				</div>
				
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Create CSV') . '</label>
					<input type="checkbox" name="CreateCSV" class="btn btn-info" value=""></div>
				</div>
...
<div class="form-group"> 
				<input tabindex="4" type="submit" class="btn btn-info" name="RunReport" value="' . _('Show Customer Balance Movements') . '" />
			</div>
		 </div>
		 </div>
	</form>
 ...
 */
	...
	if($OpeningBal !=0 OR $ClosingBal!=0 OR $TransRow['localdebits']!=0 OR $TransRow['localcredits']!=0) {
		...
		if (!isset($_POST['CreateCSV'])){
			echo '<tr>
					<td>' . $myrow['name'] . ' </td>
					<td>' . locale_number_format($OpeningBal,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
					<td>' . locale_number_format($TransRow['localdebits'],$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
					<td>' . locale_number_format($TransRow['localcredits'],$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
					<td>' . locale_number_format($ClosingBal,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
				</tr>';
...
<table id="general-table" class="table table-bordered">
			...
			<tr>
				<td>' . _('TOTALS') . '</td>
				<td>' . locale_number_format($OpeningBalances,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
				<td>' . locale_number_format($Debits,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
				<td>' . locale_number_format($Credits,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
				<td>' . locale_number_format($ClosingBalances,$_SESSION['CompanyRecord']['decimalplaces']) . '</td>
			</tr>
		</table></div></div></div>';
	} else {
