...
//	$pdf = & new Cpdf($PageSize);
	...
	$pdf->addInfo('Creator','nERP http://www.netelity.com');
	$pdf->addInfo('Author','nERP ' . $Version);
...
// Javier:
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
	   echo prnMsg(_('The inventory quantities could not be retrieved by nERP because') . ' - ' . DB_error_msg(),'error');
	  echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=stock" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
	...
	while ($InventoryPlan = DB_fetch_array($InventoryResult)){
		...
		if (DB_error_no() !=0) {
	 		 $Title = _('Inventory Planning') . ' - ' . _('Problem Report') . '....';
	  		include('includes/header.php');
	   		echo prnMsg( _('The sales quantities could not be retrieved by nERP because') . ' - ' . DB_error_msg(),'error');
	   		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=stock" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
		...
		if (DB_error_no() !=0) {
	 		$Title = _('Inventory Planning') . ' - ' . _('Problem Report') . '....';
	  		include('includes/header.php');
	   		echo prnMsg( _('The sales order demand quantities could not be retrieved by nERP because') . ' - ' . DB_error_msg(),'error');
	   		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=stock" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
...
// Also need to add in the demand as a component of an assembly items if this items has any assembly parents.
		...
		if (DB_error_no() !=0) {
	 		$Title = _('Inventory Planning') . ' - ' . _('Problem Report') . '....';
	  		include('includes/header.php');
	   		echo prnMsg( _('The sales order demand quantities from parent assemblies could not be retrieved by nERP because') . ' - ' . DB_error_msg(),'error');
	   		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=stock" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
	...
	if ($ListCount == 0){
		...
		include('includes/header.php');
		echo prnMsg( _('There were no items in the range and location specified'), 'error');
		echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=stock" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p>';
...
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Select Inventory Categories') . '</label>
	...
	echo '
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('For Inventory in Location') . '</label>
	...
	echo '
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Stock Planning') . '</label>
			<select name="NumberMonthsHolding" class="form-control">
...
<div class="form-group">
		...
		<div class="col-xs-8">
<div class="form-group">
<strong>Report of last 24 months</strong>
		<input type="submit" class="btn btn-warning" name="ExportToCSV" value="' . _('Export to CSV') . '" />
	</div>
	</div></div>
	</form></div></div>';
