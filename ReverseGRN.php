...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Reverse Goods Received from') . ' ' . $_POST['SuppName'] .  '</h1></a></div> ';
...
/* SQL to process the postings for the GRN reversal.. */
	...
	if ($QtyToReverse ==0){
		echo '<br />
				<<p class="text-info">' . _('The GRN') . ' ' . $_GET['GRNNo'] . ' ' . _('has already been reversed or fully invoiced by the supplier - it cannot be reversed - stock quantities must be corrected by stock adjustments - the stock is paid for'),'</p>';
	...
	if ($ControlledRow[0]==1) { /*Then its a controlled item */
		...
		while ($SerialStockMoves = DB_fetch_array($GetStockMoveResult)){
			...
			if ($GetQOH[0] < $SerialStockMoves['moveqty']){
				...
				echo prnMsg(_('Unfortunately, of the original number') . ' (' . $SerialStockMoves['moveqty'] . ') ' . _('that were received on serial number') . ' ' . $SerialStockMoves['serialno'] . ' ' . _('only') . ' ' . $GetQOH[0] . ' ' . _('remain') . '. ' . _('The GRN can only be reversed if all the original serial number items are still in stock in the location they were received into'),'error');
				include ('includes/footer.php');
				exit;
			}
		}
...
/*now the GRN suspense entry*/
	...
	echo '<br /><h4 class="text-info">' . _('GRN number') . ' ' . $_GET['GRNNo'] . ' ' . _('for') . ' ' . $QtyToReverse . ' x ' . $GRN['itemcode'] . ' - ' . $GRN['itemdescription'] . ' ' . _('has been reversed') . '</h4>';
	unset($_GET['GRNNo']);  // to ensure it cant be done again!!
	echo '<br /><p align="left"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn  btn-info">' . _('Back to Search') . '</a></p>';
...
<div class="col-xs-12">';
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Goods Received after') . ' </label>
			<input type="text" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" name="RecdAfterDate" value="' . $_POST['RecdAfterDate'] . '" maxlength="10" size="11" /></div>
		</div>
		<div class="col-xs-4">
<div class="form-group"> <br />
			<input type="submit" name="ShowGRNS" class="btn btn-success" value="' . _('Show') . '" />
		</div>
		</div>
		</div>
	</form></div></div>';
	...
	if (isset($_POST['ShowGRNS'])){
		...
		if (DB_num_rows($result) ==0){
			echo prnMsg(_('There are no outstanding goods received yet to be invoiced for') . ' ' . $_POST['SuppName'] . '.<br />' . _('To reverse a GRN that has been invoiced first it must be credited'),'warn');
