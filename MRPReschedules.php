...
/*Find mrpsupplies records where the duedate is not the same as the mrpdate */
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
	   echo prnMsg( _('The MRP reschedules could not be retrieved by nERP because') . ' '  . DB_error_msg(),'error');
	  echo '<br /><p align="center"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('enu') . '</a></p><br />';
	   if ($debug==1){
		  echo '<br />' . $sql;
	   }
	   include('includes/footer.php');
	...
	if (DB_num_rows($result) == 0) {
	  $Title = _('MRP Reschedules') . ' - ' . _('Problem Report');
	  include('includes/header.php');
	   echo prnMsg( _('No MRP reschedule retrieved'), 'warn');
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Print Option') . '</label>
		...
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Selection') . '</label>
