...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' .
		_('Tax Group Maintenance') . '</h1></a></div>';
...
if(isset($_POST['submit']) OR isset($_GET['remove']) OR isset($_GET['add']) ) {
	...
	if(isset($_POST['GroupName']) AND mb_strlen($_POST['GroupName'])<4) {
		$InputError = 1;
		echo prnMsg(_('The Group description entered must be at least 4 characters long'),'error');
	}
	...
	if(isset($_POST['GroupName']) ) { // Update or Add a tax group
		if(isset($SelectedGroup)) { // Update a tax group
			...
			if(DB_num_rows($result)==1) {
				echo prnMsg( _('A new tax group could not be added because a tax group already exists for') . ' ' . $_POST['GroupName'],'warn');
				unset($sql);
			} else {
	...
	if(isset($sql) AND $InputError != 1 ) {
		...
		if( $result ) {
			echo prnMsg( $SuccessMsg,'success');
		}
	}
	...
	if(DB_num_rows($Result)>0) {
		...
		if($myrow['taxontax']==1) {
			echo prnMsg(_('It is inappropriate to set tax on tax where the tax is the first in the calculation order. The system has changed it back to no tax on tax for this tax authority'),'warn');
	...
	if($myrow[0]>0) {
		echo prnMsg( _('Cannot delete this tax group because some customer branches are setup using it'),'warn');
		...
		$myrow = DB_fetch_row($result);
		if($myrow[0]>0) {
			echo prnMsg( _('Cannot delete this tax group because some suppliers are setup using it'),'warn');
			...
			$sql="DELETE FROM taxgroups
					WHERE taxgroupid='" . $_GET['SelectedGroup'] . "'";
			$result = DB_query($sql);
			echo prnMsg( $_GET['GroupID'] . ' ' . _('tax group has been deleted') . '!','success');
		}
...
/* If its the first time the page has been displayed with no parameters then none of the above are true and the list of tax groups will be displayed with links to delete or edit each. These will call the same page again and allow update/input or deletion of tax group taxes*/
	...
	if( DB_num_rows($result) == 0 ) {
		...
		echo prnMsg(_('There are no tax groups configured.'),'info');
		echo '</div>';
	} else {
...
<table id="general-table" class="table table-bordered">
		...
		while($myrow = DB_fetch_array($result)) {
			printf('<tr class="striped_row">
					...
					<td>%s</td>
					<td><a href="%s&amp;SelectedGroup=%s" class="btn btn-info">' . _('Edit') . '</a></td>
					<td><a href="%s&amp;SelectedGroup=%s&amp;Delete=1&amp;GroupID=%s" class="btn btn-danger" onclick="return confirm(\'' . _('Are you sure you wish to delete this tax group?') . '\');">' . _('Delete') . '</a></td>
					</tr>',
					$myrow['taxgroupid'],
...
if(isset($SelectedGroup)) {
	echo '<div class="row">
<div class="col-xs-4">
			<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-info">' . _('Back to  Groups') . '</a>
		</div></div><br />
';
}
...
if(isset($SelectedGroup)) {
	...
	$result = DB_query($sql);
	if( DB_num_rows($result) == 0 ) {
		echo prnMsg( _('The selected tax group is no longer available.'),'warn');
	} else {
		$myrow = DB_fetch_array($result);
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Tax Group') . '</label>
		<input pattern="(?!^ +$)[^><+-]{4,}" title="'._('The group name must be more 4 and less than 40 characters and cannot be left blank').'" placeholder="'._('4 to 40 legal characters').'" class="form-control" type="text" name="GroupName" size="40" maxlength="40" value="' . $_POST['GroupName'] . '" /></div></div>';
echo '<div class="col-xs-4">
<div class="form-group"> <br />
<input type="submit" name="submit" class="btn btn-success" value="' . _('Enter Information') . '" /></div>
	</div>
    </div>
   <br />
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Calculation Order') . '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">
		...
		for ($i=1;$i < count($TaxAuthRow)+1;$i++) {
			...
			echo '<tr class="striped_row">
				...
				<td><input type="text" class="form-control" pattern="(?!^0*$)(\d+)" title="'._('The input must be positive integer and less than 10').'" name="CalcOrder_' . $TaxAuthRow[$i]['taxauthid'] . '" value="' . $TaxAuthRow[$i]['calculationorder'] . '" size="1" maxlength="1" style="width: 90%" /></td>
				<td><select name="TaxOnTax_' . $TaxAuthRow[$i]['taxauthid'] . '" class="form-control">';
		...
		echo '<br />
			<div class="row">
			<div class="col-xs-4">
				<input type="submit" class="btn btn-info" name="UpdateOrder" value="' . _('Update Order') . '" />
			</div></div><br />';
	}
	echo '</form>';
...
<table id="general-table" class="table table-bordered">
		...
		echo '<br /><div class="row"><p class="text-info">' .
	...
	while($AvailRow = DB_fetch_array($Result)) {
		...
		if($TaxAuthUsedPointer) {
			...
			printf('<tr class="striped_row">
				...
				<td class="number">%s</td>
				<td>%s</td>
				<td><a href="%sSelectedGroup=%s&amp;remove=1&amp;TaxAuthority=%s" class="btn btn-danger" onclick="return confirm(\'' .
			...
			printf('<tr class="striped_row">
				...
				<td class="number">&nbsp;</td>
				<td>&nbsp;</td>
				<td><a href="%sSelectedGroup=%s&amp;add=1&amp;TaxAuthority=%s" class="btn btn-success">' .
					_('Add') . '</a></td>
				<td class="number">%s</td>
				<td>%s</td>
				</tr>',
