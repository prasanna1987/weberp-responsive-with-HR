...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Update Price By Cost') . '</h1></a></div>';
...
if (isset($_POST['submit']) OR isset($_POST['update'])) {
	...
	echo '<div class="text-info">' . _('Items in') . ' ' . $CategoryText . ' ' . _('With Prices') . ' ' . $Comparator . '' . $_POST['Margin'] . ' ' . _('times') . ' ' . _('Cost in Price List') . ' ' . $SalesTypeRow['sales_type'] . '</div><br />';
...
<table id="general-table" class="table table-bordered">
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			echo '<tr class="striped_row">
					...
					<td><input type="text" class="form-control" name="Price_' . $PriceCounter . '" maxlength="14" size="10" value="' . locale_number_format($myrow['price'],$myrow['decimalplaces']) . '" /></td>
				</tr> ';
			$PriceCounter++;
		} //end of looping
		...
		echo '</tbody></table>
			...
			<div class="row">
			<div class="col-xs-4">
			<input type="submit" name="submit" value="' . _('Update') . '" class="btn btn-success" onclick="return confirm(\'' . _('If the prices above do not have a commencement date as today, this will create new prices with commencement date of today at the entered figures and update the existing prices with historical start dates to have an end date of yesterday. Are You Sure?') . '\');" /></div>
			<div class="col-xs-4"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '"><input type="submit" value="' . _('Back') . '" class="btn btn-info" /></a></div>
			 </div>
			<br />
			
             </form>';
	} else {
		echo prnMsg(_('There were no prices meeting the criteria specified to review'),'info');
		echo '<br /><p align="right"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Back') . '<a/></p>';
	}
} else { /*The option to submit was not hit so display form */
	echo '<div class="text-info">' . _('Prices can be displayed based on their relation to cost') . '</div><br />';
	echo '<br />
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Price') . '</label>
	...
	if ($_SESSION['WeightedAverageCosting']==1) {
		echo '</select></div></div><div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . ' '. _('Average Cost') . ' x </label>';
	} else {
		echo '</select></div></div><div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . ' '. _('Standard Cost') . ' x </label>';
	}
	if (!isset($_POST['Margin'])){
		$_POST['Margin']=1;
	}
	echo '<input type="text" class="form-control" name="Margin" maxlength="8" size="8" value="' .$_POST['Margin'] . '" /></div>
		</div></div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Currency') . '</label>
	...
	echo '</select></div></div>';
	echo '<div class="col-xs-4">
<div class="form-group"> <br /><input type="submit" class="btn btn-success" name="submit" value="' . _('Submit') . '" /></div>
		</div></div><br />
	</form>';
