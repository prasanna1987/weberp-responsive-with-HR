...
echo '<p align="leftt"><a href="' . $RootPath . '/SelectAsset.php" class="btn btn-info">' . _('Back to Search') . '</a></p>';
...
if (isset($_POST['submit'])) {
	...
	if (!isset($_POST['Description']) or mb_strlen($_POST['Description']) > 50 OR mb_strlen($_POST['Description'])==0) {
		$InputError = 1;
		echo prnMsg (_('The asset description must be entered and be fifty characters or less long. It cannot be a zero length string either, a description is required'),'error');
		$Errors[$i] = 'Description';
		$i++;
	}
	if (mb_strlen($_POST['LongDescription'])==0) {
		$InputError = 1;
		echo prnMsg (_('The asset long description cannot be a zero length string, a long description is required'),'error');
	...
	if (mb_strlen($_POST['BarCode']) >20) {
		$InputError = 1;
		echo prnMsg(_('The barcode must be 20 characters or less long'),'error');
		$Errors[$i] = 'BarCode';
		$i++;
	}
	...
	if (trim($_POST['AssetCategoryID'])==''){
		$InputError = 1;
		echo prnMsg(_('There are no asset categories defined. All assets must belong to a valid category,'),'error');
		$Errors[$i] = 'AssetCategoryID';
		$i++;
	}
	if (trim($_POST['AssetLocation'])==''){
		$InputError = 1;
		echo prnMsg(_('There are no asset locations defined. All assets must belong to a valid location,'),'error');
	...
	if (!is_numeric(filter_number_format($_POST['DepnRate']))
		...
		$InputError = 1;
		echo prnMsg(_('The depreciation rate is expected to be a number between 0 and 100'),'error');
		$Errors[$i] = 'DepnRate';
		$i++;
	}
	if (filter_number_format($_POST['DepnRate'])>0 AND filter_number_format($_POST['DepnRate'])<1){
		echo prnMsg(_('Numbers less than 1 are interpreted as less than 1%. The depreciation rate should be entered as a number between 0 and 100'),'warn');
	}
	...
	if ($InputError !=1){
		...
		if ($_POST['submit']==_('Update')) { /*so its an existing one */
			...
			echo prnMsg( _('Asset') . ' ' . $AssetID . ' ' . _('has been updated'), 'success');
			echo '<br />';
			...
			if (DB_error_no() ==0) {
				...
				echo prnMsg( _('The new asset has been added to the database with an asset code of:') . ' ' . $NewAssetID,'success');
...
//the button to delete a selected record was clicked instead of the submit button
	...
	if ($NBV!=0) {
		...
		echo prnMsg(_('The asset still has a net book value - only assets with a zero net book value can be deleted'),'error');
	}
	...
	if (DB_num_rows($result) > 0){
		...
		echo prnMsg(_('The asset has transactions associated with it. The asset can only be deleted when the fixed asset transactions are purged, otherwise the integrity of fixed asset reports may be compromised'),'error');
	}
	...
	if (DB_num_rows($result) > 0){
		...
		echo prnMsg(_('There is a purchase order set up for this asset. The purchase order line must be deleted first'),'error');
	}
	if ($CancelDelete==0) {
		$result = DB_Txn_Begin();
		...
		echo prnMsg(_('Deleted the asset  record for asset number' ) . ' ' . $AssetID );
		unset($_POST['LongDescription']);
		unset($_POST['Description']);
...
/*If the page was called without $AssetID passed to page then assume a new asset is to be entered other wise the form showing the fields with the existing entries against the asset will show for editing with a hidden AssetID field. New is set to flag that the page may have called itself and still be entering a new asset, in which case the page needs to know not to go looking up details for an existing asset*/
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Asset Code') . '</label>
			' . $AssetID . '</div>
		</div></div>';
...
if (isset($AssetRow['disposaldate']) AND $AssetRow['disposaldate'] !='0000-00-00'){
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Asset Already disposed on') . '</label>
...
echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Asset Description') . ' (' . _('short') . '):</label>
		<input ' . (in_array('Description',$Errors) ?  'class="inputerror"' : '' ) .' type="text" required="required" title="' . _('Enter the description of the item. Up to 50 characters can be used.') . '" name="Description" class="form-control" size="52" maxlength="50" value="' . $Description . '" /></div>
	</div>';
...
<div class="form-group has-error"> <label class="col-md-8 control-label">' . _('Asset Description') . ' (' . _('long') . '):</label>
		<textarea ' . (in_array('LongDescription',$Errors) ?  'class="texterror"' : '' ) .'  name="LongDescription" class="form-control" required="required" title="' . _('Enter the lond description of the asset including specs etc. Up to 255 characters are allowed.') . '" cols="40" rows="4">' . stripslashes($LongDescription) . '</textarea></div>
	</div>';

if (!isset($New) ) { //ie not new at all!

	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' .  _('Image File (' . implode(", ", $SupportedImgExt) . ')') . ':</label>
...
echo '</select> <a target="_blank" class="btn btn-info" href="'. $RootPath . '/FixedAssetCategories.php">' . ' ' . _('Add or Modify Asset Categories') . '</a></div></div>';
...
if (isset($AssetRow) AND ($AssetRow['datepurchased']!='0000-00-00' AND $AssetRow['datepurchased']!='')){
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Date Purchased') . ':</label>
			' . ConvertSQLDate($AssetRow['datepurchased']) . '</div>
		</div>';
}
...
echo '</select>
	<a target="_blank" href="'. $RootPath . '/FixedAssetLocations.php" class="btn btn-info">' . ' ' . _('Add Asset Location') . '</a></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Bar Code') . '</label>
		<input ' . (in_array('BarCode',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" class="form-control" name="BarCode" size="22" maxlength="20" value="' . $_POST['BarCode'] . '" /></div>
	</div></div>
	<div class="row">
		<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Serial Number') . '</label>
		<input ' . (in_array('SerialNo',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" class="form-control" name="SerialNo" size="32" maxlength="30" value="' . $_POST['SerialNo'] . '" /></div>
	</div>
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Depreciation Type') . '</label>
		<select name="DepnType" class="form-control">';
...
echo '</select></div>
	...
	<div class="col-xs-4">
<div class="form-group"> <label class="col-md-8 control-label">' . _('Depreciation Rate') . ' %</label>
		<input ' . (in_array('DepnRate',$Errors) ?  'class="inputerror number"' : 'class="form-control"' ) .'  type="text" name="DepnRate" size="4" maxlength="4" value="' . $_POST['DepnRate'] . '" /></div>
	</div>
	</div><br />
';
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . _('Asset Financial Summary') . '</h3></div>
...
if (isset($New)) {
	echo '<div class="row" align="center">
			...
			<input type="submit" class="btn btn-success" name="submit" value="' . _('Submit') . '" /></div><br />';
} else {
	echo '
		<div class="row" align="center"><div>
			<input type="submit" class="btn btn-success" name="submit" value="' . _('Submit') . '" />
		</div></div>';
		echo '<br /><div class="alert alert-warning alert-dismissable" id="MessageContainerHead">
	...
	echo '<div class="row" align="center">
		<div>
			<input type="submit" name="delete" class="btn btn-danger" value="' . _('Delete') . '" onclick="return confirm(\'' . _('Are You Sure? Only assets with a zero book value can be deleted.') . '\');" /></div></div><br />
';
}
