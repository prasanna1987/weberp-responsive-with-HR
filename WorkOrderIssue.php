...
//	';
//echo '<div class="col-xs-4"><a href="'. $RootPath . '/WorkOrderCosting.php?WO=' .  $_POST['WO'] . '" class="btn btn-default">' . _('Back to Costing'). '</a></div></div>
//	<br /><br />';
...
if (!isset($_POST['WO']) OR !isset($_POST['StockID'])) {
	...
	echo '<div class="row" align="center"><a href="' . $RootPath . '/SelectWorkOrder.php" class="btn btn-info">' .
		_('Select a work order to issue materials to') . '</a></div><br />';
	echo   prnMsg(_('This page can only be opened if a work order has been selected. Please select a work order to issue materials to first'),'info');
...
if (isset($_POST['Process'])){ //user hit the process the work order issues entered.
	...
	if ($WORow['closed']==1){
		echo prnMsg(_('The work order is closed - no more materials or components can be issued to it.'),'error');
		$InputError=true;
	}
	...
	if (isset($_POST['SerialNos']) AND is_array($_POST['SerialNos'])){ //then we are issuing a serialised item
		...
		if (!isset($_POST['LotCounter']) OR !is_numeric($_POST['LotCounter'])) {
			$InputError = true;
			echo prnMsg(_('The line counter is not set up or not numeric, please ask administrator for help'),'error');
					...
					$QuantityIssued += filter_number_format($_POST['Qty'.$i]);
					 ...
					 if ($_SESSION['ProhibitNegativeStock']==1 AND $_POST['BatchRef'.$i] > "" AND $_POST['Qty' . $i]>0) {
						...
						if (DB_num_rows($Result)==0){
							$InputError = true;
							echo prnMsg(_('This issue cannot be processed because the system parameter is set to prohibit negative stock and this batch does not exist'),'error');
						}
						else {
							$CheckLotRow = DB_fetch_row($Result);
							if (($_POST['Qty'.$i]-$CheckLotRow[0])>$VarianceAllowed){
								$InputError = true;
								echo prnMsg(_('This issue cannot be processed because the system parameter is set to prohibit negative stock and this issue would result in this batch going into negative. Please correct the stock first before attempting another issue'),'error');
							}
						}
					}
	...
	if ($IssueItemRow['cost']==0){
		echo  prnMsg(_('The item being issued has a zero cost. The issue will still be processed '),'warn');
	}
	...
	if ($_SESSION['ProhibitNegativeStock']==1
		...
		if (($QuantityIssued-$CheckNegRow[0])>$VarianceAllowed){
			$InputError = true;
			echo prnMsg(_('This issue cannot be processed because the system parameter is set to prohibit negative stock and this issue would result in stock going into negative. Please correct the stock first before attempting another issue'),'error');
		}
	}
...
/************************ BEGIN SQL TRANSACTIONS ************************/
		...
		echo   prnMsg(_('The issue of') . ' ' . $QuantityIssued . ' ' . _('of')  . ' ' . $_POST['IssueItem'] . ' ' . _('against work order') . ' '. $_POST['WO'] . ' ' . _('has been processed'),'info');
			  
		echo '<div class="row">
<div class="col-xs-4"><a href="' . $RootPath . '/WorkOrderIssue.php?WO=' . $_POST['WO'] . '&amp;StockID=' . $_POST['StockID'] . '" class="btn btn-info">' . _('Issue more components to this work order') . '</a></div>';
		echo '<div class="col-xs-4"><a href="' . $RootPath . '/SelectWorkOrder.php" class="btn btn-info">' . _('Select a different work order for issuing materials and components against'). '</a></div></div><br />';
		unset($_POST['WO']);
		unset($_POST['StockID']);
...
<table id="general-table" class="table table-bordered">
		<tr>
			<td><strong>' . _('Issue to work order') . ':</strong></td>
			<td>' . $_POST['WO']  . '</td>
		</tr>
		<tr>
			<td><strong>' . _('Manufactured at') . ':</strong></td>
			<td>' . $WORow['locationname'] . '</td>
			<td><strong>' . _('Required By') . ':</strong></td>
			<td>' . ConvertSQLDate($WORow['requiredby']) . '</td>
		</tr>
		';

if ($WORow['closed']==1){
	echo   prnMsg(_('The selected work order has been closed and variances calculated and posted. No more issues of materials and components can be made against this work order.'),'info');
	include('includes/footer.php');
	exit;
}
...
echo '<tr>
		<td><strong>' . _('Date Material Issued') . ':</strong></td>
		<td><input type="text" name="IssuedDate" value="' . Date($_SESSION['DefaultDateFormat']) . '" class="form-control input-datepicker-close" data-date-format="dd/mm/yyyy" id="example-datepicker" maxlength="10" size="11" /></td>
		<td><strong>' . _('Issued From') . ':</strong></td>
		<td>';
...
if (!isset($_POST['IssueItem'])){
	...
	echo '<select name="FromLocation" class="form-control">';
...
<table id="general-table" class="table table-bordered"><thead><tr>
	...
	while ($RequirementsRow = DB_fetch_array($RequirmentsResult)){
		if ($RequirementsRow['autoissue']==0){
			echo '<tr>
					<td><input type="submit" class="btn btn-info" name="IssueItem" value="' .$RequirementsRow['stockid'] . '" /></td>
		...
		if ($RequirementsRow['controlled'] == 0) {
			echo '<td><input type="text" class="form-control" name="IssueQty' . $i . '" id="IssueQty' . $i . '" /></td>
	...
	if (isset($IssuedMaterials) AND count($IssuedMaterials)>0){
		...
		while($myrow = DB_fetch_array($result)){
			echo '<tr>
					<td><input type="submit" class="btn btn-info" name="IssueItem' . $i . '" value="' . $myrow['stockid'] . '" /></td>
			...
			if ($RequirementsRow['controlled'] == 0) {
				echo '<td><input type="text" class="form-control" name="IssueQty' . $i . '"  /></td>
...
echo '</table>
	...
	echo '
		...
		<div class="row" align="center">
			<input type="submit" name="ProcessMultiple" class="btn btn-success" value="' . _('Process Items Issued') . '" />
		</div><div class="page-header"></div><br/>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Enter text extracts in the') . ' ' . _('description') . '</label>
	    <input type="text" name="Keywords" class="form-control" size="20" maxlength="25" value="';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Enter extract of the') . '' . _('Stock Code') . '</label>
		<input type="text" class="form-control" name="StockCode" size="15" maxlength="18" value="';
            if (isset($_POST['StockCode'])) echo $_POST['StockCode'];
            echo '" /></div>
			</div>
			</div>
			
			<div class="row" align="center"><input type="submit" class="btn btn-info" name="Search" value="' . _('Search Now') . '" /></div><br />';
			...
			while ($myrow=DB_fetch_array($SearchResult)) {
				...
				if (!in_array($myrow['stockid'],$ItemCodes)){
					...
					printf('<tr class="striped_row">
							...
							<td>%s</td>
							<td>%s</td>
							<td><a href="%s" class="btn btn-info">'
	...
	if ($IssueItemRow['controlled']==1){ //controlled
		...
		if ($IssueItemRow['serialised']==1){ //serialised
			...
			if (DB_num_rows($SerialNoResult)==0){
				...
				echo '<tr>
						<td colspan="2" align="center"><input type="submit" class="btn btn-danger" name="Retry" value="' . _('Reselect Location or Issued Item') . '" /></td>
					</tr>';
			} else {
				echo '<tr>
						<td><select name="SerialNos[]" multiple="multiple" class="form-control">';
				...
				echo '<tr>
						<td colspan="2" align="center"><input type="submit" class="btn btn-success" name="Process" value="' . _('Process Items Issued') . '" /></td>
					</tr>';
			}
			...
			if (DB_num_rows($LotResult)>0) {
				...
				while($LotRow = DB_fetch_array($LotResult)) {
					echo '<tr>
						<td><input type="text" class="form-control" name="BatchRef' . $i .'" title="' . _('Enter a batch/roll reference being used with this work order') . '" value="' . $LotRow['serialno'] . '"  /></td>
						<td><input class="form-control" title="' . _('Enter the quantity of this batch/roll to issue to the work order') . '" name="Qty' . $i .'"  placeholder="' . $LotRow['quantity'] . '" /></td>
						</tr>';
					$i++;
				}
				...
				echo '<tr>
					...
					echo '<tr>
						<td colspan="2">' . _('Lot No') .': <input type="text" class="form-control" name="BatchRef' . $i .'" title="' . _('Enter a batch/roll reference being used with this work order') . '" value=""  />
						<td colspan="2">' . _('Quantity') . ': <input class="form-control" title="' . _('Enter the quantity of this batch/roll to issue to the work order') . '" name="Qty' . $i .'" /></td>
						</tr>';
					$i++;
					echo '<input type="hidden" name="LotCounter" value="' . $i . '" />';
			}
			echo '<input type="hidden" name="IssueItem" value="' . $_POST['IssueItem'] . '" />';
			echo '<tr>
					<td colspan="2" align="center"><input type="submit" name="Process" class="btn btn-success" value="' . _('Process Items Issued') . '" /></td>
				</tr>';
		} //end of lot/batch control
	} else { //not controlled - an easy one!
		echo '<input type="hidden" name="IssueItem" value="' . $_POST['IssueItem'] . '" />';
		echo '<tr><td>' . _('Quantity Issued') . ':</td>
			  <td class="has-error"><input class="form-control" type="text" size="10" maxlength="10" title="' . _('Enter the quantity of this item to issue to the work order') . '" name="Qty" required="required" value="0"/></td></tr>';
		echo '<tr>
				<td colspan="2" align="center"><input type="submit" class="btn btn-success" name="Process" value="' . _('Process Items Issued') . '" /></td>
			</tr>';
	}
