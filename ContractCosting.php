...
if (!isset($_GET['SelectedContract'])){
	echo '<br />';
	echo prnMsg( _('This page is expected to be called with the contract reference to show the costing for'), 'error');
...
echo _('Contract') . ' - ' . $_SESSION['Contract'.$identifier]->CustomerName . '<br /></h1></a></div>';
...
echo '<tr>
		<td class="number" colspan="5">' . _('<strong>Total Inventory Estimated Cost</strong>') . '</td>
		<td class="number">' . locale_number_format($ContractBOMBudget,$_SESSION['CompanyRecord']['decimalplaces'])  . '</td>
		<td class="number" colspan="5">' . _('<strong>Total Inventory Actual Cost</strong>') . '</td>
...
echo '<tr>
		<td colspan="6"><div class="table-responsive">
			<table id="general-table" class="table table-bordered">
		<thead><tr>
			<th>' . _('Requirement') . '</th>
			<th>' . _('Quantity') . '</th>
...
echo '<tr>
		<th colspan="3" align="right"><b>' . _('Estimated Other Costs') . '</b></th>
...
echo '<tr>
	...
	<tr>
		<td colspan="5"><b>' . _('Total Estimated Contract Cost') . '</b></td>
...
/*Check if the contract work order is still open */
	...
	echo prnMsg(_('The contract has been closed. No further charges can be posted against this contract.'),'success');
...
<div class="col-xs-12">';
	...
	echo '<br />
		...
		<input type="submit" class="btn btn-danger" name="CloseContract" value="' . _('Close Contract') .  '" onclick="return confirm(\'' . _('Closing the contract will prevent further stock being issued to it and charges being made against it. Variances will be taken to the profit and loss account. Are You Sure?') . '\');" />
		</div>
        </div>
		</div>
		</form>
