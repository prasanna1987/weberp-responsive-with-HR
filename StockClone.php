...
if (isset($_GET['OldStockID']) || isset($_POST['OldStockID']) ){ //we are cloning
    ...
    $InputError = 1;
    $Errors[0] = 'OldStockID';
    echo prnMsg(_('To use this script it must be called with the Stock ID of the item to be cloned passed in as $OldStockID. Please use the Clone This Item option in the Items Menu.'),'error');
}
...
if (isset($_POST['StockID']) && !empty($_POST['StockID']) && !isset($_POST['UpdateCategories'])) {
	...
	if (($myrow[0]==0) && ($_POST['OldStockID'] != '')) {
    	...
    	echo prnMsg(_('The stock code entered is already in the system - duplicate stock codes are prohibited by the system. Try choosing an alternative stock code'),'error');
...
if (!empty($_POST['OldStockID'])) { //only show this if there is a valid call to this script
    if (isset($_FILES['ItemPicture']) AND $_FILES['ItemPicture']['name'] !='') { //we are uploading a new file
        ...
        if (!in_array ($ext, $SupportedImgExt)) {
            ...
            echo prnMsg(_('The image file size is over the maximum allowed. The maximum size allowed in KB is') . ' ' . $_SESSION['MaxImageSize'],'warn');
            $UploadTheFile ='No';
            ...
            echo prnMsg(_('Attempting to overwrite an existing item image'),'warn');
        ...
        if (!copy($oldfile, $_SESSION['part_pics_dir'] . '/' .$_POST['StockID'] . '.' . $ext)) {
             echo prnMsg(_('There was an image file to clone but there was an error copying. Please upload a new image if required.'),'warn');
        }
         @unlink($_SESSION['part_pics_dir'] . '/' .$_POST['OldStockID'].'-TEMP' . '.' . $ext);
        if (is_file($_SESSION['part_pics_dir'] . '/' .$_POST['OldStockID'].'-TEMP' . '.' . $ext)) {
             echo prnMsg(_('Unable to delete the temporary image file for cloned item.'),'error');
        } else {
...
if (isset($_POST['submit'])) {
	...
	if ($_POST['StockID'] == $_POST['OldStockID']) {
	    $InputError = 1;
		echo prnMsg (_('The Stock Item code must be unique. Please re-enter a unique Stock Item code.'),'error');
		$Errors[$i] = 'StockID';
		$i++;
	}
	if (mb_strlen($_POST['StockID']) ==0) {
		$InputError = 1;
		echo prnMsg (_('The Stock Item code cannot be empty. Please enter a unique Stock Item code.'),'error');
		$Errors[$i] = 'StockID';
		$i++;
	}
		...
		if ($_POST['New'] !=0) { //it is a NEW CLONED part
			...
			if (DB_num_rows($result)==1){
				...
				if (DB_error_no() ==0) {
                    ...
                    //finish up
					if (DB_error_no() ==0) {
						echo prnMsg( _('New cloned Item') .' ' . '<a href="SelectProduct.php?StockID=' . $_POST['StockID'] . '" class="btn btn-info">' . $_POST['StockID'] . '</a> '. _('has been added to the system') .
							'<br />' . _('We also attempted to setup item purchase data and pricing.'));

                            if (isset($NoPricingData))
                            {
                                echo prnMsg(_('There is no pricing data to clone. Use the following link to add pricing.'));
                            }

                            echo prnMsg('<br />' . '<a target="_blank" href="Prices.php?Item=' . $_POST['StockID'] . '" class="btn btn-info">' . _('Review Item Prices') . '</a> ','success');

                            if ($NoPurchasingData==1)
                            {
                                 echo prnMsg(_('There is no purchasing data to clone .Use the following link to add purchasing data.'));
                            }
                            echo prnMsg('<br />' . '<a target="_blank" href="PurchData.php?StockID=' . $_POST['StockID'] . '" class="btn btn-info">' . _('Review Item Purchase Data') . '</a> ','success')
                 ;
                          echo prnMsg(_('Costing was updated for this cloned item.').
                            '<br />' . '<a target="_blank" href="StockCostUpdate.php?StockID=' . $_POST['StockID'] . '" class="btn btn-info">' . _('Review Item Cost') . '</a>', 'success');
...
echo '<form name="ItemForm" id="ItemForm" enctype="multipart/form-data" method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">';
...
/*If the page was called without $StockID or empty $StockID  then a new cloned stock item is to be entered. Show a form with a part Code field,
  otherwise show form for editing with only a hidden OldStockID field. */
	    ...
	    $StockIDStyle= !empty($_POST['StockID'])  && ($_POST['StockID'] != $_POST['OldStockID'])? '' : ' ';
	    $StockID= !empty($_POST['StockID'])? $_POST['StockID']:$_POST['OldStockID'];
		echo '<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">'. _('Cloned Item Code'). '</label>
				<input type="text" ' . (in_array('StockID',$Errors) ?  'class="inputerror"' : '' ) .'"  "'.$StockIDStyle.'" data-type="no-illegal-chars" class="form-control" autofocus="autofocus" required="required" value="' . $StockID . '" name="StockID" size="21" maxlength="20" />
				<input type="hidden" name="OldStockID" value="'.$_POST['OldStockID'].'" /> <span class="text-danger"><strong> '. _('Enter a unique item code for the new item.') .'</strong></span>
				</div>
...
//if ($_POST['New'] == 1) {
    ...
    echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Part Description') . ' (' . _('short') . ')</label>
            <input ' . (in_array('Description',$Errors) ?  'class="inputerror"' : '' ) .' type="text" name="Description" class="form-control" size="52" maxlength="50" value="' . $Description . '" /></div></div>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Part Description') . ' (' . _('long') . ')</label>
	   ...
	   <div class="row">
        <div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">'. _('Image File (.jpg)') . '</label>
    ...
    if (extension_loaded('gd') && function_exists ('gd_info') && isset ($tempfile) ) {
        ...
        if( !empty($tempid) AND file_exists($_SESSION['part_pics_dir'] . '/' .$tempid.'.' . $ext) ) {
            $StockImgLink = '<img src="' . $_SESSION['part_pics_dir'] . '/' . $tempid . '.' . $ext . '" height="50" width="50" />';
			...
			if (!copy($_SESSION['part_pics_dir'] . '/' .$_POST['OldStockID'].'.' . $ext, $_SESSION['part_pics_dir'] . '/' .$_POST['OldStockID'].'-TEMP' . '.' . $ext)) {
                ...
                $StockImgLink = '<img src="' . $_SESSION['part_pics_dir'] . '/' .$_POST['OldStockID'].'-TEMP' . '.' . $ext . '" height="50" width="50" />';
            }
        } else {
            $StockImgLink = _('No Image');
        }
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Category') . '</label>
    ...
    echo '</select><a target="_blank" href="'. $RootPath . '/StockCategories.php" class="btn btn-info">' . _('Add or Modify Stock Categories') . '</a></div></div>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Packaged Volume (metres cubed)') . '</label>
            <input ' . (in_array('Volume',$Errors) ?  'class="inputerror"' : '' ) .'   type="text" class="form-control" name="Volume" size="12" maxlength="10" value="' . locale_number_format($_POST['Volume'],'Variable') . '" /></div></div>';

    echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Packaged Gross Weight (KGs)') . '</label><input ' . (in_array('GrossWeight',$Errors) ?  'class="inputerror"' : '' ) .'   type="text" class="form-control" name="GrossWeight" size="12" maxlength="10" value="' . locale_number_format($_POST['GrossWeight'],'Variable') . '" /></div></div>';

    echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Net Weight (KGs)') . '</label><input ' . (in_array('NetWeight',$Errors) ?  'class="inputerror"' : '' ) .'   type="text" class="form-control" name="NetWeight" size="12" maxlength="10" value="' . locale_number_format($_POST['NetWeight'],'Variable') . '" /></div></div></div>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Units of Measure') . '</label>
    ...
    echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Assembly, Kit, Manufactured or Service/Labour') . '</label>
    ...
    echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Current or Obsolete') . '</label>
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Serialised') . '</label><select ' . (in_array('Serialised',$Errors) ?  'class="selecterror"' : '' ) .'  name="Serialised" class="form-control">';
    ...
    echo '</select><i>' . _('Note') . ', ' . _('this has no effect if the item is not Controlled') . '</i></div></div>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Next Serial No (>0 for auto numbering)') . '</label>
                <input ' . (in_array('NextSerialNo',$Errors) ?  'class="inputerror"' : '' ) .' type="text" class="form-control" name="NextSerialNo" size="15" maxlength="15" value="' . $_POST['NextSerialNo'] . '" /></div></div>';
    } else {
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Decimal Places for display Quantity') . '</label>
            <input type="text" class="form-control" name="DecimalPlaces" size="1" maxlength="1" value="' . $_POST['DecimalPlaces'] . '" /></div></div>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Bar Code') . '</label>
            <input ' . (in_array('BarCode',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" class="form-control" name="BarCode" size="22" maxlength="20" value="' . $BarCode . '" /></div></div></div>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Discount Category') . '</label>
            <input type="text" name="DiscountCategory" class="form-control" size="2" maxlength="2" value="' . $DiscountCategory . '" /></div></div>	';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Tax Category') . '</label>
    ...
    echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Pan Size') . '</label>
            <input type="text" class="form-control" name="Pansize" size="6" maxlength="6" value="' . locale_number_format($_POST['Pansize'],0) . '" /></div></div></div>
			<div class="row">
        <div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Shrinkage Factor') . '</label>
            <input type="text" class="form-control" name="ShrinkFactor" size="6" maxlength="6" value="' . locale_number_format($_POST['ShrinkFactor'],0) . '" /></div></div>';
...
<div class="form-group"> <label class="col-md-12 control-label">' . _('Item Category Properties') . '</label>
           ';
	...
	echo '</div><div class="row">
<div class="col-xs-4"><input type="submit" name="submit" value="' . _('Add') . '" class="btn btn-success" /></div>';
	echo '<div class="col-xs-4"><input type="submit" class="btn btn-info" name="UpdateCategories" style="visibility:hidden;width:1px" value="' . _('Categories') . '" />';
