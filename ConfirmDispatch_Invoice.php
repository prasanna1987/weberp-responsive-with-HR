...
<div>
			<a href="' . $RootPath . '/SelectSalesOrder.php" class="btn btn-info">' . _('Back to Invoicing'). '</a>
		</div></div>
		
		<br />';
	echo prnMsg( _('Please select an order first'), 'error' ); 
...
/*now populate the line items array with the sales order details records */
		...
		if(DB_num_rows($LineItemsResult)>0) {
			...
			echo '<br />';
			echo prnMsg( _('There are no ordered items with a quantity left to deliver. There is nothing left to invoice'), 'error');
			include('includes/footer.php');
			exit;
		...
		echo prnMsg( _('This order item could not be retrieved. Please select another order'), 'warn');
		include ('includes/footer.php');
		exit;
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $Itm) {
		if(sizeOf($Itm->SerialItems) > 0) {
			...
			if($_SESSION['Items'.$identifier]->LineItems[$Itm->LineNumber]->QtyDispatched > ($_SESSION['Items'.$identifier]->LineItems[$Itm->LineNumber]->Quantity - $_SESSION['Items'.$identifier]->LineItems[$Itm->LineNumber]->QtyInv)) {
				echo prnMsg(_('Dispatched Quantity should not be more than order balance quantity').'. '._('To dispatch quantity is').' '.$_SESSION['Items'.$identifier]->LineItems[$Itm->LineNumber]->QtyDispatched.' '._('And the order balance is ').' '.($_SESSION['Items'.$identifier]->LineItems[$Itm->LineNumber]->Quantity - $_SESSION['Items'.$identifier]->LineItems[$Itm->LineNumber]->QtyInv),'error');
				include('includes/footer.php');
				exit;
			}
...
echo '<div class="block-header"><a href="" class="header-title-link"><h1>' . ' ' . _('Confirm Dispatch and Invoice'). '</h1></a></div>';
...
<div class="col-xs-12">
<div class="block">
<div class="block-title"><h3>' . ' ' . _('Customer Code') . '-' . $_SESSION['Items'.$identifier]->DebtorNo . '' . _('Customer Name') . ' : ' . $_SESSION['Items'.$identifier]->CustomerName. '</h3></div>
<div class="table-responsive">
<table id="general-table" class="table table-bordered">

			<tr><th colspan ="11"></th>
				<th colspan ="2"><strong>' . _('Currency:') . ' ' . $_SESSION['Items'.$identifier]->DefaultCurrency . '</strong></th>
			</tr>
	<thead>
	<tr>
		<th>' . _('Item Code') . '</th>
		...
		<th>' . _('Ordered') . '</th>
		<th>' . _('Units') . '</th>
		<th>' . _('Already') . '<br />' . _('Dispatched') . '</th>
		<th>' . _('Dispatching') . '<br />' . _('Now') . '</th>
		<th>' . _('Price') . '</th>
		<th>' . _('Discount') . '</th>
...
foreach ($_SESSION['Items'.$identifier]->LineItems as $LnItm) {
	...
	if ($LnItm->QOHatLoc < $LnItm->Quantity and ($LnItm->MBflag == 'B' or $LnItm->MBflag == 'M')) {
		...
		$RowStarter = '<tr style="background:#ff5e4c;color:#FFFFFF">'; //rows show red where stock deficiency
	} else {
	...
	if($LnItm->Controlled==1) {
		...
		if(isset($_POST['ProcessInvoice'])) {
			...
			echo '<td class="has-error"><input type="hidden" name="' . $LnItm->LineNumber . '_QtyDispatched" required="required" maxlength="11"  value="' . $LnItm->QtyDispatched . '" />';
			
			echo'
			<a href="' . $RootPath .'/ConfirmDispatchControlled_Invoice.php?identifier=' . urlencode($identifier) . '&LineNo=' . urlencode($LnItm->LineNumber) . '" class="btn btn-info">' . locale_number_format($LnItm->QtyDispatched,$LnItm->DecimalPlaces) . '</a></td>';
		}
	} else {
		if(isset($_POST['ProcessInvoice'])) {
			echo '<td class="number">' . locale_number_format($LnItm->QtyDispatched,$LnItm->DecimalPlaces) . '</td>';
		} else {
			echo '<td class="has-error"><input tabindex="'.$j.'" type="text" ' . ($j==1 ? 'autofocus="autofocus" ':'') . ' class="form-control" required="required" title="' . _('Enter the quantity to charge the customer for, that has been dispatched') . '" name="' . $LnItm->LineNumber . '_QtyDispatched" maxlength="12" size="12" value="' . locale_number_format($LnItm->QtyDispatched,$LnItm->DecimalPlaces) . '" /></td>';
		}
	}
	...
	foreach ($LnItm->Taxes AS $Tax) {
		...
		if(isset($_POST['ProcessInvoice'])) {
			...
			echo '<input type="text" class="form-control" required="required" title="' . _('Enter the tax rate applicable as a number') . '" name="' . $LnItm->LineNumber . $i . '_TaxRate" maxlength="4" size="4" value="' . $Tax->TaxRate*100 . '" />
			<input type="hidden" name="' . $LnItm->LineNumber . $i . '_TaxAuthID" value="' . $LnItm->TaxAuthID . '" />
			';
		}
		$i++;
		if($Tax->TaxOnTax ==1) {
	...
	//if($LnItm->Controlled==1) {
//		if(!isset($_POST['ProcessInvoice'])) {
//			echo '<td><a href="' . $RootPath . '/ConfirmDispatchControlled_Invoice.php?identifier=' . urlencode($identifier) . '&LineNo=' . urlencode($LnItm->LineNumber) . '" class="btn btn-info">';
//			if($LnItm->Serialised==1) {
//				echo _('Enter Serial Numbers');
//			} else { /*Just batch/roll/lot control */
//				echo _('Batch/Serial Nos.');
//			}
//			echo '</a></td>';
//		}
//	}
	...
	if(!is_numeric($BestShipper)) {
		...
		if(DB_num_rows($TestShipperExists)==1) {
			...
			if(DB_num_rows($TestShipperExists)>=1) {
				...
				echo prnMsg( _('There are no shippers defined') . '. ' . _('Please use the link below to set up shipping companies, the nERP needs the shipping company to be selected or a default freight company to be used'),'error');
				echo '<p align="left"><a href="' . $RootPath . 'Shippers.php" class="btn btn-info">' . _('Enter') . '/' . _('Amend Freight Companies'). '</a></p><br />';
			}
		}
	}
}
...
//	$_POST['ChargeFreightCost']=0;
//}
//echo '<td class="number" colspan="2">', _('Freight Cost(Excl.Tax)'), '</td>';
...
//
//	echo '<td><input class="form-control" maxlength="12" name="ChargeFreightCost" required="required" size="10" tabindex="', $j, '" type="text" value="0" /></td>';
...
//	} else {
//		echo '<td class="number"><input class="form-control" maxlength="12" name="ChargeFreightCost" size="10" tabindex="'. $j. '" type="text" value="'. locale_number_format($_SESSION['Items'.$identifier]->FreightCost,$_SESSION['Items'.$identifier]->CurrDecimalPlaces). '" /></td>';
//	}
...
//	} else {
//		echo '<input class="form-control" maxlength="4" name="FreightTaxRate'. $FreightTaxLine->TaxCalculationOrder. '" size="4" type="text" value="'. locale_number_format($FreightTaxLine->TaxRate * 100, $_SESSION['Items'.$identifier]->CurrDecimalPlaces) . '" />';
//	}
//
...
echo '<tr>
	<td colspan="8" class="number"><strong>' . _('Invoice Totals'). '</strong></td>
	<td class="number"><strong>' . $DisplaySubTotal . '</strong></td>
	<td colspan="2"></td>
	<td class="number"><strong>' . locale_number_format($TaxTotal,$_SESSION['Items'.$identifier]->CurrDecimalPlaces) . '</strong></td>
	<td class="number"><strong>' . locale_number_format($TaxTotal+($_SESSION['Items'.$identifier]->total + $_POST['ChargeFreightCost']),$_SESSION['Items'.$identifier]->CurrDecimalPlaces) . '</strong></td>
</tr>';
...
invoices can have a zero amount but there must be a quantity to invoice */
	...
	if(! $QuantityInvoicedIsPositive) {
		echo prnMsg( _('There are no lines on this order with a quantity to invoice') . '. ' . _('No further processing has been done'),'error');
		include('includes/footer.php');
		exit;
	}
	...
	//sadly this check does not combine quantities occuring twice on and order and each line is considered individually :-(
		...
		if($NegativesFound) {
			...
			echo '<div class="row"><div class="col-xs-4">
					<input type="submit" class="btn btn-info" name="Update" value="' . _('Update'). '" /></div></div><br />
';
			include('includes/footer.php');
			exit;
		}
...
/*company record read in on login with info on GL Links and debtors GL account*/
	...
	if($_SESSION['CompanyRecord']==0) {
		...
		echo prnMsg( _('The company information and preferences could not be retrieved') . ' - ' . _('see your system administrator'), 'error');
		include('includes/footer.php');
		exit;
	}
...
/*Now need to check that the order details are the same as they were when they were read into the Items array. If they've changed then someone else may have invoiced them */
	...
	/*there should be the same number of items returned from this query as there are lines on the invoice - if not 	then someone has already invoiced or credited some lines */
		...
		echo '<br />';
		echo prnMsg( _('This order has been changed or invoiced since this delivery processing started') . '. ' . _('Processing halted') . '. ' . _('To enter and confirm this dispatch') . '/' . _('invoice the order must be re-selected and re-processed again to update the changes made by the other user'), 'error');
	...
	while ($myrow = DB_fetch_array($Result)) {
		...
		if($_SESSION['Items'.$identifier]->LineItems[$myrow['orderlineno']]->Quantity != $myrow['quantity']
			...
			echo prnMsg( _('This order has been changed or invoiced since this delivery processing started') . ' ' . _('Processing halted.') . ' ' . _('To enter and confirm this dispatch, it must be re-selected and re-processed again to update the changes made by the other user'), 'error');

			echo '<br />';

			echo '<div class="row"><div class="col-xs-8"><a href="'. $RootPath . '/SelectSalesOrder.php" class="btn btn-info">' . _('Select a sales order for confirming deliveries and invoicing'). '</a></div></div><br />
';
...
/* If balance of the order cancelled update sales order details quantity. Also insert log records for OrderDeliveryDifferencesLog */
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
		...
		if(mb_substr($OrderLine->StockID,0,6)=='ASSET-') {
			...
			echo prnMsg (_('The asset number being disposed of is:') . ' ' . $AssetNumber, 'info');
		} else {
			$IsAsset = false;
			$AssetNumber = 0;
		}
...
// *************************************************************************
	...
	echo '<div class="col-xs-6"><a href="'.$RootPath.'/SelectSalesOrder.php" class="btn btn-info">' . _('Back to Invoicing'). '</a></div>';
	if($_SESSION['InvoicePortraitFormat']==0) {
		echo '<div class="col-xs-4">' . ' ' . '<a target="_blank" href="'.$RootPath.'/PrintCustTrans.php?FromTransNo='.$InvoiceNo.'&amp;InvOrCredit=Invoice&amp;PrintPDF=True" class="btn btn-warning">' . _('Print'). ' </a></div>';
	} else {
		echo '<div class="col-xs-4">' . ' ' . '<a target="_blank" href="'.$RootPath.'/PrintCustTransPortrait.php?FromTransNo='.$InvoiceNo.'&amp;InvOrCredit=Invoice&amp;PrintPDF=True" class="btn btn-warning">' . _('Print'). ' </a></div>';
	}
	echo '<div class="col-xs-2"><a href="'.$RootPath.'/SelectOrderItems.php?NewOrder=Yes" class="btn btn-info">' . _('Back to Sales Orders') . '</a></div></div><br />
';
/*end of process invoice */
	...
	echo '<div class="row">
<div class="col-xs-4">
<div class="form-group has-error"> <label class="col-md-12 control-label">' ._('Date Of Invoice'). '</label>
			<input tabindex="'.$j.'" type="text" required="required" maxlength="10" size="11" required="required" name="DispatchDate" value="' . $DefaultDispatchDate . '" class="form-control input-datepicker" data-date-format="dd/mm/yyyy" id="example-datepicker"
 /></div>
		</div>';
	$j++;
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Consignment Ref. Note'). '</label>
			<input tabindex="'.$j.'" type="text" data-type="no-illegal-chars" class="form-control" title="' . _('Enter the consignment note reference to enable tracking of the delivery in the event of customer proof of delivery issues') . '" maxlength="20" size="20" name="Consignment" value="' . $_POST['Consignment'] . '" /></div>
		</div>';
	$j++;
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('No Of Packages in Delivery'). '</label>
			<input tabindex="'.$j.'" type="number" maxlength="6" size="6" class="form-control" name="Packages" value="' . $_POST['Packages'] . '" /></div>
		</div></div>';

	$j++;
	echo '<div class="row">
			<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Action For Balance'). '</label>
			
				<select required="required" tabindex="' . $j . '" name="BOPolicy" class="form-control">
	...
	$j++;
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' ._('Invoice Text'). '</label>
			<textarea tabindex="'.$j.'" name="InvoiceText" pattern=".{0,20}" class="form-control">' . reverse_escape($_POST['InvoiceText']) . '</textarea></div>
		</div>';

	++$j;
	echo '<div class="col-xs-4">
<div class="form-group"> <label class="col-md-12 control-label">' . _('Internal Comments') . '</label>
			<textarea tabindex="' . $j . '" name="InternalComments" pattern=".{0,20}" class="form-control">' . reverse_escape($_SESSION['Items'.$identifier]->InternalComments) . '</textarea></div>
		</div></div>';
	...
	echo '
		<div class="row">
		<div class="col-xs-4">
			<input type="submit" class="btn btn-info" tabindex="'.$j.'" name="Update" value="' . _('Update'). '" />
			</div>';

	$j++;
	echo '<div class="col-xs-4">
		<input type="submit" tabindex="'.$j.'" name="ProcessInvoice" value="'._('Process Invoice').'" class="btn btn-success" />
		</div> </div><br />
