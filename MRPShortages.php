...
/* 21/03/2010: Ricard modification to allow items with total supply = 0 be included in the report */
	...
	if (DB_error_no() !=0) {
	  ...
	  include('includes/header.php');
	   echo prnMsg( _('The MRP shortages and excesses could not be retrieved by nERP because') . ' '  . DB_error_msg(),'error');
	...
	if (DB_num_rows($result) == 0) {
	  $Title = _('MRP Shortages and Excesses') . ' - ' . _('Problem Report');
	  include('includes/header.php');
	   echo prnMsg( _('No MRP shortages - Excess retrieved'), 'warn');
	  echo '<br /><p align="right"><a href="' . $RootPath . '/menu_data.php?Application=manuf" class="btn btn-default">' . _('<i class="fa fa-hand-o-left fa-fw"></i> Menu') . '</a></p><br />';
	   if ($debug==1){
		  echo '<br />' . $sql;
	   }
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Sort by') . '</label>
			<select name="Sort" class="form-control">
				<option selected="selected" value="extcost">' . _('Total Shortage Amount') . '</option>
				<option value="stockid">' . _('Stock ID') . '</option>
				</select>
			</div>
		</div>';
...
<div class="form-group"> <label class="col-md-8 control-label">' . _('Report Type') . '</label>
			<select name="ReportType" class="form-control">
				<option selected="selected" value="Shortage">' . _('MRP Shortages') . '</option>
				<option value="Excess">' . _('MRP Excesses') . '</option>
				</select>
			</div>
		</div>';
